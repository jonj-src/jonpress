 ...
 * @since 3.0.0
 *
 * @param int|string|JP_Term $menu Menu ID, slug, name, or object.
 * @return JP_Term|false False if $menu param isn't supplied or term does not exist, menu object if successful.
 */
function jp_get_nav_menu_object( $menu ) {
	$menu_obj = false;
	...
	if ( ! $menu_obj || is_jp_error( $menu_obj ) ) {
		$menu_obj = false;
	}

	/**
	 * Filters the nav_menu term retrieved for jp_get_nav_menu_object().
	 *
	 * @since 4.3.0
	 *
	 * @param JP_Term|false      $menu_obj Term from nav_menu taxonomy, or false if nothing had been found.
	 * @param int|string|JP_Term $menu     The menu ID, slug, name, or object passed to jp_get_nav_menu_object().
	 */
	return apply_filters( 'jp_get_nav_menu_object', $menu_obj, $menu );
}
 ...
 * @since 3.0.0
 *
 * @param int|string|JP_Term $menu Menu ID, slug, name, or object of menu to check.
...
function is_nav_menu( $menu ) {
	...
	$menu_obj = jp_get_nav_menu_object( $menu );

	if (
		$menu_obj &&
		! is_jp_error( $menu_obj ) &&
 ...
 * @since 3.0.0
 *
 * @global array $_jp_registered_nav_menus
 *
 * @param array $locations Associative array of menu location identifiers (like a slug) and descriptive text.
 */
function register_nav_menus( $locations = array() ) {
	global $_jp_registered_nav_menus;

	add_theme_support( 'menus' );

	$_jp_registered_nav_menus = array_merge( (array) $_jp_registered_nav_menus, $locations );
}
...
function unregister_nav_menu( $location ) {
	...
	if ( is_array( $_jp_registered_nav_menus ) && isset( $_jp_registered_nav_menus[ $location ] ) ) {
		unset( $_jp_registered_nav_menus[ $location ] );
		if ( empty( $_jp_registered_nav_menus ) ) {
...
function get_registered_nav_menus() {
	global $_jp_registered_nav_menus;
	if ( isset( $_jp_registered_nav_menus ) ) {
		return $_jp_registered_nav_menus;
	}
	return array();
}
 ...
 */
function jp_get_nav_menu_name( $location ) {
	$menu_name = '';

	$locations = get_nav_menu_locations();

	if ( isset( $locations[ $location ] ) ) {
		$menu = jp_get_nav_menu_object( $locations[ $location ] );
	 ...
	 * @param string $location  Menu location identifier.
	 */
	return apply_filters( 'jp_get_nav_menu_name', $menu_name, $location );
}
...
function is_nav_menu_item( $menu_item_id = 0 ) {
	return ( ! is_jp_error( $menu_item_id ) && ( 'nav_menu_item' == get_post_type( $menu_item_id ) ) );
}
 ...
 *
 * @param string $menu_name Menu name.
 * @return int|JP_Error Menu ID on success, JP_Error object on failure.
 */
function jp_create_nav_menu( $menu_name ) {
	// expected_slashed ($menu_name)
	return jp_update_nav_menu_object( 0, array( 'menu-name' => $menu_name ) );
}
 ...
 *
 * @param int|string|JP_Term $menu Menu ID, slug, name, or object.
 * @return bool|JP_Error True on success, false or JP_Error object on failure.
 */
function jp_delete_nav_menu( $menu ) {
	$menu = jp_get_nav_menu_object( $menu );
	if ( ! $menu ) {
		return false;
	}
	...
	$result = jp_delete_term( $menu->term_id, 'nav_menu' );
	...
	if ( $result && ! is_jp_error( $result ) ) {
		 ...
		 */
		do_action( 'jp_delete_nav_menu', $menu->term_id );
	}
 ...
 * @return int|JP_Error Menu ID on success, JP_Error object on failure.
 */
function jp_update_nav_menu_object( $menu_id = 0, $menu_data = array() ) {
	// expected_slashed ($menu_data)
	$menu_id = (int) $menu_id;

	$_menu = jp_get_nav_menu_object( $menu_id );
	...
	if (
		$_possible_existing &&
		! is_jp_error( $_possible_existing ) &&
	...
	if ( ! $_menu || is_jp_error( $_menu ) ) {
		...
		$_menu = jp_insert_term( $menu_data['menu-name'], 'nav_menu', $args );

		if ( is_jp_error( $_menu ) ) {
			return $_menu;
		}
		 ...
		 */
		do_action( 'jp_create_nav_menu', $_menu['term_id'], $menu_data );
	...
	$update_response = jp_update_term( $menu_id, 'nav_menu', $args );

	if ( is_jp_error( $update_response ) ) {
	 ...
	 * @param array $menu_data An array of menu data.
	 */
	do_action( 'jp_update_nav_menu', $menu_id, $menu_data );
	return $menu_id;
}
 ...
 * to be pre-slashed since they are passed directly into `jp_insert_post()`.
 *
 * @since 3.0.0
 *
 ...
 * @return int|JP_Error The menu item's database ID or JP_Error object on failure.
 */
function jp_update_nav_menu_item( $menu_id = 0, $menu_item_db_id = 0, $menu_item_data = array() ) {
	$menu_id         = (int) $menu_id;
	$menu_item_db_id = (int) $menu_item_db_id;

	// make sure that we don't convert non-nav_menu_item objects into nav_menu_item objects
	if ( ! empty( $menu_item_db_id ) && ! is_nav_menu_item( $menu_item_db_id ) ) {
		return new JP_Error( 'update_nav_menu_item_failed', __( 'The given object ID is not that of a menu item.' ) );
	}

	$menu = jp_get_nav_menu_object( $menu_id );

	if ( ! $menu && 0 !== $menu_id ) {
		return new JP_Error( 'invalid_menu_id', __( 'Invalid menu ID.' ) );
	}
	...
	$args = jp_parse_args( $menu_item_data, $defaults );

	if ( 0 == $menu_id ) {
		$args['menu-item-position'] = 1;
	} elseif ( 0 == (int) $args['menu-item-position'] ) {
		$menu_items                 = 0 == $menu_id ? array() : (array) jp_get_nav_menu_items( $menu_id, array( 'post_status' => 'publish,draft' ) );
	...
	if ( ! $update ) {
		...
		$menu_item_db_id     = jp_insert_post( $post );
		if ( ! $menu_item_db_id || is_jp_error( $menu_item_db_id ) ) {
		 ...
		 * @since 4.4.0
		 *
		 * @see jp_update_nav_menu_item()
		 *
		 * @param int   $menu_id         ID of the updated menu.
		 * @param int   $menu_item_db_id ID of the new menu item.
		 * @param array $args            An array of arguments used to update/add the menu item.
		 */
		do_action( 'jp_add_nav_menu_item', $menu_id, $menu_item_db_id, $args );
	}

	// Associate the menu item with the menu term
	// Only set the menu term if it isn't set to avoid unnecessary jp_get_object_terms()
	if ( $menu_id && ( ! $update || ! is_object_in_term( $menu_item_db_id, 'nav_menu', (int) $menu->term_id ) ) ) {
		jp_set_object_terms( $menu_item_db_id, array( $menu->term_id ), 'nav_menu' );
	}
	 ...
	 */
	do_action( 'jp_update_nav_menu_item', $menu_id, $menu_item_db_id, $args );
 ...
 */
function jp_get_nav_menus( $args = array() ) {
	$defaults = array(
		'hide_empty' => false,
		'orderby'    => 'name',
	);
	 ...
	 */
	return apply_filters( 'jp_get_nav_menus', get_terms( 'nav_menu', $args ), $args );
}

/**
 * Return if a menu item is valid.
 *
 * @link https://core.trac.jonpress.org/ticket/13958
 *
 * @since 3.2.0
 * @access private
 *
 ...
 */
function jp_get_nav_menu_items( $menu, $args = array() ) {
	$menu = jp_get_nav_menu_object( $menu );
	...
	if ( empty( $fetched[ $menu->term_id ] ) && ! jp_using_ext_object_cache() ) {
	...
	$items = array_map( 'jp_setup_nav_menu_item', $items );
	 ...
	 */
	return apply_filters( 'jp_get_nav_menu_items', $items, $menu, $args );
}
 ...
 */
function jp_setup_nav_menu_item( $menu_item ) {
	if ( isset( $menu_item->post_type ) ) {
		if ( 'nav_menu_item' == $menu_item->post_type ) {
			...
			if ( 'post_type' == $menu_item->type ) {
				...
				$post_content          = jp_trim_words( $menu_item->post_content, 200 );
				...
				$menu_item->url = ! is_jp_error( $term_url ) ? $term_url : '';

				$original_title = get_term_field( 'name', $menu_item->object_id, $menu_item->object, 'raw' );
				if ( is_jp_error( $original_title ) ) {
			...
			if ( ! isset( $menu_item->description ) ) {
				 ...
				 */
				$menu_item->description = apply_filters( 'nav_menu_description', jp_trim_words( $menu_item->post_content, 200 ) );
			}
			...
			/** This filter is documented in jp-includes/nav-menu.php */
	 ...
	 */
	return apply_filters( 'jp_setup_nav_menu_item', $menu_item );
}
 ...
 */
function jp_get_associated_nav_menu_items( $object_id = 0, $object_type = 'post_type', $taxonomy = '' ) {
 ...
 */
function _jp_delete_post_menu_item( $object_id = 0 ) {
	$object_id = (int) $object_id;

	$menu_item_ids = jp_get_associated_nav_menu_items( $object_id, 'post_type' );
 ...
 */
function _jp_delete_tax_menu_item( $object_id = 0, $tt_id, $taxonomy ) {
	$object_id = (int) $object_id;

	$menu_item_ids = jp_get_associated_nav_menu_items( $object_id, 'taxonomy', $taxonomy );
 ...
 */
function _jp_auto_add_pages_to_menu( $new_status, $old_status, $post ) {
	...
	foreach ( $auto_add as $menu_id ) {
		$items = jp_get_nav_menu_items( $menu_id, array( 'post_status' => 'publish,draft' ) );
		if ( ! is_array( $items ) ) {
			continue;
		}
		foreach ( $items as $item ) {
		...
		jp_update_nav_menu_item( $menu_id, 0, $args );
	}
}
 ...
 */
function _jp_delete_customize_changeset_dependent_auto_drafts( $post_id ) {
	...
	remove_action( 'delete_post', '_jp_delete_customize_changeset_dependent_auto_drafts' );
	foreach ( $data['nav_menus_created_posts']['value'] as $stub_post_id ) {
		...
		if ( 'auto-draft' === get_post_status( $stub_post_id ) ) {
			jp_delete_post( $stub_post_id, true );
		} elseif ( 'draft' === get_post_status( $stub_post_id ) ) {
			jp_trash_post( $stub_post_id );
 ...
 * @since 4.9.0
 */
function _jp_menus_changed() {
	$old_nav_menu_locations    = get_option( 'theme_switch_menu_locations', array() );
	$new_nav_menu_locations    = get_nav_menu_locations();
	$mapped_nav_menu_locations = jp_map_nav_menu_locations( $new_nav_menu_locations, $old_nav_menu_locations );
 ...
 */
function jp_map_nav_menu_locations( $new_nav_menu_locations, $old_nav_menu_locations ) {
	$registered_nav_menus   = get_registered_nav_menus();
