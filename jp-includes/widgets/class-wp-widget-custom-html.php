<?php
/**
 * Widget API: JP_Widget_Custom_HTML class
 *
 * @package JonPress
 * @subpackage Widgets
 * @since 4.8.1
 ...
 * @see JP_Widget
 */
class JP_Widget_Custom_HTML extends JP_Widget {
	...
	public function _register_one( $number = -1 ) {
		...
		jp_add_inline_script( 'custom-html-widgets', sprintf( 'wp.customHtmlWidgets.idBases.push( %s );', jp_json_encode( $this->id_base ) ) );
		...
		add_action( 'admin_footer-widgets.php', array( 'JP_Widget_Custom_HTML', 'render_control_template_scripts' ) );

		// Note this action is used to ensure the help text is added to the end.
		add_action( 'admin_head-widgets.php', array( 'JP_Widget_Custom_HTML', 'add_help_text' ) );
	}
	...
	public function widget( $args, $instance ) {
		...
		/** This filter is documented in jp-includes/widgets/class-jp-widget-text.php */
		 ...
		 * @param JP_Widget_Custom_HTML $this     Current Custom HTML widget instance.
		 */
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( current_user_can( 'unfiltered_html' ) ) {
			...
			$instance['content'] = jp_kses_post( $new_instance['content'] );
		}
		return $instance;
	}
	...
	public function enqueue_admin_scripts() {
		$settings = jp_enqueue_code_editor(
			array(
				'type'       => 'text/html',
				'codemirror' => array(
					'indentUnit' => 2,
		...
		jp_enqueue_script( 'custom-html-widgets' );
		if ( empty( $settings ) ) {
			$settings = array(
				'disabled' => true,
			);
		}
		jp_add_inline_script( 'custom-html-widgets', sprintf( 'wp.customHtmlWidgets.init( %s );', jp_json_encode( $settings ) ), 'after' );
		...
		jp_add_inline_script( 'custom-html-widgets', sprintf( 'jQuery.extend( wp.customHtmlWidgets.l10n, %s );', jp_json_encode( $l10n ) ), 'after' );
	}
	 ...
	 *
	 * @since 4.8.1
	 * @since 4.9.0 The form contains only hidden sync inputs. For the control UI, see `JP_Widget_Custom_HTML::render_control_template_scripts()`.
	 *
	 * @see JP_Widget_Custom_HTML::render_control_template_scripts()
	 * @param array $instance Current instance.
	 * @returns void
	 */
	public function form( $instance ) {
		$instance = jp_parse_args( (array) $instance, $this->default_instance );
		?>
