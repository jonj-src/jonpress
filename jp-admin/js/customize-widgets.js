...
(function( wp, $ ){
	...
	api.Widgets.WidgetControl = api.Control.extend(/** @lends wp.customize.Widgets.WidgetControl.prototype */{
		 ...
		 * Note that 'widget_form' must match the JP_Widget_Form_Customize_Control::$type
		 *
		 * @since 4.1.0
		 *
		...
		_setupWideWidget: function() {
			...
			$customizeSidebar = $( '.jp-full-overlay-sidebar-content:first' );
		...
		_setupRemoveUI: function() {
			...
			replaceDeleteWithRemove = function() {
				$removeBtn.text( l10n.removeBtnLabel ); // jp_widget_control() outputs the button as "Delete"
	...
	api.Widgets.WidgetsPanel = api.Panel.extend(/** @lends wp.customize.Widgets.WigetsPanel.prototype */{
		...
		ready: function () {
			...
			panel.deferred.embedded.done(function() {
				...
				// @todo This should use the Notifications API introduced to panels. See <https://core.trac.jonpress.org/ticket/38794>.
	 ...
	 *
	 * Customizer control for widgets.
	 * Note that 'sidebar_widgets' must match the JP_Widget_Area_Customize_Control::$type
	 *
	 * @since 3.9.0
	 *
				...
				_( removedWidgetIds ).each( function( removedWidgetId ) {
					...
					setTimeout( function() {
						...
						api.each( function( otherSetting ) {
							if ( otherSetting.id === self.setting.id || 0 !== otherSetting.id.indexOf( 'sidebars_widgets[' ) || otherSetting.id === 'sidebars_widgets[jp_inactive_widgets]' ) {
								return;
							}
						...
						if ( api.Widgets.savedWidgetIds[removedWidgetId] ) {
							inactiveWidgets = api.value( 'sidebars_widgets[jp_inactive_widgets]' )().slice();
							inactiveWidgets.push( removedWidgetId );
							api.value( 'sidebars_widgets[jp_inactive_widgets]' )( _( inactiveWidgets ).unique() );
						}
