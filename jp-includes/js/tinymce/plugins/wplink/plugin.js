( function( tinymce ) {
	tinymce.ui.Factory.add( 'WPLinkPreview', tinymce.ui.Control.extend( {
		...
		renderHtml: function() {
			return (
				'<div id="' + this._id + '" class="jp-link-preview">' +
	...
	tinymce.ui.Factory.add( 'WPLinkInput', tinymce.ui.Control.extend( {
		renderHtml: function() {
			return (
				'<div id="' + this._id + '" class="jp-link-input">' +
	...
	tinymce.PluginManager.add( 'wplink', function( editor ) {
		...
		function removePlaceholders() {
			editor.$( 'a' ).each( function( i, element ) {
				...
				if ( $element.attr( 'href' ) === '_jp_link_placeholder' ) {
					editor.dom.remove( element, true );
				} else if ( $element.attr( 'data-jplink-edit' ) ) {
					$element.attr( 'data-jplink-edit', null );
				}
			});
		}
		...
		function removePlaceholderStrings( content, dataAttr ) {
			return content.replace( /(<a [^>]+>)([\s\S]*?)<\/a>/g, function( all, tag, text ) {
				if ( tag.indexOf( ' href="_jp_link_placeholder"' ) > -1 ) {
					return text;
				}

				if ( dataAttr ) {
					tag = tag.replace( / data-jplink-edit="true"/g, '' );
				}

				tag = tag.replace( / data-jplink-url-error="true"/g, '' );
		...
		editor.on( 'preinit', function() {
			if ( editor.wp && editor.wp._createToolbar ) {
				...
				if ( typeof window.wpLink !== 'undefined' ) {
					editButtons.push( 'jp_link_advanced' );
				}
				...
				editToolbar.on( 'hide', function() {
					if ( ! editToolbar.scrolling ) {
						editor.execCommand( 'jp_link_cancel' );
					}
				} );
			}
		} );

		editor.addCommand( 'JP_Link', function() {
			...
			if ( linkNode ) {
				editor.dom.setAttribs( linkNode, { 'data-jplink-edit': true } );
			} else {
				removePlaceholders();
				editor.execCommand( 'mceInsertLink', false, { href: '_jp_link_placeholder' } );

				linkNode = editor.$( 'a[href="_jp_link_placeholder"]' )[0];
				editor.nodeChanged();
			}
		} );

		editor.addCommand( 'jp_link_apply', function() {
			if ( editToolbar.scrolling ) {
				return;
			}
			...
			if ( linkNode ) {
				...
				editor.dom.setAttribs( linkNode, { href: href, 'data-jplink-edit': null } );
		...
		editor.addCommand( 'jp_link_cancel', function() {
		...
		editor.addCommand( 'jp_unlink', function() {
		...
		editor.addShortcut( 'access+a', '', 'JP_Link' );
		editor.addShortcut( 'access+s', '', 'jp_unlink' );
		// The "de-facto standard" shortcut, see #27305
		editor.addShortcut( 'meta+k', '', 'JP_Link' );
		...
		editor.addButton( 'jp_link_preview', {
			type: 'WPLinkPreview',
		...
		editor.addButton( 'jp_link_input', {
			type: 'WPLinkInput',
			onPostRender: function() {
				...
				if ( $ && $.ui && $.ui.autocomplete ) {
					...
					.autocomplete( {
						...
						focus: function( event, ui ) {
							$input.attr( 'aria-activedescendant', 'mce-jp-autocomplete-' + ui.item.ID );
							/*
						...
						return $( '<li role="option" id="mce-jp-autocomplete-' + item.ID + '">' )
						.append( '<span>' + title + '</span>&nbsp;<span class="jp-editor-float-right">' + item.info + '</span>' )
						.appendTo( ul );
					};
				...
				tinymce.$( input ).on( 'keydown', function( event ) {
					if ( event.keyCode === 13 ) {
						editor.execCommand( 'jp_link_apply' );
						event.preventDefault();
					}
				} );
			}
		...
		editor.on( 'wptoolbar', function( event ) {
			...
			if ( linkNode ) {
				...
				edit = $linkNode.attr( 'data-jplink-edit' );

				if ( href === '_jp_link_placeholder' || edit ) {
					if ( href !== '_jp_link_placeholder' && ! inputInstance.getURL() ) {
					...
					if ( $linkNode.attr( 'data-jplink-url-error' ) === 'true' ) {
						toolbar.$el.find( '.jp-link-preview a' ).addClass( 'wplink-url-error' );
					} else {
						toolbar.$el.find( '.jp-link-preview a' ).removeClass( 'wplink-url-error' );
						hasLinkError = false;
					}
				}
		...
		editor.addButton( 'jp_link_edit', {
			tooltip: 'Edit|button', // '|button' is not displayed, only used for context
			icon: 'dashicon dashicons-edit',
			cmd: 'JP_Link'
		} );

		editor.addButton( 'jp_link_remove', {
			tooltip: 'Remove link',
			icon: 'dashicon dashicons-editor-unlink',
			cmd: 'jp_unlink'
		} );

		editor.addButton( 'jp_link_advanced', {
		...
		editor.addButton( 'jp_link_apply', {
			tooltip: 'Apply',
