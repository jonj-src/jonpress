...
( function( exports, $ ) {
	 ...
	 * JonPress back end and the JonPress media experience.
	 *
	 * @class
	 * @memberOf wp
	 * @alias wp.Uploader
	...
	Uploader = function( options ) {
		...
		for ( key in elements ) {
			...
			if ( ! this[ key ].prop('id') ) {
				this[ key ].prop( 'id', '__jp-uploader-id-' + Uploader.uuid++ );
			}
		...
		this.uploader.bind( 'init', function( uploader ) {
			...
			if ( ! dragdrop ) {
				return dropzone.unbind('.jp-uploader');
			}

			// 'dragenter' doesn't fire correctly, simulate it with a limited 'dragover'.
			dropzone.bind( 'dragover.jp-uploader', function() {
				if ( timer ) {
					clearTimeout( timer );
				}
			...
			dropzone.bind('dragleave.jp-uploader, drop.jp-uploader', function() {
				// Using an instant timer prevents the drag-over class from
				// being quickly removed and re-added when elements inside the
				// dropzone are repositioned.
				//
				// @see https://core.trac.jonpress.org/ticket/21705
	...
	$.extend( Uploader.prototype, /** @lends wp.Uploader.prototype */{
		...
		refresh:  function() {
			...
			if ( this.browser ) {
				...
				if ( ! attached ) {
					id = 'jp-uploader-browser-' + this.uploader.id;

					container = $( '#' + id );
					if ( ! container.length ) {
						container = $('<div class="jp-uploader-browser" />').css({
							position: 'fixed',
							top: '-1000px',
							left: '-1000px',
							height: 0,
							width: 0
						}).attr( 'id', 'jp-uploader-browser-' + this.uploader.id ).appendTo('body');
					}
