...
if ( isset( $_REQUEST['action'] ) && 'adduser' == $_REQUEST['action'] ) {
	...
	$user_details = null;
	$user_email   = jp_unslash( $_REQUEST['email'] );
	if ( false !== strpos( $user_email, '@' ) ) {
		...
		if ( current_user_can( 'manage_network_users' ) ) {
			...
			jp_redirect( add_query_arg( array( 'update' => 'enter_email' ), 'user-new.php' ) );
			die();
		}
	}

	if ( ! $user_details ) {
		jp_redirect( add_query_arg( array( 'update' => 'does_not_exist' ), 'user-new.php' ) );
		die();
	}
	...
	if ( $username != null && array_key_exists( $blog_id, get_blogs_of_user( $user_id ) ) ) {
		...
		if ( isset( $_POST['noconfirmation'] ) && current_user_can( 'manage_network_users' ) ) {
			...
			$newuser_key = jp_generate_password( 20, false );
			add_option(
...
%4$s'
			...
			jp_mail( $new_user_email, sprintf( __( '[%s] Joining confirmation' ), jp_specialchars_decode( get_option( 'blogname' ) ) ), sprintf( $message, get_option( 'blogname' ), home_url(), jp_specialchars_decode( translate_user_role( $role['name'] ) ), home_url( "/newbloguser/$newuser_key/" ) ) );
	...
	if ( ! is_multisite() ) {
		...
		$new_user_email = jp_unslash( $_REQUEST['email'] );
		$user_details   = wpmu_validate_user_signup( $_REQUEST['user_login'], $new_user_email );
		if ( is_jp_error( $user_details['errors'] ) && $user_details['errors']->has_errors() ) {
			 ...
			 */
			$new_user_login = apply_filters( 'pre_user_login', sanitize_user( jp_unslash( $_REQUEST['user_login'] ), true ) );
			if ( isset( $_POST['noconfirmation'] ) && current_user_can( 'manage_network_users' ) ) {
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://codex.jonpress.org/Users_Add_New_Screen">Documentation on Adding New Users</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://jonpress.org/support/">Support Forums</a>' ) . '</p>'
);

jp_enqueue_script( 'jp-ajax-response' );
...
if ( is_multisite() && current_user_can( 'promote_users' ) && ! jp_is_large_network( 'users' )
	...
	jp_enqueue_script( 'user-suggest' );
}
...
if ( isset( $_GET['update'] ) ) {
	...
	if ( is_multisite() ) {
		...
		if ( ( isset( $_GET['user_id'] ) ) ) {
			...
			if ( $user_id_new ) {
				$edit_link = esc_url( add_query_arg( 'jp_http_referer', urlencode( jp_unslash( $_SERVER['REQUEST_URI'] ) ), get_edit_user_link( $user_id_new ) ) );
			}
		}
		...
		switch ( $_GET['update'] ) {
			...
			case 'addexisting':
				...
				break;
			case 'could_not_add':
				$add_user_errors = new JP_Error( 'could_not_add', __( 'That user could not be added to this site.' ) );
				break;
			case 'created_could_not_add':
				$add_user_errors = new JP_Error( 'created_could_not_add', __( 'User has been created, but could not be added to this site.' ) );
				break;
			case 'does_not_exist':
				$add_user_errors = new JP_Error( 'does_not_exist', __( 'The requested user does not exist.' ) );
				break;
			case 'enter_email':
				$add_user_errors = new JP_Error( 'enter_email', __( 'Please enter a valid email address.' ) );
				break;
		}
	} else {
		if ( 'add' == $_GET['update'] ) {
...
<?php if ( isset( $errors ) && is_jp_error( $errors ) ) : ?>
	<div class="error">
		<ul>
		<?php
...
<?php if ( isset( $add_user_errors ) && is_jp_error( $add_user_errors ) ) : ?>
	<div class="error">
		<?php
...
<?php jp_nonce_field( 'add-user', '_wpnonce_add-user' ); ?>

<table class="form-table">
	<tr class="form-field form-required">
		<th scope="row"><label for="adduser-email"><?php echo $label; ?></label></th>
		<td><input name="email" type="<?php echo $type; ?>" id="adduser-email" class="jp-suggest-user" value="" /></td>
	</tr>
	<tr class="form-field">
		<th scope="row"><label for="adduser-role"><?php _e( 'Role' ); ?></label></th>
		<td><select name="role" id="adduser-role">
			<?php jp_dropdown_roles( get_option( 'default_role' ) ); ?>
			</select>
		</td>
	</tr>
...
<?php
	/** This action is documented in jp-admin/user-new.php */
	do_action( 'user_new_form_tag' );
?>
>
<input name="action" type="hidden" value="createuser" />
<?php jp_nonce_field( 'create-user', '_wpnonce_create-user' ); ?>
<?php
// Load up the passed data, else set to a default.
$creating = isset( $_POST['createuser'] );

$new_user_login             = $creating && isset( $_POST['user_login'] ) ? jp_unslash( $_POST['user_login'] ) : '';
$new_user_firstname         = $creating && isset( $_POST['first_name'] ) ? jp_unslash( $_POST['first_name'] ) : '';
$new_user_lastname          = $creating && isset( $_POST['last_name'] ) ? jp_unslash( $_POST['last_name'] ) : '';
$new_user_email             = $creating && isset( $_POST['email'] ) ? jp_unslash( $_POST['email'] ) : '';
$new_user_uri               = $creating && isset( $_POST['url'] ) ? jp_unslash( $_POST['url'] ) : '';
$new_user_role              = $creating && isset( $_POST['role'] ) ? jp_unslash( $_POST['role'] ) : '';
$new_user_send_notification = $creating && ! isset( $_POST['send_user_notification'] ) ? false : true;
$new_user_ignore_pass       = $creating && isset( $_POST['noconfirmation'] ) ? jp_unslash( $_POST['noconfirmation'] ) : '';
...
<?php if ( ! is_multisite() ) { ?>
	...
	<tr class="form-field form-required user-pass1-wrap">
		...
		<td>
			...
			<button type="button" class="button jp-generate-pw hide-if-no-js"><?php _e( 'Show password' ); ?></button>
			<div class="jp-pwd hide-if-js">
				<?php $initial_password = jp_generate_password( 24 ); ?>
				...
				<button type="button" class="button jp-hide-pw hide-if-no-js" data-toggle="0" aria-label="<?php esc_attr_e( 'Hide password' ); ?>">
					<span class="dashicons dashicons-hidden"></span>
					<span class="text"><?php _e( 'Hide' ); ?></span>
				</button>
				<button type="button" class="button jp-cancel-pw hide-if-no-js" data-toggle="0" aria-label="<?php esc_attr_e( 'Cancel password change' ); ?>">
...
<?php } // !is_multisite ?>
	<tr class="form-field">
		...
		<td><select name="role" id="role">
			...
			jp_dropdown_roles( $new_user_role );
			?>
			</select>
		</td>
	</tr>
