...
if ( ! current_user_can( 'manage_sites' ) ) {
	jp_die( __( 'Sorry, you are not allowed to access this page.' ), 403 );
}

$jp_list_table = _get_list_table( 'JP_MS_Sites_List_Table' );
...
if ( isset( $_GET['action'] ) ) {
	/** This action is documented in jp-admin/network/edit.php */
	...
	if ( 'confirm' === $_GET['action'] ) {
		...
		if ( ! array_key_exists( $site_action, $manage_actions ) ) {
			jp_die( __( 'The requested action is not valid.' ) );
		}
		...
		if ( get_network()->site_id == $id ) {
			jp_die( __( 'Sorry, you are not allowed to change the current site.' ) );
		}
		...
		?>
			<div class="wrap">
				...
				<form action="sites.php?action=<?php echo esc_attr( $site_action ); ?>" method="post">
					...
					<input type="hidden" name="_jp_http_referer" value="<?php echo esc_attr( jp_get_referer() ); ?>" />
					<?php jp_nonce_field( $site_action . '_' . $id, '_wpnonce', false ); ?>
	...
	switch ( $_GET['action'] ) {
		...
		case 'deleteblog':
			if ( ! current_user_can( 'delete_sites' ) ) {
				jp_die( __( 'Sorry, you are not allowed to access this page.' ), '', array( 'response' => 403 ) );
			}
		...
		case 'delete_sites':
			...
			foreach ( (array) $_POST['site_ids'] as $site_id ) {
				...
				if ( ! current_user_can( 'delete_site', $site_id ) ) {
					...
					jp_die( sprintf( __( 'Sorry, you are not allowed to delete the site %s.' ), $site_address ), 403 );
				}
							...
							case 'delete':
								...
								<div class="wrap">
									...
									<form action="sites.php?action=delete_sites" method="post">
										...
										<input type="hidden" name="_jp_http_referer" value="<?php echo esc_attr( jp_get_referer() ); ?>" />
										<?php jp_nonce_field( 'ms-delete-sites', '_wpnonce', false ); ?>
	...
	if ( ! empty( $updated_action ) ) {
		jp_safe_redirect( add_query_arg( array( 'updated' => $updated_action ), jp_get_referer() ) );
		exit();
	}
}
...
<div class="wrap">
<h1 class="jp-heading-inline"><?php _e( 'Sites' ); ?></h1>
...
<form method="get" id="ms-search" class="jp-clearfix">
<?php $jp_list_table->search_box( __( 'Search Sites' ), 'site' ); ?>
