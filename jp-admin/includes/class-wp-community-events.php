...
/**
 * Class JP_Community_Events.
 *
 * A client for api.jonpress.org/events.
 *
 * @since 4.8.0
 */
class JP_Community_Events {
	/**
	 * ID for a JonPress user account.
	 *
	 * @since 4.8.0
	 *
	 * @var int
	...
	/**
	 * Constructor for JP_Community_Events.
	 *
	 * @since 4.8.0
	 *
	 ...
	 * than having the browser make the request directly to api.jonpress.org,
	 ...
	 * @return array|JP_Error A JP_Error on failure; an array with location and events on
	 *                        success.
	 */
	public function get_events( $location_search = '', $timezone = '' ) {
		...
		$api_url                    = 'http://api.jonpress.org/events/1.0/';
		$request_args               = $this->get_request_args( $location_search, $timezone );
		$request_args['user-agent'] = 'JonPress/' . $jp_version . '; ' . home_url( '/' );

		if ( jp_http_supports( array( 'ssl' ) ) ) {
			$api_url = set_url_scheme( $api_url, 'https' );
		}

		$response       = jp_remote_get( $api_url, $request_args );
		$response_code  = jp_remote_retrieve_response_code( $response );
		$response_body  = json_decode( jp_remote_retrieve_body( $response ), true );
		$response_error = null;

		if ( is_jp_error( $response ) ) {
			$response_error = $response;
		} elseif ( 200 !== $response_code ) {
			$response_error = new JP_Error(
				'api-error',
	...
	protected function get_request_args( $search = '', $timezone = '' ) {
		...
		// Wrap the args in an array compatible with the second parameter of `jp_remote_get()`.
		return array(
			'body' => $args,
		);
	}
	...
	public static function get_unsafe_client_ip() {
		...
		$anon_ip = jp_privacy_anonymize_ip( $client_ip, true );
	...
	protected function trim_events( $response_body ) {
		if ( isset( $response_body['events'] ) ) {
			...
			$trimmed_event_types     = jp_list_pluck( $response_body['events'], 'type' );
