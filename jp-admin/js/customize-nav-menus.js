...
( function( api, wp, $ ) {
	 ...
	 *
	 * A single available menu item model. See PHP's JP_Customize_Nav_Menu_Item_Setting class.
	 *
	 ...
	 * Note that 'menus' must match the JP_Customize_Menu_Panel::$type.
	 *
	 ...
	 * Note that 'nav_menu' must match the JP_Customize_Menu_Section::$type.
	 *
	 ...
	 * Note that 'nav_menu_location' must match the JP_Customize_Nav_Menu_Location_Control::$type.
	 *
	...
	api.Menus.MenuItemControl = api.Control.extend(/** @lends wp.customize.Menus.MenuItemControl.prototype */{
		 ...
		 * Note that 'menu_item' must match the JP_Customize_Menu_Item_Control::$type.
		 *
	 ...
	 * Note that 'nav_menu' must match the JP_Menu_Customize_Control::$type
	 *
	...
	api.Menus.NewMenuControl = api.Control.extend(/** @lends wp.customize.Menus.NewMenuControl.prototype */{
		 ...
		 * Note that 'new_menu' must match the JP_Customize_New_Menu_Control::$type.
		 *
			...
			if ( 'inserted' === update.status ) {
				...
				api.each( function( setting ) {
					...
					if ( /^nav_menu_locations\[/.test( setting.id ) && setting.get() === update.previous_term_id ) {
						...
						setting._dirty = false; // Not dirty because this is has also just been done on server in JP_Customize_Nav_Menu_Setting::update().
	...
	function displayNavMenuName( name ) {
		name = name || '';
		name = $( '<div>' ).text( name ).html(); // Emulate esc_html() which is used in jp-admin/nav-menus.php.
		name = $.trim( name );
