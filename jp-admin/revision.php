 ...
 *
 * Requires jp-admin/includes/revision.php.
 *
 * @package JonPress
 ...
 */
jp_reset_vars( array( 'revision', 'action', 'from', 'to' ) );
...
switch ( $action ) {
	case 'restore':
		...
		if ( ! jp_revisions_enabled( $post ) && ! jp_is_post_autosave( $revision ) ) {
		...
		jp_restore_post_revision( $revision->ID );
...
jp_enqueue_script( 'revisions' );
jp_localize_script( 'revisions', '_wpRevisionsSettings', jp_prepare_revisions_for_js( $post, $revision_id, $from ) );
...
$revisions_sidebar .= '<p>' . __( '<a href="https://codex.jonpress.org/Revision_Management">Revisions Management</a>' ) . '</p>';
$revisions_sidebar .= '<p>' . __( '<a href="https://jonpress.org/support/">Support Forums</a>' ) . '</p>';
