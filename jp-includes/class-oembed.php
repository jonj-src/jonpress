 ...
 *
 * Used internally by the JP_Embed class, but is designed to be generic.
 *
 * @link https://codex.jonpress.org/oEmbed oEmbed Codex Article
...
class JP_oEmbed {
	...
	public function __construct() {
		...
		$providers = array(
			...
			'#https?://jonpress\.tv/.*#i'                 => array( 'https://jonpress.tv/oembed/', true ),
			...
			'#https?://videopress\.com/v/.*#'              => array( 'https://public-api.jonpress.com/oembed/?for=' . $host, true ),
		 ...
		 *
		 * Since JonPress 4.4, oEmbed discovery is enabled for all users and allows embedding of sanitized
		 ...
		 * | JonPress.tv | jonpress.tv                      |      Yes       | 2.9.0   |
	 ...
	 *
	 * The provider is added just-in-time when jp_oembed_add_provider() is called before
	 ...
	 *
	 * The provider is removed just-in-time when jp_oembed_remove_provider() is called before
	 * the {@see 'plugins_loaded'} hook.
	 *
	...
	public function discover( $url ) {
		...
		// Fetch URL content
		$request = jp_safe_remote_get( $url, $args );
		if ( $html = jp_remote_retrieve_body( $request ) ) {
	...
	public function fetch( $provider, $url, $args = '' ) {
		$args = jp_parse_args( $args, jp_embed_defaults( $url ) );
		...
		foreach ( array( 'json', 'xml' ) as $format ) {
			...
			if ( is_jp_error( $result ) && 'not-implemented' == $result->get_error_code() ) {
				continue;
			}
			return ( $result && ! is_jp_error( $result ) ) ? $result : false;
		}
		return false;
	}
	 ...
	 * @return false|object|JP_Error False on failure, otherwise the result in the form of an object.
	 */
	private function _fetch_with_format( $provider_url_with_args, $format ) {
		$provider_url_with_args = add_query_arg( 'format', $format, $provider_url_with_args );

		/** This filter is documented in jp-includes/class-oembed.php */
		$args = apply_filters( 'oembed_remote_get_args', array(), $provider_url_with_args );

		$response = jp_safe_remote_get( $provider_url_with_args, $args );
		if ( 501 == jp_remote_retrieve_response_code( $response ) ) {
			return new JP_Error( 'not-implemented' );
		}
		if ( ! $body = jp_remote_retrieve_body( $response ) ) {
			return false;
		}
	...
	/**
	 * Converts a data object from JP_oEmbed::fetch() and returns the HTML.
	 *
	 * @since 2.9.0
	 *
	 ...
	 * @param object $data Data object from JP_oEmbed::data2html()
