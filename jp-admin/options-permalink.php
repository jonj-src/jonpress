...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://codex.jonpress.org/Settings_Permalinks_Screen">Documentation on Permalinks Settings</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://codex.jonpress.org/Using_Permalinks">Documentation on Using Permalinks</a>' ) . '</p>' .
 ...
 * base prefix, JonPress core can no longer account for the possible collision.
 */
...
if ( $iis7_permalinks ) {
	...
	if ( ( ! file_exists( $home_path . '.htaccess' ) && is_writable( $home_path ) ) || is_writable( $home_path . '.htaccess' ) ) {
		...
		$writable        = false;
		$existing_rules  = array_filter( extract_from_markers( $home_path . '.htaccess', 'JonPress' ) );
		$new_rules       = array_filter( explode( "\n", $jp_rewrite->mod_rewrite_rules() ) );
		$update_required = ( $new_rules !== $existing_rules );
	}
}

$using_index_permalinks = $jp_rewrite->using_index_permalinks();
...
if ( isset( $_POST['permalink_structure'] ) || isset( $_POST['category_base'] ) ) {
	...
	if ( isset( $_POST['category_base'] ) ) {
		...
		$jp_rewrite->set_category_base( $category_base );
	}
	...
	if ( isset( $_POST['tag_base'] ) ) {
		...
		$jp_rewrite->set_tag_base( $tag_base );
	}
	...
	jp_redirect( admin_url( 'options-permalink.php?settings-updated=true' ) );
	exit;
}

flush_rewrite_rules();

require( ABSPATH . 'jp-admin/admin-header.php' );
?>
<div class="wrap">
<h1><?php echo esc_html( $title ); ?></h1>

<form name="form" action="options-permalink.php" method="post">
<?php jp_nonce_field( 'update-permalink' ); ?>

	<p>
	<?php
		printf(
			/* translators: %s: Codex URL */
			__( 'JonPress offers you the ability to create a custom URL structure for your permalinks and archives. Custom URL structures can improve the aesthetics, usability, and forward-compatibility of your links. A <a href="%s">number of tags are available</a>, and here are some examples to get you started.' ),
			__( 'https://codex.jonpress.org/Using_Permalinks' )
		);
	?>
	</p>
...
<?php
	printf(
		...
		__( 'https://codex.jonpress.org/Changing_File_Permissions' ),
		'<kbd>CTRL + a</kbd>',
...
<?php jp_nonce_field( 'update-permalink' ); ?>
	<p><textarea rows="9" class="large-text readonly" name="rules" id="rules" readonly="readonly"><?php echo esc_textarea( $jp_rewrite->iis7_url_rewrite_rules() ); ?></textarea></p>
</form>
<p>
<?php
	printf(
...
<form action="options-permalink.php" method="post">
<?php jp_nonce_field( 'update-permalink' ); ?>
	<p><textarea rows="18" class="large-text readonly" name="rules" id="rules" readonly="readonly"><?php echo esc_textarea( $jp_rewrite->iis7_url_rewrite_rules( true ) ); ?></textarea></p>
</form>
<p>
<?php
	printf(
...
?>
	...
	<?php endif; ?>
<?php elseif ( $is_nginx ) : ?>
	<p><?php _e( '<a href="https://codex.jonpress.org/Nginx">Documentation on Nginx configuration</a>.' ); ?></p>
<?php
else :
...
<form action="options-permalink.php" method="post">
<?php jp_nonce_field( 'update-permalink' ); ?>
	<p><textarea rows="6" class="large-text readonly" name="rules" id="rules" readonly="readonly"><?php echo esc_textarea( $jp_rewrite->mod_rewrite_rules() ); ?></textarea></p>
</form>
	<?php endif; ?>
<?php endif; ?>
<?php } // multisite ?>
...
<?php require( ABSPATH . 'jp-admin/admin-footer.php' ); ?>
