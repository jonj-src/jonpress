 ...
 * @return JP_Site|void The blog object
 */
 ...
 * @return JP_Post|null JP_Post on success or null on failure
 */
...
function add_user_to_blog( $blog_id, $user_id, $role ) {
	...
	if ( ! $user ) {
		restore_current_blog();
		return new JP_Error( 'user_does_not_exist', __( 'The requested user does not exist.' ) );
	}
	 ...
	 * @since 4.9.0
	 *
	 * @param bool|JP_Error $retval  True if the user should be added to the site, false
	...
	if ( true !== $can_add_user ) {
		...
		if ( is_jp_error( $can_add_user ) ) {
			return $can_add_user;
		}

		return new JP_Error( 'user_cannot_be_added', __( 'User cannot be added to this site.' ) );
	}
	...
	jp_cache_delete( $blog_id . '_user_count', 'blog-details' );
	restore_current_blog();
	return true;
}
...
function remove_user_from_blog( $user_id, $blog_id = '', $reassign = '' ) {
	...
	// jp_revoke_user($user_id);
	$user = get_userdata( $user_id );
	if ( ! $user ) {
		restore_current_blog();
		return new JP_Error( 'user_does_not_exist', __( 'That user does not exist.' ) );
	}
...
function get_blog_id_from_url( $domain, $path = '/' ) {
	...
	$id     = jp_cache_get( md5( $domain . $path ), 'blog-id-cache' );
	...
	if ( ! $id ) {
		jp_cache_set( md5( $domain . $path ), -1, 'blog-id-cache' );
		return 0;
	}

	jp_cache_set( md5( $domain . $path ), $id, 'blog-id-cache' );
 ...
 * at jp-admin/network/settings.php. The check is only run on
 * self-registrations; user creation at jp-admin/network/users.php
 * bypasses this check.
 *
 * @since MU (3.0.0)
 *
...
function wpmu_validate_user_signup( $user_name, $user_email ) {
	 ...
	 *     @type JP_Error $errors        JP_Error object containing any errors found.
	 * }
	 */
 ...
 * the way that JonPress validates new site signups.
 *
 * @since MU (3.0.0)
 *
 * @global wpdb   $wpdb
 ...
 * @param JP_User|string $user       Optional. The user object to check against the new site name.
...
function wpmu_validate_blog_signup( $blogname, $blog_title, $user = '' ) {
	...
	$blog_title = jp_unslash( $blog_title );
	 ...
	 *     @type string|JP_User $user       By default, an empty string. A user object if provided.
	 *     @type JP_Error       $errors     JP_Error containing any errors found.
	 * }
	 */
...
function wpmu_signup_blog( $domain, $path, $title, $user, $user_email, $meta = array() ) {
	...
	$key = substr( md5( time() . jp_rand() . $domain ), 0, 16 );
...
function wpmu_signup_user( $user, $user_email, $meta = array() ) {
	...
	$user_email = sanitize_email( $user_email );
	$key        = substr( md5( time() . jp_rand() . $user_email ), 0, 16 );
...
function wpmu_signup_blog_notification( $domain, $path, $title, $user_login, $user_email, $key, $meta = array() ) {
	...
	if ( ! is_subdomain_install() || get_current_network_id() != 1 ) {
		$activate_url = network_site_url( "jp-activate.php?key=$key" );
	} else {
		$activate_url = "http://{$domain}{$path}jp-activate.php?key=$key"; // @todo use *_url() API
	}
	...
	$from_name       = get_site_option( 'site_name' ) == '' ? 'JonPress' : esc_html( get_site_option( 'site_name' ) );
	...
	$message = sprintf(
		 ...
		 *
		 * Content should be formatted for transmission via jp_mail().
		 *
		 * @since MU (3.0.0)
		 *
	...
	jp_mail( $user_email, jp_specialchars_decode( $subject ), $message, $message_headers );
...
function wpmu_signup_user_notification( $user_login, $user_email, $key, $meta = array() ) {
	...
	$message         = sprintf(
		...
		site_url( "jp-activate.php?key=$key" )
	);
	// TODO: Don't hard code activation link.
	$subject = sprintf(
		/**
 ...
 * @return array|JP_Error An array containing information about the activated user and/or blog
 */
function wpmu_activate_signup( $key ) {
	...
	if ( empty( $signup ) ) {
		return new JP_Error( 'invalid_key', __( 'Invalid activation key.' ) );
	}

	if ( $signup->active ) {
		if ( empty( $signup->domain ) ) {
			return new JP_Error( 'already_active', __( 'The user is already active.' ), $signup );
		} else {
			return new JP_Error( 'already_active', __( 'The site is already active.' ), $signup );
		}
	}

	$meta     = maybe_unserialize( $signup->meta );
	$password = jp_generate_password( 12, false );
	...
	if ( ! $user_id ) {
		return new JP_Error( 'create_user', __( 'Could not create user' ), $signup );
	}
	...
	if ( empty( $signup->domain ) ) {
		...
		if ( isset( $user_already_exists ) ) {
			return new JP_Error( 'user_already_exists', __( 'That username is already activated.' ), $signup );
		}
...
function wpmu_create_user( $user_name, $password, $email ) {
	...
	$user_id = jp_create_user( $user_name, $password, $email );
 ...
 * @return int|JP_Error Returns JP_Error object on failure, the new site ID on success.
 */
function wpmu_create_blog( $domain, $path, $title, $user_id, $meta = array(), $network_id = 1 ) {
	...
	$meta     = jp_parse_args( $meta, $defaults );
	...
	if ( domain_exists( $domain, $path, $network_id ) ) {
		return new JP_Error( 'blog_taken', __( 'Sorry, that site already exists!' ) );
	}
	...
	if ( ! $blog_id = insert_blog( $domain, $path, $network_id ) ) {
		return new JP_Error( 'insert_blog', __( 'Could not create site.' ) );
	}
	...
	jp_cache_set( 'last_changed', microtime(), 'sites' );
...
Disable these notifications: %4$s'
		), $blogname, $siteurl, jp_unslash( $_SERVER['REMOTE_ADDR'] ), $options_site_url
	);
	/**
	...
	jp_mail( $email, sprintf( __( 'New Site Registration: %s' ), $siteurl ), $msg );
	return true;
}
...
Disable these notifications: %3$s'
		), $user->user_login, jp_unslash( $_SERVER['REMOTE_ADDR'] ), $options_site_url
	);
	 ...
	 *
	 * @param string  $msg  Email body.
	 * @param JP_User $user JP_User instance of the new user.
	 */
	$msg = apply_filters( 'newuser_notify_siteadmin', $msg, $user );
	jp_mail( $email, sprintf( __( 'New User Registration: %s' ), $user->user_login ), $msg );
	return true;
}
 ...
 *
 * This function creates a row in the jp_blogs table and returns
...
function insert_blog( $domain, $path, $network_id ) {
	...
	jp_maybe_update_network_site_counts( $network_id );
...
function install_blog( $blog_id, $blog_title = '' ) {
	...
	if ( $wpdb->get_results( "DESCRIBE {$wpdb->posts}" ) ) {
		die( '<h1>' . __( 'Already Installed' ) . '</h1><p>' . __( 'You appear to have already installed JonPress. To reinstall please clear your old database tables first.' ) . '</p></body></html>' );
	}
	...
	update_option( 'blogname', jp_unslash( $blog_title ) );
 ...
 * Set blog defaults.
 *
 * This function creates a row in the jp_blogs table.
 *
 * @since MU (3.0.0)
 * @deprecated MU
 * @deprecated Use jp_install_defaults()
 *
 * @global wpdb $wpdb JonPress database abstraction object.
 *
...
Username: USERNAME
Password: PASSWORD
Log in here: BLOG_URLjp-login.php
...
--The Team @ SITE_NAME'
	...
	if ( empty( $current_network->site_name ) ) {
		$current_network->site_name = 'JonPress';
	}
	 ...
	 */
	$subject = apply_filters( 'update_welcome_subject', sprintf( $subject, $current_network->site_name, jp_unslash( $title ) ) );
	jp_mail( $user->user_email, jp_specialchars_decode( $subject ), $message, $message_headers );
...
function wpmu_welcome_user_notification( $user_id, $password, $meta = array() ) {
	...
	$welcome_email = str_replace( 'USERNAME', $user->user_login, $welcome_email );
	$welcome_email = str_replace( 'PASSWORD', $password, $welcome_email );
	$welcome_email = str_replace( 'LOGINLINK', jp_login_url(), $welcome_email );
 ...
 * @since MU (3.0.0)
 *
 * @global JP_Network $current_site
 *
 * @return JP_Network
 */
 ...
 *
 * JonPress ships with a set of allowed upload filetypes,
 * which is defined in jp-includes/functions.php in
 * get_allowed_mime_types(). This function is used to filter
 * that list against the filetype whitelist provided by Multisite
 * Super Admins at jp-admin/network/settings.php.
 *
 * @since MU (3.0.0)
 *
 * @param array $mimes
 ...
 *
 * JonPress MS stores a blog's post count as an option so as
	...
	if ( $user ) {
		$wpdb->insert(
			$wpdb->registration_log, array(
				...
				'IP'              => preg_replace( '/[^0-9., ]/', '', jp_unslash( $_SERVER['REMOTE_ADDR'] ) ),
				'blog_id'         => $blog_id,
...
function upload_is_file_too_big( $upload ) {
	if ( ! is_array( $upload ) || defined( 'JP_IMPORTING' ) || get_site_option( 'upload_space_check_disabled' ) ) {
		return $upload;
	}
...
function signup_nonce_fields() {
	...
	jp_nonce_field( 'signup_form_' . $id, '_signup_form', false );
}
...
function signup_nonce_check( $result ) {
	if ( ! strpos( $_SERVER['PHP_SELF'], 'jp-signup.php' ) ) {
		return $result;
	}

	if ( ! jp_verify_nonce( $_POST['_signup_form'], 'signup_form_' . $_POST['signup_form_id'] ) ) {
...
function maybe_add_existing_user_to_blog() {
	...
	if ( empty( $details ) || is_jp_error( add_existing_user_to_blog( $details ) ) ) {
		jp_die( sprintf( __( 'An error occurred adding you to this site. Back to the <a href="%s">homepage</a>.' ), home_url() ) );
	}

	jp_die( sprintf( __( 'You have been added to this site. Please visit the <a href="%1$s">homepage</a> or <a href="%2$s">log in</a> using your username and password.' ), home_url(), admin_url() ), __( 'JonPress &rsaquo; Success' ), array( 'response' => 200 ) );
}
...
function add_existing_user_to_blog( $details = false ) {
	if ( is_array( $details ) ) {
		 ...
		 *
		 * @param int   $user_id User ID.
		 * @param mixed $result  True on success or a JP_Error object if the user doesn't exist
 ...
 * @since MU (3.0.0)
 *
 * @param string|JP_User $user Optional. Defaults to current user. JP_User object,
 ...
 * @since 3.1.0
 */
function jp_schedule_update_network_counts() {
	if ( ! is_main_site() ) {
		return;
	}

	if ( ! jp_next_scheduled( 'update_network_counts' ) && ! jp_installing() ) {
		jp_schedule_event( time(), 'twicedaily', 'update_network_counts' );
	}
}
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function jp_update_network_counts( $network_id = null ) {
	jp_update_network_user_counts( $network_id );
	jp_update_network_site_counts( $network_id );
}
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function jp_maybe_update_network_site_counts( $network_id = null ) {
	$is_small_network = ! jp_is_large_network( 'sites', $network_id );
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function jp_maybe_update_network_user_counts( $network_id = null ) {
	$is_small_network = ! jp_is_large_network( 'users', $network_id );

	/** This filter is documented in jp-includes/ms-functions.php */
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function jp_update_network_site_counts( $network_id = null ) {
	$network_id = (int) $network_id;
	if ( ! $network_id ) {
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function jp_update_network_user_counts( $network_id = null ) {
	global $wpdb;
 ...
 * Plugins can alter this criteria using the {@see 'jp_is_large_network'} filter.
 *
 * @since 3.3.0
 * @since 4.8.0 The `$network_id` parameter has been added.
 *
 ...
 */
function jp_is_large_network( $using = 'sites', $network_id = null ) {
	$network_id = (int) $network_id;
	if ( ! $network_id ) {
		$network_id = get_current_network_id();
	}
	...
	if ( 'users' == $using ) {
		 ...
		 */
		return apply_filters( 'jp_is_large_network', $count > 10000, 'users', $count, $network_id );
	}

	$count = get_blog_count( $network_id );
	/** This filter is documented in jp-includes/ms-functions.php */
	return apply_filters( 'jp_is_large_network', $count > 10000, 'sites', $count, $network_id );
}
...
function get_subdirectory_reserved_names() {
	 ...
	 *
	 * @since 3.0.0
	 * @since 4.4.0 'jp-admin', 'jp-content', 'jp-includes', 'jp-json', and 'embed' were added
...
###SITEURL###'
	...
	$content      = str_replace( '###SITENAME###', jp_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES ), $content );
	$content      = str_replace( '###SITEURL###', network_home_url(), $content );

	jp_mail( $value, sprintf( __( '[%s] New Network Admin Email Address' ), jp_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES ) ), $content );
 ...
 */
function jp_network_admin_email_change_notification( $option_name, $new_email, $old_email, $network_id ) {
	$send = true;
...
###SITEURL###'
	...
	// get network name
	$network_name = jp_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES );
