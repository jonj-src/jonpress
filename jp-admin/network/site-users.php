...
if ( ! current_user_can( 'manage_sites' ) ) {
	jp_die( __( 'Sorry, you are not allowed to edit this site.' ), 403 );
}
...
$referer                = remove_query_arg( 'update', jp_get_referer() );
...
if ( ! $id ) {
	jp_die( __( 'Invalid site ID.' ) );
}

$details = get_site( $id );
if ( ! $details ) {
	jp_die( __( 'The requested site does not exist.' ) );
}
...
if ( $action ) {
	...
	jp_safe_redirect( add_query_arg( 'update', $update, $referer ) );
	exit();
}
 ...
 */
if ( ! jp_is_large_network( 'users' ) && apply_filters( 'show_network_site_users_add_existing_form', true ) ) {
...
/** This filter is documented in jp-admin/network/site-users.php */
...
<form action="site-users.php?action=adduser" id="adduser" method="post">
	...
	<table class="form-table">
		<tr>
			...
			<td><input type="text" class="regular-text jp-suggest-user" name="newuser" id="newuser" /></td>
		</tr>
		<tr>
			...
			<?php
			switch_to_blog( $id );
			jp_dropdown_roles( get_option( 'default_role' ) );
			restore_current_blog();
			?>
			</select></td>
		</tr>
...
<form action="<?php echo network_admin_url( 'site-users.php?action=newuser' ); ?>" id="newuser" method="post">
	...
	<?php jp_nonce_field( 'add-user', '_wpnonce_add-new-user' ); ?>
