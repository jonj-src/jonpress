...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://codex.jonpress.org/Network_Admin_Settings_Screen">Documentation on Network Settings</a>' ) . '</p>' .
...
if ( $_POST ) {
	...
	if ( ! empty( $_POST['WPLANG'] ) && current_user_can( 'install_languages' ) && jp_can_install_language_pack() ) {
		$language = jp_download_language_pack( $_POST['WPLANG'] );
		if ( $language ) {
	...
	foreach ( $options as $option_name ) {
		...
		$value = jp_unslash( $_POST[ $option_name ] );
	...
	jp_redirect( add_query_arg( 'updated', 'true', network_admin_url( 'settings.php' ) ) );
	exit();
}
...
<div class="wrap">
	...
	<form method="post" action="settings.php" novalidate="novalidate">
		<?php jp_nonce_field( 'siteoptions' ); ?>
				...
				<td>
					...
					?>
						...
						<?php
							...
							printf(
								' <a href="%1$s">%2$s</a>',
								esc_url( jp_nonce_url( network_admin_url( 'settings.php?dismiss=new_network_admin_email' ), 'dismiss_new_network_admin_email' ) ),
								__( 'Cancel' )
							);
						?>
						</p>
			...
			<tr>
				...
				<td>
					...
					if ( is_subdomain_install() ) {
						...
						/* translators: 1: NOBLOGREDIRECT, 2: jp-config.php */
						printf(
