 ...
 * https://codex.jonpress.org/The_Loop to understand it and
 * https://codex.jonpress.org/Template_Tags to understand
 * the tags used in it.
 *
...
if ( have_posts() ) {
				...
				<?php endif; ?>
					...
					<div class="entry-meta">
						<?php
							...
							if ( jp_attachment_is_image() ) {
								echo ' <span class="meta-sep">|</span> ';
								$metadata = jp_get_attachment_metadata();
								printf(
									__( 'Full size is %s pixels', 'twentyten' ),
									sprintf(
										'<a href="%1$s" title="%2$s">%3$s &times; %4$s</a>',
										esc_url( jp_get_attachment_url() ),
	...
	?>
						<p class="attachment"><a href="<?php echo esc_url( $next_attachment_url ); ?>" title="<?php the_title_attribute(); ?>" rel="attachment">
																	...
																	echo jp_get_attachment_image( $post->ID, array( $attachment_width, $attachment_height ) ); // filterable image width with, essentially, no limit for image height.
							?>
							</a></p>
	...
	<?php else : ?>
						<a href="<?php echo esc_url( jp_get_attachment_url() ); ?>" title="<?php the_title_attribute(); ?>" rel="attachment"><?php echo esc_html( basename( get_permalink() ) ); ?></a>
	<?php endif; ?>
