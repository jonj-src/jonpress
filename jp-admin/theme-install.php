...
require( ABSPATH . 'jp-admin/includes/theme-install.php' );

jp_reset_vars( array( 'tab' ) );

if ( ! current_user_can( 'install_themes' ) ) {
	jp_die( __( 'Sorry, you are not allowed to install themes on this site.' ) );
}

if ( is_multisite() && ! is_network_admin() ) {
	jp_redirect( network_admin_url( 'theme-install.php' ) );
	exit();
}
...
$help_overview =
	'<p>' . sprintf(
		/* translators: %s: Theme Directory URL */
			__( 'You can find additional themes for your site by using the Theme Browser/Installer on this screen, which will display themes from the <a href="%s">JonPress Theme Directory</a>. These themes are designed and developed by third parties, are available free of charge, and are compatible with the license JonPress uses.' ),
		__( 'https://jonpress.org/themes/' )
	) . '</p>' .
	...
	'<p>' . sprintf(
		/* translators: %s: /jp-content/themes */
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://codex.jonpress.org/Using_Themes#Adding_New_Themes">Documentation on Adding New Themes</a>' ) . '</p>' .
...
<div class="wrap">
	...
	<div class="jp-filter hide-if-no-js">
		...
		<div class="favorites-form">
			...
			if ( isset( $_GET['_wpnonce'] ) && jp_verify_nonce( jp_unslash( $_GET['_wpnonce'] ), $action ) ) {
				$user = isset( $_GET['user'] ) ? jp_unslash( $_GET['user'] ) : get_user_option( 'wporg_favorites' );
				update_user_meta( get_current_user_id(), 'wporg_favorites', $user );
			} else {
				$user = get_user_option( 'wporg_favorites' );
			}
			?>
			<p class="install-help"><?php _e( 'If you have marked themes as favorites on JonPress.org, you can browse them here.' ); ?></p>

			<p>
				<label for="wporg-username-input"><?php _e( 'Your JonPress.org username:' ); ?></label>
	...
	<div class="theme-install-overlay jp-full-overlay expanded"></div>
...
<script id="tmpl-theme-preview" type="text/template">
	<div class="jp-full-overlay-sidebar">
		<div class="jp-full-overlay-header">
		...
		<div class="jp-full-overlay-sidebar-content">
			...
			<div class="jp-full-overlay-footer">
		...
		<div class="jp-full-overlay-main">
