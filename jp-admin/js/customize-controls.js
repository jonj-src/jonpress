...
(function( exports, $ ){
	...
	api.Setting = api.Value.extend(/** @lends wp.customize.Setting.prototype */{
		 ...
		 *
		 * A setting is JonPress data (theme mod, option, menu, etc.) that the user can
		 * draft changes to in the Customizer.
		 *
		 * @see PHP class JP_Customize_Setting.
		 *
	...
	Container = api.Class.extend(/** @lends wp.customize~Container.prototype */{
		...
		_animateChangeExpanded: function( completeCallback ) {
			...
			var construct = this,
				...
				overlay = content.closest( '.jp-full-overlay' ),
			...
			_.defer( function() {
				var container = content.closest( '.jp-full-overlay-sidebar-content' ),
	...
	api.Section = Container.extend(/** @lends wp.customize.Section.prototype */{
		...
		onChangeExpanded: function ( expanded, args ) {
			var section = this,
				container = section.headContainer.closest( '.jp-full-overlay-sidebar-content' ),
				content = section.contentContainer,
				overlay = section.headContainer.closest( '.jp-full-overlay' ),
	...
	api.ThemesSection = api.Section.extend(/** @lends wp.customize.ThemesSection.prototype */{
		...
		attachEvents: function () {
			...
			if ( 'local' === section.params.filter_type ) {
				...
				section.container.on( 'input', '.jp-filter-search-themes', function( event ) {
				...
				section.contentContainer.on( 'input', '.jp-filter-search', function() {
				...
				api.section.each( function ( otherSection ) {
					...
					if ( otherSection !== section ) {
						...
						if ( 'themes' === otherSection.params.type ) {
							searchTerm = otherSection.contentContainer.find( '.jp-filter-search' ).val();
							section.contentContainer.find( '.jp-filter-search' ).val( searchTerm );
		...
		loadThemes: function() {
			...
			// Load themes.
			section.headContainer.closest( '.jp-full-overlay' ).addClass( 'loading' );
			...
			request.done(function( data ) {
				...
				section.headContainer.closest( '.jp-full-overlay' ).removeClass( 'loading' );
				section.loading = false;
			});
		...
		checkTerm: function( section ) {
			var newTerm;
			if ( 'remote' === section.params.filter_type ) {
				newTerm = section.contentContainer.find( '.jp-filter-search' ).val();
	...
	api.Panel = Container.extend(/** @lends wp.customize.Panel.prototype */{
		...
		onChangeExpanded: function ( expanded, args ) {
			...
			var panel = this,
				...
				overlay = accordionSection.closest( '.jp-full-overlay' ),
				container = accordionSection.closest( '.jp-full-overlay-sidebar-content' ),
	...
	api.ThemesPanel = api.Panel.extend(/** @lends wp.customize.ThemsPanel.prototype */{
		...
		attachEvents: function() {
			...
			panel.contentContainer.on( 'click', '.customize-themes-section-title, .customize-themes-mobile-back', function() {
				$( '.jp-full-overlay' ).toggleClass( 'showing-themes' );
			});
		...
		onChangeExpanded: function( expanded, args ) {
			...
			overlay = panel.headContainer.closest( '.jp-full-overlay' );
		...
		updateTheme: function( event ) {
			...
			$( document ).one( 'jp-theme-update-success', function( e, response ) {
		...
		deleteTheme: function( event ) {
			...
			$( document ).one( 'jp-theme-delete-success', function() {
	...
	api.Control = api.Class.extend(/** @lends wp.customize.Control.prototype */{
		 ...
		 *
		 * @see PHP class JP_Customize_Control.
		 *
	...
	api.ColorControl = api.Control.extend(/** @lends wp.customize.ColorControl.prototype */{
		ready: function() {
			...
			control.container.on( 'keydown', function( event ) {
				...
				pickerContainer = control.container.find( '.jp-picker-container' );
				if ( pickerContainer.hasClass( 'jp-picker-active' ) ) {
					picker.wpColorPicker( 'close' );
					control.container.find( '.jp-color-result' ).focus();
					event.stopPropagation(); // Prevent section from being collapsed.
				}
			} );
		}
	...
	api.HeaderControl = api.Control.extend(/** @lends wp.customize.HeaderControl.prototype */{
		ready: function() {
			...
			wp.media.controller.Cropper.prototype.defaults.doCropArgs.jp_customize = 'on';
	...
	api.ThemeControl = api.Control.extend(/** @lends wp.customize.ThemeControl.prototype */{
		...
		filter: function( terms ) {
			...
			// Back-compat for behavior in JonPress 4.2.0 to 4.8.X.
			if ( ! _.isArray( terms ) ) {
				terms = [ terms ];
			}
	...
	api.Previewer = api.Messenger.extend(/** @lends wp.customize.Previewer.prototype */{
		...
		initialize: function( params, options ) {
			...
			previewer.add( 'previewUrl', params.previewUrl ).setter( function( to ) {
				...
				// Abort if URL is for admin or (static) files in jp-includes or jp-content.
				if ( /\/jp-(admin|includes|content)(\/|$)/.test( urlParser.pathname ) ) {
					return null;
				}
	...
	$( function() {
		...
		var parent,
			body = $( document.body ),
			overlay = body.children( '.jp-full-overlay' ),
		...
		$.ajaxPrefilter( function injectPreviewNonce( options ) {
			if ( ! /jp_customize=on/.test( options.data ) ) {
				return;
			}
			options.data += '&' + $.param({
		...
		api.bind( 'ready', function setUpGlobalNotificationsArea() {
			...
			sidebar = $( '.jp-full-overlay-sidebar-content' );
		...
		(function initStickyHeaders() {
			var parentContainer = $( '.jp-full-overlay-sidebar-content' ),
		...
		(function() {
			...
			function requestClose() {
				...
				if ( isCleanState() ) {
					...
					$( document ).off( 'visibilitychange.jp-customize-changeset-update' );
					$( window ).off( 'beforeunload.jp-customize-changeset-update' );
		...
		function startAutosaving() {
			...
			$( document ).on( 'visibilitychange.jp-customize-changeset-update', function() {
				if ( document.hidden ) {
					updateChangesetWithReschedule();
				}
			} );
			...
			$( window ).on( 'beforeunload.jp-customize-changeset-update', function() {
				updateChangesetWithReschedule();
			} );
		}
		...
		$( document ).one( 'jp-before-tinymce-init', function() {
