<?php
/**
 * Customize API: JP_Customize_Selective_Refresh class
 *
 * @package JonPress
 ...
 * @since 4.5.0
 */
final class JP_Customize_Selective_Refresh {
	 ...
	 * @since 4.5.0
	 */
	const RENDER_QUERY_VAR = 'jp_customize_render_partials';
	...
	/**
	 * Registered instances of JP_Customize_Partial.
	 *
	 * @since 4.5.0
	 * @var JP_Customize_Partial[]
	 */
	 ...
	 * @since 4.5.0
	 *
	 * @param JP_Customize_Manager $manager Manager instance.
	 */
	public function __construct( JP_Customize_Manager $manager ) {
		$this->manager = $manager;
		require_once( ABSPATH . WPINC . '/customize/class-jp-customize-partial.php' );
	 ...
	 * @since 4.5.0
	 *
	 * @param JP_Customize_Partial|string $id   Customize Partial object, or Panel ID.
	 * @param array                       $args {
	 ...
	 *                                        Callback is called with one argument, the instance of JP_Customize_Partial.
	 ...
	 *                                        false.
	 * }
	 * @return JP_Customize_Partial             The instance of the panel that was added.
	 */
	public function add_partial( $id, $args = array() ) {
		if ( $id instanceof JP_Customize_Partial ) {
			$partial = $id;
		} else {
			$class = 'JP_Customize_Partial';

			/** This filter is documented in jp-includes/customize/class-jp-customize-selective-refresh.php */
	 ...
	 * @return JP_Customize_Partial|null The partial, if set. Otherwise null.
	 */
	...
	public function init_preview() {
		...
		add_action( 'jp_enqueue_scripts', array( $this, 'enqueue_preview_scripts' ) );
	}
	...
	public function enqueue_preview_scripts() {
		jp_enqueue_script( 'customize-selective-refresh' );
		add_action( 'jp_footer', array( $this, 'export_preview_data' ), 1000 );
	}
	...
	public function export_preview_data() {
		...
		// Export data to JS.
		echo sprintf( '<script>var _customizePartialRefreshExports = %s;</script>', jp_json_encode( $exports ) );
	}
	 ...
	 * @since 4.5.0
	 *
	 * @see JP_Customize_Manager::add_dynamic_settings()
	 *
	 * @param string[] $partial_ids Array of the partial IDs to add.
	 * @return JP_Customize_Partial[] Array of added JP_Customize_Partial instances.
	 */
	public function add_dynamic_partials( $partial_ids ) {
		...
		foreach ( $partial_ids as $partial_id ) {
			...
			$partial_args  = false;
			$partial_class = 'JP_Customize_Partial';
			 ...
			 * the JP_Customize_Partial constructor.
			 *
			 * @since 4.5.0
			 *
			 * @param false|array $partial_args The arguments to the JP_Customize_Partial constructor.
			 ...
			 *
			 * Allow non-statically created partials to be constructed with custom JP_Customize_Partial subclass.
			 *
			 * @since 4.5.0
			 *
			 * @param string $partial_class JP_Customize_Partial or a subclass.
			 * @param string $partial_id    ID for dynamic partial.
			 * @param array  $partial_args  The arguments to the JP_Customize_Partial constructor.
			 */
	...
	public function handle_render_partials_request() {
		 ...
		 * JP_Customize_Manager::setup_theme() is where the previewing flag is set.
		 */
		if ( ! is_customize_preview() ) {
			jp_send_json_error( 'expected_customize_preview', 403 );
		} elseif ( ! isset( $_POST['partials'] ) ) {
			jp_send_json_error( 'missing_partials', 400 );
		}

		// Ensure that doing selective refresh on 404 template doesn't result in fallback rendering behavior (full refreshes).
		status_header( 200 );

		$partials = json_decode( jp_unslash( $_POST['partials'] ), true );

		if ( ! is_array( $partials ) ) {
			jp_send_json_error( 'malformed_partials' );
		}
		 ...
		 *
		 * Plugins may do things like call jp_enqueue_scripts() and gather a list of the scripts
		 * and styles which may get enqueued in the response.
		 *
		 * @since 4.5.0
		 *
		 * @param JP_Customize_Selective_Refresh $this     Selective refresh component.
		...
		foreach ( $partials as $partial_id => $container_contexts ) {
			...
			if ( ! is_array( $container_contexts ) ) {
				jp_send_json_error( 'malformed_container_contexts' );
			}
		 ...
		 *
		 * Plugins may do things like call jp_footer() to scrape scripts output and return them
		...
		if ( defined( 'JP_DEBUG_DISPLAY' ) && JP_DEBUG_DISPLAY ) {
		 ...
		 *     @type array $errors   List of errors triggered during rendering of partials, if `JP_DEBUG_DISPLAY`
		 *                           is enabled.
		 * }
		 * @param JP_Customize_Selective_Refresh $this     Selective refresh component.
		 * @param array                          $partials Placements' context data for the partials rendered in the request.
