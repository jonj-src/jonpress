<?php
/**
 * List Table API: JP_Plugin_Install_List_Table class
 *
 * @package JonPress
 ...
 * @see JP_List_Table
 */
class JP_Plugin_Install_List_Table extends JP_List_Table {
	...
	public function prepare_items() {
		include( ABSPATH . 'jp-admin/includes/plugin-install.php' );
		...
		if ( $installed_plugins ) {
			...
			$js_plugins['all'] = array_values( jp_list_pluck( $installed_plugins, 'plugin' ) );
			$upgrade_plugins   = jp_filter_object_list( $installed_plugins, array( 'upgrade' => true ), 'and', 'plugin' );
	...
	public function views() {
		...
		/** This filter is documented in jp-admin/inclues/class-jp-list-table.php */
...
<?php
	...
	public function display() {
		...
		if ( $singular ) {
			$data_attr = " data-jp-lists='list:$singular'";
		}

		$this->display_tablenav( 'top' );

?>
<div class="jp-list-table <?php echo implode( ' ', $this->get_table_classes() ); ?>">
<?php
...
<?php
	...
	public function display_rows() {
		...
		foreach ( (array) $this->items as $plugin ) {
			...
			$title = jp_kses( $plugin['name'], $plugins_allowedtags );

			// Remove any HTML from the description.
			$description = strip_tags( $plugin['short_description'] );
			$version     = jp_kses( $plugin['version'], $plugins_allowedtags );

			$name = strip_tags( $title . ' ' . $version );

			$author = jp_kses( $plugin['author'], $plugins_allowedtags );
			if ( ! empty( $author ) ) {
					...
					case 'newer_installed':
						if ( is_plugin_active( $status['file'] ) ) {
							...
							$activate_url = add_query_arg(
								array(
									'_wpnonce' => jp_create_nonce( 'activate-plugin_' . $status['file'] ),
									'action'   => 'activate',
									'plugin'   => $status['file'],
...
?>
				...
				<div class="column-compatibility">
					...
					if ( ! empty( $plugin['tested'] ) && version_compare( substr( $jp_version, 0, strlen( $plugin['tested'] ) ), $plugin['tested'], '>' ) ) {
						echo '<span class="compatibility-untested">' . __( 'Untested with your version of JonPress' ) . '</span>';
					} elseif ( ! empty( $plugin['requires'] ) && version_compare( substr( $jp_version, 0, strlen( $plugin['requires'] ) ), $plugin['requires'], '<' ) ) {
						echo '<span class="compatibility-incompatible">' . __( '<strong>Incompatible</strong> with your version of JonPress' ) . '</span>';
					} else {
						echo '<span class="compatibility-compatible">' . __( '<strong>Compatible</strong> with your version of JonPress' ) . '</span>';
					}
					?>
				</div>
			</div>
