<?php
/**
 * Facilitates adding of the JonPress editor as used on the Write and Edit screens.
 *
 * @package JonPress
 * @since 3.3.0
 *
 * Private, not included by default. See jp_editor() in jp-includes/general-template.php.
 */

final class _JP_Editors {
	 ...
	 *     @type bool       $dfw               Deprecated in 4.1. Since 4.3 used only to enqueue jp-fullscreen-stub.js
	...
	public static function parse_settings( $editor_id, $settings ) {
		...
		/**
		 * Filters the jp_editor() settings.
		 *
		 * @since 4.0.0
		 *
		 ...
		 * @param string $editor_id ID for the current editor instance.
		 */
		$settings = apply_filters( 'jp_editor_settings', $settings, $editor_id );
		...
		if ( self::$this_tinymce ) {
			if ( false !== strpos( $editor_id, '[' ) ) {
				...
				_deprecated_argument( 'jp_editor()', '3.9.0', 'TinyMCE editor IDs cannot have brackets.' );
			}
		}
		...
		if ( 'content' === $editor_id && empty( $set['tinymce']['jp_autoresize_on'] ) ) {
	 ...
	 * @param array $settings See _JP_Editors::parse_settings() for description.
	 */
	public static function editor( $content, $editor_id, $settings = array() ) {
		$set            = self::parse_settings( $editor_id, $settings );
		$editor_class   = ' class="' . trim( esc_attr( $set['editor_class'] ) . ' jp-editor-area' ) . '"';
		...
		if ( self::$this_tinymce ) {
			...
			if ( self::$this_quicktags ) {
				$default_editor = $set['default_editor'] ? $set['default_editor'] : jp_default_editor();
				// 'html' is used for the "Text" editor tab.
				if ( 'html' !== $default_editor ) {
					$default_editor = 'tinymce';
				}

				$buttons .= '<button type="button" id="' . $editor_id_attr . '-tmce" class="jp-switch-editor switch-tmce"' .
					' data-jp-editor-id="' . $editor_id_attr . '">' . _x( 'Visual', 'Name for the Visual editor tab' ) . "</button>\n";
				$buttons .= '<button type="button" id="' . $editor_id_attr . '-html" class="jp-switch-editor switch-html"' .
					' data-jp-editor-id="' . $editor_id_attr . '">' . _x( 'Text', 'Name for the Text editor tab (formerly HTML)' ) . "</button>\n";
			} else {
				$default_editor = 'tinymce';
			}
		}
		...
		$wrap_class   = 'jp-core-ui jp-editor-wrap ' . $switch_class;

		if ( $set['_content_editor_dfw'] ) {
			$wrap_class .= ' has-dfw';
		}

		echo '<div id="jp-' . $editor_id_attr . '-wrap" class="' . $wrap_class . '">';

		if ( self::$editor_buttons_css ) {
			jp_print_styles( 'editor-buttons' );
		...
		if ( ! empty( $buttons ) || $set['media_buttons'] ) {
			echo '<div id="jp-' . $editor_id_attr . '-editor-tools" class="jp-editor-tools hide-if-no-js">';

			if ( $set['media_buttons'] ) {
				self::$has_medialib = true;

				if ( ! function_exists( 'media_buttons' ) ) {
					include( ABSPATH . 'jp-admin/includes/media.php' );
				}

				echo '<div id="jp-' . $editor_id_attr . '-media-buttons" class="jp-media-buttons">';
			...
			echo '<div class="jp-editor-tabs">' . $buttons . "</div>\n";
			echo "</div>\n";
		}
		...
		$the_editor = apply_filters(
			'the_editor', '<div id="jp-' . $editor_id_attr . '-editor-container" class="jp-editor-container">' .
			$quicktags_toolbar .
	...
	public static function editor_settings( $editor_id, $set ) {
		...
		if ( empty( self::$first_init ) ) {
			if ( is_admin() ) {
				...
				add_action( 'jp_print_footer_scripts', array( __CLASS__, 'editor_js' ), 50 );
				add_action( 'jp_print_footer_scripts', array( __CLASS__, 'enqueue_scripts' ), 1 );
			}
		}
		...
		if ( self::$this_tinymce ) {
			...
			if ( empty( self::$first_init ) ) {
				...
				if ( $set['teeny'] ) {
					 ...
					 * @param string $editor_id Unique editor identifier, e.g. 'content'.
					 */
					$plugins = apply_filters( 'teeny_mce_plugins', array( 'colorpicker', 'lists', 'fullscreen', 'image', 'jonpress', 'wpeditimage', 'wplink' ), $editor_id );
				} else {
					 ...
					 * 'myplugin' => 'http://mysite.com/jp-content/plugins/myfolder/mce_plugin.js'.
					 *
					 ...
					 * in JonPress should be added to the TinyMCE instance.
					 *
					 * @since 3.3.0
					 *
					...
					if ( ! empty( $mce_external_plugins ) ) {
						 ...
						 *
						 * The language file should follow the same format as jp_mce_translation(),
						...
						foreach ( $mce_external_plugins as $name => $url ) {
							...
							if ( ! in_array( $name, $loaded_langs, true ) ) {
								...
								$path = JP_CONTENT_DIR . $path . '/langs/';
				...
				if ( ! empty( $mce_external_plugins ) ) {
					$settings['external_plugins'] = jp_json_encode( $mce_external_plugins );
				}
			...
			if ( $set['teeny'] ) {
				...
				$mce_buttons = array( 'formatselect', 'bold', 'italic', 'bullist', 'numlist', 'blockquote', 'alignleft', 'aligncenter', 'alignright', 'link', 'jp_more', 'spellchecker' );
			...
			if ( $post = get_post() ) {
				...
				if ( post_type_supports( $post->post_type, 'post-formats' ) ) {
					...
					if ( $post_format && ! is_jp_error( $post_format ) ) {
	...
	public static function enqueue_scripts( $default_scripts = false ) {
		if ( $default_scripts || self::$has_tinymce ) {
			jp_enqueue_script( 'editor' );
		}

		if ( $default_scripts || self::$has_quicktags ) {
			jp_enqueue_script( 'quicktags' );
			jp_enqueue_style( 'buttons' );
		}

		if ( $default_scripts || in_array( 'wplink', self::$plugins, true ) || in_array( 'link', self::$qt_buttons, true ) ) {
			jp_enqueue_script( 'wplink' );
			jp_enqueue_script( 'jquery-ui-autocomplete' );
		}

		if ( self::$old_dfw_compat ) {
			jp_enqueue_script( 'jp-fullscreen-stub' );
		}

		if ( self::$has_medialib ) {
			add_thickbox();
			jp_enqueue_script( 'media-upload' );
	...
	public static function enqueue_default_editor() {
		...
		if ( did_action( 'jp_enqueue_editor' ) ) {
			return;
		}

		self::enqueue_scripts( true );

		// Also add jp-includes/css/editor.css
		jp_enqueue_style( 'editor-buttons' );

		if ( is_admin() ) {
			add_action( 'admin_print_footer_scripts', array( __CLASS__, 'print_default_editor_scripts' ), 45 );
		} else {
			add_action( 'jp_print_footer_scripts', array( __CLASS__, 'print_default_editor_scripts' ), 45 );
		}
	}
	...
	public static function print_default_editor_scripts() {
		...
		if ( $user_can_richedit ) {
			...
			// In production all plugins are loaded (they are in jp-editor.js.gz).
			// The 'wpview', 'wpdialogs', and 'media' TinyMCE plugins are not initialized by default.
			// Can be added from js by using the 'jp-before-tinymce-init' event.
	...
	private static function default_settings() {
		...
		$settings = array(
			...
			'formats'                      => '{' .
				'alignleft: [' .
					...
					'{selector: "img,table,dl.jp-caption", classes: "alignleft"}' .
				'],' .
				'aligncenter: [' .
					'{selector: "p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li", styles: {textAlign:"center"}},' .
					'{selector: "img,table,dl.jp-caption", classes: "aligncenter"}' .
				'],' .
				'alignright: [' .
					'{selector: "p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li", styles: {textAlign:"right"}},' .
					'{selector: "img,table,dl.jp-caption", classes: "alignright"}' .
				'],' .
			...
			'entity_encoding'              => 'raw',
			'keep_styles'                  => false,
			'cache_suffix'                 => 'jp-mce-' . $tinymce_version,
			'resize'                       => 'vertical',
			'menubar'                      => false,
			'branding'                     => false,
			...
			'jp_lang_attr'                 => get_bloginfo( 'language' ),
			'jp_keep_scroll_position'      => false,
			'jp_shortcut_labels'           => jp_json_encode( $shortcut_labels ),
		);
		...
		$settings['content_css'] = includes_url( "css/dashicons$suffix.css?$version" ) . ',' .
			includes_url( "js/tinymce/skins/jonpress/jp-content.css?$version" );
	 ...
	 */
	public static function jp_mce_translation( $mce_locale = '', $json_only = false ) {
		if ( ! $mce_locale ) {
			$mce_locale = self::get_mce_locale();
		}
		 ...
		 */
		$mce_translation = apply_filters( 'jp_mce_translation', $mce_translation, $mce_locale );
		...
		if ( $json_only ) {
			return jp_json_encode( $mce_translation );
		}

		$baseurl = self::get_baseurl();

		return "tinymce.addI18n( '$mce_locale', " . jp_json_encode( $mce_translation ) . ");\n" .
	...
	public static function print_tinymce_scripts() {
		...
		// Load tinymce.js when running from /src, else load jp-tinymce.js.gz (production) or tinymce.min.js (SCRIPT_DEBUG)
		$mce_suffix = false !== strpos( get_bloginfo( 'version' ), '-src' ) ? '' : '.min';

		if ( $compressed ) {
			echo "<script type='text/javascript' src='{$baseurl}/jp-tinymce.php?c=1&amp;$version'></script>\n";
		} else {
		...
		echo "<script type='text/javascript'>\n" . self::jp_mce_translation() . "</script>\n";
	}
	...
	public static function editor_js() {
		 ...
		 */
		do_action( 'before_jp_tiny_mce', self::$mce_settings );
		?>
		...
		if ( $tmce_on ) {
			...
			if ( self::$ext_plugins ) {
				...
				echo "<script type='text/javascript' src='{$baseurl}/langs/jp-langs-en.js?$version'></script>\n";
			}
		}
		 ...
		 * @param array $mce_settings TinyMCE settings array.
		 */
		do_action( 'jp_tiny_mce_init', self::$mce_settings );
		...
		( function() {
			...
			if ( typeof tinymce !== 'undefined' ) {
				if ( tinymce.Env.ie && tinymce.Env.ie < 11 ) {
					tinymce.$( '.jp-editor-wrap ' ).removeClass( 'tmce-active' ).addClass( 'html-active' );
					return;
				}

				for ( id in tinyMCEPreInit.mceInit ) {
					init = tinyMCEPreInit.mceInit[id];
					$wrap = tinymce.$( '#jp-' + id + '-wrap' );

					if ( ( $wrap.hasClass( 'tmce-active' ) || ! tinyMCEPreInit.qtInit.hasOwnProperty( id ) ) && ! init.jp_skip_init ) {
						tinymce.init( init );
		 ...
		 * @param array $mce_settings TinyMCE settings array.
		 */
		do_action( 'after_jp_tiny_mce', self::$mce_settings );
	}
	 ...
	 * @deprecated 4.3.0
	 */
	public static function jp_fullscreen_html() {
	 ...
	 */
	public static function jp_link_query( $args = array() ) {
		 ...
		 *
		 * @see JP_Query for a full list of arguments
		 *
		 * @since 3.7.0
		 *
		 * @param array $query An array of JP_Query arguments.
		 */
		$query = apply_filters( 'jp_link_query_args', $query );
		 ...
		 * @param array $query  An array of JP_Query arguments.
		 */
		$results = apply_filters( 'jp_link_query', $results, $query );
	 ...
	 * @since 3.1.0
	 */
	public static function jp_link_dialog() {
		// Run once
		...
		?>
		<div id="jp-link-backdrop" style="display: none"></div>
		<div id="jp-link-wrap" class="jp-core-ui" style="display: none" role="dialog" aria-labelledby="link-modal-title">
		<form id="jp-link" tabindex="-1">
		<?php jp_nonce_field( 'internal-linking', '_ajax_linking_nonce', false ); ?>
		<h1 id="link-modal-title"><?php _e( 'Insert/edit link' ); ?></h1>
		<button type="button" id="jp-link-close"><span class="screen-reader-text"><?php _e( 'Close' ); ?></span></button>
		<div id="link-selector">
			<div id="link-options">
				<p class="howto" id="wplink-enter-url"><?php _e( 'Enter the destination URL' ); ?></p>
				<div>
					<label><span><?php _e( 'URL' ); ?></span>
					<input id="jp-link-url" type="text" aria-describedby="wplink-enter-url" /></label>
				</div>
				<div class="jp-link-text-field">
					<label><span><?php _e( 'Link Text' ); ?></span>
					<input id="jp-link-text" type="text" /></label>
				</div>
				<div class="link-target">
					<label><span></span>
					<input type="checkbox" id="jp-link-target" /> <?php _e( 'Open link in a new tab' ); ?></label>
				</div>
			</div>
			...
			<div id="search-panel">
				<div class="link-search-wrapper">
					<label>
						...
						<input type="search" id="jp-link-search" class="link-search-field" autocomplete="off" aria-describedby="wplink-link-existing-content" />
		...
		<div class="submitbox">
			<div id="jp-link-cancel">
				<button type="button" class="button"><?php _e( 'Cancel' ); ?></button>
			</div>
			<div id="jp-link-update">
				<input type="submit" value="<?php esc_attr_e( 'Add Link' ); ?>" class="button button-primary" id="jp-link-submit" name="jp-link-submit">
			</div>
		</div>
		</form>
		</div>
