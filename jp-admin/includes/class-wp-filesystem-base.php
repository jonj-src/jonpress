...
/**
 * Base JonPress Filesystem class for which Filesystem implementations extend
 *
 * @since 2.5.0
 */
class JP_Filesystem_Base {
	/**
	...
	public function abspath() {
		...
		// Perhaps the FTP folder is rooted at the JonPress install, Check for jp-includes folder in root, Could have some false positives, but rare.
	...
	/**
	 * Return the path on the remote filesystem of JP_CONTENT_DIR.
	 *
	 * @since 2.7.0
	 *
	 * @return string The location of the remote path.
	 */
	public function jp_content_dir() {
		return $this->find_folder( JP_CONTENT_DIR );
	}

	/**
	 * Return the path on the remote filesystem of JP_PLUGIN_DIR.
	 *
	 * @since 2.7.0
	 *
	 * @return string The location of the remote path.
	 */
	public function jp_plugins_dir() {
		return $this->find_folder( JP_PLUGIN_DIR );
	}
	 ...
	 * @return string The location of the remote path.
	 */
	public function jp_themes_dir( $theme = false ) {
	...
	/**
	 * Return the path on the remote filesystem of JP_LANG_DIR.
	 *
	 * @since 3.2.0
	 *
	 * @return string The location of the remote path.
	 */
	public function jp_lang_dir() {
		return $this->find_folder( JP_LANG_DIR );
	}
	 ...
	 *
	 * @since 2.5.0
	 * @deprecated 2.7.0 use JP_Filesystem::abspath() or JP_Filesystem::jp_*_dir() instead.
	 * @see JP_Filesystem::abspath()
	 * @see JP_Filesystem::jp_content_dir()
	 * @see JP_Filesystem::jp_plugins_dir()
	 * @see JP_Filesystem::jp_themes_dir()
	 * @see JP_Filesystem::jp_lang_dir()
	 *
	...
	public function find_base_dir( $base = '.', $echo = false ) {
		_deprecated_function( __FUNCTION__, '2.7.0', 'JP_Filesystem::abspath() or JP_Filesystem::jp_*_dir()' );
		$this->verbose = $echo;
		return $this->abspath();
	}
	 ...
	 *
	 * @since 2.5.0
	 * @deprecated 2.7.0 use JP_Filesystem::abspath() or JP_Filesystem::jp_*_dir() methods instead.
	 * @see JP_Filesystem::abspath()
	 * @see JP_Filesystem::jp_content_dir()
	 * @see JP_Filesystem::jp_plugins_dir()
	 * @see JP_Filesystem::jp_themes_dir()
	...
	public function find_folder( $folder ) {
		...
		if ( stripos( $this->method, 'ftp' ) !== false ) {
			$constant_overrides = array(
				'FTP_BASE'        => ABSPATH,
				'FTP_CONTENT_DIR' => JP_CONTENT_DIR,
				'FTP_PLUGIN_DIR'  => JP_PLUGIN_DIR,
				'FTP_LANG_DIR'    => JP_LANG_DIR,
			);
	...
	public function search_for_folder( $folder, $base = '.', $loop = false ) {
		...
		foreach ( $folder_parts as $index => $key ) {
			...
			/*
			 * Working from /home/ to /user/ to /jonpress/ see if that file exists within
			 * the current folder, If it's found, change into it and follow through looking
			 * for it. If it cant find JonPress down that route, it'll continue onto the next
	 ...
	 * @abstract
	 *
	 * @return bool True on success or false on failure (always true for JP_Filesystem_Direct).
	 */
