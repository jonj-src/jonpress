/**
 * Functions for ajaxified updates, deletions and installs inside the JonPress admin.
 *
 * @version 4.2.0
 */
...
(function( $, wp, settings ) {
	 ...
	 * @type {function}
	 */
	wp.updates.adminNotice = wp.template( 'jp-updates-admin-notice' );
	...
	wp.updates.addAdminNotice = function( data ) {
		...
		$document.trigger( 'jp-updates-notice-added' );
	};

	/**
	 * Handles Ajax requests to JonPress.
	 *
	 * @since 4.6.0
	 *
	...
	wp.updates.refreshCount = function() {
		var $adminBarUpdates              = $( '#jp-admin-bar-updates' ),
	...
	wp.updates.updatePlugin = function( args ) {
		...
		$document.trigger( 'jp-plugin-updating', args );
	...
	wp.updates.updatePluginSuccess = function( response ) {
		...
		$document.trigger( 'jp-plugin-update-success', response );
	};
	...
	wp.updates.updatePluginError = function( response ) {
		...
		$document.trigger( 'jp-plugin-update-error', response );
	};
	 ...
	 *
	 * @param {object}                args         Arguments.
	 * @param {string}                args.slug    Plugin identifier in the JonPress.org Plugin repository.
	...
	wp.updates.installPlugin = function( args ) {
		...
		$document.trigger( 'jp-plugin-installing', args );
	...
	wp.updates.installPluginSuccess = function( response ) {
		...
		$document.trigger( 'jp-plugin-install-success', response );
	...
	wp.updates.installPluginError = function( response ) {
		...
		$document.trigger( 'jp-plugin-install-error', response );
	};
	...
	wp.updates.installImporterSuccess = function( response ) {
		...
		$document.trigger( 'jp-importer-install-success', response );
	};
	...
	wp.updates.installImporterError = function( response ) {
		...
		$document.trigger( 'jp-importer-install-error', response );
	};
	...
	wp.updates.deletePlugin = function( args ) {
		...
		$document.trigger( 'jp-plugin-deleting', args );
	...
	wp.updates.deletePluginSuccess = function( response ) {
		...
		$document.trigger( 'jp-plugin-delete-success', response );
	};
	...
	wp.updates.deletePluginError = function( response ) {
		...
		$document.trigger( 'jp-plugin-delete-error', response );
	};
	...
	wp.updates.updateTheme = function( args ) {
		...
		$document.trigger( 'jp-theme-updating', args );
	...
	wp.updates.updateThemeSuccess = function( response ) {
		...
		$document.trigger( 'jp-theme-update-success', response );
	...
	wp.updates.updateThemeError = function( response ) {
		...
		$document.trigger( 'jp-theme-update-error', response );
	};
	...
	wp.updates.installTheme = function( args ) {
		...
		$document.trigger( 'jp-theme-installing', args );
	...
	wp.updates.installThemeSuccess = function( response ) {
		var $card = $( '.jp-full-overlay-header, [data-slug=' + response.slug + ']' ),
			$message;

		$document.trigger( 'jp-theme-install-success', response );
	...
	wp.updates.installThemeError = function( response ) {
		...
		$document.trigger( 'jp-theme-install-error', response );
	};
	...
	wp.updates.deleteTheme = function( args ) {
		...
		$document.trigger( 'jp-theme-deleting', args );
	...
	wp.updates.deleteThemeSuccess = function( response ) {
		...
		$document.trigger( 'jp-theme-delete-success', response );
	};
	...
	wp.updates.deleteThemeError = function( response ) {
		...
		$document.trigger( 'jp-theme-delete-error', response );
	};
	...
	$( function() {
		var $pluginFilter        = $( '#plugin-filter' ),
			...
			$pluginSearch        = $( '.plugins-php .jp-filter-search' ),
			$pluginInstallSearch = $( '.plugin-install-php .jp-filter-search' );
		 ...
		 *
		 * @see {jp_print_request_filesystem_credentials_modal() in PHP}
		 */
		...
		$bulkActionForm.on( 'click', '[type="submit"]:not([name="clear-recent-list"])', function( event ) {
			...
			$document.trigger( 'jp-' + type + '-bulk-' + bulkAction, itemsSelected );
			...
			$document.on( 'jp-plugin-update-success jp-plugin-update-error jp-theme-update-success jp-theme-update-error', function( event, response ) {
				var $itemRow = $( '[data-slug="' + response.slug + '"]' ),
					$bulkActionNotice, itemName;

				if ( 'jp-' + response.update + '-update-success' === event.type ) {
					success++;
				} else {
				...
				wp.updates.adminNotice = wp.template( 'jp-bulk-updates-admin-notice' );
			...
			$document.on( 'jp-updates-notice-added', function() {
				wp.updates.adminNotice = wp.template( 'jp-updates-admin-notice' );
			} );
		...
		$pluginInstallSearch.on( 'keyup input', _.debounce( function( event, eventtype ) {
			...
			if ( ! $searchTab.length ) {
				...
				$( '.jp-filter .filter-links .current' )
					.removeClass( 'current' )
		...
		$document.on( 'submit', '.search-plugins', function( event ) {
			...
			$( 'input.jp-filter-search' ).trigger( 'input' );
		} );
		...
		$( window ).on( 'message', function( event ) {
			...
			switch ( message.action ) {
				...
				// Called from `jp-admin/includes/class-jp-upgrader-skins.php`.
