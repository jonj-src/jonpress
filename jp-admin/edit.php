...
if ( 'attachment' === $typenow ) {
	if ( jp_redirect( admin_url( 'upload.php' ) ) ) {
		exit;
	}
}
...
$jp_list_table = _get_list_table( 'JP_Posts_List_Table' );
$pagenum       = $jp_list_table->get_pagenum();

// Back-compat for viewing comments of an entry
foreach ( array( 'p', 'attachment_id', 'page_id' ) as $_redirect ) {
	if ( ! empty( $_REQUEST[ $_redirect ] ) ) {
		jp_redirect( admin_url( 'edit-comments.php?p=' . absint( $_REQUEST[ $_redirect ] ) ) );
		exit;
	}
}
unset( $_redirect );
...
if ( $doaction ) {
	...
	$sendback = remove_query_arg( array( 'trashed', 'untrashed', 'deleted', 'locked', 'ids' ), jp_get_referer() );
	if ( ! $sendback ) {
		...
		case 'delete':
			...
			foreach ( (array) $post_ids as $post_id ) {
				...
				if ( $post_del->post_type == 'attachment' ) {
					if ( ! jp_delete_attachment( $post_id ) ) {
						jp_die( __( 'Error in deleting.' ) );
					}
				} else {
					if ( ! jp_delete_post( $post_id ) ) {
						jp_die( __( 'Error in deleting.' ) );
					}
				}
				$deleted++;
	...
	jp_redirect( $sendback );
	exit();
} elseif ( ! empty( $_REQUEST['_jp_http_referer'] ) ) {
...
jp_enqueue_script( 'inline-edit-post' );
...
if ( 'post' == $post_type ) {
	...
	get_current_screen()->set_help_sidebar(
		...
		'<p>' . __( '<a href="https://codex.jonpress.org/Posts_Screen">Documentation on Managing Posts</a>' ) . '</p>' .
	...
	get_current_screen()->set_help_sidebar(
		'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
		'<p>' . __( '<a href="https://codex.jonpress.org/Pages_Screen">Documentation on Managing Pages</a>' ) . '</p>' .
		'<p>' . __( '<a href="https://jonpress.org/support/">Support Forums</a>' ) . '</p>'
	);
...
foreach ( $bulk_counts as $message => $count ) {
	...
	if ( $message == 'trashed' && isset( $_REQUEST['ids'] ) ) {
		...
		$messages[] = '<a href="' . esc_url( jp_nonce_url( "edit.php?post_type=$post_type&doaction=undo&action=untrash&ids=$ids", 'bulk-posts' ) ) . '">' . __( 'Undo' ) . '</a>';
	}
}
...
<?php $jp_list_table->search_box( $post_type_object->labels->search_items, 'post' ); ?>
