...
jp_reset_vars( array( 'action', 'option_page' ) );
...
if ( ! empty( $_GET['adminhash'] ) ) {
	...
	jp_redirect( admin_url( $redirect ) );
	exit;
} elseif ( ! empty( $_GET['dismiss'] ) && 'new_admin_email' == $_GET['dismiss'] ) {
	check_admin_referer( 'dismiss-' . get_current_blog_id() . '-new_admin_email' );
	delete_option( 'adminhash' );
	delete_option( 'new_admin_email' );
	jp_redirect( admin_url( 'options-general.php?updated=true' ) );
	exit;
}
...
if ( ! is_multisite() ) {
	...
	if ( get_option( 'upload_url_path' ) || ( get_option( 'upload_path' ) != 'jp-content/uploads' && get_option( 'upload_path' ) ) ) {
...
if ( 'update' == $action ) {
	...
	if ( ! isset( $whitelist_options[ $option_page ] ) ) {
		jp_die( __( '<strong>ERROR</strong>: options page not found.' ) );
	}

	if ( 'options' == $option_page ) {
		if ( is_multisite() && ! current_user_can( 'manage_network_options' ) ) {
			jp_die( __( 'Sorry, you are not allowed to modify unregistered settings for this site.' ) );
		}
		$options = explode( ',', jp_unslash( $_POST['page_options'] ) );
	} else {
	...
	if ( 'general' == $option_page ) {
		...
		if ( ! empty( $_POST['date_format'] ) && isset( $_POST['date_format_custom'] ) && '\c\u\s\t\o\m' == jp_unslash( $_POST['date_format'] ) ) {
			$_POST['date_format'] = $_POST['date_format_custom'];
		}
		if ( ! empty( $_POST['time_format'] ) && isset( $_POST['time_format_custom'] ) && '\c\u\s\t\o\m' == jp_unslash( $_POST['time_format'] ) ) {
		...
		if ( ! empty( $_POST['WPLANG'] ) && current_user_can( 'install_languages' ) ) {
			...
			if ( jp_can_install_language_pack() ) {
			...
			if ( $unregistered ) {
				_deprecated_argument(
					...
					sprintf(
						...
						__( 'The %s setting is unregistered. Unregistered settings are deprecated. See https://codex.jonpress.org/Settings_API' ),
						'<code>' . $option . '</code>'
					)
				);
			}
	 ...
	 */
	$goback = add_query_arg( 'settings-updated', 'true', jp_get_referer() );
	jp_redirect( $goback );
	exit;
}
...
<div class="wrap">
	...
	<form name="form" action="options.php" method="post" id="all-options">
		<?php jp_nonce_field( 'options-options' ); ?>
