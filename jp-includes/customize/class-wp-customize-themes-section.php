<?php
/**
 * Customize API: JP_Customize_Themes_Section class
 *
 * @package JonPress
 ...
 */
class JP_Customize_Themes_Section extends JP_Customize_Section {
		...
		<li id="accordion-section-{{ data.id }}" class="theme-section">
			...
			<div class="customize-themes-section themes-section-{{ data.id }} control-section-content themes-php">
				...
				<div class="theme-browser rendered">
					...
					<div class="error unexpected-error" style="display: none; "><p><?php _e( 'An unexpected error occurred. Something may be wrong with JonPress.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="https://jonpress.org/support/">support forums</a>.' ); ?></p></div>
					<ul class="themes">
					</ul>
					<p class="no-themes"><?php _e( 'No themes found. Try a different search.' ); ?></p>
					<p class="no-themes-local">
						<?php
						/* translators: %s: "Search JonPress.org themes" button */
						printf(
							__( 'No themes found. Try a different search, or %s.' ),
							sprintf( '<button type="button" class="button-link search-dotorg-themes">%s</button>', __( 'Search JonPress.org themes' ) )
						);
						?>
					</p>
					<p class="spinner"></p>
	...
	protected function filter_bar_content_template() {
		...
		<# if ( 'wporg' === data.action ) { #>
			<div class="search-form">
				<label for="jp-filter-search-input-{{ data.id }}" class="screen-reader-text"><?php _e( 'Search themes&hellip;' ); ?></label>
				<input type="search" id="jp-filter-search-input-{{ data.id }}" placeholder="<?php esc_attr_e( 'Search themes&hellip;' ); ?>" aria-describedby="{{ data.id }}-live-search-desc" class="jp-filter-search">
		...
		<# } else { #>
			<div class="themes-filter-container">
				...
				<input type="search" id="{{ data.id }}-themes-filter" placeholder="<?php esc_attr_e( 'Search themes&hellip;' ); ?>" aria-describedby="{{ data.id }}-live-search-desc" class="jp-filter-search jp-filter-search-themes" />
				<div class="search-icon" aria-hidden="true"></div>
				<span id="{{ data.id }}-live-search-desc" class="screen-reader-text"><?php _e( 'The search results will be updated as you type.' ); ?></span>
			</div>
		<# } #>
