<?php
/**
 * JonPress Network Administration API.
 *
 * @package JonPress
...
function network_step1( $errors = false ) {
	...
	jp_nonce_field( 'install-network-1' );
	...
	<p><?php _e( 'Fill in the information below and you&#8217;ll be on your way to creating a network of JonPress sites. We will create configuration files in the next step.' ); ?></p>
	<?php
	...
	?>
		...
		<p><?php _e( 'Please choose whether you would like sites in your JonPress network to use sub-domains or sub-directories.' ); ?>
...
<?php
	...
	if ( JP_CONTENT_DIR != ABSPATH . 'jp-content' && ( allow_subdirectory_install() || ! allow_subdomain_install() ) ) {
		echo '<div class="error inline"><p><strong>' . __( 'Warning:' ) . '</strong> ' . __( 'Subdirectory networks may not be fully compatible with custom jp-content directories.' ) . '</p></div>';
	}
	...
	if ( $is_www ) :
			...
			<tr>
				...
				<?php
					printf(
						...
						__( 'Because you are using %1$s, the sites in your JonPress network must use sub-directories. Consider using %2$s if you wish to use sub-domains.' ),
						'<code>localhost</code>',
		...
		<?php elseif ( ! allow_subdomain_install() ) : ?>
			<tr>
				...
				<td>
				<?php
					_e( 'Because your installation is in a directory, the sites in your JonPress network must use sub-directories.' );
					// Uh oh:
				if ( ! allow_subdirectory_install() ) {
					echo ' <strong>' . __( 'Warning:' ) . ' ' . __( 'The main site in a sub-directory installation will need to use a modified permalink structure, potentially breaking existing links.' ) . '</strong>';
				}
		...
		<?php elseif ( ! allow_subdirectory_install() ) : ?>
			<tr>
				...
				<td>
				<?php
				_e( 'Because your installation is not new, the sites in your JonPress network must use sub-domains.' );
...
function network_step2( $errors = false ) {
	...
	$jp_siteurl_subdir = preg_replace( '#^' . preg_quote( $home_path, '#' ) . '#', '', $abspath_fix );
	$rewrite_base      = ! empty( $jp_siteurl_subdir ) ? ltrim( trailingslashit( $jp_siteurl_subdir ), '/' ) : '';

	$location_of_jp_config = $abspath_fix;
	if ( ! file_exists( ABSPATH . 'jp-config.php' ) && file_exists( dirname( ABSPATH ) . '/jp-config.php' ) ) {
		$location_of_jp_config = dirname( $abspath_fix );
	}
	$location_of_jp_config = trailingslashit( $location_of_jp_config );
...
?>
	<div class="error"><p><strong><?php _e( 'Warning:' ); ?></strong> <?php _e( 'An existing JonPress network was detected.' ); ?></p></div>
...
?>
		...
		if ( file_exists( $home_path . '.htaccess' ) ) {
			...
			printf(
				/* translators: 1: jp-config.php, 2: .htaccess */
			...
			echo '<strong>' . __( 'Caution:' ) . '</strong> ';
			printf(
				/* translators: 1: jp-config.php, 2: web.config */
				__( 'We recommend you back up your existing %1$s and %2$s files.' ),
				'<code>jp-config.php</code>',
...
?>
		<ol>
			...
			<?php
			printf(
				/* translators: 1: jp-config.php, 2: location of jp-config file, 3: translated version of "That's all, stop editing! Happy blogging." */
				__( 'Add the following to your %1$s file in %2$s <strong>above</strong> the line reading %3$s:' ),
				'<code>jp-config.php</code>',
				'<code>' . $location_of_jp_config . '</code>',
				/*
				 * translators: This string should only be translated if jp-config-sample.php is localized.
				 * You can check the localized release package or
				 * https://i18n.svn.jonpress.org/<locale code>/branches/<wp version>/dist/jp-config-sample.php
				 */
...
if ( ! empty( $keys_salts ) ) {
	$keys_salts_str = '';
	$from_api       = jp_remote_get( 'https://api.jonpress.org/secret-key/1.1/salt/' );
	if ( is_jp_error( $from_api ) ) {
		foreach ( $keys_salts as $c => $v ) {
			$keys_salts_str .= "\ndefine( '$c', '" . jp_generate_password( 64, true, true ) . "' );";
		}
	} else {
		$from_api = explode( "\n", jp_remote_retrieve_body( $from_api ) );
		foreach ( $keys_salts as $c => $v ) {
...
<configuration>
    <system.webServer>
        <rewrite>
            <rules>
                <rule name="JonPress Rule 1" stopProcessing="true">
                    <match url="^index\.php$" ignoreCase="false" />
                    <action type="None" />
                </rule>';
	if ( is_multisite() && get_site_option( 'ms_files_rewriting' ) ) {
		$web_config_file .= '
                <rule name="JonPress Rule for Files" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . 'files/(.+)" ignoreCase="false" />
                    <action type="Rewrite" url="' . $iis_rewrite_base . WPINC . '/ms-files.php?file={R:1}" appendQueryString="false" />
                </rule>';
	}
			$web_config_file .= '
                <rule name="JonPress Rule 2" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . 'jp-admin$" ignoreCase="false" />
                    <action type="Redirect" url="' . $iis_subdir_replacement . 'jp-admin/" redirectType="Permanent" />
                </rule>
                <rule name="JonPress Rule 3" stopProcessing="true">
                ...
                <rule name="JonPress Rule 4" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . '(jp-(content|admin|includes).*)" ignoreCase="false" />
                    <action type="Rewrite" url="' . $iis_rewrite_base . '{R:1}" />
                </rule>
                <rule name="JonPress Rule 5" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . '([_0-9a-zA-Z-]+/)?(.*\.php)$" ignoreCase="false" />
                    <action type="Rewrite" url="' . $iis_rewrite_base . '{R:2}" />
                </rule>
                <rule name="JonPress Rule 6" stopProcessing="true">
                    <match url="." ignoreCase="false" />
...
';
			...
			printf(
				...
				__( 'Add the following to your %1$s file in %2$s, <strong>replacing</strong> other JonPress rules:' ),
				'<code>web.config</code>',
				'<code>' . $home_path . '</code>'
			);
	echo '</p>';
	if ( ! $subdomain_install && JP_CONTENT_DIR != ABSPATH . 'jp-content' ) {
		echo '<p><strong>' . __( 'Warning:' ) . ' ' . __( 'Subdirectory networks may not be fully compatible with custom jp-content directories.' ) . '</strong></p>';
	}
	?>
...
{$ms_files_rewriting}
# add a trailing slash to /jp-admin
RewriteRule ^{$subdir_match}jp-admin$ {$subdir_replacement_01}jp-admin/ [R=301,L]

RewriteCond %{REQUEST_FILENAME} -f [OR]
RewriteCond %{REQUEST_FILENAME} -d
RewriteRule ^ - [L]
RewriteRule ^{$subdir_match}(jp-(content|admin|includes).*) {$rewrite_base}{$subdir_replacement_12} [L]
...
<?php echo esc_textarea( $htaccess_file ); ?></textarea></li>
	...
	if ( ! is_multisite() ) {
		?>
		<p><?php _e( 'Once you complete these steps, your network is enabled and configured. You will have to log in again.' ); ?> <a href="<?php echo esc_url( jp_login_url() ); ?>"><?php _e( 'Log In' ); ?></a></p>
	<?php
	}
}
