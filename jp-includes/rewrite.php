...
function add_rewrite_rule( $regex, $query, $after = 'bottom' ) {
	...
	$jp_rewrite->add_rule( $regex, $query, $after );
}
...
function add_rewrite_tag( $tag, $regex, $query = '' ) {
	...
	$jp_rewrite->add_rewrite_tag( $tag, $regex, $query );
}
...
function remove_rewrite_tag( $tag ) {
	global $jp_rewrite;
	$jp_rewrite->remove_rewrite_tag( $tag );
}
...
function add_permastruct( $name, $struct, $args = array() ) {
	...
	$jp_rewrite->add_permastruct( $name, $struct, $args );
}
 ...
 * @since 4.5.0
 *
 * @see JP_Rewrite::remove_permastruct()
 * @global JP_Rewrite $jp_rewrite JonPress rewrite component.
 *
 * @param string $name Name for permalink structure.
 */
function remove_permastruct( $name ) {
	...
	$jp_rewrite->remove_permastruct( $name );
}
...
function add_feed( $feedname, $function ) {
	...
	if ( ! in_array( $feedname, $jp_rewrite->feeds ) ) {
		$jp_rewrite->feeds[] = $feedname;
	}
...
function add_rewrite_endpoint( $name, $places, $query_var = true ) {
	global $jp_rewrite;
	$jp_rewrite->add_endpoint( $name, $places, $query_var );
}
 ...
 * @return string
 */
function _jp_filter_taxonomy_base( $base ) {
	if ( ! empty( $base ) ) {
 ...
 *
 * Permalinks of posts with numeric slugs can sometimes look to JP_Query::parse_query()
 ...
 * post permalink.
 *
 * Note that, since 4.3.0, jp_unique_post_slug() prevents the creation of post slugs
 ...
 */
function jp_resolve_numeric_slug_conflicts( $query_vars = array() ) {
...
function url_to_postid( $url ) {
	...
	if ( trim( $url, '/' ) === home_url() && 'page' == get_option( 'show_on_front' ) ) {
		...
		if ( $page_on_front && get_post( $page_on_front ) instanceof JP_Post ) {
	...
	if ( ! $jp_rewrite->using_index_permalinks() ) {
		$url = str_replace( $jp_rewrite->index . '/', '', $url );
	}
	...
	foreach ( (array) $rewrite as $match => $query ) {
		...
		if ( preg_match( "#^$match#", $request_match, $matches ) ) {
			...
			foreach ( (array) $query_vars as $key => $value ) {
				if ( in_array( $key, $jp->public_query_vars ) ) {
					$query[ $key ] = $value;
			...
			$query = jp_resolve_numeric_slug_conflicts( $query );
