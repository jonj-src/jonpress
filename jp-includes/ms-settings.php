 ...
 *
 * Allows for some configuration in jp-config.php (see ms-default-constants.php)
 *
 * @package JonPress
...
/** JP_Network class */
require_once( ABSPATH . WPINC . '/class-jp-network.php' );

/** JP_Site class */
require_once( ABSPATH . WPINC . '/class-jp-site.php' );
...
if ( defined( 'SUNRISE' ) ) {
	include_once( JP_CONTENT_DIR . '/sunrise.php' );
}
...
if ( ! isset( $current_site ) || ! isset( $current_blog ) ) {
	...
	if ( is_admin() ) {
		$path = preg_replace( '#(.*)/jp-admin/.*#', '$1/', $path );
	}
	...
	jp_load_core_site_options( $site_id );
}

$wpdb->set_prefix( $table_prefix, false ); // $table_prefix can be set in sunrise.php
$wpdb->set_blog_id( $current_blog->blog_id, $current_blog->site_id );
$table_prefix       = $wpdb->get_blog_prefix();
$_jp_switched_stack = array();
$switched           = false;

// need to init cache again after blog_id is set
jp_start_object_cache();

if ( ! $current_site instanceof JP_Network ) {
	$current_site = new JP_Network( $current_site );
}

if ( ! $current_blog instanceof JP_Site ) {
	$current_blog = new JP_Site( $current_blog );
}
