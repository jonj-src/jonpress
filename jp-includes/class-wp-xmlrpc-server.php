...
/**
 * JonPress XMLRPC server implementation.
 *
 * Implements compatibility for Blogger API, MetaWeblog API, MovableType, and
 * pingback. Additional JonPress API for managing comments, pages, posts,
 * options, etc.
 *
 * As of JonPress 3.5.0, XML-RPC is enabled by default. It can be disabled
 * via the {@see 'xmlrpc_enabled'} filter found in jp_xmlrpc_server::login().
 *
 * @since 1.5.0
 *
 * @see IXR_Server
 */
class jp_xmlrpc_server extends IXR_Server {
	/**
	 * Methods.
	 *
	 * @var array
	...
	/**
	 * Flags that the user authentication has failed in this instance of jp_xmlrpc_server.
	 *
	 * @var bool
	 */
	...
	public function __construct() {
		$this->methods = array(
			// JonPress API
			'wp.getUsersBlogs'                 => 'this:jp_getUsersBlogs',
			'wp.newPost'                       => 'this:jp_newPost',
			'wp.editPost'                      => 'this:jp_editPost',
			'wp.deletePost'                    => 'this:jp_deletePost',
			'wp.getPost'                       => 'this:jp_getPost',
			'wp.getPosts'                      => 'this:jp_getPosts',
			'wp.newTerm'                       => 'this:jp_newTerm',
			'wp.editTerm'                      => 'this:jp_editTerm',
			'wp.deleteTerm'                    => 'this:jp_deleteTerm',
			'wp.getTerm'                       => 'this:jp_getTerm',
			'wp.getTerms'                      => 'this:jp_getTerms',
			'wp.getTaxonomy'                   => 'this:jp_getTaxonomy',
			'wp.getTaxonomies'                 => 'this:jp_getTaxonomies',
			'wp.getUser'                       => 'this:jp_getUser',
			'wp.getUsers'                      => 'this:jp_getUsers',
			'wp.getProfile'                    => 'this:jp_getProfile',
			'wp.editProfile'                   => 'this:jp_editProfile',
			'wp.getPage'                       => 'this:jp_getPage',
			'wp.getPages'                      => 'this:jp_getPages',
			'wp.newPage'                       => 'this:jp_newPage',
			'wp.deletePage'                    => 'this:jp_deletePage',
			'wp.editPage'                      => 'this:jp_editPage',
			'wp.getPageList'                   => 'this:jp_getPageList',
			'wp.getAuthors'                    => 'this:jp_getAuthors',
			'wp.getCategories'                 => 'this:mw_getCategories',     // Alias
			'wp.getTags'                       => 'this:jp_getTags',
			'wp.newCategory'                   => 'this:jp_newCategory',
			'wp.deleteCategory'                => 'this:jp_deleteCategory',
			'wp.suggestCategories'             => 'this:jp_suggestCategories',
			'wp.uploadFile'                    => 'this:mw_newMediaObject',    // Alias
			'wp.deleteFile'                    => 'this:jp_deletePost',        // Alias
			'wp.getCommentCount'               => 'this:jp_getCommentCount',
			'wp.getPostStatusList'             => 'this:jp_getPostStatusList',
			'wp.getPageStatusList'             => 'this:jp_getPageStatusList',
			'wp.getPageTemplates'              => 'this:jp_getPageTemplates',
			'wp.getOptions'                    => 'this:jp_getOptions',
			'wp.setOptions'                    => 'this:jp_setOptions',
			'wp.getComment'                    => 'this:jp_getComment',
			'wp.getComments'                   => 'this:jp_getComments',
			'wp.deleteComment'                 => 'this:jp_deleteComment',
			'wp.editComment'                   => 'this:jp_editComment',
			'wp.newComment'                    => 'this:jp_newComment',
			'wp.getCommentStatusList'          => 'this:jp_getCommentStatusList',
			'wp.getMediaItem'                  => 'this:jp_getMediaItem',
			'wp.getMediaLibrary'               => 'this:jp_getMediaLibrary',
			'wp.getPostFormats'                => 'this:jp_getPostFormats',
			'wp.getPostType'                   => 'this:jp_getPostType',
			'wp.getPostTypes'                  => 'this:jp_getPostTypes',
			'wp.getRevisions'                  => 'this:jp_getRevisions',
			'wp.restoreRevision'               => 'this:jp_restoreRevision',
	 ...
	 * @return JP_User|bool JP_User object if authentication passed, false otherwise
	 */
	public function login( $username, $password ) {
		...
		if ( $this->auth_failed ) {
			$user = new JP_Error( 'login_prevented' );
		} else {
			$user = jp_authenticate( $username, $password );
		}

		if ( is_jp_error( $user ) ) {
			$this->error = new IXR_Error( 403, __( 'Incorrect username or password.' ) );

			// Flag that authentication has failed once on this jp_xmlrpc_server instance
	 ...
	 *
	 * @since 1.5.0
	 * @deprecated 2.8.0 Use jp_xmlrpc_server::login()
	 * @see jp_xmlrpc_server::login()
	 *
	 * @param string $username User's username.
	 * @param string $password User's password.
	...
	public function set_custom_fields( $post_id, $fields ) {
		...
		foreach ( (array) $fields as $meta ) {
			if ( isset( $meta['id'] ) ) {
				...
				if ( isset( $meta['key'] ) ) {
					$meta['key'] = jp_unslash( $meta['key'] );
					if ( $meta['key'] !== $pmeta->meta_key ) {
						continue;
					}
					$meta['value'] = jp_unslash( $meta['value'] );
			...
			} elseif ( current_user_can( 'add_post_meta', $post_id, jp_unslash( $meta['key'] ) ) ) {
	 ...
	 */
	public function jp_getUsersBlogs( $args ) {
	...
	protected function _prepare_taxonomy( $taxonomy, $fields ) {
		 ...
		 * @param JP_Taxonomy $taxonomy  Taxonomy object.
		 * @param array       $fields    The subset of taxonomy fields to return.
		 */
	...
	/**
	 * Convert a JonPress date string to an IXR_Date object.
	 *
	...
	/**
	 * Convert a JonPress GMT date string to an IXR_Date object.
	 *
	 * @param string $date_gmt JonPress GMT date string.
	...
	protected function _prepare_post( $post, $fields ) {
		...
		if ( $all_taxonomy_fields || in_array( 'terms', $fields ) ) {
			...
			$terms                = jp_get_object_terms( $post['ID'], $post_type_taxonomies );
			$_post['terms']       = array();
	 ...
	 *
	 * @param JP_Post_Type $post_type Post type object.
	...
	protected function _prepare_media_item( $media_item, $thumbnail_size = 'thumbnail' ) {
		$_media_item = array(
			...
			'link'             => jp_get_attachment_url( $media_item->ID ),
			'title'            => $media_item->post_title,
			'caption'          => $media_item->post_excerpt,
			'description'      => $media_item->post_content,
			'metadata'         => jp_get_attachment_metadata( $media_item->ID ),
	...
	protected function _prepare_page( $page ) {
		...
		if ( is_object_in_taxonomy( 'page', 'category' ) ) {
			foreach ( jp_get_post_categories( $page->ID ) as $cat_id ) {
		...
		$_page = array(
			...
			'jp_slug'                => $page->post_name,
			'jp_password'            => $page->post_password,
			'jp_author'              => $author->display_name,
			'jp_page_parent_id'      => $page->post_parent,
			'jp_page_parent_title'   => $parent_title,
			'jp_page_order'          => $page->menu_order,
			'jp_author_id'           => (string) $author->ID,
			'jp_author_display_name' => $author->display_name,
			'date_created_gmt'       => $page_date_gmt,
			'custom_fields'          => $this->get_custom_fields( $page->ID ),
			'jp_page_template'       => $page_template,
		);
	 ...
	 *
	 * @param JP_User $user   The unprepared user object.
	 ...
	 *         Content struct for adding a new post. See jp_insert_post() for information on
	 ...
	 */
	public function jp_newPost( $args ) {
		...
		/** This action is documented in jp-includes/class-jp-xmlrpc-server.php */
	...
	private function _toggle_sticky( $post_data, $update = false ) {
		...
		if ( 'private' === $post_data['post_status'] || ! empty( $post_data['post_password'] ) ) {
			...
			$sticky = jp_validate_boolean( $post_data['sticky'] );
			if ( $sticky ) {
	...
	/**
	 * Helper method for jp_newPost() and jp_editPost(), containing shared logic.
	 *
	 * @since 3.4.0
	 *
	 * @see jp_insert_post()
	 *
	 * @param JP_User         $user           The post author if post_author isn't set in $content_struct.
	...
	protected function _insert_post( $user, $content_struct ) {
		...
		$post_data = jp_parse_args( array_intersect_key( $content_struct, $defaults ), $defaults );
				...
				foreach ( $taxonomies as $taxonomy ) {
					...
					foreach ( $term_names as $term_name ) {
						...
						if ( ! $term ) {
							...
							// Create the new term.
							$term_info = jp_insert_term( $term_name, $taxonomy );
		 ...
		 */
		$post_data = apply_filters( 'xmlrpc_jp_insert_post_data', $post_data, $content_struct );

		$post_ID = $update ? jp_update_post( $post_data, true ) : jp_insert_post( $post_data, true );
	 ...
	 */
	public function jp_editPost( $args ) {
	 ...
	 */
	public function jp_deletePost( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
	 ...
	 *  - 'enclosure'
	 */
	public function jp_getPost( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
	 ...
	 * @since 3.4.0
	 *
	 * @see jp_get_recent_posts()
	 * @see jp_getPost() for more on `$fields`
	 ...
	 */
	public function jp_getPosts( $args ) {
		...
		$posts_list = jp_get_recent_posts( $query );
	 ...
	 */
	public function jp_newTerm( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		$term = jp_insert_term( $term_data['name'], $taxonomy['name'], $term_data );
	 ...
	 * @return true|IXR_Error True on success, IXR_Error instance on failure.
	 */
	public function jp_editTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) ) {
			return $this->error;
		}
		...
		$term = jp_update_term( $term_id, $taxonomy['name'], $term_data );
	 ...
	 */
	public function jp_deleteTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) ) {
			return $this->error;
		}
		...
		$result = jp_delete_term( $term_id, $taxonomy->name );
	 ...
	 *  - 'count'
	 */
	public function jp_getTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) ) {
			return $this->error;
		}
	 ...
	 */
	public function jp_getTerms( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
	 ...
	 */
	public function jp_getTaxonomy( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
	 ...
	 */
	public function jp_getTaxonomies( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
	 ...
	 *  - 'roles'
	 */
	public function jp_getUser( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
	 ...
	 *
	 * @uses get_users()
	 * @see jp_getUser() for more on $fields and return values
	 *
	 * @param array  $args {
	 *     Method arguments. Note: arguments must be ordered as documented.
	 *
	 ...
	 */
	public function jp_getUsers( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
	 ...
	 *     @type array  $fields (optional)
	 * }
	 * @return array|IXR_Error (@see jp_getUser)
	 */
	public function jp_getProfile( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
	 ...
	 */
	public function jp_editProfile( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		$result = jp_update_user( $user_data );
	 ...
	 */
	public function jp_getPage( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function jp_getPages( $args ) {
		$this->escape( $args );
	 ...
	 * @since 2.2.0
	 *
	 * @see jp_xmlrpc_server::mw_newPost()
	 *
	 * @param array  $args {
	 *     Method arguments. Note: arguments must be ordered as documented.
	 *
	 ...
	 */
	public function jp_newPage( $args ) {
	 ...
	 */
	public function jp_deletePage( $args ) {
		$this->escape( $args );
		 ...
		 */
		do_action( 'xmlrpc_call_success_jp_deletePage', $page_id, $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function jp_editPage( $args ) {
	 ...
	 * @return array|IXR_Error
	 */
	public function jp_getPageList( $args ) {
		global $wpdb;
	 ...
	 * @return array|IXR_Error
	 */
	public function jp_getAuthors( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function jp_getTags( $args ) {
		$this->escape( $args );
	 ...
	 */
	public function jp_newCategory( $args ) {
		$this->escape( $args );
		...
		$cat_id = jp_insert_category( $new_category, true );
		if ( is_jp_error( $cat_id ) ) {
		 ...
		 */
		do_action( 'xmlrpc_call_success_jp_newCategory', $cat_id, $args );
	 ...
	 * }
	 * @return bool|IXR_Error See jp_delete_term() for return info.
	 */
	public function jp_deleteCategory( $args ) {
		$this->escape( $args );
		...
		$status = jp_delete_term( $category_id, 'category' );
		...
		if ( true == $status ) {
			 ...
			 */
			do_action( 'xmlrpc_call_success_jp_deleteCategory', $category_id, $args );
		}
	 ...
	 * @return array|IXR_Error
	 */
	public function jp_suggestCategories( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function jp_getComment( $args ) {
		$this->escape( $args );
	 ...
	 * - 'offset' - Default is 0. See JP_Query::query() for more.
	 *
	 * @since 2.7.0
	 *
	 * @param array  $args {
	 ...
	 *     @type array  $struct
	 * }
	 * @return array|IXR_Error Contains a collection of comments. See jp_xmlrpc_server::jp_getComment() for a description of each item contents
	 */
	public function jp_getComments( $args ) {
		$this->escape( $args );
	 ...
	 * See jp_delete_comment() for more information on this behavior.
	 *
	 * @since 2.7.0
	 *
	 * @param array  $args {
	 ...
	 *     @type int    $comment_ID
	 * }
	 * @return bool|IXR_Error See jp_delete_comment().
	 */
	public function jp_deleteComment( $args ) {
		$this->escape( $args );
		...
		$status = jp_delete_comment( $comment_ID );
		...
		if ( $status ) {
			 ...
			 */
			do_action( 'xmlrpc_call_success_jp_deleteComment', $comment_ID, $args );
		}
	 ...
	 * @return true|IXR_Error True, on success.
	 */
	public function jp_editComment( $args ) {
		$this->escape( $args );
		...
		$result = jp_update_comment( $comment );
		if ( is_jp_error( $result ) ) {
			return new IXR_Error( 500, $result->get_error_message() );
		}
		 ...
		 */
		do_action( 'xmlrpc_call_success_jp_editComment', $comment_ID, $args );
	 ...
	 *     @type array      $content_struct
	 * }
	 * @return int|IXR_Error See jp_new_comment().
	 */
	public function jp_newComment( $args ) {
		$this->escape( $args );
		...
		$comment_ID = jp_new_comment( $comment, true );
		 ...
		 */
		do_action( 'xmlrpc_call_success_jp_newComment', $comment_ID, $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function jp_getCommentStatusList( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function jp_getCommentCount( $args ) {
		$this->escape( $args );
		...
		$count = jp_count_comments( $post_id );
	 ...
	 * @return array|IXR_Error
	 */
	public function jp_getPostStatusList( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function jp_getPageStatusList( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function jp_getPageTemplates( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function jp_getOptions( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function jp_setOptions( $args ) {
		$this->escape( $args );
		...
		foreach ( $options as $o_name => $o_value ) {
			...
			update_option( $this->blog_options[ $o_name ]['option'], jp_unslash( $o_value ) );
		}
	 ...
	 *  - 'metadata'
	 */
	public function jp_getMediaItem( $args ) {
		$this->escape( $args );
	 ...
	 *     @type array  $struct
	 * }
	 * @return array|IXR_Error Contains a collection of media items. See jp_xmlrpc_server::jp_getMediaItem() for a description of each item contents
	 */
	public function jp_getMediaLibrary( $args ) {
		$this->escape( $args );
	 ...
	 */
	public function jp_getPostFormats( $args ) {
		$this->escape( $args );
	 ...
	 *  - 'supports'
	 */
	public function jp_getPostType( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
	 ...
	 * @return array|IXR_Error
	 */
	public function jp_getPostTypes( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
	 ...
	 */
	public function jp_getRevisions( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		if ( ! jp_revisions_enabled( $post ) ) {
			return new IXR_Error( 401, __( 'Sorry, revisions are disabled.' ) );
		}

		$revisions = jp_get_post_revisions( $post_id );
		...
		foreach ( $revisions as $revision ) {
			...
			// Skip autosaves
			if ( jp_is_post_autosave( $revision ) ) {
				continue;
			}
	 ...
	 */
	public function jp_restoreRevision( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
		...
		if ( ! $revision = jp_get_post_revision( $revision_id ) ) {
			return new IXR_Error( 404, __( 'Invalid post ID.' ) );
		}
		...
		$post = jp_restore_post_revision( $revision_id );
	...
	public function blogger_getPost( $args ) {
		...
		$categories = implode( ',', jp_get_post_categories( $post_ID ) );

		$content  = '<title>' . jp_unslash( $post_data['post_title'] ) . '</title>';
		$content .= '<category>' . $categories . '</category>';
		$content .= jp_unslash( $post_data['post_content'] );
	...
	public function blogger_getRecentPosts( $args ) {
		...
		foreach ( $posts_list as $entry ) {
			...
			$categories = implode( ',', jp_get_post_categories( $entry['ID'] ) );

			$content  = '<title>' . jp_unslash( $entry['post_title'] ) . '</title>';
			$content .= '<category>' . $categories . '</category>';
			$content .= jp_unslash( $entry['post_content'] );
	...
	public function blogger_newPost( $args ) {
		...
		$post_ID = jp_insert_post( $post_data );
		if ( is_jp_error( $post_ID ) ) {
			return new IXR_Error( 500, $post_ID->get_error_message() );
		}
	...
	public function blogger_editPost( $args ) {
		...
		$result = jp_update_post( $postdata );
	...
	public function mw_newPost( $args ) {
		...
		if ( ! empty( $content_struct['post_type'] ) ) {
			if ( $content_struct['post_type'] == 'page' ) {
				...
				$post_type     = 'page';
				if ( ! empty( $content_struct['jp_page_template'] ) ) {
					$page_template = $content_struct['jp_page_template'];
				}
		...
		if ( isset( $content_struct['jp_post_format'] ) ) {
			$content_struct['jp_post_format'] = sanitize_key( $content_struct['jp_post_format'] );
			if ( ! array_key_exists( $content_struct['jp_post_format'], get_post_format_strings() ) ) {
				return new IXR_Error( 404, __( 'Invalid post format.' ) );
			}
		}

		// Let JonPress generate the post_name (slug) unless
		// one has been provided.
		$post_name = '';
		if ( isset( $content_struct['jp_slug'] ) ) {
			$post_name = $content_struct['jp_slug'];
		}

		// Only use a password if one was given.
		if ( isset( $content_struct['jp_password'] ) ) {
			$post_password = $content_struct['jp_password'];
		}

		// Only set a post parent if one was provided.
		if ( isset( $content_struct['jp_page_parent_id'] ) ) {
			$post_parent = $content_struct['jp_page_parent_id'];
		}

		// Only set the menu_order if it was provided.
		if ( isset( $content_struct['jp_page_order'] ) ) {
			$menu_order = $content_struct['jp_page_order'];
		}

		$post_author = $user->ID;

		// If an author id was provided then use it instead.
		if ( isset( $content_struct['jp_author_id'] ) && ( $user->ID != $content_struct['jp_author_id'] ) ) {
			switch ( $post_type ) {
				case 'post':
			...
			$author = get_userdata( $content_struct['jp_author_id'] );
			if ( ! $author ) {
				return new IXR_Error( 404, __( 'Invalid author ID.' ) );
			}
			$post_author = $content_struct['jp_author_id'];
		}
		...
		if ( isset( $content_struct['jp_post_thumbnail'] ) ) {
			if ( set_post_thumbnail( $post_ID, $content_struct['jp_post_thumbnail'] ) === false ) {
				return new IXR_Error( 404, __( 'Invalid attachment ID.' ) );
			}

			unset( $content_struct['jp_post_thumbnail'] );
		}
		...
		// in this function
		if ( isset( $content_struct['jp_post_format'] ) ) {
			set_post_format( $post_ID, $content_struct['jp_post_format'] );
		}

		$post_ID = jp_insert_post( $postdata, true );
		if ( is_jp_error( $post_ID ) ) {
			return new IXR_Error( 500, $post_ID->get_error_message() );
		}
	...
	public function mw_editPost( $args ) {
		...
		// Let JonPress manage slug if none was provided.
		...
		$page_template = null;
		if ( ! empty( $content_struct['jp_page_template'] ) && 'page' == $post_type ) {
			$page_template = $content_struct['jp_page_template'];
		}
		...
		if ( isset( $content_struct['jp_author_id'] ) ) {
			// Check permissions if attempting to switch author to or from another user.
			if ( $user->ID != $content_struct['jp_author_id'] || $user->ID != $post_author ) {
				switch ( $post_type ) {
					case 'post':
						if ( ! current_user_can( 'edit_others_posts' ) ) {
		...
		$result = jp_update_post( $newpost, true );
		if ( is_jp_error( $result ) ) {
			return new IXR_Error( 500, $result->get_error_message() );
		}
		...
		if ( isset( $content_struct['jp_post_thumbnail'] ) ) {
			...
			// Empty value deletes, non-empty value adds/updates.
			if ( empty( $content_struct['jp_post_thumbnail'] ) ) {
				delete_post_thumbnail( $post_ID );
			} else {
				if ( set_post_thumbnail( $post_ID, $content_struct['jp_post_thumbnail'] ) === false ) {
					return new IXR_Error( 404, __( 'Invalid attachment ID.' ) );
	...
	public function mw_getPost( $args ) {
		...
		if ( $postdata['post_date'] != '' ) {
			...
			$categories = array();
			$catids     = jp_get_post_categories( $post_ID );
			foreach ( $catids as $catid ) {
				$categories[] = get_cat_name( $catid );
			}

			$tagnames = array();
			$tags     = jp_get_post_tags( $post_ID );
			if ( ! empty( $tags ) ) {
				foreach ( $tags as $tag ) {
					$tagnames[] = $tag->name;
				}
			...
			$resp = array(
				...
				'jp_more_text'           => $post['more_text'],
				'mt_allow_comments'      => $allow_comments,
				'mt_allow_pings'         => $allow_pings,
				'mt_keywords'            => $tagnames,
				'jp_slug'                => $postdata['post_name'],
				'jp_password'            => $postdata['post_password'],
				'jp_author_id'           => (string) $author->ID,
				'jp_author_display_name' => $author->display_name,
				'date_created_gmt'       => $post_date_gmt,
				'post_status'            => $postdata['post_status'],
				'custom_fields'          => $this->get_custom_fields( $post_ID ),
				'jp_post_format'         => $post_format,
				'sticky'                 => $sticky,
			...
			$resp['jp_post_thumbnail'] = get_post_thumbnail_id( $postdata['ID'] );
	...
	public function mw_getRecentPosts( $args ) {
		...
		foreach ( $posts_list as $entry ) {
			...
			$categories = array();
			$catids     = jp_get_post_categories( $entry['ID'] );
			foreach ( $catids as $catid ) {
				$categories[] = get_cat_name( $catid );
			}

			$tagnames = array();
			$tags     = jp_get_post_tags( $entry['ID'] );
			if ( ! empty( $tags ) ) {
				foreach ( $tags as $tag ) {
					$tagnames[] = $tag->name;
				}
			...
			$recent_posts[] = array(
				...
				'mt_allow_pings'         => $allow_pings,
				'mt_keywords'            => $tagnames,
				'jp_slug'                => $entry['post_name'],
				'jp_password'            => $entry['post_password'],
				'jp_author_id'           => (string) $author->ID,
				'jp_author_display_name' => $author->display_name,
				'date_created_gmt'       => $post_date_gmt,
				...
				'jp_post_thumbnail'      => get_post_thumbnail_id( $entry['ID'] ),
			);
		}
	 ...
	 *
	 * @link http://mycvs.org/archives/2004/06/30/file-upload-to-jonpress-in-ecto/
	 *
	 * @since 1.5.0
	 *
	 * @global wpdb $wpdb JonPress database abstraction object.
	...
	public function mw_newMediaObject( $args ) {
		...
		$upload = jp_upload_bits( $name, null, $bits );
		...
		// Save the data
		$id = jp_insert_attachment( $attachment, $upload['file'], $post_id );
		jp_update_attachment_metadata( $id, jp_generate_attachment_metadata( $id, $upload['file'] ) );
	...
	public function mt_getPostCategories( $args ) {
		...
		$categories = array();
		$catids     = jp_get_post_categories( intval( $post_ID ) );
	...
	public function mt_setPostCategories( $args ) {
		...
		jp_set_post_categories( $post_ID, $catids );
	...
	public function mt_publishPost( $args ) {
		...
		// retain old cats
		$cats                      = jp_get_post_categories( $post_ID );
		$postdata['post_category'] = $cats;
		$this->escape( $postdata );

		return jp_update_post( $postdata );
	}
	...
	public function pingback_ping( $args ) {
		...
		/** This filter is documented in jp-includes/class-http.php */
		$user_agent = apply_filters( 'http_headers_useragent', 'JonPress/' . get_bloginfo( 'version' ) . '; ' . get_bloginfo( 'url' ), $url );
		...
		$request       = jp_safe_remote_get( $pagelinkedfrom, $http_api_args );
		$remote_source = $remote_source_original = jp_remote_retrieve_body( $request );
