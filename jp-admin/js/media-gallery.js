...
jQuery(function($) {
	/**
	 * Adds a click event handler to the element with a 'jp-gallery' class.
	 */
	$( 'body' ).bind( 'click.jp-gallery', function(e) {
		var target = $( e.target ), id, img_size;

		if ( target.hasClass( 'jp-set-header' ) ) {
			// Opens the image to preview it full size.
			( window.dialogArguments || opener || parent || top ).location.href = target.data( 'location' );
			e.preventDefault();
		} else if ( target.hasClass( 'jp-set-background' ) ) {
