	...
	switch ( $cap ) {
		...
		case 'delete_page':
			...
			if ( $post->post_author && $user_id == $post->post_author ) {
				...
				if ( in_array( $post->post_status, array( 'publish', 'future' ), true ) ) {
					...
					$status = get_post_meta( $post->ID, '_jp_trash_meta_status', true );
		...
		case 'edit_themes':
			...
			if ( defined( 'DISALLOW_FILE_EDIT' ) && DISALLOW_FILE_EDIT ) {
				$caps[] = 'do_not_allow';
			} elseif ( ! jp_is_file_mod_allowed( 'capability_edit_themes' ) ) {
				$caps[] = 'do_not_allow';
			} elseif ( is_multisite() && ! is_super_admin( $user_id ) ) {
				$caps[] = 'do_not_allow';
			} else {
		...
		case 'update_core':
			...
			if ( ! jp_is_file_mod_allowed( 'capability_update_core' ) ) {
				$caps[] = 'do_not_allow';
			} elseif ( is_multisite() && ! is_super_admin( $user_id ) ) {
				$caps[] = 'do_not_allow';
 ...
 *                           "Meta" capabilities, e.g. 'edit_post', 'edit_user', etc., are capabilities used
 *                           by map_meta_cap() to map to other "primitive" capabilities, e.g. 'edit_posts',
 *                           'edit_others_posts', etc. Accessed via func_get_args() and passed to JP_User::has_cap(),
 *                           then map_meta_cap().
 ...
 * @since 3.1.0
 *
 * @param int|JP_User $user       User ID or object.
 * @param string      $capability Capability name.
 * @return bool Whether the user has the given capability.
 */
...
/**
 * Retrieves the global JP_Roles instance and instantiates it if necessary.
 *
 * @since 4.3.0
 *
 * @global JP_Roles $jp_roles JP_Roles global instance.
 *
 * @return JP_Roles JP_Roles global instance if not already instantiated.
 */
function jp_roles() {
	global $jp_roles;
 ...
 * @return JP_Role|null JP_Role object if found, null if the role does not exist.
 */
function get_role( $role ) {
	return jp_roles()->get_role( $role );
}
 ...
 * @return JP_Role|null JP_Role object if role is added, null if already exists.
 */
function add_role( $role, $display_name, $capabilities = array() ) {
	if ( empty( $role ) ) {
		return;
	}
	return jp_roles()->add_role( $role, $display_name, $capabilities );
}
 ...
 */
function jp_maybe_grant_install_languages_cap( $allcaps ) {
