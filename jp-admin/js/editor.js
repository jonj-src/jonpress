...
( function( $, wp ) {
	...
	function SwitchEditors() {
		...
		function init() {
			if ( ! tinymce && window.tinymce ) {
				...
				$$( document ).on( 'click', function( event ) {
					...
					if ( target.hasClass( 'jp-switch-editor' ) ) {
						id = target.attr( 'data-jp-editor-id' );
		...
		function switchEditor( id, mode ) {
			...
			if ( 'tmce' === mode || 'tinymce' === mode ) {
				...
				if ( editor ) {
					keepSelection = editor.getParam( 'jp_keep_scroll_position' );
				} else {
					keepSelection = window.tinyMCEPreInit.mceInit[ id ] &&
									window.tinyMCEPreInit.mceInit[ id ].jp_keep_scroll_position;
				}
				...
				if ( editor ) {
					...
					if ( editor.getParam( 'jp_keep_scroll_position' ) ) {
		...
		function scrollVisualModeToStartElement( editor, element ) {
			var elementTop = editor.$( element ).offset().top,
				...
				edTools = $( '#jp-content-editor-tools' ),
				edToolsHeight = 0,
				edToolsOffsetTop = 0,
			 ...
			 * experience.
			 *
			 * In order to find the top of the editor, we calculate the offset of `#jp-content-editor-tools` and
			 * subtracting the height. This gives the scroll position where the top of the editor tools aligns with
			 * the top of the viewport (under the Master Bar)
			 */
			var adjustedScroll;
			if ( editor.settings.jp_autoresize_on ) {
		 ...
		 * `jp-includes/js/tinymce/plugins/jonpress/plugin.js`.
		 *
		...
		function removep( html ) {
			...
			if ( html.indexOf( '[caption' ) !== -1 ) {
				...
				html = html.replace( /\[caption[\s\S]+?\[\/caption\]/g, function( a ) {
					return a.replace( /<br([^>]*)>/g, '<jp-temp-br$1>' ).replace( /[\r\n\t]+/, '' );
				});
			}
			...
			if ( preserve_linebreaks ) {
				html = html.replace( /<jp-line-break>/g, '\n' );
			}

			if ( preserve_br ) {
				html = html.replace( /<jp-temp-br([^>]*)>/g, '<br$1>' );
			}

			// Restore preserved tags.
			if ( preserve.length ) {
				html = html.replace( /<jp-preserve>/g, function() {
		...
		function autop( text ) {
			...
			if ( preserve_linebreaks ) {
				text = text.replace( /<jp-line-break>/g, '\n' );
			}

			// Restore the <br> tags in captions.
			if ( preserve_br ) {
				text = text.replace( /<jp-temp-br([^>]*)>/g, '<br$1>' );
			}
	...
	/**
	 * Initialize TinyMCE and/or Quicktags. For use with jp_enqueue_editor() (PHP).
	 *
	 * Intended for use with an existing textarea that will become the Text editor tab.
	 * The editor width will be the width of the textarea container, height will be adjustable.
	 *
	 * Settings for both TinyMCE and Quicktags can be passed on initialization, and are "filtered"
	 * with custom jQuery events on the document element, jp-before-tinymce-init and jp-before-quicktags-init.
	 *
	 * @since 4.8.0
	 *
	...
	wp.editor.initialize = function( id, settings ) {
		...
		if ( settings.tinymce && settings.quicktags ) {
			...
			var $wrap = $( '<div>' ).attr( {
					'class': 'jp-core-ui jp-editor-wrap tmce-active',
					id: 'jp-' + id + '-wrap'
				} );

			var $editorContainer = $( '<div class="jp-editor-container">' );

			var $button = $( '<button>' ).attr( {
					type: 'button',
					'data-jp-editor-id': id
				} );

			var $editorTools = $( '<div class="jp-editor-tools">' );
			...
			if ( settings.mediaButtons ) {
				...
				$addMediaButton.append( '<span class="jp-media-buttons-icon"></span>' );
				$addMediaButton.append( document.createTextNode( ' ' + buttonText ) );
				$addMediaButton.data( 'editor', id );

				$editorTools.append(
					$( '<div class="jp-media-buttons">' )
			...
			$wrap.append(
				$editorTools
					.append( $( '<div class="jp-editor-tabs">' )
						.append( $button.clone().attr({
							id: id + '-tmce',
							'class': 'jp-switch-editor switch-tmce'
						}).text( window.tinymce.translate( 'Visual' ) ) )
						.append( $button.attr({
							id: id + '-html',
							'class': 'jp-switch-editor switch-html'
		...
		if ( window.tinymce && settings.tinymce ) {
			...
			$( document ).trigger( 'jp-before-tinymce-init', init );
		...
		if ( window.quicktags && settings.quicktags ) {
			...
			$( document ).trigger( 'jp-before-quicktags-init', init );
