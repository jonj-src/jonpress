...
tinymce.PluginManager.add( 'jonpress', function( editor ) {
	...
	function toggleToolbars( state ) {
		...
		editor.fire( 'jp-toolbar-toggle' );
	}

	// Add the kitchen sink button :)
	editor.addButton( 'jp_adv', {
	...
	editor.on( 'PostRender', function() {
		if ( editor.getParam( 'jonpress_adv_hidden', true ) && getUserSetting( 'hidetb', '0' ) === '0' ) {
			toggleToolbars( 'hide' );
		}
	});

	editor.addCommand( 'JP_Adv', function() {
		toggleToolbars();
	});
		...
		if ( event.content ) {
			if ( event.content.indexOf( '<!--more' ) !== -1 ) {
				...
				event.content = event.content.replace( /<!--more(.*?)-->/g, function( match, moretext ) {
					return '<img src="' + tinymce.Env.transparentSrc + '" data-jp-more="more" data-jp-more-text="' + moretext + '" ' +
						'class="jp-more-tag mce-jp-more" alt="" title="' + title + '" data-mce-resize="false" data-mce-placeholder="1" />';
				});
			}
			...
			if ( event.content.indexOf( '<!--nextpage-->' ) !== -1 ) {
				...
				event.content = event.content.replace( /<!--nextpage-->/g,
					'<img src="' + tinymce.Env.transparentSrc + '" data-jp-more="nextpage" class="jp-more-tag mce-jp-nextpage" ' +
			...
			if ( event.content.indexOf( '<script' ) !== -1 || event.content.indexOf( '<style' ) !== -1 ) {
				event.content = event.content.replace( /<(script|style)[^>]*>[\s\S]*?<\/\1>/g, function( match, tag ) {
					return '<img ' +
						...
						'data-jp-preserve="' + encodeURIComponent( match ) + '" ' +
	...
	editor.on( 'PostProcess', function( event ) {
		if ( event.get ) {
			event.content = event.content.replace(/<img[^>]+>/g, function( image ) {
				...
				if ( image.indexOf( 'data-jp-more="more"' ) !== -1 ) {
					if ( match = image.match( /data-jp-more-text="([^"]+)"/ ) ) {
						moretext = match[1];
					}

					string = '<!--more' + moretext + '-->';
				} else if ( image.indexOf( 'data-jp-more="nextpage"' ) !== -1 ) {
					string = '<!--nextpage-->';
				} else if ( image.indexOf( 'data-jp-preserve' ) !== -1 ) {
					if ( match = image.match( / data-jp-preserve="([^"]+)"/ ) ) {
	...
	editor.on( 'ResolveName', function( event ) {
		...
		if ( event.target.nodeName === 'IMG' && ( attr = editor.dom.getAttrib( event.target, 'data-jp-more' ) ) ) {
			event.name = attr;
		}
	});

	// Register commands
	editor.addCommand( 'JP_More', function( tag ) {
		var parent, html, title,
			classname = 'jp-more-tag',
			dom = editor.dom,
			node = editor.selection.getNode(),
			rootNode = editor.getBody();

		tag = tag || 'more';
		classname += ' mce-jp-' + tag;
		title = tag === 'more' ? 'Read more...' : 'Next page';
		title = __( title );
		html = '<img src="' + tinymce.Env.transparentSrc + '" alt="" title="' + title + '" class="' + classname + '" ' +
			'data-jp-more="' + tag + '" data-mce-resize="false" data-mce-placeholder="1" />';
	...
	editor.addCommand( 'JP_Code', function() {
		editor.formatter.toggle('code');
	});

	editor.addCommand( 'JP_Page', function() {
		editor.execCommand( 'JP_More', 'nextpage' );
	});

	editor.addCommand( 'JP_Help', function() {
		var access = tinymce.Env.mac ? __( 'Ctrl + Alt + letter:' ) : __( 'Shift + Alt + letter:' ),
			...
			i1 = 0,
			i2 = 0,
			labels = editor.settings.jp_shortcut_labels,
		...
		if ( dialog.$el ) {
			...
			$wrap = dialog.$el.find( '.mce-jp-help' );
	...
	editor.addCommand( 'JP_Medialib', function() {
	...
	// Register buttons
	editor.addButton( 'jp_more', {
		tooltip: 'Insert Read More tag',
		onclick: function() {
			editor.execCommand( 'JP_More', 'more' );
		}
	});

	editor.addButton( 'jp_page', {
		tooltip: 'Page break',
		onclick: function() {
			editor.execCommand( 'JP_More', 'nextpage' );
		}
	...
	editor.addButton( 'jp_help', {
		tooltip: 'Keyboard Shortcuts',
		cmd: 'JP_Help'
	});

	editor.addButton( 'jp_code', {
		tooltip: 'Code',
		cmd: 'JP_Code',
		stateSelector: 'code'
	});
	...
	editor.on( 'init', function() {
		...
		dom.setAttrib( doc.documentElement, 'lang', editor.getParam( 'jp_lang_attr' ) );
		...
		if ( editor.getParam( 'jp_paste_filters', true ) ) {
		...
		if ( editor.settings.jp_shortcut_labels && editor.theme.panel ) {
			var labels = {};
			...
			each( editor.settings.jp_shortcut_labels, function( value, name ) {
	...
	editor.on( 'preinit', function() {
		...
		function create( buttons, bottom ) {
			...
			toolbar.on( 'show', function() {
				...
				if ( isChromeRtl ) {
					tinymce.$( '.mce-widget.mce-tooltip' ).addClass( 'jp-hide-mce-tooltip' );
				}
			} );

			toolbar.on( 'hide', function() {
				if ( isChromeRtl ) {
					tinymce.$( '.mce-widget.mce-tooltip' ).removeClass( 'jp-hide-mce-tooltip' );
				}
			} );
