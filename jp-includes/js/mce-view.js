...
( function( window, wp, shortcode, $ ) {
	...
	wp.mce.views = {
		...
		setMarkers: function( content, editor ) {
			...
			_.each( views, function( view, type ) {
				...
				_.each( current, function( piece ) {
					...
					while ( remaining && ( result = view.prototype.match( remaining ) ) ) {
						...
						pieces.push( {
							content: instance.ignore ? text : '<p data-jpview-marker="' + instance.encodedText + '">' + text + '</p>',
							processed: true
						} );
			...
			return content.replace( /<p>\s*<p data-jpview-marker=/g, '<p data-jpview-marker=' ).replace( /<\/p>\s*<\/p>/g, '</p>' );
		},
		...
		getInstance: function( object ) {
			...
			return instances[ $( object ).attr( 'data-jpview-text' ) ];
		},
		...
		getText: function( node ) {
			return decodeURIComponent( $( node ).attr( 'data-jpview-text' ) || '' );
		},
	...
	_.extend( wp.mce.View.prototype, /** @lends wp.mce.View.prototype */{
		...
		getNodes: function( callback, rendered ) {
			this.getEditors( function( editor ) {
				...
				$( editor.getBody() )
					.find( '[data-jpview-text="' + self.encodedText + '"]' )
					.filter( function() {
						var data;
		...
		getMarkers: function( callback ) {
			this.getEditors( function( editor ) {
				...
				$( editor.getBody() )
					.find( '[data-jpview-marker="' + this.encodedText + '"]' )
					.each( function() {
		...
		replaceMarkers: function() {
			this.getMarkers( function( editor, node ) {
				...
				if ( ! this.loader && $( node ).text() !== tinymce.DOM.decode( this.text ) ) {
					editor.dom.setAttrib( node, 'data-jpview-marker', null );
					return;
				}

				$viewNode = editor.$(
					'<div class="wpview wpview-wrap" data-jpview-text="' + this.encodedText + '" data-jpview-type="' + this.type + '" contenteditable="false"></div>'
				);
			...
			this.getNodes( function( editor, node ) {
				...
				tinymce.each( dom.$( 'link[rel="stylesheet"]', editorHead ), function( link ) {
					if ( link.href && link.href.indexOf( 'skins/lightgray/content.min.css' ) === -1 &&
						link.href.indexOf( 'skins/jonpress/jp-content.css' ) === -1 ) {
		 ...
		 * @param {String} dashicon A dashicon ID. Optional. {@link https://developer.jonpress.org/resource/dashicons/}
		 */
		...
		update: function( text, editor, node, force ) {
			_.find( views, function( view, type ) {
				...
				if ( match ) {
					...
					editor.dom.setAttrib( node, 'data-jpview-text', encodeURIComponent( text ) );
...
/*
 * The JonPress core TinyMCE views.
