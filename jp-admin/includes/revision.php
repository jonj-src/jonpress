<?php
/**
 * JonPress Administration Revisions API
 *
 * @package JonPress
 ...
 *                    Or, false on failure.
 */
function jp_get_revision_ui_diff( $post, $compare_from, $compare_to ) {
	...
	foreach ( _jp_post_revision_fields( $post ) as $field => $name ) {
		/**
		 ...
		 * @param JP_Post $compare_from         The revision post object to compare to or from.
		 * @param string  null                  The context of whether the current revision is the old
		 *                                      or the new one. Values are 'to' or 'from'.
		 */
		$content_from = $compare_from ? apply_filters( "_jp_post_revision_field_{$field}", $compare_from->$field, $field, $compare_from, 'from' ) : '';

		/** This filter is documented in jp-admin/includes/revision.php */
		$content_to = apply_filters( "_jp_post_revision_field_{$field}", $compare_to->$field, $field, $compare_to, 'to' );
		 ...
		 *
		 * Filters the options passed to jp_text_diff() when viewing a post revision.
		 *
		 * @since 4.1.0
		 *
		 * @param array   $args {
		 *     Associative array of options to pass to jp_text_diff().
		 *
		 *     @type bool $show_split_view True for split view (two columns), false for
		 *                                 un-split view (single column). Default true.
		 * }
		 * @param string  $field        The current revision field.
		 * @param JP_Post $compare_from The revision post to compare from.
		 * @param JP_Post $compare_to   The revision post to compare to.
		 */
		$args = apply_filters( 'revision_text_diff_options', $args, $field, $compare_from, $compare_to );

		$diff = jp_text_diff( $content_from, $content_to, $args );
	 ...
	 * @param JP_Post $compare_to   The revision post to compare to.
	 */
	return apply_filters( 'jp_get_revision_ui_diff', $return, $compare_from, $compare_to );
 ...
 */
function jp_prepare_revisions_for_js( $post, $selected_revision_id, $from = null ) {
	$post    = get_post( $post );
	$authors = array();
	$now_gmt = time();
	...
	cache_users( jp_list_pluck( $revisions, 'post_author' ) );
	...
	foreach ( $revisions as $revision ) {
		...
		$autosave = (bool) jp_is_post_autosave( $revision );
		 ...
		 *     @type string     $title      Title for the revision's parent JP_Post object.
		 ...
		 * }
		 * @param JP_Post $revision       The revision's JP_Post object.
		 * @param JP_Post $post           The revision's parent JP_Post object.
		 */
		$revisions[ $revision->ID ] = apply_filters( 'jp_prepare_revision_for_js', $revisions_data, $revision, $post );
	}
	...
	$diffs = array(
		array(
			...
			'fields' => jp_get_revision_ui_diff( $post->ID, $from, $selected_revision_id ),
		),
	);

	return array(
		'postId'         => $post->ID,
		'nonce'          => jp_create_nonce( 'revisions-ajax-nonce' ),
 ...
 * @since 4.1.0
 *
 * @global JP_Post $post The global `$post` object.
 */
function jp_print_revision_templates() {
	global $post;
		...
		<# if ( ! _.isUndefined( data.attributes ) ) { #>
			<div class="diff-title">
				...
				<# if ( 'to' === data.type && data.attributes.restoreUrl ) { #>
					<input  <?php if ( jp_check_post_lock( $post->ID ) ) { ?>
						disabled="disabled"
					<?php } else { ?>
