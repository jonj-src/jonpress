<?php
/**
 * List Table API: JP_Terms_List_Table class
 *
 * @package JonPress
 ...
 * @see JP_List_Table
 */
class JP_Terms_List_Table extends JP_List_Table {
	...
	public function prepare_items() {
		...
		$search = ! empty( $_REQUEST['s'] ) ? trim( jp_unslash( $_REQUEST['s'] ) ) : '';
		...
		if ( ! empty( $_REQUEST['orderby'] ) ) {
			$args['orderby'] = trim( jp_unslash( $_REQUEST['orderby'] ) );
		}

		if ( ! empty( $_REQUEST['order'] ) ) {
			$args['order'] = trim( jp_unslash( $_REQUEST['order'] ) );
		}
		...
		$this->set_pagination_args(
			array(
				'total_items' => jp_count_terms( $this->screen->taxonomy, compact( 'search' ) ),
	...
	public function column_name( $tag ) {
		 ...
		 * @since 2.5.0
		 *
		 * @see JP_Terms_List_Table::column_name()
		 *
		...
		$uri = jp_doing_ajax() ? jp_get_referer() : $_SERVER['REQUEST_URI'];
	 ...
	 * @since 4.3.0
	 *
	 * @param JP_Term $tag         Tag being acted upon.
	...
	protected function handle_row_actions( $tag, $column_name, $primary ) {
		...
		if ( current_user_can( 'delete_term', $tag->term_id ) ) {
			$actions['delete'] = sprintf(
				...
				jp_nonce_url( "edit-tags.php?action=delete&amp;taxonomy=$taxonomy&amp;tag_ID=$tag->term_id", 'delete-tag_' . $tag->term_id ),
				/* translators: %s: taxonomy term name */
...
?>
	...
	?>
		...
		<div class="inline-edit-save submit">
			...
			<?php jp_nonce_field( 'taxinlineeditnonce', '_inline_edit', false ); ?>
