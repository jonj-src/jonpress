...
if ( ! current_user_can( 'manage_options' ) ) {
	jp_die( __( 'Sorry, you are not allowed to manage privacy on this site.' ) );
}
...
if ( ! empty( $action ) ) {
	...
	if ( 'set-privacy-page' === $action ) {
		...
		update_option( 'jp_page_for_privacy_policy', $privacy_policy_page_id );
		...
		$privacy_policy_page_content = JP_Privacy_Policy_Content::get_default_content();
		$privacy_policy_page_id = jp_insert_post(
			array(
		...
		if ( is_jp_error( $privacy_policy_page_id ) ) {
			add_settings_error(
				'page_for_privacy_policy',
				'page_for_privacy_policy',
...
$privacy_policy_page_id     = (int) get_option( 'jp_page_for_privacy_policy' );

if ( ! empty( $privacy_policy_page_id ) ) {

	$privacy_policy_page = get_post( $privacy_policy_page_id );

	if ( ! $privacy_policy_page instanceof JP_Post ) {
		add_settings_error(
			'page_for_privacy_policy',
			'page_for_privacy_policy',
		...
		<tr>
			...
			<td>
				<form method="post" action="">
					...
					jp_nonce_field( 'set-privacy-page' );
				...
				<form method="post" action="">
					...
					<?php
					jp_nonce_field( 'create-privacy-page' );
