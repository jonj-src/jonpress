<?php
/**
 * JonPress Administration for Navigation Menus
 * Interface functions
 *
 * @version 2.0.0
 *
...
if ( ! current_theme_supports( 'menus' ) && ! current_theme_supports( 'widgets' ) ) {
	jp_die( __( 'Your theme does not support navigation menus or widgets.' ) );
}
...
switch ( $action ) {
	case 'add-menu-item':
		...
		if ( isset( $_REQUEST['nav-menu-locations'] ) ) {
			...
			jp_save_nav_menu_items( $nav_menu_selected_id, $_REQUEST['menu-item'] );
		}
		break;
	case 'move-down-menu-item':
		...
		if ( is_nav_menu_item( $menu_item_id ) ) {
			$menus = isset( $_REQUEST['menu'] ) ? array( (int) $_REQUEST['menu'] ) : jp_get_object_terms( $menu_item_id, 'nav_menu', array( 'fields' => 'ids' ) );
			if ( ! is_jp_error( $menus ) && ! empty( $menus[0] ) ) {
				$menu_id            = (int) $menus[0];
				$ordered_menu_items = jp_get_nav_menu_items( $menu_id );
				$menu_item_data     = (array) jp_setup_nav_menu_item( get_post( $menu_item_id ) );
				...
				if (
					...
					$next_item_data = (array) jp_setup_nav_menu_item( get_post( $next_item_id ) );
					...
					if (
						...
						$parent_object = jp_setup_nav_menu_item( get_post( $parent_db_id ) );

						if ( ! is_jp_error( $parent_object ) ) {
						...
						jp_update_post( $menu_item_data );
						jp_update_post( $next_item_data );
					}
	...
	case 'delete-menu-item':
		...
		if ( is_nav_menu_item( $menu_item_id ) && jp_delete_post( $menu_item_id, true ) ) {
	...
	case 'delete':
		...
		if ( is_nav_menu( $nav_menu_selected_id ) ) {
			$deletion = jp_delete_nav_menu( $nav_menu_selected_id );
		} else {
	...
	case 'delete_menus':
		...
		foreach ( $_REQUEST['delete_menus'] as $menu_id_to_delete ) {
			...
			$deletion = jp_delete_nav_menu( $menu_id_to_delete );
			if ( is_jp_error( $deletion ) ) {
				$messages[]     = '<div id="message" class="error notice is-dismissible"><p>' . $deletion->get_error_message() . '</p></div>';
				$deletion_error = true;
			}
	...
	case 'update':
		...
		if ( 0 == $nav_menu_selected_id ) {
			...
			if ( $new_menu_title ) {
				$_nav_menu_selected_id = jp_update_nav_menu_object( 0, array( 'menu-name' => $new_menu_title ) );

				if ( is_jp_error( $_nav_menu_selected_id ) ) {
					$messages[] = '<div id="message" class="error notice is-dismissible"><p>' . $_nav_menu_selected_id->get_error_message() . '</p></div>';
				} else {
					$_menu_object            = jp_get_nav_menu_object( $_nav_menu_selected_id );
					$nav_menu_selected_id    = $_nav_menu_selected_id;
					$nav_menu_selected_title = $_menu_object->name;
					if ( isset( $_REQUEST['menu-item'] ) ) {
						jp_save_nav_menu_items( $nav_menu_selected_id, absint( $_REQUEST['menu-item'] ) );
					}
					if ( isset( $_REQUEST['zero-menu-state'] ) ) {
						// If there are menu items, add them
						jp_nav_menu_update_menu_items( $nav_menu_selected_id, $nav_menu_selected_title );
					...
					jp_redirect( admin_url( 'nav-menus.php?menu=' . $_nav_menu_selected_id ) );
					exit();
				}
			} else {
			...
			$_menu_object = jp_get_nav_menu_object( $nav_menu_selected_id );
			...
			if ( ! is_jp_error( $_menu_object ) ) {
				$_nav_menu_selected_id = jp_update_nav_menu_object( $nav_menu_selected_id, array( 'menu-name' => $menu_title ) );
				if ( is_jp_error( $_nav_menu_selected_id ) ) {
			...
			// Update menu items.
			if ( ! is_jp_error( $_menu_object ) ) {
				$messages = array_merge( $messages, jp_nav_menu_update_menu_items( $_nav_menu_selected_id, $nav_menu_selected_title ) );

				// If the menu ID changed, redirect to the new URL.
				if ( $nav_menu_selected_id != $_nav_menu_selected_id ) {
					jp_redirect( admin_url( 'nav-menus.php?menu=' . intval( $_nav_menu_selected_id ) ) );
					exit();
				}
			}
		}
	...
	case 'locations':
		if ( ! $num_locations ) {
			jp_redirect( admin_url( 'nav-menus.php' ) );
			exit();
		}
 ...
 */
$page_count                  = jp_count_posts( 'page' );
...
if ( 0 == $menu_count && ! $add_new_screen && ! $one_theme_location_no_menus ) {
	jp_redirect( admin_url( 'nav-menus.php?action=edit&menu=0' ) );
}
...
if ( ! $nav_menu_selected_title && is_nav_menu( $nav_menu_selected_id ) ) {
	$_menu_object            = jp_get_nav_menu_object( $nav_menu_selected_id );
	$nav_menu_selected_title = ! is_jp_error( $_menu_object ) ? $_menu_object->name : '';
}

// Generate truncated menu names.
foreach ( (array) $nav_menus as $key => $_nav_menu ) {
	$nav_menus[ $key ]->truncated_name = jp_html_excerpt( $_nav_menu->name, 40, '&hellip;' );
}
 ...
 *
 * @global int $_jp_nav_menu_max_depth
 */
global $_jp_nav_menu_max_depth;
$_jp_nav_menu_max_depth = 0;

// Calling jp_get_nav_menu_to_edit generates $_jp_nav_menu_max_depth.
if ( is_nav_menu( $nav_menu_selected_id ) ) {
	$menu_items  = jp_get_nav_menu_items( $nav_menu_selected_id, array( 'post_status' => 'any' ) );
	$edit_markup = jp_get_nav_menu_to_edit( $nav_menu_selected_id );
}
 ...
 * @return string
 */
function jp_nav_menu_max_depth( $classes ) {
	global $_jp_nav_menu_max_depth;
	return "$classes menu-max-depth-$_jp_nav_menu_max_depth";
}

add_filter( 'admin_body_class', 'jp_nav_menu_max_depth' );

jp_nav_menu_setup();
jp_initial_nav_menu_meta_boxes();
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://codex.jonpress.org/Appearance_Menus_Screen">Documentation on Menus</a>' ) . '</p>' .
...
?>
<div class="wrap">
	<h1 class="jp-heading-inline"><?php echo esc_html( __( 'Menus' ) ); ?></h1>
	<?php
			...
			esc_url(
				add_query_arg(
					array(
						...
						'return' => urlencode( remove_query_arg( jp_removable_query_args(), jp_unslash( $_SERVER['REQUEST_URI'] ) ) ),
							...
							<select name="menu-locations[<?php echo $_location; ?>]" id="locations-<?php echo $_location; ?>">
								...
								foreach ( $nav_menus as $menu ) :
									...
									<option <?php echo $data_orig; ?> <?php selected( $selected ); ?> value="<?php echo $menu->term_id; ?>">
										<?php echo jp_html_excerpt( $menu->name, 40, '&hellip;' ); ?>
									</option>
								<?php endforeach; ?>
							</select>
...
">
			...
			<p class="button-controls jp-clearfix"><?php submit_button( __( 'Save Changes' ), 'primary left', 'nav-menu-locations', false ); ?></p>
			<?php jp_nonce_field( 'save-menu-locations' ); ?>
				...
				<?php foreach ( (array) $nav_menus as $_nav_menu ) : ?>
					<option value="<?php echo esc_attr( $_nav_menu->term_id ); ?>" <?php selected( $_nav_menu->term_id, $nav_menu_selected_id ); ?>>
						...
						if ( ! empty( $menu_locations ) && in_array( $_nav_menu->term_id, $menu_locations ) ) {
							 ...
							 */
							$assigned_locations = array_slice( $locations_assigned_to_this_menu, 0, absint( apply_filters( 'jp_nav_locations_listed_per_menu', 3 ) ) );
...
if ( isset( $_GET['menu'] ) && '0' == $_GET['menu'] ) {
	...
	<div id="nav-menus-frame" class="jp-clearfix">
	<div id="menu-settings-column" class="metabox-holder<?php echo $metabox_holder_disabled_class; ?>">
		...
		<form id="nav-menu-meta" class="nav-menu-meta" method="post" enctype="multipart/form-data">
			...
			<?php jp_nonce_field( 'add-menu_item', 'menu-settings-column-nonce' ); ?>
		...
		<div id="menu-management">
			...
			?>
				...
				<div class="menu-edit <?php echo $new_screen_class; ?>">
					...
					jp_nonce_field( 'update-nav_menu', 'update-nav-menu-nonce' );
					...
					<div id="nav-menu-header">
						<div class="major-publishing-actions jp-clearfix">
					...
					<div id="post-body">
						<div id="post-body-content" class="jp-clearfix">
							<?php if ( ! $add_new_screen ) : ?>
							<?php
								$hide_style = '';
							...
							<?php
							if ( isset( $edit_markup ) && ! is_jp_error( $edit_markup ) ) {
								echo $edit_markup;
							} else {
							?>
											...
											<?php if ( ! empty( $menu_locations[ $location ] ) && $menu_locations[ $location ] != $nav_menu_selected_id ) : ?>
												...
												<?php
													...
													printf(
														...
														jp_get_nav_menu_object( $menu_locations[ $location ] )->name
													);
												?>
												</span>
											<?php endif; ?>
