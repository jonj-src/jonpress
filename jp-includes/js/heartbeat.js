 ...
 * @see jp_ajax_nopriv_heartbeat(), jp_ajax_heartbeat()
 *
	...
	var Heartbeat = function() {
		...
		function initialize() {
			...
			if ( hidden ) {
				...
				$document.on( visibilitychange + '.jp-heartbeat', function() {
			...
			$(window).on( 'unload.jp-heartbeat', function() {
		...
		function userIsActive() {
			...
			$document.off( '.jp-heartbeat-active' );

			$('iframe').each( function( i, frame ) {
				if ( isLocalFrame( frame ) ) {
					$( frame.contentWindow ).off( '.jp-heartbeat-active' );
				}
			});
		...
		function checkUserActivity() {
			...
			if ( ! settings.userActivityEvents ) {
				$document.on( 'mouseover.jp-heartbeat-active keyup.jp-heartbeat-active touchend.jp-heartbeat-active', function() {
					userIsActive();
				});

				$('iframe').each( function( i, frame ) {
					if ( isLocalFrame( frame ) ) {
						$( frame.contentWindow ).on( 'mouseover.jp-heartbeat-active keyup.jp-heartbeat-active touchend.jp-heartbeat-active', function() {
							userIsActive();
						});
					}
				});
