...
( function( $, wpLinkL10n, wp ) {
	...
	wpLink = {
		...
		init: function() {
			inputs.wrap = $('#jp-link-wrap');
			inputs.dialog = $( '#jp-link' );
			inputs.backdrop = $( '#jp-link-backdrop' );
			inputs.submit = $( '#jp-link-submit' );
			inputs.close = $( '#jp-link-close' );

			// Input
			inputs.text = $( '#jp-link-text' );
			inputs.url = $( '#jp-link-url' );
			inputs.nonce = $( '#_ajax_linking_nonce' );
			inputs.openInNewTab = $( '#jp-link-target' );
			inputs.search = $( '#jp-link-search' );
			...
			inputs.close.add( inputs.backdrop ).add( '#jp-link-cancel button' ).click( function( event ) {
				event.preventDefault();
				wpLink.close();
			});
		...
		mceUpdate: function() {
			...
			editor.undoManager.transact( function() {
				if ( ! $link.length ) {
					editor.execCommand( 'mceInsertLink', false, { href: '_jp_link_placeholder', 'data-jp-temp-link': 1 } );
					$link = editor.$( 'a[data-jp-temp-link="1"]' ).removeAttr( 'data-jp-temp-link' );
		...
		keydown: function( event ) {
			...
			// Tab key.
				...
				// jp-link-submit must always be the last focusable element in the dialog.
				// following focusable elements will be skipped on keyboard navigation.
				if ( id === 'jp-link-submit' && ! event.shiftKey ) {
					inputs.close.focus();
					event.preventDefault();
				} else if ( id === 'jp-link-close' && event.shiftKey ) {
					inputs.submit.focus();
					event.preventDefault();
				}
			}
	...
	River = function( element, search ) {
		...
		$( '#jp-link .query-results, #jp-link #link-selector' ).scroll( function() {
			self.maybeLoad();
		});
