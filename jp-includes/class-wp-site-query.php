 ...
 * @since 4.6.0
 *
 * @see JP_Site_Query::__construct() for accepted arguments.
 */
class JP_Site_Query {
	 ...
	 *     @type array        $meta_query             Meta query clauses to limit retrieved sites by. See `JP_Meta_Query`.
	 *                                                Default empty.
	 ...
	 * @since 4.6.0
	 *
	 * @see JP_Site_Query::__construct()
	 *
	 * @param string|array $query Array or string of JP_Site_Query arguments. See JP_Site_Query::__construct().
	 */
	public function parse_query( $query = '' ) {
		 ...
		 * @since 4.6.0
		 *
		 * @param JP_Site_Query $this The JP_Site_Query instance (passed by reference).
		 */
		do_action_ref_array( 'parse_site_query', array( &$this ) );
	}

	/**
	 * Sets up the JonPress query for retrieving sites.
	 *
	 * @since 4.6.0
	 *
	...
	public function get_sites() {
		 ...
		 * @since 4.6.0
		 *
		 * @param JP_Site_Query $this Current instance of JP_Site_Query (passed by reference).
		 */
		...
		$last_changed = jp_cache_get_last_changed( 'sites' );

		$cache_key   = "get_sites:$key:$last_changed";
		$cache_value = jp_cache_get( $cache_key, 'sites' );
		...
		if ( false === $cache_value ) {
			...
			jp_cache_add( $cache_key, $cache_value, 'sites' );
		} else {
		 ...
		 * @since 4.6.0
		 *
		 * @param JP_Site[]     $_sites An array of JP_Site objects.
		 * @param JP_Site_Query $this   Current instance of JP_Site_Query (passed by reference).
		 */
		$_sites = apply_filters_ref_array( 'the_sites', array( $_sites, &$this ) );

		// Convert to JP_Site instances.
	...
	protected function get_site_ids() {
		...
		if ( ! empty( $this->query_vars['site__in'] ) ) {
			$this->sql_clauses['where']['site__in'] = "{$wpdb->blogs}.blog_id IN ( " . implode( ',', jp_parse_id_list( $this->query_vars['site__in'] ) ) . ' )';
		}

		// Parse site IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['site__not_in'] ) ) {
			$this->sql_clauses['where']['site__not_in'] = "{$wpdb->blogs}.blog_id NOT IN ( " . implode( ',', jp_parse_id_list( $this->query_vars['site__not_in'] ) ) . ' )';
		}
		...
		if ( ! empty( $this->query_vars['network__in'] ) ) {
			$this->sql_clauses['where']['network__in'] = 'site_id IN ( ' . implode( ',', jp_parse_id_list( $this->query_vars['network__in'] ) ) . ' )';
		}

		// Parse site network IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['network__not_in'] ) ) {
			$this->sql_clauses['where']['network__not_in'] = 'site_id NOT IN ( ' . implode( ',', jp_parse_id_list( $this->query_vars['network__not_in'] ) ) . ' )';
		}
		...
		if ( ! empty( $this->query_vars['lang__in'] ) ) {
			$this->sql_clauses['where']['lang__in'] = 'lang_id IN ( ' . implode( ',', jp_parse_id_list( $this->query_vars['lang__in'] ) ) . ' )';
		}

		// Parse site language IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['lang__not_in'] ) ) {
			$this->sql_clauses['where']['lang__not_in'] = 'lang_id NOT IN ( ' . implode( ',', jp_parse_id_list( $this->query_vars['lang__not_in'] ) ) . ' )';
		}
		...
		if ( strlen( $this->query_vars['search'] ) ) {
			...
			/**
			 * Filters the columns to search in a JP_Site_Query search.
			 *
			 ...
			 * @param JP_Site_Query $this           The current JP_Site_Query instance.
			 */
		...
		if ( ! empty( $date_query ) && is_array( $date_query ) ) {
			$this->date_query                         = new JP_Date_Query( $date_query, 'registered' );
	...
	private function set_found_sites() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			 ...
			 * @param JP_Site_Query $site_query        The `JP_Site_Query` instance.
			 */
