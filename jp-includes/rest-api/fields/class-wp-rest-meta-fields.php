<?php
/**
 * REST API: JP_REST_Meta_Fields class
 *
 * @package JonPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 * @since 4.7.0
 */
abstract class JP_REST_Meta_Fields {
	 ...
	 * @return JP_Error|object Object containing the meta values by name, otherwise JP_Error object.
	 */
	 ...
	 * @param JP_REST_Request $request Current request object.
	 ...
	 * @param int             $object_id Object ID to fetch meta for.
	 * @return JP_Error|null JP_Error if one occurs, null on success.
	 */
	 ...
	 * @return bool|JP_Error True if meta field is deleted, JP_Error otherwise.
	 */
	protected function delete_meta_value( $object_id, $meta_key, $name ) {
		...
		if ( ! delete_metadata( $meta_type, $object_id, jp_slash( $meta_key ) ) ) {
			return new JP_Error(
				'rest_meta_database_error',
				__( 'Could not delete meta value from database.' ),
				array(
					'key'    => $name,
					'status' => JP_Http::INTERNAL_SERVER_ERROR,
				)
			);
		}
	 ...
	 * @return bool|JP_Error True if meta fields are updated, JP_Error otherwise.
	 */
	protected function update_multi_meta_value( $object_id, $meta_key, $name, $values ) {
		...
		foreach ( $to_remove as $value ) {
			if ( ! delete_metadata( $meta_type, $object_id, jp_slash( $meta_key ), jp_slash( $value ) ) ) {
				return new JP_Error(
					'rest_meta_database_error',
		...
		foreach ( $to_add as $value ) {
			if ( ! add_metadata( $meta_type, $object_id, jp_slash( $meta_key ), jp_slash( $value ) ) ) {
				return new JP_Error(
					'rest_meta_database_error',
					__( 'Could not update meta value in database.' ),
					array(
	 ...
	 * @return bool|JP_Error True if the meta field was updated, JP_Error otherwise.
	 */
	protected function update_meta_value( $object_id, $meta_key, $name, $value ) {
		...
		$meta_key   = jp_slash( $meta_key );
		$meta_value = jp_slash( $value );
	 ...
	 * @return JP_Error|string The meta array, if valid, otherwise an error.
	 */
