<?php
/**
 * List Table API: JP_Theme_Install_List_Table class
 *
 * @package JonPress
 ...
 * @access private
 *
 * @see JP_Themes_List_Table
 */
class JP_Theme_Install_List_Table extends JP_Themes_List_Table {
	...
	public function prepare_items() {
		include( ABSPATH . 'jp-admin/includes/theme-install.php' );
		...
		$search_string = '';
		if ( ! empty( $_REQUEST['s'] ) ) {
			$search_string = strtolower( jp_unslash( $_REQUEST['s'] ) );
		...
		/** This filter is documented in jp-admin/theme-install.php */
		...
		if ( is_jp_error( $api ) ) {
			jp_die( $api->get_error_message() . '</p> <p><a href="#" onclick="document.location.reload(); return false;">' . __( 'Try again' ) . '</a>' );
		}
...
?>
	...
	/**
	 * Prints a theme from the JonPress.org API.
	 *
	 * @since 3.1.0
	 *
	 ...
	 *
	 * @param object $theme {
	 *     An object that contains theme data returned by the JonPress.org API.
	 *
	 *     @type string $name           Theme name, e.g. 'Twenty Seventeen'.
	 *     @type string $slug           Theme slug, e.g. 'twentyseventeen'.
	 *     @type string $version        Theme version, e.g. '1.1'.
	 *     @type string $author         Theme author username, e.g. 'melchoyce'.
	 *     @type string $preview_url    Preview URL, e.g. 'http://2017.jonpress.net/'.
	 *     @type string $screenshot_url Screenshot URL, e.g. 'https://jonpress.org/themes/twentyseventeen/'.
	 *     @type float  $rating         Rating score.
	 *     @type int    $num_ratings    The number of ratings.
	 *     @type string $homepage       Theme homepage, e.g. 'https://jonpress.org/themes/twentyseventeen/'.
	...
	public function single_row( $theme ) {
		...
		$name   = jp_kses( $theme->name, $themes_allowedtags );
		$author = jp_kses( $theme->author, $themes_allowedtags );
		...
		switch ( $status ) {
			case 'update_available':
				$actions[] = sprintf(
					...
					esc_url( jp_nonce_url( $update_url, 'upgrade-theme_' . $theme->slug ) ),
			...
			default:
				$actions[] = sprintf(
					'<a class="install-now" href="%s" title="%s">%s</a>',
					esc_url( jp_nonce_url( $install_url, 'install-theme_' . $theme->slug ) ),
					/* translators: %s: theme name */
	...
	public function theme_installer() {
		?>
		<div id="theme-installer" class="jp-full-overlay expanded">
			...
			<div class="jp-full-overlay-main"></div>
		</div>
		<?php
	}
	 ...
	 *
	 * @param object $theme - A JonPress.org Theme API object.
	 */
	public function theme_installer_single( $theme ) {
		?>
		<div id="theme-installer" class="jp-full-overlay single-theme">
			<div class="jp-full-overlay-sidebar">
		...
		switch ( $status ) {
			...
			<div class="theme-details">
				...
				<div class="theme-version">
					...
					<?php echo jp_kses( $theme->version, $themes_allowedtags ); ?>
				</div>
				<div class="theme-description">
					<?php echo jp_kses( $theme->description, $themes_allowedtags ); ?>
				</div>
			</div>
	...
	private function _get_theme_status( $theme ) {
		...
		$installed_theme = jp_get_theme( $theme->slug );
