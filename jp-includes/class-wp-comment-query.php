<?php
/**
 * Comment API: JP_Comment_Query class
 *
 * @package JonPress
 * @subpackage Comments
 * @since 4.4.0
 ...
 * @since 3.1.0
 *
 * @see JP_Comment_Query::__construct() for accepted arguments.
 */
class JP_Comment_Query {
	 ...
	 *     @type array        $date_query                Date query clauses to limit comments by. See JP_Date_Query.
	 *                                                   Default null.
	 ...
	 *                                                   See JP_Meta_Query. Default empty.
	 ...
	 *                                                   stored in a `children` property on the `JP_Comment`
	...
	public function __construct( $query = '' ) {
		$this->query_var_defaults = array(
			...
			'meta_value'                => '',
			'meta_query'                => '',
			'date_query'                => null, // See JP_Date_Query
			'hierarchical'              => false,
			'cache_domain'              => 'core',
	 ...
	 *
	 * @since 4.2.0 Extracted from JP_Comment_Query::query().
	 *
	 * @param string|array $query JP_Comment_Query arguments. See JP_Comment_Query::__construct()
	 */
	public function parse_query( $query = '' ) {
		...
		$this->query_vars = jp_parse_args( $query, $this->query_var_defaults );
		 ...
		 * @since 4.2.0
		 *
		 * @param JP_Comment_Query $this The JP_Comment_Query instance (passed by reference).
		 */
		do_action_ref_array( 'parse_comment_query', array( &$this ) );
	}

	/**
	 * Sets up the JonPress query for retrieving comments.
	 *
	 * @since 3.1.0
	...
	public function get_comments() {
		 ...
		 * @since 3.1.0
		 *
		 * @param JP_Comment_Query $this Current instance of JP_Comment_Query (passed by reference).
		 */
		 ...
		 */
		$_args = jp_array_slice_assoc( $this->query_vars, array_keys( $this->query_var_defaults ) );
		unset( $_args['fields'] );

		$key          = md5( serialize( $_args ) );
		$last_changed = jp_cache_get_last_changed( 'comment' );

		$cache_key   = "get_comments:$key:$last_changed";
		$cache_value = jp_cache_get( $cache_key, 'comment' );
		if ( false === $cache_value ) {
			...
			jp_cache_add( $cache_key, $cache_value, 'comment' );
		} else {
		 ...
		 * @since 3.1.0
		 *
		 * @param JP_Comment[]     $_comments An array of comments.
		 * @param JP_Comment_Query $this      Current instance of JP_Comment_Query (passed by reference).
		 */
		$_comments = apply_filters_ref_array( 'the_comments', array( $_comments, &$this ) );

		// Convert to JP_Comment instances
	...
	protected function get_comment_ids() {
		...
		if ( ! empty( $this->query_vars['comment__in'] ) ) {
			$this->sql_clauses['where']['comment__in'] = "$wpdb->comments.comment_ID IN ( " . implode( ',', jp_parse_id_list( $this->query_vars['comment__in'] ) ) . ' )';
		}

		// Parse comment IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['comment__not_in'] ) ) {
			$this->sql_clauses['where']['comment__not_in'] = "$wpdb->comments.comment_ID NOT IN ( " . implode( ',', jp_parse_id_list( $this->query_vars['comment__not_in'] ) ) . ' )';
		}

		// Parse comment parent IDs for an IN clause.
		if ( ! empty( $this->query_vars['parent__in'] ) ) {
			$this->sql_clauses['where']['parent__in'] = 'comment_parent IN ( ' . implode( ',', jp_parse_id_list( $this->query_vars['parent__in'] ) ) . ' )';
		}

		// Parse comment parent IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['parent__not_in'] ) ) {
			$this->sql_clauses['where']['parent__not_in'] = 'comment_parent NOT IN ( ' . implode( ',', jp_parse_id_list( $this->query_vars['parent__not_in'] ) ) . ' )';
		}

		// Parse comment post IDs for an IN clause.
		if ( ! empty( $this->query_vars['post__in'] ) ) {
			$this->sql_clauses['where']['post__in'] = 'comment_post_ID IN ( ' . implode( ',', jp_parse_id_list( $this->query_vars['post__in'] ) ) . ' )';
		}

		// Parse comment post IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['post__not_in'] ) ) {
			$this->sql_clauses['where']['post__not_in'] = 'comment_post_ID NOT IN ( ' . implode( ',', jp_parse_id_list( $this->query_vars['post__not_in'] ) ) . ' )';
		}
		...
		$plucked          = jp_array_slice_assoc( $this->query_vars, array( 'post_author', 'post_name', 'post_parent' ) );
		...
		if ( ! empty( $this->query_vars['author__in'] ) ) {
			$this->sql_clauses['where']['author__in'] = 'user_id IN ( ' . implode( ',', jp_parse_id_list( $this->query_vars['author__in'] ) ) . ' )';
		}

		// Comment author IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['author__not_in'] ) ) {
			$this->sql_clauses['where']['author__not_in'] = 'user_id NOT IN ( ' . implode( ',', jp_parse_id_list( $this->query_vars['author__not_in'] ) ) . ' )';
		}

		// Post author IDs for an IN clause.
		if ( ! empty( $this->query_vars['post_author__in'] ) ) {
			$join_posts_table                              = true;
			$this->sql_clauses['where']['post_author__in'] = 'post_author IN ( ' . implode( ',', jp_parse_id_list( $this->query_vars['post_author__in'] ) ) . ' )';
		}

		// Post author IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['post_author__not_in'] ) ) {
			$join_posts_table                                  = true;
			$this->sql_clauses['where']['post_author__not_in'] = 'post_author NOT IN ( ' . implode( ',', jp_parse_id_list( $this->query_vars['post_author__not_in'] ) ) . ' )';
		}
		...
		if ( ! empty( $this->query_vars['date_query'] ) && is_array( $this->query_vars['date_query'] ) ) {
			$this->date_query                         = new JP_Date_Query( $this->query_vars['date_query'], 'comment_date' );
	...
	private function set_found_comments() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			 ...
			 * @param JP_Comment_Query $comment_query        The `JP_Comment_Query` instance.
			 */
	 ...
	 * @global wpdb $wpdb JonPress database abstraction object.
	 *
	 * @param JP_Comment[] $comments Array of top-level comments whose descendants should be filled in.
	 * @return array
	 */
	protected function fill_descendants( $comments ) {
		...
		$levels = array(
			0 => jp_list_pluck( $comments, 'comment_ID' ),
		);

		$key          = md5( serialize( jp_array_slice_assoc( $this->query_vars, array_keys( $this->query_var_defaults ) ) ) );
		$last_changed = jp_cache_get_last_changed( 'comment' );
		...
		do {
			...
			foreach ( $_parent_ids as $parent_id ) {
				...
				$parent_child_ids = jp_cache_get( $cache_key, 'comment' );
			...
			if ( $uncached_parent_ids ) {
				...
				foreach ( $parent_map as $parent_id => $children ) {
					$cache_key = "get_comment_child_ids:$parent_id:$key:$last_changed";
					jp_cache_set( $cache_key, $children, 'comment' );
				}
			}
