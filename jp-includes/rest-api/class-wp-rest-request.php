<?php
/**
 * REST API: JP_REST_Request class
 *
 * @package JonPress
 * @subpackage REST_API
 * @since 4.4.0
 ...
 *
 * Note: When using features provided by ArrayAccess, be aware that JonPress deliberately
 ...
 * JP_REST_Request::get_body_params(), JP_REST_Request::get_url_params(), etc.
 *
 * @since 4.4.0
 *
 * @see ArrayAccess
 */
class JP_REST_Request implements ArrayAccess {
	...
	protected function get_parameter_order() {
		 ...
		 * }
		 * @param JP_REST_Request $this The request object.
		 */
	 ...
	 * @return true|JP_Error True if the JSON data was passed or no JSON data was provided, JP_Error if invalid JSON was passed.
	 */
	protected function parse_json_params() {
		 ...
		 * might not be defined: https://core.trac.jonpress.org/ticket/27799
		 */
		if ( null === $params && ( ! function_exists( 'json_last_error' ) || JSON_ERROR_NONE !== json_last_error() ) ) {
			...
			return new JP_Error( 'rest_invalid_json', __( 'Invalid JSON body passed.' ), $error_data );
		}
	...
	protected function parse_body_params() {
		 ...
		 *
		 * NOTE: Do not refactor to use `jp_unslash`.
		 */
	 ...
	 * @since 4.4.0
	 *
	 * @return true|JP_Error True if parameters were sanitized, JP_Error if an error occurred during sanitization.
	 */
	public function sanitize_params() {
		...
		foreach ( $order as $type ) {
			...
			foreach ( $this->params[ $type ] as $key => $value ) {
				...
				if ( is_jp_error( $sanitized_value ) ) {
	 ...
	 * @since 4.4.0
	 *
	 * @return bool|JP_Error True if there are no parameters to validate or if all pass validation,
	 *                       JP_Error if required parameters are missing.
	 */
	public function has_valid_params() {
		// If JSON data was passed, check for errors.
		$json_error = $this->parse_json_params();
		if ( is_jp_error( $json_error ) ) {
			return $json_error;
		}
	...
	/**
	 * Retrieves a JP_REST_Request object from a full URL.
	 *
	 * @since 4.5.0
	 *
	 * @param string $url URL with protocol, domain, path and query args.
	 * @return JP_REST_Request|false JP_REST_Request object on success, false on failure.
	 */
	public static function from_url( $url ) {
		...
		if ( ! empty( $bits['query'] ) ) {
			jp_parse_str( $bits['query'], $query_params );
		}
		...
		$request = false;
		if ( ! empty( $route ) ) {
			$request = new JP_REST_Request( 'GET', $route );
		 ...
		 * @since 4.5.0
		 *
		 * @param JP_REST_Request|false $request Generated request object, or false if URL
		 *                                       could not be parsed.
