 ...
 * @since 2.0.0
 *
 * @return int|JP_Error Value 0 or JP_Error on failure. The link ID on success.
 */
function add_link() {
	return edit_link();
}
...
function edit_link( $link_id = 0 ) {
	...
	if ( ! empty( $link_id ) ) {
		...
		return jp_update_link( $_POST );
	} else {
		return jp_insert_link( $_POST );
	}
}
...
function get_default_link_to_edit() {
	...
	if ( isset( $_GET['linkurl'] ) ) {
		$link->link_url = esc_url( jp_unslash( $_GET['linkurl'] ) );
	} else {
		$link->link_url = '';
	}

	if ( isset( $_GET['name'] ) ) {
		$link->link_name = esc_attr( jp_unslash( $_GET['name'] ) );
	} else {
		$link->link_name = '';
	}
 ...
 */
function jp_delete_link( $link_id ) {
	global $wpdb;
	/**
	...
	jp_delete_object_term_relationships( $link_id, 'link_category' );
 ...
 */
function jp_get_link_cats( $link_id = 0 ) {
	$cats = jp_get_object_terms( $link_id, 'link_category', array( 'fields' => 'ids' ) );
 ...
 * @param bool  $jp_error Optional. Whether to return a JP_Error object on failure. Default false.
 * @return int|JP_Error Value 0 or JP_Error on failure. The link ID on success.
 */
function jp_insert_link( $linkdata, $jp_error = false ) {
	global $wpdb;
	...
	$args = jp_parse_args( $linkdata, $defaults );
	$r    = jp_unslash( sanitize_bookmark( $args, 'db' ) );
	...
	if ( $update ) {
		if ( false === $wpdb->update( $wpdb->links, compact( 'link_url', 'link_name', 'link_image', 'link_target', 'link_description', 'link_visible', 'link_rating', 'link_rel', 'link_notes', 'link_rss' ), compact( 'link_id' ) ) ) {
			if ( $jp_error ) {
				return new JP_Error( 'db_update_error', __( 'Could not update link in the database' ), $wpdb->last_error );
			} else {
				return 0;
			}
		}
		...
		if ( false === $wpdb->insert( $wpdb->links, compact( 'link_url', 'link_name', 'link_image', 'link_target', 'link_description', 'link_visible', 'link_owner', 'link_rating', 'link_rel', 'link_notes', 'link_rss' ) ) ) {
			if ( $jp_error ) {
				return new JP_Error( 'db_insert_error', __( 'Could not insert link into the database' ), $wpdb->last_error );
			} else {
				return 0;
			}
		}
	...
	jp_set_link_cats( $link_id, $link_category );
 ...
 */
function jp_set_link_cats( $link_id = 0, $link_categories = array() ) {
	...
	jp_set_object_terms( $link_id, $link_categories, 'link_category' );
 ...
 * @return int|JP_Error Value 0 or JP_Error on failure. The updated link ID on success.
 */
function jp_update_link( $linkdata ) {
	...
	return jp_insert_link( $linkdata );
}

/**
 * Outputs the 'disabled' message for the JonPress Link Manager.
 *
 * @since 3.5.0
 * @access private
 *
 ...
 */
function jp_link_manager_disabled_message() {
	global $pagenow;
	...
	if ( $really_can_manage_links && current_user_can( 'install_plugins' ) ) {
		...
		jp_die( sprintf( __( 'If you are looking to use the link manager, please install the <a href="%s">Link Manager</a> plugin.' ), $link ) );
	}

	jp_die( __( 'Sorry, you are not allowed to edit the links for this site.' ) );
}
