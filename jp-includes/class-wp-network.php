 ...
 *
 * This class is most useful in JonPress multi-network installations where the
...
class JP_Network {
	 ...
	 * @return JP_Network|bool The network's object if found. False if not.
	 */
	public static function get_instance( $network_id ) {
		...
		$_network = jp_cache_get( $network_id, 'networks' );

		if ( ! $_network ) {
			$_network = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->site} WHERE id = %d LIMIT 1", $network_id ) );

			if ( empty( $_network ) || is_jp_error( $_network ) ) {
				return false;
			}

			jp_cache_add( $network_id, $_network, 'networks' );
		}

		return new JP_Network( $_network );
	}

	/**
	 * Create a new JP_Network object.
	 *
	 ...
	 * @since 4.4.0
	 *
	 * @param JP_Network|object $network A network object.
	 */
	...
	private function get_main_site_id() {
		 ...
		 * @param JP_Network $network      The network object for which the main site was detected.
		 */
		...
		if ( $site->domain === $this->domain && $site->path === $this->path ) {
			...
			$main_site_id = jp_cache_get( $cache_key, 'site-options' );
			if ( false === $main_site_id ) {
				...
				jp_cache_add( $cache_key, $main_site_id, 'site-options' );
			}
		}
	 ...
	 * @return JP_Network|bool Network object if successful. False when no network is found.
	 */
	public static function get_by_path( $domain = '', $path = '', $segments = null ) {
		...
		if ( jp_using_ext_object_cache() ) {
			$using_paths = jp_cache_get( 'networks_have_paths', 'site-options' );
			if ( false === $using_paths ) {
				...
				jp_cache_add( 'networks_have_paths', $using_paths, 'site-options' );
			}
		}
		 ...
		 * an object from jp_get_network().
		 *
		 * @since 3.9.0
		 *
		 * @param null|bool|JP_Network $network  Network value to return by path.
		 * @param string               $domain   The requested domain.
		 * @param string               $path     The requested path, in full.
