...
if ( is_multisite() && ! is_network_admin() ) {
	jp_redirect( network_admin_url( 'theme-editor.php' ) );
	exit();
}

if ( ! current_user_can( 'edit_themes' ) ) {
	jp_die( '<p>' . __( 'Sorry, you are not allowed to edit templates for this site.' ) . '</p>' );
}
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
				 ...
				 '<p>' . sprintf( __( 'Upgrading to a newer version of the same theme will override changes made here. To avoid this, consider creating a <a href="%s">child theme</a> instead.' ), __( 'https://codex.jonpress.org/Child_Themes' ) ) . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://codex.jonpress.org/Theme_Development">Documentation on Theme Development</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://codex.jonpress.org/Using_Themes">Documentation on Using Themes</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://codex.jonpress.org/Editing_Files">Documentation on Editing Files</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://codex.jonpress.org/Template_Tags">Documentation on Template Tags</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://jonpress.org/support/">Support Forums</a>' ) . '</p>'
);

jp_reset_vars( array( 'action', 'error', 'file', 'theme' ) );
...
if ( ! $theme->exists() ) {
	jp_die( __( 'The requested theme does not exist.' ) );
}

if ( $theme->errors() && 'theme_no_stylesheet' == $theme->errors()->get_error_code() ) {
	jp_die( __( 'The requested theme does not exist.' ) . ' ' . $theme->errors()->get_error_message() );
}

$allowed_files = $style_files = array();
$has_templates = false;

$file_types = jp_get_theme_file_editable_extensions( $theme );
...
if ( 'POST' === $_SERVER['REQUEST_METHOD'] ) {
	...
	$settings = array(
		'codeEditor' => jp_enqueue_code_editor( compact( 'file' ) ),
	);
	jp_enqueue_script( 'jp-theme-plugin-editor' );
	jp_add_inline_script( 'jp-theme-plugin-editor', sprintf( 'jQuery( function( $ ) { wp.themePluginEditor.init( $( "#template" ), %s ); } )', jp_json_encode( $settings ) ) );
	jp_add_inline_script( 'jp-theme-plugin-editor', 'wp.themePluginEditor.themeOrPlugin = "theme";' );
	...
	if ( ! empty( $posted_content ) ) {
		...
		if ( '.php' == substr( $file, strrpos( $file, '.' ) ) ) {
			$functions = jp_doc_link_parse( $content );
...
<?php elseif ( is_jp_error( $edit_error ) ) : ?>
...
<?php
foreach ( jp_get_themes( array( 'errors' => null ) ) as $a_stylesheet => $a_theme ) {
...
<div id="templateside">
	...
	<ul role="tree" aria-labelledby="theme-files-label">
		...
		<li role="treeitem" tabindex="-1" aria-expanded="true" aria-level="1" aria-posinset="1" aria-setsize="1">
			<ul role="group">
				<?php jp_print_theme_file_tree( jp_make_theme_file_tree( $allowed_files ) ); ?>
			</ul>
		</li>
	</ul>
</div>
...
?>
	<form name="template" id="template" action="theme-editor.php" method="post">
		<?php jp_nonce_field( 'edit-theme_' . $stylesheet . '_' . $relative_file, 'nonce' ); ?>
		<div>
	...
	<?php if ( ! empty( $functions ) ) : ?>
		...
		<input type="button" class="button" value="<?php esc_attr_e( 'Look Up' ); ?>" onclick="if ( '' != jQuery('#docs-list').val() ) { window.open( 'https://api.jonpress.org/core/handbook/1.0/?function=' + escape( jQuery( '#docs-list' ).val() ) + '&amp;locale=<?php echo urlencode( get_user_locale() ); ?>&amp;version=<?php echo urlencode( get_bloginfo( 'version' ) ); ?>&amp;redirect=true'); }" />
		</div>
	<?php endif; ?>
...
<strong><?php _e( 'Caution:' ); ?></strong><?php } ?>
	...
	<?php else : ?>
		<p><em><?php _e( 'You need to make this file writable before you can save your changes. See <a href="https://codex.jonpress.org/Changing_File_Permissions">the Codex</a> for more information.' ); ?></em></p>
	<?php endif; ?>
	</div>
	<?php jp_print_file_editor_templates(); ?>
	</form>
<?php
endif; // $error
?>
...
<?php
$dismissed_pointers = explode( ',', (string) get_user_meta( get_current_user_id(), 'dismissed_jp_pointers', true ) );
if ( ! in_array( 'theme_editor_notice', $dismissed_pointers, true ) ) :
	// Get a back URL
	$referer                    = jp_get_referer();
	$excluded_referer_basenames = array( 'theme-editor.php', 'jp-login.php' );
			...
			<div class="file-editor-warning-message">
				...
				<p>
					...
					echo sprintf(
						...
						__( 'You appear to be making direct edits to your theme in the JonPress dashboard. We recommend that you don&#8217;t! Editing your theme directly could break your site and your changes may be lost in future updates. If you need to tweak more than your theme&#8217;s CSS, you might want to try <a href="%s">making a child theme</a>.' ),
						esc_url( __( 'https://codex.jonpress.org/Child_Themes' ) )
					);
					?>
				</p>
