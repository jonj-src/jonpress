 ...
 * Core Metadata API
 *
 * Functions for retrieving and manipulating metadata of various JonPress object types. Metadata
...
function add_metadata( $meta_type, $object_id, $meta_key, $meta_value, $unique = false ) {
	...
	jp_cache_delete( $object_id, $meta_type . '_meta' );
...
function delete_metadata( $meta_type, $object_id, $meta_key, $meta_value = '', $delete_all = false ) {
	...
	if ( $delete_all ) {
		foreach ( (array) $object_ids as $o_id ) {
			jp_cache_delete( $o_id, $meta_type . '_meta' );
		}
	} else {
		jp_cache_delete( $object_id, $meta_type . '_meta' );
	}
...
function get_metadata( $meta_type, $object_id, $meta_key = '', $single = false ) {
	...
	$meta_cache = jp_cache_get( $object_id, $meta_type . '_meta' );
...
function metadata_exists( $meta_type, $object_id, $meta_key ) {
	...
	/** This filter is documented in jp-includes/meta.php */
...
function update_metadata_by_mid( $meta_type, $meta_id, $meta_value, $meta_key = false ) {
	...
	if ( $meta = get_metadata_by_mid( $meta_type, $meta_id ) ) {
		...
		/** This action is documented in jp-includes/meta.php */
		do_action( "update_{$meta_type}_meta", $meta_id, $object_id, $meta_key, $_meta_value );
...
function update_meta_cache( $meta_type, $object_ids ) {
	...
	foreach ( $object_ids as $id ) {
		$cached_object = jp_cache_get( $id, $cache_key );
	...
	foreach ( $ids as $id ) {
		...
		jp_cache_add( $id, $cache[ $id ], $cache_key );
	}
 ...
 * @since 4.5.0
 *
 * @return JP_Metadata_Lazyloader $lazyloader Metadata lazyloader queue.
 */
function jp_metadata_lazyloader() {
	static $jp_metadata_lazyloader;

	if ( null === $jp_metadata_lazyloader ) {
		$jp_metadata_lazyloader = new JP_Metadata_Lazyloader();
	}

	return $jp_metadata_lazyloader;
}
...
function get_meta_sql( $meta_query, $type, $primary_table, $primary_id_column, $context = null ) {
	$meta_query_obj = new JP_Meta_Query( $meta_query );
 ...
 *
 * @since 3.3.0
 * @since 4.6.0 {@link https://core.trac.jonpress.org/ticket/35658 Modified
...
function register_meta( $object_type, $meta_key, $args, $deprecated = null ) {
	...
	if ( ! $has_old_auth_cb && ! $has_old_sanitize_cb ) {
		$jp_meta_keys[ $object_type ][ $meta_key ] = $args;
...
function registered_meta_key_exists( $object_type, $meta_key ) {
	...
	if ( ! isset( $jp_meta_keys[ $object_type ] ) ) {
		return false;
	}

	if ( isset( $jp_meta_keys[ $object_type ][ $meta_key ] ) ) {
		return true;
	}
...
function unregister_meta_key( $object_type, $meta_key ) {
	...
	$args = $jp_meta_keys[ $object_type ][ $meta_key ];
	...
	unset( $jp_meta_keys[ $object_type ][ $meta_key ] );

	// Do some clean up
	if ( empty( $jp_meta_keys[ $object_type ] ) ) {
		unset( $jp_meta_keys[ $object_type ] );
	}
...
function get_registered_meta_keys( $object_type ) {
	...
	if ( ! is_array( $jp_meta_keys ) || ! isset( $jp_meta_keys[ $object_type ] ) ) {
		return array();
	}

	return $jp_meta_keys[ $object_type ];
}
 ...
 */
function _jp_register_meta_args_whitelist( $args, $default_args ) {
