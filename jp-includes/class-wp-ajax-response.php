...
class JP_Ajax_Response {
	...
	/**
	 * Constructor - Passes args to JP_Ajax_Response::add().
	 *
	 * @since 2.1.0
	 * @see JP_Ajax_Response::add()
	 *
	 ...
	 *     @type int|JP_Error    $id           The response ID, used as the response type `id` attribute. Also
	 *                                         accepts a `JP_Error` object if the ID does not exist. Default 0.
	 ...
	 *     @type string|JP_Error $data         The response content/message. Also accepts a JP_Error object if the
	...
	public function add( $args = '' ) {
		...
		if ( is_jp_error( $data ) ) {
			foreach ( (array) $data->get_error_codes() as $code ) {
				$response .= "<jp_error code='$code'><![CDATA[" . $data->get_error_message( $code ) . ']]></jp_error>';
				...
				$response .= "<jp_error_data code='$code'$class>";
	...
	public function send() {
		...
		echo "<?xml version='1.0' encoding='" . get_option( 'blog_charset' ) . "' standalone='yes'?><jp_ajax>";
