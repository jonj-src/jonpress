 ...
 * @since 0.71
 *
 * @global array $jp_cockneyreplace Array of formatted entities for certain common phrases
...
function wptexturize( $text, $reset = false ) {
	global $jp_cockneyreplace, $shortcode_tags;
	...
	if ( $reset || ! isset( $static_characters ) ) {
		...
		if ( isset( $jp_cockneyreplace ) ) {
			$cockney        = array_keys( $jp_cockneyreplace );
			$cockneyreplace = array_values( $jp_cockneyreplace );
		} else {
...
function wpautop( $pee, $br = true ) {
	...
	$pee = jp_replace_in_html_tags( $pee, array( "\n" => ' <!-- wpnl --> ' ) );
...
function _get_wptexturize_split_regex( $shortcode_regex = '' ) {
	...
	if ( ! isset( $html_regex ) ) {
		...
		$html_regex = // Needs replaced with jp_html_split() per Shortcode API Roadmap.
			'<'                  // Find start of element.
			. '(?(?=!--)'        // Is this a comment?
 ...
 */
function jp_replace_in_html_tags( $haystack, $replace_pairs ) {
	// Find all elements.
	$textarr = jp_html_split( $haystack );
	$changed = false;
 ...
 */
function _jp_specialchars( $string, $quote_style = ENT_NOQUOTES, $charset = false, $double_encode = false ) {
	...
	// Store the site charset as a static to avoid multiple calls to jp_load_alloptions()
	if ( ! $charset ) {
 ...
 *                                Also compatible with old _jp_specialchars() values;
 *                                converting single quotes if set to 'single',
 *                                double if set to 'double' or both if otherwise set.
 *                                Default is ENT_NOQUOTES.
 * @return string The decoded text without HTML entities.
 */
function jp_specialchars_decode( $string, $quote_style = ENT_NOQUOTES ) {
	$string = (string) $string;
	...
	// Match the previous behaviour of _jp_specialchars() when the $quote_style is not an accepted value
	if ( empty( $quote_style ) ) {
		$quote_style = ENT_NOQUOTES;
	} elseif ( ! in_array( $quote_style, array( 0, 2, 3, 'single', 'double' ), true ) ) {
		$quote_style = ENT_QUOTES;
 ...
 */
function jp_check_invalid_utf8( $string, $strip = false ) {
	$string = (string) $string;
...
function sanitize_file_name( $filename ) {
	...
	if ( false === strpos( $filename, '.' ) ) {
		$mime_types = jp_get_mime_types();
		$filetype   = jp_check_filetype( 'test.' . $filename, $mime_types );
...
function sanitize_user( $username, $strict = false ) {
	...
	$username     = jp_strip_all_tags( $username );
...
function convert_invalid_entities( $content ) {
	...
	if ( strpos( $content, '&#1' ) !== false ) {
		$content = strtr( $content, $jp_htmltranswinuni );
	}
 ...
 */
function jp_rel_nofollow( $text ) {
	// This is a pre save filter, so text is already escaped.
	$text = stripslashes( $text );
	$text = preg_replace_callback( '|<a (.+?)>|i', 'jp_rel_nofollow_callback', $text );
	return jp_slash( $text );
}
 ...
 */
function jp_rel_nofollow_callback( $matches ) {
	$text = $matches[1];
 ...
 * @return string Converted content.
 */
function jp_targeted_link_rel( $text ) {
	// Don't run (more expensive) regex if no links with targets.
	if ( stripos( $text, 'target' ) !== false && stripos( $text, '<a ' ) !== false ) {
		$text = preg_replace_callback( '|<a\s([^>]*target\s*=[^>]*)>|i', 'jp_targeted_link_rel_callback', $text );
	}
 ...
 */
function jp_targeted_link_rel_callback( $matches ) {
	$link_html = $matches[1];
	$rel_match = array();
	 ...
	 */
	$rel = apply_filters( 'jp_targeted_link_rel', 'noopener noreferrer', $link_html );
...
function translate_smiley( $matches ) {
	...
	return sprintf( '<img src="%s" alt="%s" class="jp-smiley" style="height: 1em; max-height: 1em;" />', esc_url( $src_url ), esc_attr( $smiley ) );
}
 ...
 * @since 0.71
 *
 * @global string|array $jp_smiliessearch
 *
 * @param string $text Content to convert smilies from text.
 * @return string Converted content with text smilies replaced with images.
 */
function convert_smilies( $text ) {
	global $jp_smiliessearch;
	$output = '';
	if ( get_option( 'use_smilies' ) && ! empty( $jp_smiliessearch ) ) {
		...
		for ( $i = 0; $i < $stop; $i++ ) {
			...
			if ( '' == $ignore_block_element && strlen( $content ) > 0 && '<' != $content[0] ) {
				$content = preg_replace_callback( $jp_smiliessearch, 'translate_smiley', $content );
			}
 ...
 */
function jp_iso_descrambler( $string ) {
	/* this may only work with iso-8859-1, I'm afraid */
	if ( ! preg_match( '#\=\?(.+)\?Q\?(.+)\?\=#i', $string, $matches ) ) {
		return $string;
	} else {
		$subject = str_replace( '_', ' ', $matches[2] );
		return preg_replace_callback( '#\=([0-9a-f]{2})#i', '_jp_iso_convert', $subject );
	}
}
 ...
 */
function _jp_iso_convert( $match ) {
...
function jp_trim_excerpt( $text = '' ) {
	...
	if ( '' == $text ) {
		...
		$text         = jp_trim_words( $text, $excerpt_length, $excerpt_more );
	}
	/**
	 ...
	 */
	return apply_filters( 'jp_trim_excerpt', $text, $raw_excerpt );
}
 ...
 */
function jp_trim_words( $text, $num_words = 55, $more = null ) {
	if ( null === $more ) {
		$more = __( '&hellip;' );
	}

	$original_text = $text;
	$text          = jp_strip_all_tags( $text );
	 ...
	 */
	return apply_filters( 'jp_trim_words', $text, $num_words, $more, $original_text );
}
 ...
 *                          Defaults to return value of jp_allowed_protocols()
...
function esc_url( $url, $protocols = null, $_context = 'display' ) {
	...
	if ( 'display' == $_context ) {
		$url = jp_kses_normalize_entities( $url );
		$url = str_replace( '&amp;', '&#038;', $url );
		$url = str_replace( "'", '&#039;', $url );
	}
	...
	if ( '/' === $url[0] ) {
		...
		if ( ! is_array( $protocols ) ) {
			$protocols = jp_allowed_protocols();
		}
		$good_protocol_url = jp_kses_bad_protocol( $url, $protocols );
...
function esc_js( $text ) {
	$safe_text = jp_check_invalid_utf8( $text );
	$safe_text = _jp_specialchars( $safe_text, ENT_COMPAT );
...
function esc_html( $text ) {
	$safe_text = jp_check_invalid_utf8( $text );
	$safe_text = _jp_specialchars( $safe_text, ENT_QUOTES );
	/**
 ...
 */
function jp_make_link_relative( $link ) {
	...
	switch ( $option ) {
		...
		case 'blogname':
			...
			if ( $value !== $original_value ) {
				$value = $wpdb->strip_invalid_text_for_column( $wpdb->options, 'option_value', jp_encode_emoji( $original_value ) );
			}
		...
		case 'siteurl':
			...
			if ( is_jp_error( $value ) ) {
				...
				if ( preg_match( '#http(s?)://(.+)#i', $value ) ) {
					...
					$error = __( 'The JonPress address you entered did not appear to be a valid URL. Please enter a valid URL.' );
				}
			}
			break;
		...
		case 'tag_base':
			...
			if ( 'permalink_structure' === $option && '' !== $value && ! preg_match( '/%[^\/%]+%/', $value ) ) {
				$error = sprintf(
					...
					__( 'https://codex.jonpress.org/Using_Permalinks#Choosing_your_permalink_structure' )
				);
			}
			break;
 ...
 */
function jp_parse_str( $string, &$array ) {
	 ...
	 */
	$array = apply_filters( 'jp_parse_str', $array );
}
 ...
 * @return string Converted text.
 */
function jp_pre_kses_less_than( $text ) {
	return preg_replace_callback( '%<[^>]*?((?=<)|>|$)%', 'jp_pre_kses_less_than_callback', $text );
}
 ...
 */
function jp_pre_kses_less_than_callback( $matches ) {
...
/**
 * JonPress implementation of PHP sprintf() with filters.
 *
 * @since 2.5.0
...
function jp_sprintf( $pattern ) {
	...
	while ( $len > $start ) {
		...
		if ( $pattern[ $start ] == '%' ) {
			...
			/**
			 * Filters a fragment from the pattern passed to jp_sprintf().
			 *
			 ...
			 */
			$_fragment = apply_filters( 'jp_sprintf', $fragment, $arg );
 ...
 */
function jp_sprintf_l( $pattern, $args ) {
	// Not a match
	...
	/**
	 * Filters the translated delimiters used by jp_sprintf_l().
 ...
 * @return string The excerpt.
 */
function jp_html_excerpt( $str, $count, $more = null ) {
	if ( null === $more ) {
		$more = '';
	}
	$str     = jp_strip_all_tags( $str, true );
...
function _links_add_base( $m ) {
	...
	return $m[1] . '=' . $m[2] .
		( preg_match( '#^(\w{1,20}):#', $m[3], $protocol ) && in_array( $protocol[1], jp_allowed_protocols() ) ?
			$m[3] :
			JP_Http::make_absolute_url( $m[3], $_links_add_base )
		)
		. $m[2];
}
 ...
 * will return 'something'. jp_strip_all_tags will return ''
 *
 * @since 2.9.0
 *
 ...
 */
function jp_strip_all_tags( $string, $remove_breaks = false ) {
 ...
 * @see jp_check_invalid_utf8()
 * @see jp_strip_all_tags()
 *
...
function _sanitize_text_fields( $str, $keep_newlines = false ) {
	$filtered = jp_check_invalid_utf8( $str );

	if ( strpos( $filtered, '<' ) !== false ) {
		$filtered = jp_pre_kses_less_than( $filtered );
		// This will strip extra whitespace for us.
		$filtered = jp_strip_all_tags( $filtered, false );
 ...
 * @return string
 */
function jp_basename( $path, $suffix = '' ) {
	return urldecode( basename( str_replace( array( '%2F', '%5C' ), '/', urlencode( $path ) ), $suffix ) );
}

// phpcs:disable JonPress.WP.CapitalPDangit.Misspelled, JonPress.NamingConventions.ValidFunctionName.FunctionNameInvalid -- 8-)
/**
...
function capital_P_dangit( $text ) {
	...
	if ( 'the_title' === $current_filter || 'jp_title' === $current_filter ) {
		return str_replace( 'Wordpress', 'JonPress', $text );
	}
	...
	return str_replace(
		...
		array( ' JonPress', '&#8216;JonPress', $dblq . 'JonPress', '>JonPress', '(JonPress' ),
		$text
	);
}
// phpcs:enable
...
function jp_spaces_regexp() {
	...
	if ( empty( $spaces ) ) {
		 ...
		 */
		$spaces = apply_filters( 'jp_spaces_regexp', '[\r\n\t ]|\xC2\xA0|&nbsp;' );
	}
...
function _print_emoji_detection_script() {
	...
	if ( SCRIPT_DEBUG ) {
		$settings['source'] = array(
			/** This filter is documented in jp-includes/class.jp-scripts.php */
			'wpemoji' => apply_filters( 'script_loader_src', includes_url( "js/jp-emoji.js?$version" ), 'wpemoji' ),
			/** This filter is documented in jp-includes/class.jp-scripts.php */
		...
		<script type="text/javascript">
			window._wpemojiSettings = <?php echo jp_json_encode( $settings ); ?>;
			<?php readfile( ABSPATH . WPINC . '/js/jp-emoji-loader.js' ); ?>
		</script>
		<?php
	} else {
		$settings['source'] = array(
			/** This filter is documented in jp-includes/class.jp-scripts.php */
			'concatemoji' => apply_filters( 'script_loader_src', includes_url( "js/jp-emoji-release.min.js?$version" ), 'concatemoji' ),
		);

		/*
		 * If you're looking at a src version of this file, you'll see an "include"
		 * statement below. This is used by the `grunt build` process to directly
		 * include a minified version of jp-emoji-loader.js, instead of using the
		 * readfile() method from above.
		 *
		 * If you're looking at a build version of this file, you'll see a string of
		 * minified JavaScript. If you need to debug it, please turn on SCRIPT_DEBUG
		 * and edit jp-emoji-loader.js directly.
		 */
		?>
		<script type="text/javascript">
			window._wpemojiSettings = <?php echo jp_json_encode( $settings ); ?>;
 ...
 */
function jp_encode_emoji( $content ) {
	$emoji = _jp_emoji_list( 'partials' );
 ...
 * @return string The encoded content.
 */
function jp_staticize_emoji( $text ) {
	if ( false === strpos( $text, '&#x' ) ) {
		if ( ( function_exists( 'mb_check_encoding' ) && mb_check_encoding( $text, 'ASCII' ) ) || ! preg_match( '/[^\x00-\x7F]/', $text ) ) {
			...
			$encoded_text = jp_encode_emoji( $text );
	...
	for ( $i = 0; $i < $stop; $i++ ) {
		...
		if ( '' == $ignore_block_element && strlen( $content ) > 0 && '<' != $content[0] && false !== strpos( $content, '&#x' ) ) {
			foreach ( $possible_emoji as $emojum => $emoji_char ) {
				...
				$entity = sprintf( '<img src="%s" alt="%s" class="jp-smiley" style="height: 1em; max-height: 1em;" />', $cdn_url . $file . $ext, $emoji_char );
 ...
 */
function jp_staticize_emoji_for_email( $mail ) {
	 ...
	 * in jp_mail() - get the Content-Type from the headers, if there is one,
	 * then pass it through the jp_mail_content_type filter, in case a plugin
	...
	if ( 'text/html' === $content_type ) {
		$mail['message'] = jp_staticize_emoji( $mail['message'] );
	}
 ...
 * @return array An array to match all emoji that JonPress recognises.
 */
function _jp_emoji_list( $type = 'entities' ) {
 ...
 *
 * @since 1.2.0
 * @since 4.4.0 Moved to jp-includes/formatting.php from jp-admin/includes/misc.php and added $length param.
 *
