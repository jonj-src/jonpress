...
wp.customize.navMenusPreview = wp.customize.MenusCustomizerPreview = ( function( $, _, wp, api ) {
	...
	self.init = function() {
		...
		if ( api.selectiveRefresh ) {
			...
			api.bind( 'add', function( setting ) {
				 ...
				 * being exported to the frontend by the _is_valid_nav_menu_item filter in jp_get_nav_menu_items(),
			...
			/*
			 * Ensure that jp_nav_menu() instances nested inside of other partials
	...
	if ( api.selectiveRefresh ) {
		...
		/**
		 * Partial representing an invocation of jp_nav_menu().
		 *
		...
		self.onChangeNavMenuLocationsSetting = function() {
			...
			// If there are no jp_nav_menu() instances that refer to the theme location, do full refresh.
