...
(function( exports, $ ){
	...
	api.isLinkPreviewable = function isLinkPreviewable( element, options ) {
		...
		if ( /\/jp-(login|signup)\.php$/.test( element.pathname ) ) {
			return false;
		}

		// Allow links to admin ajax as faux frontend URLs.
		if ( /\/jp-admin\/admin-ajax\.php$/.test( element.pathname ) ) {
			return args.allowAdminAjax;
		}

		// Disallow links to admin, includes, and content.
		if ( /\/jp-(admin|includes|content)(\/|$)/.test( element.pathname ) ) {
			return false;
		}
	...
	api.prepareLinkPreview = function prepareLinkPreview( element ) {
		...
		// Ignore links with class jp-playlist-caption
		if ( $element.hasClass( 'jp-playlist-caption' ) ) {
			return;
		}
	...
	api.settingPreviewHandlers = {
		...
		custom_logo: function( attachmentId ) {
			$( 'body' ).toggleClass( 'jp-custom-logo', !! attachmentId );
		},
		...
		background: function() {
			 ...
			 *
			 * See get_body_class() in /jp-includes/post-template.php
			 */
	...
	$( function() {
		...
		api.preview.bind( 'loading-initiated', function () {
			$( 'body' ).addClass( 'jp-customizer-unloading' );
		});
		api.preview.bind( 'loading-failed', function () {
			$( 'body' ).removeClass( 'jp-customizer-unloading' );
		});
		 ...
		 * Custom Logo
		 *
		 * Toggle the jp-custom-logo body class when a logo is added or removed.
		 *
		 * @since 4.5.0
		 */
