...
if ( is_multisite() && ! is_network_admin() ) {
	jp_redirect( network_admin_url( 'plugin-editor.php' ) );
	exit();
}

if ( ! current_user_can( 'edit_plugins' ) ) {
	jp_die( __( 'Sorry, you are not allowed to edit plugins for this site.' ) );
}
...
if ( isset( $_REQUEST['file'] ) ) {
	$file = jp_unslash( $_REQUEST['file'] );
}

if ( isset( $_REQUEST['plugin'] ) ) {
	$plugin = jp_unslash( $_REQUEST['plugin'] );
}
...
if ( ! is_file( $real_file ) ) {
	jp_die( sprintf( '<p>%s</p>', __( 'No such file exists! Double check the name and try again.' ) ) );
} else {
	...
	if ( preg_match( '/\.([^.]+)$/', $real_file, $matches ) ) {
		...
		if ( ! in_array( $ext, $editable_extensions ) ) {
			jp_die( sprintf( '<p>%s</p>', __( 'Files of this type are not editable.' ) ) );
		}
	}
}
	...
	get_current_screen()->set_help_sidebar(
		...
		'<p>' . __( '<a href="https://codex.jonpress.org/Plugins_Editor_Screen">Documentation on Editing Plugins</a>' ) . '</p>' .
		'<p>' . __( '<a href="https://codex.jonpress.org/Writing_a_Plugin">Documentation on Writing Plugins</a>' ) . '</p>' .
		'<p>' . __( '<a href="https://jonpress.org/support/">Support Forums</a>' ) . '</p>'
	);

	$settings = array(
		'codeEditor' => jp_enqueue_code_editor( array( 'file' => $real_file ) ),
	);
	...
	jp_add_inline_script( 'jp-theme-plugin-editor', sprintf( 'wp.themePluginEditor.themeOrPlugin = "plugin";' ) );

	require_once( ABSPATH . 'jp-admin/admin-header.php' );

	update_recently_edited( JP_PLUGIN_DIR . '/' . $file );
...
<div id="templateside">
	...
	<li role="treeitem" tabindex="-1" aria-expanded="true" aria-level="1" aria-posinset="1" aria-setsize="1">
		<ul role="group">
			<?php jp_print_plugin_file_tree( jp_make_plugin_file_tree( $plugin_editable_files ) ); ?>
		</ul>
	</ul>
</div>
<form name="template" id="template" action="plugin-editor.php" method="post">
	<?php jp_nonce_field( 'edit-plugin_' . $file, 'nonce' ); ?>
		<div>
		...
		<div id="documentation" class="hide-if-no-js"><label for="docs-list"><?php _e( 'Documentation:' ); ?></label> <?php echo $docs_select; ?> <input type="button" class="button" value="<?php esc_attr_e( 'Look Up' ); ?> " onclick="if ( '' != jQuery('#docs-list').val() ) { window.open( 'https://api.jonpress.org/core/handbook/1.0/?function=' + escape( jQuery( '#docs-list' ).val() ) + '&amp;locale=<?php echo urlencode( get_user_locale() ); ?>&amp;version=<?php echo urlencode( get_bloginfo( 'version' ) ); ?>&amp;redirect=true'); }" /></div>
		<?php endif; ?>
...
if ( ! in_array( 'plugin_editor_notice', $dismissed_pointers, true ) ) :
	...
	$excluded_referer_basenames = array( 'plugin-editor.php', 'jp-login.php' );
	...
	<div class="notification-dialog">
		<div class="file-editor-warning-content">
			<div class="file-editor-warning-message">
				...
				<p><?php _e( 'You appear to be making direct edits to your plugin in the JonPress dashboard. We recommend that you don&#8217;t! Editing plugins directly may introduce incompatibilities that break your site and your changes may be lost in future updates.' ); ?></p>
