...
/**
 * Core class used to implement the JP_User object.
 *
 * @since 2.0.0
 *
...
class JP_User {
	 ...
	 * Constructor.
	 *
	 * Retrieves the userdata and passes it to JP_User::init().
	 *
	 * @since 2.0.0
	 *
	 * @global wpdb $wpdb JonPress database abstraction object.
	 *
	 * @param int|string|stdClass|JP_User $id User's ID, a JP_User object, or a user object from the DB.
	...
	public static function get_data_by( $field, $value ) {
		...
		switch ( $field ) {
			case 'id':
				...
				break;
			case 'slug':
				$user_id  = jp_cache_get( $value, 'userslugs' );
				$db_field = 'user_nicename';
				break;
			case 'email':
				$user_id  = jp_cache_get( $value, 'useremail' );
				$db_field = 'user_email';
				break;
			case 'login':
				$value    = sanitize_user( $value );
				$user_id  = jp_cache_get( $value, 'userlogins' );
				$db_field = 'user_login';
				break;
			default:
				return false;
		...
		if ( false !== $user_id ) {
			if ( $user = jp_cache_get( $user_id, 'users' ) ) {
				return $user;
			}
		}
	...
	public function __isset( $key ) {
		if ( 'id' == $key ) {
			_deprecated_argument(
				'JP_User->id', '2.1.0',
				sprintf(
					/* translators: %s: JP_User->ID */
					__( 'Use %s instead.' ),
					'<code>JP_User->ID</code>'
				)
			);
			$key = 'ID';
		}
	 ...
	 * the value on the JP_User instance.
	 *
	 * @since 3.3.0
	 *
	 ...
	 *
	 * @since 2.1.0
	 * @deprecated 4.9.0 Use JP_User::for_site()
	 *
	 * @global wpdb $wpdb JonPress database abstraction object.
	 *
	...
	protected function _init_caps( $cap_key = '' ) {
		...
		_deprecated_function( __METHOD__, '4.9.0', 'JP_User::for_site()' );
	...
	public function get_role_caps() {
		...
		if ( is_array( $this->caps ) ) {
			$this->roles = array_filter( array_keys( $this->caps ), array( $jp_roles, 'is_role' ) );
		}

		// Build $allcaps from role caps, overlay user's $caps.
		$this->allcaps = array();
		foreach ( (array) $this->roles as $role ) {
			$the_role      = $jp_roles->get_role( $role );
	...
	public function has_cap( $cap ) {
		 ...
		 * @param JP_User  $user    The user object.
		 */
