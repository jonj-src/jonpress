<?php
/**
 * JonPress Link Template Functions
 *
 * @package JonPress
 * @subpackage Template
 */
 ...
 *
 * @param int|JP_Post $post Optional. Post ID or post object. Default is the global `$post`.
 */
function the_permalink( $post = 0 ) {
	 ...
	 * @param int|JP_Post $post      Post ID, JP_Post object, or 0. Default 0.
	 */
...
function user_trailingslashit( $string, $type_of_url = '' ) {
	global $jp_rewrite;
	if ( $jp_rewrite->use_trailing_slashes ) {
...
function get_permalink( $post = 0, $leavename = false ) {
	 ...
	 * @param JP_Post $post      The post in question.
	...
	if ( '' != $permalink && ! in_array( $post->post_status, array( 'draft', 'pending', 'auto-draft', 'future' ) ) ) {
		...
		if ( strpos( $permalink, '%category%' ) !== false ) {
			...
			if ( $cats ) {
				 ...
				 * @since 3.5.0
				 *
				 * @param JP_Term  $cat  The category to use in the permalink.
				 * @param array    $cats Array of all categories (JP_Term objects) associated with the post.
				 * @param JP_Post  $post The post in question.
				 */
			...
			if ( empty( $category ) ) {
				...
				if ( $default_category && ! is_jp_error( $default_category ) ) {
 ...
 * @global JP_Rewrite $jp_rewrite
 *
 * @param int|JP_Post $id        Optional. Post ID or post object. Default is the global `$post`.
 ...
 * @return string|JP_Error The post permalink.
 */
function get_post_permalink( $id = 0, $leavename = false, $sample = false ) {
	...
	$post_link = $jp_rewrite->get_extra_permastruct( $post->post_type );
 ...
 * @since 1.5.0
 *
 * @param int|JP_Post $post      Optional. Post ID or object. Default uses the global `$post`.
...
function _get_page_link( $post = false, $leavename = false, $sample = false ) {
	...
	$link = $jp_rewrite->get_page_permastruct();
 ...
 *
 * This can be used in the JonPress Loop or outside of it.
 *
 * @since 2.0.0
 *
 * @global JP_Rewrite $jp_rewrite
...
function get_attachment_link( $post = null, $leavename = false ) {
	...
	if ( $jp_rewrite->using_permalinks() && $parent ) {
	...
	} elseif ( $jp_rewrite->using_permalinks() && ! $leavename ) {
...
function get_year_link( $year ) {
	...
	$yearlink = $jp_rewrite->get_year_permastruct();
	if ( ! empty( $yearlink ) ) {
...
function get_month_link( $year, $month ) {
	...
	$monthlink = $jp_rewrite->get_month_permastruct();
	if ( ! empty( $monthlink ) ) {
...
function get_day_link( $year, $month, $day ) {
	...
	$daylink = $jp_rewrite->get_day_permastruct();
	if ( ! empty( $daylink ) ) {
...
function get_feed_link( $feed = '' ) {
	...
	$permalink = $jp_rewrite->get_feed_permastruct();
	if ( '' != $permalink ) {
		if ( false !== strpos( $feed, 'comments_' ) ) {
			$feed      = str_replace( 'comments_', '', $feed );
			$permalink = $jp_rewrite->get_comment_feed_permastruct();
		}
...
function get_term_feed_link( $term_id, $taxonomy = 'category', $feed = '' ) {
	...
	if ( empty( $term ) || is_jp_error( $term ) ) {
		return false;
	}
 ...
 * @param JP_Term $tag    Optional. Term object. If null, the queried object will be inspected.
 *                        Default null.
 */
...
function get_search_link( $query = '' ) {
	...
	$permastruct = $jp_rewrite->get_search_permastruct();
...
function get_post_type_archive_link( $post_type ) {
	...
	if ( get_option( 'permalink_structure' ) && is_array( $post_type_obj->rewrite ) ) {
		...
		if ( $post_type_obj->rewrite['with_front'] ) {
			$struct = $jp_rewrite->front . $struct;
		} else {
			$struct = $jp_rewrite->root . $struct;
		}
 ...
 * @since 4.4.0
 *
 * @param int|JP_Post $post         Optional. Post ID or `JP_Post` object. Defaults to global `$post`.
 ...
 *
 * Can be used within the JonPress loop or outside of it. Can be used with
 ...
 *
 * Can be used within the JonPress loop or outside of it, with any post type.
 *
 * @since 2.9.0
 *
 * @param int|JP_Post $id           Optional. Post ID or post object. Default is the global `$post`.
...
function get_delete_post_link( $id = 0, $deprecated = '', $force_delete = false ) {
	 ...
	 */
	return apply_filters( 'get_delete_post_link', jp_nonce_url( $delete_link, "$action-post_{$post->ID}" ), $post->ID, $force_delete );
}
 ...
 * @since 2.3.0
 *
 * @param int|JP_Comment $comment_id Optional. Comment ID or JP_Comment object.
 ...
 * @return null|string|JP_Post Post object if successful. Null if global $post is not set. Empty string if no
...
function get_adjacent_post( $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	...
	if ( $in_same_term || ! empty( $excluded_terms ) ) {
		...
		if ( $in_same_term ) {
			...
			$term_array = jp_get_object_terms( $post->ID, $taxonomy, array( 'fields' => 'ids' ) );

			// Remove any exclusions from the term array to include.
			$term_array = array_diff( $term_array, (array) $excluded_terms );
			$term_array = array_map( 'intval', $term_array );

			if ( ! $term_array || is_jp_error( $term_array ) ) {
				return '';
			}
	 ...
	 * @param JP_Post $post           JP_Post object.
	 */
	...
	$result    = jp_cache_get( $query_key, 'counts' );
	if ( false !== $result ) {
		if ( $result ) {
	...
	jp_cache_set( $query_key, $result, 'counts' );
 ...
 *
 * This is meant to be attached to actions like 'jp_head'. Do not call this directly in plugins
 * or theme templates.
 *
 * @since 3.0.0
 *
 ...
 */
function adjacent_posts_rel_link_jp_head() {
...
function get_adjacent_post_link( $format, $link, $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	 ...
	 * @param string  $format   Link anchor format.
	 * @param string  $link     Link permalink format.
	 * @param JP_Post $post     The adjacent post.
...
function get_pagenum_link( $pagenum = 1, $escape = true ) {
	...
	if ( ! $jp_rewrite->using_permalinks() || is_admin() ) {
		...
		$request = preg_replace( "|$jp_rewrite->pagination_base/\d+/?$|", '', $request );
		$request = preg_replace( '|^' . preg_quote( $jp_rewrite->index, '|' ) . '|i', '', $request );
		$request = ltrim( $request, '/' );

		$base = trailingslashit( get_bloginfo( 'url' ) );

		if ( $jp_rewrite->using_index_permalinks() && ( $pagenum > 1 || '' != $request ) ) {
			$base .= $jp_rewrite->index . '/';
		}

		if ( $pagenum > 1 ) {
			$request = ( ( ! empty( $request ) ) ? trailingslashit( $request ) : $request ) . user_trailingslashit( $jp_rewrite->pagination_base . '/' . $pagenum, 'paged' );
		}
...
function get_next_posts_link( $label = null, $max_page = 0 ) {
	global $paged, $jp_query;

	if ( ! $max_page ) {
		$max_page = $jp_query->max_num_pages;
	}
...
function get_posts_nav_link( $args = array() ) {
	...
	if ( ! is_singular() ) {
		...
		$max_num_pages = $jp_query->max_num_pages;
 ...
 * @since 4.1.0
 *
 * @global JP_Query $jp_query JonPress Query object.
 *
 * @param array $args {
...
function get_the_posts_navigation( $args = array() ) {
	...
	if ( $GLOBALS['jp_query']->max_num_pages > 1 ) {
		$args = jp_parse_args(
			$args, array(
	...
	if ( 'newest' == get_option( 'default_comments_page' ) ) {
		if ( $pagenum != $max_page ) {
			if ( $jp_rewrite->using_permalinks() ) {
				$result = user_trailingslashit( trailingslashit( $result ) . $jp_rewrite->comments_pagination_base . '-' . $pagenum, 'commentpaged' );
			} else {
...
function get_next_comments_link( $label = '', $max_page = 0 ) {
	...
	if ( empty( $max_page ) ) {
		$max_page = $jp_query->max_num_comment_pages;
	}
...
function paginate_comments_links( $args = array() ) {
	...
	if ( $jp_rewrite->using_permalinks() ) {
		$defaults['base'] = user_trailingslashit( trailingslashit( get_permalink() ) . $jp_rewrite->comments_pagination_base . '-%#%', 'commentpaged' );
	}
...
function get_home_url( $blog_id = null, $path = '', $scheme = null ) {
	...
	if ( ! in_array( $scheme, array( 'http', 'https', 'relative' ) ) ) {
		if ( is_ssl() && ! is_admin() && 'jp-login.php' !== $pagenow ) {
			$scheme = 'https';
		} else {
...
/**
 * Retrieves the URL for the current site where JonPress application files
 * (e.g. jp-blog-header.php or the jp-admin/ folder) are accessible.
 *
...
/**
 * Retrieves the URL for a given site where JonPress application files
 * (e.g. jp-blog-header.php or the jp-admin/ folder) are accessible.
 *
 * Returns the 'site_url' option with the appropriate protocol, 'https' if
...
function get_admin_url( $blog_id = null, $path = '', $scheme = 'admin' ) {
	$url = get_site_url( $blog_id, 'jp-admin/', $scheme );
...
function includes_url( $path = '', $scheme = null ) {
	 ...
	 * @param string $path Path relative to the URL to the jp-includes directory. Blank string
...
function content_url( $path = '' ) {
	$url = set_url_scheme( JP_CONTENT_URL );
...
function plugins_url( $path = '', $plugin = '' ) {
	...
	$path          = jp_normalize_path( $path );
	$plugin        = jp_normalize_path( $plugin );
	$mu_plugin_dir = jp_normalize_path( WPMU_PLUGIN_DIR );
...
function network_admin_url( $path = '', $scheme = 'admin' ) {
	...
	$url = network_site_url( 'jp-admin/network/', $scheme );
...
function user_admin_url( $path = '', $scheme = 'admin' ) {
	$url = network_site_url( 'jp-admin/user/', $scheme );
 ...
 *                      been published yet.
 */
function jp_get_canonical_url( $post = null ) {
	$post = get_post( $post );
 ...
 *
 * @since 2.9.0
 * @since 4.6.0 Adjusted to use `jp_get_canonical_url()`.
 */
 ...
 *                are not enabled.
 */
function jp_get_shortlink( $id = 0, $context = 'post', $allow_slugs = true ) {
	/**
 ...
 *
 * Attached to the {@see 'jp_head'} action.
 *
 * @since 3.0.0
 */
function jp_shortlink_jp_head() {
	$shortlink = jp_get_shortlink( 0, 'query' );
...
function the_shortlink( $text = '', $title = '', $before = '', $after = '' ) {
	...
	$shortlink = jp_get_shortlink( $post->ID );
...
function get_avatar_data( $id_or_email, $args = null ) {
	...
	if ( is_numeric( $id_or_email ) ) {
	...
	} elseif ( $id_or_email instanceof JP_User ) {
		// User Object
		$user = $id_or_email;
	} elseif ( $id_or_email instanceof JP_Post ) {
		// Post Object
		$user = get_user_by( 'id', (int) $id_or_email->post_author );
	} elseif ( $id_or_email instanceof JP_Comment ) {
		/**
		...
		if ( ( ! $user || is_jp_error( $user ) ) && ! empty( $id_or_email->comment_author_email ) ) {
