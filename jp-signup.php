...
/** Sets up the JonPress Environment. */
...
if ( is_array( get_site_option( 'illegal_names' ) ) && isset( $_GET['new'] ) && in_array( $_GET['new'], get_site_option( 'illegal_names' ) ) ) {
	jp_redirect( network_home_url() );
	die();
}

/**
 * Prints signup_header via jp_head
 *
 * @since MU (3.0.0)
 */
...
add_action( 'jp_head', 'do_signup_header' );

if ( ! is_multisite() ) {
	jp_redirect( jp_registration_url() );
	die();
}

if ( ! is_main_site() ) {
	jp_redirect( network_site_url( 'jp-signup.php' ) );
	die();
}

// Fix for page title
$jp_query->is_404 = false;
...
add_action( 'jp_head', 'wpmu_signup_stylesheet' );
get_header( 'jp-signup' );
...
<div class="mu_register jp-signup-container">
<?php
/**
 ...
 * @param JP_Error|string $errors     A JP_Error object containing existing errors. Defaults to empty string.
 */
function show_blog_form( $blogname = '', $blog_title = '', $errors = '' ) {
	 ...
	 * @since 3.0.0
	 *
	 * @param JP_Error $errors A JP_Error object possibly containing 'blogname' or 'blog_title' errors.
	 */
...
function show_user_form( $user_name = '', $user_email = '', $errors = '' ) {
	 ...
	 * @since 3.0.0
	 *
	 * @param JP_Error $errors A JP_Error object containing 'user_name' or 'user_email' errors.
	 */
...
function signup_another_blog( $blogname = '', $blog_title = '', $errors = '' ) {
	 ...
	 *     @type JP_Error $errors     A JP_Error object possibly containing 'blogname' or 'blog_title' errors.
	 * }
	 */
	...
	<form id="setupform" method="post" action="jp-signup.php">
...
function validate_another_blog_signup() {
	...
	if ( ! empty( $_POST['WPLANG'] ) ) {
		...
		if ( in_array( $_POST['WPLANG'], $languages ) ) {
			$language = jp_unslash( sanitize_text_field( $_POST['WPLANG'] ) );
...
function confirm_another_blog_signup( $domain, $path, $blog_title, $user_name, $user_email = '', $meta = array(), $blog_id = 0 ) {
	...
	if ( $blog_id ) {
		...
		$login_url = 'http://' . $domain . $path . 'jp-login.php';
	}
...
function signup_user( $user_name = '', $user_email = '', $errors = '' ) {
	 ...
	 *     @type JP_Error $errors     A JP_Error object with possible errors relevant to the sign-up user.
	 * }
	 */
	...
	<form id="setupform" method="post" action="jp-signup.php" novalidate="novalidate">
		<input type="hidden" name="stage" value="validate-user-signup" />
		<?php
		/** This action is documented in jp-signup.php */
...
function validate_user_signup() {
	...
	/** This filter is documented in jp-signup.php */
...
function signup_blog( $user_name = '', $user_email = '', $blogname = '', $blog_title = '', $errors = '' ) {
	 ...
	 *     @type JP_Error $errors     A JP_Error object with possible errors relevant to new site creation variables.
	 * }
	 */
...
if ( $active_signup == 'none' ) {
	...
	$login_url = jp_login_url( network_site_url( 'jp-signup.php' ) );
...
<?php
/**
 * Fires after the sign-up forms, before jp_footer.
 *
 * @since 3.0.0
 */
