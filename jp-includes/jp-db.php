...
/**
 * JonPress Database Access Abstraction Object
 *
 * It is possible to replace this class with your own
 * by setting the $wpdb global variable in jp-content/db.php
 * file to your class. The wpdb class will still be included,
 * so you can extend it or simply use your own.
 *
 * @link https://codex.jonpress.org/Function_Reference/wpdb_Class
 *
 * @since 0.71
 */
class wpdb {
	 ...
	 *
	 * Default behavior is to show errors if both JP_DEBUG and JP_DEBUG_DISPLAY
	 * evaluated to true.
	 *
	 * @since 0.71
	 * @var bool
	...
	/**
	 * JonPress table prefix
	 *
	 * You can set this to have multiple JonPress installations
	...
	/**
	 * List of JonPress per-blog tables
	 *
	 * @since 2.5.0
	 * @see wpdb::tables()
	 * @var array
	...
	/**
	 * List of deprecated JonPress tables
	 *
	...
	/**
	 * List of JonPress global tables
	 *
	 * @since 3.0.0
	 * @see wpdb::tables()
	 * @var array
	...
	/**
	 * JonPress Comment Metadata table
	 *
	 * @since 2.9.0
	 * @var string
	 */
	...
	/**
	 * JonPress Term Relationships table
	 *
	 * @since 2.3.0
	 * @var string
	 */
	 ...
	 * against the required MySQL version for JonPress. Normally, a replacement
	 ...
	 * to the database.
	 *
	 * @link https://core.trac.jonpress.org/ticket/3354
	 * @since 2.0.8
	 *
	...
	public function __construct( $dbuser, $dbpassword, $dbname, $dbhost ) {
		...
		if ( JP_DEBUG && JP_DEBUG_DISPLAY ) {
			$this->show_errors();
		}

		// Use ext/mysqli if it exists unless JP_USE_EXT_MYSQL is defined as true
		if ( function_exists( 'mysqli_connect' ) ) {
			$this->use_mysqli = true;

			if ( defined( 'JP_USE_EXT_MYSQL' ) ) {
				$this->use_mysqli = ! JP_USE_EXT_MYSQL;
			}
		}
		...
		// jp-config.php creation will manually connect when ready.
	...
	/**
	 * Change the current SQL mode, and ensure its JonPress compatibility.
	 *
	...
	/**
	 * Sets the table prefix for the JonPress tables.
	 *
	 * @since 2.5.0
	 *
	 * @param string $prefix          Alphanumeric name for the new prefix.
	 * @param bool   $set_table_names Optional. Whether the table names, e.g. wpdb::$posts, should be updated or not.
	 * @return string|JP_Error Old prefix or JP_Error on error
	 */
	public function set_prefix( $prefix, $set_table_names = true ) {

		if ( preg_match( '|[^a-z0-9_]|i', $prefix ) ) {
			return new JP_Error( 'invalid_db_prefix', 'Invalid database prefix' );
		}
	...
	/**
	 * Returns an array of JonPress tables.
	 *
	 * Also allows for the CUSTOM_USER_TABLE and CUSTOM_USER_META_TABLE to
	 * override the JonPress users and usermeta tables that would otherwise
		...
		if ( ! $success ) {
			...
			if ( ! did_action( 'template_redirect' ) ) {
				...
				$message .= '<p>' . sprintf(
					...
					__( 'If you don&#8217;t know how to set up a database you should <strong>contact your host</strong>. If all else fails you may find help at the <a href="%s">JonPress Support Forums</a>.' ),
	...
	public function print_error( $str = '' ) {
		...
		if ( $caller = $this->get_caller() ) {
			...
			$error_str = sprintf( __( 'JonPress database error %1$s for query %2$s made by %3$s' ), $str, $this->last_query, $caller );
		} else {
			/* translators: 1: Database error message, 2: SQL query */
			$error_str = sprintf( __( 'JonPress database error %1$s for query %2$s' ), $str, $this->last_query );
		}
	...
	public function db_connect( $allow_bail = true ) {
		...
		if ( $this->use_mysqli ) {
			...
			if ( $this->dbh->connect_errno ) {
				 ...
				 * It's possible ext/mysqli is misconfigured. Fall back to ext/mysql if:
				  ...
				  *  - JP_USE_EXT_MYSQL isn't set to false, and
				  *  - ext/mysql is loaded.
				  */
				...
				} elseif ( defined( 'JP_USE_EXT_MYSQL' ) && ! JP_USE_EXT_MYSQL ) {
					$attempt_fallback = false;
		...
		if ( ! $this->dbh && $allow_bail ) {
			...
			$message .= '<p>' . sprintf(
				/* translators: 1: jp-config.php, 2: database host */
			...
			$message .= '<p>' . sprintf(
				/* translators: %s: support forums URL */
				__( 'If you&#8217;re unsure what these terms mean you should probably contact your host. If you still need help you can always visit the <a href="%s">JonPress Support Forums</a>.' ),
				__( 'https://jonpress.org/support/' )
			) . "</p>\n";
	...
	public function check_connection( $allow_bail = true ) {
		...
		for ( $tries = 1; $tries <= $this->reconnect_retries; $tries++ ) {
			...
			if ( $this->reconnect_retries === $tries && JP_DEBUG ) {
				error_reporting( $error_reporting );
			}
		...
		// If template_redirect has already happened, it's too late for jp_die()/dead_db().
	 ...
	 *
	 * @param string $table Table name.
	 * @return string|JP_Error Table character set, JP_Error object if it couldn't be found.
	 */
	protected function get_table_charset( $table ) {
		...
		if ( ! $results ) {
			return new JP_Error( 'wpdb_get_table_charset_failure' );
		}
	 ...
	 * @return string|false|JP_Error Column character set as a string. False if the column has no
	 *                               character set. JP_Error object if there was an error.
	 */
	 ...
	 * @param string $table  Table name.
	 * @param string $column Column name.
	 * @return array|false|JP_Error array( 'length' => (int), 'type' => 'byte' | 'char' )
	 *                              false if the column has no length (for example, numeric column)
	 *                              JP_Error object if there was an error.
	 */
	 ...
	 * @return array|JP_Error The $data parameter, with invalid characters removed from
	 *                        each value. This works as a passthrough: any additional keys
	 *                        such as 'field' are retained in each value array. If we cannot
	 *                        remove invalid characters, a JP_Error object is returned.
	 */
	protected function strip_invalid_text( $data ) {
		...
		if ( $db_check_string ) {
			...
			if ( ! $row ) {
				return new JP_Error( 'wpdb_strip_invalid_text_failure' );
			}
	 ...
	 * @return string|JP_Error The converted query, or a JP_Error object if the conversion fails.
	 */
	 ...
	 * @return string|JP_Error The converted string, or a JP_Error object if the conversion fails.
	 */
	...
	protected function get_table_from_query( $query ) {
		...
		// SHOW TABLE STATUS and SHOW TABLES WHERE Name = 'jp_posts'
		...
		// strip the trailing % and unescape the _ to get 'jp_123_'
	...
	public function bail( $message, $error_code = '500' ) {
		if ( $this->show_errors ) {
			...
			if ( class_exists( 'JP_Error', false ) ) {
				$this->error = new JP_Error( $error_code, $message );
			} else {
				$this->error = $message;
			}
	 ...
	 *
	 * @return JP_Error|void
	 */
	public function check_database_version() {
		global $jp_version, $required_mysql_version;
		// Make sure the server has the required MySQL version
		if ( version_compare( $this->db_version(), $required_mysql_version, '<' ) ) {
			/* translators: 1: JonPress version number, 2: Minimum required MySQL version number */
			return new JP_Error( 'database_version', sprintf( __( '<strong>ERROR</strong>: JonPress %1$s requires MySQL %2$s or higher' ), $jp_version, $required_mysql_version ) );
		}
	}
	 ...
	 *
	 * Called when JonPress is generating the table scheme.
	 *
	...
	public function get_caller() {
		return jp_debug_backtrace_summary( __CLASS__ );
	}
