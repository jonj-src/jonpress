...
tinymce.PluginManager.add( 'wpeditimage', function( editor ) {
	...
	editor.addButton( 'jp_img_remove', {
		tooltip: 'Remove',
		...
		editor.addButton( 'jp_img_' + name, {
			...
			onPostRender: function() {
				...
				editor.on( 'NodeChange', function( event ) {
					...
					node = editor.dom.getParent( event.element, '.jp-caption' ) || event.element;
	...
	function parseShortcode( content ) {
		return content.replace( /(?:<p>)?\[(?:jp_)?caption([^\]]+)\]([\s\S]+?)\[\/(?:jp_)?caption\](?:<\/p>)?/g, function( a, b, c ) {
			...
			return '<div class="mceTemp"><dl id="' + id + '" class="jp-caption ' + align + classes + '" style="width: ' + width + 'px">' +
				'<dt class="jp-caption-dt">'+ img +'</dt><dd class="jp-caption-dd">'+ caption +'</dd></dl></div>';
		});
	}

	function getShortcode( content ) {
		return content.replace( /(?:<div [^>]+mceTemp[^>]+>)?\s*(<dl [^>]+jp-caption[^>]+>[\s\S]+?<\/dl>)\s*(?:<\/div>)?/g, function( all, dl ) {
			var out = '';
			...
			out = dl.replace( /\s*<dl ([^>]+)>\s*<dt [^>]+>([\s\S]+?)<\/dt>\s*<dd [^>]+>([\s\S]*?)<\/dd>\s*<\/dl>\s*/gi, function( a, b, c, caption ) {
				...
				classes = classes.replace( /jp-caption ?|align[a-z]+ ?/gi, '' );
	...
	function extractImageData( imageNode ) {
		...
		tinymce.each( classes, function( name ) {
			...
			if ( /^jp-image/.test( name ) ) {
				metadata.attachment_id = parseInt( name.replace( 'jp-image-', '' ), 10 );
		...
		// Extract caption
		captionBlock = dom.getParents( imageNode, '.jp-caption' );
		...
		if ( captionBlock.length ) {
			...
			caption = dom.select( 'dd.jp-caption-dd', captionBlock );
			if ( caption.length ) {
				caption = caption[0];
	...
	function updateImage( imageNode, imageData ) {
		...
		if ( imageData.attachment_id ) {
			classes.push( 'jp-image-' + imageData.attachment_id );
		...
		if ( imageData.caption ) {
			...
			if ( captionNode ) {
				dl = dom.select( 'dl.jp-caption', captionNode );
				...
				dd = dom.select( '.jp-caption-dd', captionNode );
				...
				html =  '<dl ' + id + 'class="' + className +'" style="width: '+ width +'px">' +
					'<dt class="jp-caption-dt"></dt><dd class="jp-caption-dd">'+ imageData.caption +'</dd></dl>';
				...
				editor.$( wrap ).find( 'dt.jp-caption-dt' ).append( node );
	...
	editor.on( 'init', function() {
		...
		// Prevent IE11 from making dl.jp-caption resizable
		if ( tinymce.Env.ie && tinymce.Env.ie > 10 ) {
			// The 'mscontrolselect' event is supported only in IE11+
			dom.bind( editor.getBody(), 'mscontrolselect', function( event ) {
				if ( event.target.nodeName === 'IMG' && dom.getParent( event.target, '.jp-caption' ) ) {
					// Hide the thick border with resize handles around dl.jp-caption
					editor.getBody().focus(); // :(
				} else if ( event.target.nodeName === 'DL' && dom.hasClass( event.target, 'jp-caption' ) ) {
	...
	editor.on( 'ObjectResized', function( event ) {
		...
		if ( node.nodeName === 'IMG' ) {
			editor.undoManager.transact( function() {
				...
				if ( parent = dom.getParent( node, '.jp-caption' ) ) {
	...
	editor.on( 'pastePostProcess', function( event ) {
		...
		if ( editor.dom.getParent( editor.selection.getNode(), 'dd.jp-caption-dd' ) ) {
	...
	editor.on( 'BeforeExecCommand', function( event ) {
		...
		if ( cmd === 'mceInsertContent' || cmd === 'Indent' || cmd === 'Outdent' ) {
			...
			DL = editor.dom.getParent( node, '.jp-caption' );
	...
	editor.on( 'beforeGetContent', function( event ) {
		if ( event.format !== 'raw' ) {
			editor.$( 'img[id="__jp-temp-img-id"]' ).attr( 'id', null );
		}
	});
