 ...
 * @global JP_Post_Type $post_type_object
 * @global JP_Post      $post
 */
...
jp_enqueue_script( 'post' );
$_jp_editor_expand = $_content_editor_dfw = false;
 ...
 */
if ( post_type_supports( $post_type, 'editor' ) && ! jp_is_mobile() &&
	! ( $is_IE && preg_match( '/MSIE [5678]/', $_SERVER['HTTP_USER_AGENT'] ) ) &&
	apply_filters( 'jp_editor_expand', true, $post_type ) ) {

	jp_enqueue_script( 'editor-expand' );
	$_content_editor_dfw = true;
	$_jp_editor_expand   = ( get_user_setting( 'editor_expand', 'on' ) === 'on' );
}
...
if ( $post_ID == get_option( 'page_for_posts' ) && empty( $post->post_content ) ) {
	add_action( 'edit_form_after_title', '_jp_posts_page_notice' );
...
if ( $thumbnail_support ) {
	add_thickbox();
	jp_enqueue_media( array( 'post' => $post_ID ) );
}
...
$messages['post'] = array(
	...
	5  => isset( $_GET['revision'] ) ? sprintf( __( 'Post restored to revision from %s.' ), jp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
...
$messages['page'] = array(
	...
	4  => __( 'Page updated.' ),
	/* translators: %s: date and time of the revision */
	5  => isset( $_GET['revision'] ) ? sprintf( __( 'Page restored to revision from %s.' ), jp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
...
if ( 'auto-draft' == $post->post_status ) {
	...
	$autosave = jp_get_post_autosave( $post_ID );
}
...
if ( $autosave && mysql2date( 'U', $autosave->post_modified_gmt, false ) > mysql2date( 'U', $post->post_modified_gmt, false ) ) {
	foreach ( _jp_post_revision_fields( $post ) as $autosave_field => $_autosave_field ) {
	...
	if ( ! $notice ) {
		jp_delete_post_revision( $autosave->ID );
	}
...
require_once( ABSPATH . 'jp-admin/includes/meta-boxes.php' );
 ...
 *                                             'link' for links.
 * @param JP_Post|JP_Comment|object $post      Post, comment, or link object.
 */
 ...
 * @param JP_Post|string|object $post      Post object for posts, empty string for dashboard widgets,
...
if ( 'post' == $post_type ) {
	...
	get_current_screen()->set_help_sidebar(
		'<p>' . sprintf( __( 'You can also create posts with the <a href="%s">Press This bookmarklet</a>.' ), 'tools.php' ) . '</p>' .
			...
			'<p>' . __( '<a href="https://codex.jonpress.org/Posts_Add_New_Screen">Documentation on Writing and Editing Posts</a>' ) . '</p>' .
	...
	get_current_screen()->set_help_sidebar(
		'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
			'<p>' . __( '<a href="https://codex.jonpress.org/Pages_Add_New_Screen">Documentation on Adding New Pages</a>' ) . '</p>' .
			'<p>' . __( '<a href="https://codex.jonpress.org/Pages_Screen#Editing_Individual_Pages">Documentation on Editing Pages</a>' ) . '</p>' .
			'<p>' . __( '<a href="https://jonpress.org/support/">Support Forums</a>' ) . '</p>'
	);
	...
	get_current_screen()->set_help_sidebar(
		'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
		'<p>' . __( '<a href="https://codex.jonpress.org/Media_Add_New_Screen#Edit_Media">Documentation on Edit Media</a>' ) . '</p>' .
		'<p>' . __( '<a href="https://jonpress.org/support/">Support Forums</a>' ) . '</p>'
	);
}
...
if ( 'post' == $post_type || 'page' == $post_type ) {
	...
	$inserting_media .= '<p>' . __( 'You can also embed media from many popular websites including Twitter, YouTube, Flickr and others by pasting the media URL on its own line into the content of your post/page. Please refer to the Codex to <a href="https://codex.jonpress.org/Embeds">learn more about embeds</a>.' ) . '</p>';
...
if ( 'post' == $post_type ) {
	...
	if ( current_theme_supports( 'post-formats' ) && post_type_supports( 'post', 'post-formats' ) ) {
		$publish_box .= '<li>' . __( '<strong>Format</strong> &mdash; Post Formats designate how your theme will display a specific post. For example, you could have a <em>standard</em> blog post with a title and paragraphs, or a short <em>aside</em> that omits the title and contains a short text blurb. Please refer to the Codex for <a href="https://codex.jonpress.org/Post_Formats#Supported_Formats">descriptions of each post format</a>. Your theme could enable all or some of 10 possible formats.' ) . '</li>';
	}
	...
	$discussion_settings  = '<p>' . __( '<strong>Send Trackbacks</strong> &mdash; Trackbacks are a way to notify legacy blog systems that you&#8217;ve linked to them. Enter the URL(s) you want to send trackbacks. If you link to other JonPress sites they&#8217;ll be notified automatically using pingbacks, and this field is unnecessary.' ) . '</p>';
...
?>
>
<?php jp_nonce_field( $nonce_action ); ?>
...
if ( 'draft' != get_post_status( $post ) ) {
	jp_original_referer_field( true, 'previous' );
}
...
if ( $viewable ) :
	...
	if ( has_filter( 'pre_get_shortlink' ) || has_filter( 'get_shortlink' ) ) {
		$shortlink = jp_get_shortlink( $post->ID, 'post' );
...
<?php
jp_nonce_field( 'samplepermalink', 'samplepermalinknonce', false );
?>
</div><!-- /titlediv -->
<?php
}
...
if ( post_type_supports( $post_type, 'editor' ) ) {
	$_jp_editor_expand_class = '';
	if ( $_jp_editor_expand ) {
		$_jp_editor_expand_class = ' jp-editor-expand';
	}
?>
<div id="postdivrich" class="postarea<?php echo $_jp_editor_expand_class; ?>">
...
jp_editor(
	$post->post_content, 'content', array(
		...
		'tinymce'             => array(
			'resize'                  => false,
			'jp_autoresize_on'        => $_jp_editor_expand,
			'add_unload_trigger'      => false,
			'jp_keep_scroll_position' => ! $is_IE,
		),
	)
);
?>
<table id="post-status-info"><tbody><tr>
	<td id="jp-word-count" class="hide-if-no-js"><?php printf( __( 'Word count: %s' ), '<span class="word-count">0</span>' ); ?></td>
...
<?php if ( ! jp_is_mobile() && post_type_supports( $post_type, 'title' ) && '' === $post->post_title ) : ?>
