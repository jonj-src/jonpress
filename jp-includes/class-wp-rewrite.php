 ...
 *
 * The JonPress Rewrite class writes the rewrite module rules to the .htaccess
 * file. It also handles parsing the request to get the correct setup for the
 * JonPress Query class.
 *
 * The Rewrite along with WP class function as a front controller for JonPress.
...
class JP_Rewrite {
	 ...
	 * `JP_Rewrite::$index` with a trailing slash appended. Otherwise the root
	 * will be empty.
	 *
	 * @since 1.5.0
	 * @var string
	 *
	 * @see JP_Rewrite::init()
	 * @see JP_Rewrite::using_index_permalinks()
	 */
	public $root = '';
	...
	/**
	 * Rules that don't redirect to JonPress' index.php.
	 *
	...
	/**
	 * Whether to write every mod_rewrite rule for JonPress into the .htaccess file.
	 *
	 ...
	 * @var bool
	 *
	 * @see JP_Rewrite::mod_rewrite_rules()
	 */
	 ...
	 * set to true. Prior to JonPress 3.3 this flag indicated that every page
	 ...
	 * `JP_Rewrite::$rewritereplace` and are rewritten to the query
	 * variables listed in JP_Rewrite::$queryreplace.
	 *
	 ...
	 * of rewrite tags, see JP_Rewrite::$rewritecode.
	 *
	 * @since 1.5.0
	 * @var array
	 */
	...
	/**
	 * Query variables that rewrite tags map to, see JP_Rewrite::$rewritecode.
	 *
	 * @since 1.5.0
	 * @var array
	 */
	 ...
	 * @since 1.5.0
	 *
	 * @see JP_Rewrite::$rewritecode
	 * @see JP_Rewrite::$rewritereplace
	 * @see JP_Rewrite::$queryreplace
	 *
	 ...
	 *
	 * The main JP_Rewrite function for building the rewrite rule list. The
	 ...
	 *
	 * Shorten version of JP_Rewrite::generate_rewrite_rules() that allows for shorter
	 * list of parameters. See the method for longer description of what generating
	 * rewrite rules does.
	 *
	 * @since 1.5.0
	 *
	 * @see JP_Rewrite::generate_rewrite_rules() See for long description and rest of parameters.
	 *
	 ...
	 * reference to the current JP_Rewrite instance to further manipulate the
	...
	public function rewrite_rules() {
		...
		$deprecated_files = array(
			'.*jp-(atom|rdf|rss|rss2|feed|commentsrss2)\.php$' => $this->index . '?feed=old',
			'.*jp-app\.php(/.*)?$' => $this->index . '?error=403',
		);

		// Registration rules.
		$registration_pages = array();
		if ( is_multisite() && is_main_site() ) {
			$registration_pages['.*jp-signup.php$']   = $this->index . '?signup=true';
			$registration_pages['.*jp-activate.php$'] = $this->index . '?activate=true';
		}

		// Deprecated.
		$registration_pages['.*jp-register.php$'] = $this->index . '?register=true';
		 ...
		 * @since 1.5.0
		 *
		 * @param JP_Rewrite $this Current JP_Rewrite instance (passed by reference).
		 */
	 ...
	 *
	 * The difference between this method and JP_Rewrite::rewrite_rules() is that
	 ...
	 */
	public function jp_rewrite_rules() {
		$this->rules = get_option( 'rewrite_rules' );
		if ( empty( $this->rules ) ) {
			$this->matches = 'matches';
			$this->rewrite_rules();
			if ( ! did_action( 'jp_loaded' ) ) {
				add_action( 'jp_loaded', array( $this, 'flush_rules' ) );
				return $this->rules;
			}
	 ...
	 *
	 * Will add the non_jp_rules property rules to the .htaccess file before
	 * the JonPress rewrite rules one.
	 *
	 * @since 1.5.0
	 *
	 * @return string
	...
	public function mod_rewrite_rules() {
		...
		foreach ( (array) $this->non_jp_rules as $match => $query ) {
	...
	<system.webServer>
		...
		$rules .= '
			<rule name="JonPress: ' . esc_attr( home_url() ) . '" patternSyntax="Wildcard">
				<match url="*" />
					<conditions>
	 ...
	 * query strings. The new permastruct is added to the JP_Rewrite::$extra_permastructs array.
	 *
	 * When the rewrite rules are built by JP_Rewrite::rewrite_rules(), all of these extra
	 * permastructs are passed to JP_Rewrite::generate_rewrite_rules() which transforms them
	 * into the regular expressions that many love to hate.
	 *
	 * The `$args` parameter gives you control over how JP_Rewrite::generate_rewrite_rules()
	 ...
	 *     Default empty array.
	 *
	 *     @type bool $with_front  Whether the structure should be prepended with `JP_Rewrite::$front`.
	 *                             Default true.
	 ...
	 *
	 * Calls JP_Rewrite::jp_rewrite_rules() after removing the 'rewrite_rules' option.
	...
	public function init() {
		$this->extra_rules         = $this->non_jp_rules = $this->endpoints = array();
	 ...
	 * JP_Rewrite::init() after the option is updated.
	 *
	 ...
	 * the current category base and the parameter value. Calls JP_Rewrite::init()
	 ...
	 * current tag base and the parameter value. Calls JP_Rewrite::init() after
