<?php
/**
 * REST API: JP_REST_Taxonomies_Controller class
 *
 * @package JonPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class JP_REST_Taxonomies_Controller extends JP_REST_Controller {
	...
	public function get_items_permissions_check( $request ) {
		if ( 'edit' === $request['context'] ) {
			...
			return new JP_Error( 'rest_cannot_view', __( 'Sorry, you are not allowed to manage terms in this taxonomy.' ), array( 'status' => rest_authorization_required_code() ) );
		}
		return true;
	}
	 ...
	 *
	 * @param JP_REST_Request $request Full details about the request.
	 * @return JP_REST_Response Response object on success, or JP_Error object on failure.
	 */
	 ...
	 *
	 * @param  JP_REST_Request $request Full details about the request.
	 * @return true|JP_Error True if the request has read access for the item, otherwise false or JP_Error object.
	 */
	public function get_item_permissions_check( $request ) {
		...
		if ( $tax_obj ) {
			...
			if ( 'edit' === $request['context'] && ! current_user_can( $tax_obj->cap->manage_terms ) ) {
				return new JP_Error( 'rest_forbidden_context', __( 'Sorry, you are not allowed to manage terms in this taxonomy.' ), array( 'status' => rest_authorization_required_code() ) );
			}
		}
	...
	public function get_item( $request ) {
		$tax_obj = get_taxonomy( $request['taxonomy'] );
		if ( empty( $tax_obj ) ) {
			return new JP_Error( 'rest_taxonomy_invalid', __( 'Invalid taxonomy.' ), array( 'status' => 404 ) );
		}
