	...
	function autosave() {
		...
		function autosaveLocal() {
			...
			function checkStorage() {
				...
				try {
					window.sessionStorage.setItem( 'jp-test', test );
					result = window.sessionStorage.getItem( 'jp-test' ) === test;
					window.sessionStorage.removeItem( 'jp-test' );
				} catch(e) {}
			...
			function getStorage() {
				...
				if ( hasStorage && blog_id ) {
					stored_obj = sessionStorage.getItem( 'jp-autosave-' + blog_id );
			...
			function run() {
				...
				if ( $( '#jp-content-wrap' ).hasClass( 'tmce-active' ) ) {
				...
				$( 'form#post' ).on( 'submit.autosave-local', function() {
					...
					wpCookies.set( 'jp-saving-post', post_id + '-check', 24 * 60 * 60, false, false, secure );
				});
			}
			...
			function checkPost() {
				var content, post_title, excerpt, $notice,
					postData = getSavedPostData(),
					cookie = wpCookies.get( 'jp-saving-post' ),
					$newerAutosaveNotice = $( '#has-newer-autosave' ).parent( '.notice' ),
					$headerEnd = $( '.jp-header-end' );

				if ( cookie === post_id + '-saved' ) {
					wpCookies.remove( 'jp-saving-post' );
