<?php
/**
 * REST API: JP_REST_Post_Types_Controller class
 *
 * @package JonPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class JP_REST_Post_Types_Controller extends JP_REST_Controller {
	 ...
	 * @return JP_Error|true True if the request has read access, JP_Error object otherwise.
	 */
	public function get_items_permissions_check( $request ) {
		if ( 'edit' === $request['context'] ) {
			...
			return new JP_Error( 'rest_cannot_view', __( 'Sorry, you are not allowed to edit posts in this post type.' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function get_item( $request ) {
		...
		if ( empty( $obj ) ) {
			return new JP_Error( 'rest_type_invalid', __( 'Invalid post type.' ), array( 'status' => 404 ) );
		}

		if ( empty( $obj->show_in_rest ) ) {
			return new JP_Error( 'rest_cannot_read_type', __( 'Cannot view post type.' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function prepare_item_for_response( $post_type, $request ) {
		$taxonomies = jp_list_filter( get_object_taxonomies( $post_type->name, 'objects' ), array( 'show_in_rest' => true ) );
		$taxonomies = jp_list_pluck( $taxonomies, 'name' );
