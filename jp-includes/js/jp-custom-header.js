...
(function( window, settings ) {
			...
			if ( this.supportsVideo() ) {
				for ( var id in this.handlers ) {
					...
					if ( 'test' in handler && handler.test( settings ) ) {
						...
						trigger( document, 'jp-custom-header-video-loaded' );
						break;
					}
				}
			}
	...
	BaseHandler.prototype = {
		...
		initialize: function( settings ) {
			...
			this.container = document.getElementById( 'jp-custom-header' );
			this.button = button;

			button.setAttribute( 'type', 'button' );
			button.setAttribute( 'id', 'jp-custom-header-video-button' );
			button.setAttribute( 'class', 'jp-custom-header-video-button jp-custom-header-video-play' );
			...
			this.container.addEventListener( 'play', function() {
				button.className = 'jp-custom-header-video-button jp-custom-header-video-play';
			...
			this.container.addEventListener( 'pause', function() {
				button.className = 'jp-custom-header-video-button jp-custom-header-video-pause';
				button.innerHTML = settings.l10n.play;
				if ( 'a11y' in window.wp ) {
	...
	NativeHandler = BaseHandler.extend(/** @lends wp.NativeHandler.prototype */{
		...
		ready: function() {
			...
			video.id = 'jp-custom-header-video';
