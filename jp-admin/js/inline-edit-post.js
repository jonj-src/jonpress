...
( function( $, wp ) {
	...
	setBulk : function(){
		...
		if ( 'post' === type ) {
			$( 'tr.inline-editor textarea[data-jp-taxonomy]' ).each( function ( i, element ) {
				/*
...
$( document ).on( 'heartbeat-tick.jp-check-locked-posts', function( e, data ) {
	var locked = data['jp-check-locked-posts'] || {};

	$('#the-list tr').each( function(i, el) {
		var key = el.id, row = $(el), lock_data, avatar;

		if ( locked.hasOwnProperty( key ) ) {
			if ( ! row.hasClass('jp-locked') ) {
				...
				row.addClass('jp-locked');
			}
		} else if ( row.hasClass('jp-locked') ) {
			// Make room for the CSS animation
			row.removeClass('jp-locked').delay(1000).find('.locked-info span').empty();
		}
	});
}).on( 'heartbeat-send.jp-check-locked-posts', function( e, data ) {
	var check = [];
	...
	if ( check.length ) {
		data['jp-check-locked-posts'] = check;
	}
}).ready( function() {
