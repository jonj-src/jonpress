...
/**
 * Sets up the JonPress core custom header and custom background features.
 *
 * @since Twenty Sixteen 1.0
 *
...
function twentysixteen_custom_header_and_background() {
	 ...
	 *     @type callable $jp-head-callback Callback function used to style the header image and text
	...
	add_theme_support(
		'custom-header', apply_filters(
			'twentysixteen_custom_header_args', array(
				...
				'height'             => 280,
				'flex-height'        => true,
				'jp-head-callback'   => 'twentysixteen_header_style',
			)
		)
	);
}
 ...
 * @since Twenty Sixteen 1.0
 *
 * @param JP_Customize_Manager $jp_customize The Customizer object.
 */
function twentysixteen_customize_register( $jp_customize ) {
	$color_scheme = twentysixteen_get_color_scheme();
	...
	$jp_customize->add_control(
		new JP_Customize_Color_Control(
			$jp_customize, 'page_background_color', array(
	...
	$jp_customize->remove_control( 'header_textcolor' );
	...
	$jp_customize->add_control(
		new JP_Customize_Color_Control(
			$jp_customize, 'main_text_color', array(
	...
	$jp_customize->add_control(
		new JP_Customize_Color_Control(
			$jp_customize, 'secondary_text_color', array(
...
function twentysixteen_color_scheme_css() {
	...
	jp_add_inline_style( 'twentysixteen-style', $color_scheme_css );
}
add_action( 'jp_enqueue_scripts', 'twentysixteen_color_scheme_css' );
...
function twentysixteen_customize_control_js() {
	jp_enqueue_script( 'color-scheme-control', get_template_directory_uri() . '/js/color-scheme-control.js', array( 'customize-controls', 'iris', 'underscore', 'jp-util' ), '20160816', true );
	jp_localize_script( 'color-scheme-control', 'colorScheme', twentysixteen_get_color_schemes() );
}
...
function twentysixteen_customize_preview_js() {
	jp_enqueue_script( 'twentysixteen-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20160816', true );
}
...
function twentysixteen_page_background_color_css() {
	...
	jp_add_inline_style( 'twentysixteen-style', sprintf( $css, $page_background_color ) );
}
add_action( 'jp_enqueue_scripts', 'twentysixteen_page_background_color_css', 11 );
...
function twentysixteen_link_color_css() {
	...
	jp_add_inline_style( 'twentysixteen-style', sprintf( $css, $link_color ) );
}
add_action( 'jp_enqueue_scripts', 'twentysixteen_link_color_css', 11 );
...
function twentysixteen_main_text_color_css() {
	...
	jp_add_inline_style( 'twentysixteen-style', sprintf( $css, $main_text_color, $border_color ) );
}
add_action( 'jp_enqueue_scripts', 'twentysixteen_main_text_color_css', 11 );
...
function twentysixteen_secondary_text_color_css() {
	...
	jp_add_inline_style( 'twentysixteen-style', sprintf( $css, $secondary_text_color ) );
}
add_action( 'jp_enqueue_scripts', 'twentysixteen_secondary_text_color_css', 11 );
