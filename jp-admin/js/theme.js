...
themes.view.Appearance = wp.Backbone.View.extend({
	...
	search: function() {
		...
		this.searchContainer
			.append( $.parseHTML( '<label class="screen-reader-text" for="jp-filter-search-input">' + l10n.search + '</label>' ) )
			.append( view.el )
...
themes.Collection = Backbone.Collection.extend({
	...
	// Send request to api.jonpress.org/themes
...
themes.view.Theme = wp.Backbone.View.extend({
	...
	installTheme: function( event ) {
		...
		$( document ).on( 'jp-theme-install-success', function( event, response ) {
	...
	updateTheme: function( event ) {
		...
		$( document ).on( 'jp-theme-update-success', function( event, response ) {
...
themes.view.Details = wp.Backbone.View.extend({
	...
	containFocus: function( $el ) {
		...
		$el.on( 'keydown.jp-themes', function( event ) {
	...
	deleteTheme: function( event ) {
		...
		$( document ).one( 'jp-theme-delete-success', function( event, response ) {
...
themes.view.Preview = themes.view.Details.extend({
	...
	className: 'jp-full-overlay expanded',
	...
	tooglePreviewDeviceButtons: function( newDevice ) {
		var $devices = $( '.jp-full-overlay-footer .devices' );
	...
	installTheme: function( event ) {
		...
		$( document ).on( 'jp-theme-install-success', function() {
			_this.model.set( { 'installed': true } );
		} );
...
themes.Router = Backbone.Router.extend({
	...
	search: function( query ) {
		$( '.jp-filter-search' ).val( query.replace( /\+/g, ' ' ) );
	},
...
themes.Run = {
	...
	routes: function() {
		...
		themes.router.on( 'route:search', function() {
			$( '.jp-filter-search' ).trigger( 'keyup' );
		});
...
themes.view.InstallerSearch =  themes.view.Search.extend({
	...
	doSearch: function( value ) {
		...
		// Get the themes by sending Ajax POST request to api.jonpress.org/themes
...
themes.view.Installer = themes.view.Appearance.extend({
	...
	// Save the user's JonPress.org username and get his favorite themes.
	saveUsername: function ( event ) {
		...
		return wp.ajax.send( 'save-jporg-username', {
			data: {
				_wpnonce: nonce,
				username: username
			},
...
themes.RunInstaller = {
	...
	routes: function() {
		...
		themes.router.on( 'route:search', function() {
			$( '.jp-filter-search' ).focus().trigger( 'keyup' );
		});
