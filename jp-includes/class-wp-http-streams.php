<?php
/**
 * HTTP API: JP_Http_Streams class
 *
 * @package JonPress
 * @subpackage HTTP
 * @since 4.4.0
 ...
 */
class JP_Http_Streams {
	/**
	 * Send a HTTP request to a URI using PHP Streams.
	 *
	 * @see JP_Http::request For default options descriptions.
	 *
	 * @since 2.7.0
	 * @since 3.7.0 Combined with the fsockopen transport and switched to stream_socket_client().
	 *
	...
	public function request( $url, $args = array() ) {
		...
		if ( false === $handle ) {
			...
			if ( $secure_transport && 0 === $connection_error && '' === $connection_error_str ) {
				return new JP_Error( 'http_request_failed', __( 'The SSL certificate for the host could not be verified.' ) );
			}

			return new JP_Error( 'http_request_failed', $connection_error . ': ' . $connection_error_str );
		}
			...
			while ( ! feof( $handle ) && $keep_reading ) {
				...
				if ( ! $bodyStarted ) {
					...
					if ( strpos( $strResponse, "\r\n\r\n" ) ) {
						$process     = JP_Http::processResponse( $strResponse );
						$bodyStarted = true;
						$block       = $process['body'];
						unset( $strResponse );
						$process['body'] = '';
				...
				if ( $bytes_written_to_file != $this_block_size ) {
					...
					return new JP_Error( 'http_request_failed', __( 'Failed to write request to temporary file.' ) );
				}
		...
		$arrHeaders = JP_Http::processHeaders( $process['headers'], $url );
		...
		// Handle redirects.
		if ( false !== ( $redirect_response = JP_Http::handle_redirects( $url, $r, $response ) ) ) {
			return $redirect_response;
		}

		// If the body was chunk encoded, then decode it.
		if ( ! empty( $process['body'] ) && isset( $arrHeaders['headers']['transfer-encoding'] ) && 'chunked' == $arrHeaders['headers']['transfer-encoding'] ) {
			$process['body'] = JP_Http::chunkTransferDecode( $process['body'] );
		}

		if ( true === $r['decompress'] && true === JP_Http_Encoding::should_decode( $arrHeaders['headers'] ) ) {
			$process['body'] = JP_Http_Encoding::decompress( $process['body'] );
		}
	...
	public static function verify_ssl_certificate( $stream, $host ) {
		 ...
		 */
		$host_type = ( JP_Http::is_ip_address( $host ) ? 'ip' : 'dns' );
 ...
 * All code should make use of JP_Http directly through its API.
 *
 * @see JP_HTTP::request
 *
 * @since 2.7.0
 * @deprecated 3.7.0 Please use JP_HTTP::request() directly
 */
class JP_HTTP_Fsockopen extends JP_HTTP_Streams {
