<?php
/**
 * List Table API: JP_Users_List_Table class
 *
 * @package JonPress
 ...
 * @see JP_List_Table
 */
class JP_Users_List_Table extends JP_List_Table {
	...
	public function prepare_items() {
		...
		$usersearch = isset( $_REQUEST['s'] ) ? jp_unslash( trim( $_REQUEST['s'] ) ) : '';
		...
		if ( 'none' === $role ) {
			$args = array(
				...
				'include' => jp_get_users_with_no_role( $this->site_id ),
				'search'  => $usersearch,
		 ...
		 * @since 4.4.0
		 *
		 * @param array $args Arguments passed to JP_User_Query to retrieve items for the current
		 *                    users list table.
		 */
		$args = apply_filters( 'users_list_table_query_args', $args );

		// Query the user IDs for this page
		$jp_user_search = new JP_User_Query( $args );

		$this->items = $jp_user_search->get_results();

		$this->set_pagination_args(
			array(
				'total_items' => $jp_user_search->get_total(),
	...
	protected function get_views() {
		...
		foreach ( $jp_roles->get_names() as $this_role => $name ) {
	 ...
	 *
	 * @param JP_User $user_object The current user object.
	...
	public function single_row( $user_object, $style = '', $role = '', $numposts = 0 ) {
		if ( ! ( $user_object instanceof JP_User ) ) {
		...
		if ( current_user_can( 'list_users' ) ) {
			...
			$edit_link = esc_url( add_query_arg( 'jp_http_referer', urlencode( jp_unslash( $_SERVER['REQUEST_URI'] ) ), get_edit_user_link( $user_object->ID ) ) );
			...
			if ( ! is_multisite() && get_current_user_id() != $user_object->ID && current_user_can( 'delete_user', $user_object->ID ) ) {
				$actions['delete'] = "<a class='submitdelete' href='" . jp_nonce_url( "users.php?action=delete&amp;user=$user_object->ID", 'bulk-users' ) . "'>" . __( 'Delete' ) . '</a>';
			}
			if ( is_multisite() && get_current_user_id() != $user_object->ID && current_user_can( 'remove_user', $user_object->ID ) ) {
				$actions['remove'] = "<a class='submitdelete' href='" . jp_nonce_url( $url . "action=remove&amp;user=$user_object->ID", 'bulk-users' ) . "'>" . __( 'Remove' ) . '</a>';
			}
			 ...
			 * @param JP_User  $user_object JP_User object for the currently listed user.
			 */
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			$data = 'data-colname="' . jp_strip_all_tags( $column_display_name ) . '"';
	 ...
	 * @since 4.4.0
	 *
	 * @param JP_User $user_object The JP_User object.
	...
	protected function get_role_list( $user_object ) {
		...
		foreach ( $user_object->roles as $role ) {
			if ( isset( $jp_roles->role_names[ $role ] ) ) {
				$role_list[ $role ] = translate_user_role( $jp_roles->role_names[ $role ] );
			}
		}
		 ...
		 * @param JP_User  $user_object A JP_User object.
		 */
