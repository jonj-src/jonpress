<?php
/**
 * JonPress Customize Widgets classes
 *
 * @package JonPress
 ...
 */
final class JP_Customize_Widgets {
	 ...
	 * @since 3.9.0
	 *
	 * @param JP_Customize_Manager $manager Customize manager bootstrap instance.
	 */
	public function __construct( $manager ) {
		$this->manager = $manager;

		// See https://github.com/xwp/jp-customize-snapshots/blob/962586659688a5b1fd9ae93618b7ce2d4e7a421c/php/class-customize-snapshot-manager.php#L420-L449
		...
		add_action( 'jp_loaded', array( $this, 'override_sidebars_widgets_for_theme_switch' ) );
	...
	public function get_selective_refreshable_widgets() {
		...
		if ( ! isset( $this->selective_refreshable_widgets ) ) {
			...
			foreach ( $jp_widget_factory->widgets as $jp_widget ) {
				$this->selective_refreshable_widgets[ $jp_widget->id_base ] = ! empty( $jp_widget->widget_options['customize_selective_refresh'] );
			}
		}
	...
	public function register_settings() {
		...
		if ( $this->manager->doing_ajax( 'update-widget' ) && isset( $_REQUEST['widget-id'] ) ) {
			$widget_setting_ids[] = $this->get_setting_id( jp_unslash( $_REQUEST['widget-id'] ) );
		}
	 ...
	 * @since 4.2.0
	 *
	 * @param false|array $args       The arguments to the JP_Customize_Setting constructor.
	...
	public function override_sidebars_widgets_for_theme_switch() {
		...
		$this->old_sidebars_widgets = jp_get_sidebars_widgets();
		...
		// reset global cache var used by jp_get_sidebars_widgets()
		unset( $GLOBALS['_jp_sidebars_widgets'] );
	}
	 ...
	 * @since 3.9.0
	 *
	 * @see JP_Customize_Widgets::handle_theme_switch()
	 *
	 ...
	 *
	 * Note: these actions are also fired in jp_ajax_update_widget().
	 *
	 * @since 3.9.0
	 */
	 ...
	 *
	 * When in preview, hook to {@see 'customize_register'} for settings after JonPress is loaded
	...
	public function customize_register() {
		global $jp_registered_widgets, $jp_registered_widget_controls, $jp_registered_sidebars;

		add_filter( 'sidebars_widgets', array( $this, 'preview_sidebars_widgets' ), 1 );

		$sidebars_widgets = array_merge(
			array( 'jp_inactive_widgets' => array() ),
			array_fill_keys( array_keys( $jp_registered_sidebars ), array() ),
			jp_get_sidebars_widgets()
		);
		 ...
		 */
		foreach ( array_keys( $jp_registered_widgets ) as $widget_id ) {
		...
		foreach ( $sidebars_widgets as $sidebar_id => $sidebar_widget_ids ) {
			...
			$is_inactive_widgets   = ( 'jp_inactive_widgets' === $sidebar_id );
			...
			if ( $is_registered_sidebar || $is_inactive_widgets ) {
				...
				if ( $is_active_sidebar ) {
					...
					$section_args = array(
						'title'       => $jp_registered_sidebars[ $sidebar_id ]['name'],
						'description' => $jp_registered_sidebars[ $sidebar_id ]['description'],
						'priority'    => array_search( $sidebar_id, array_keys( $jp_registered_sidebars ) ),
						'panel'       => 'widgets',
					...
					$section = new JP_Customize_Sidebar_Section( $this->manager, $section_id, $section_args );
					$this->manager->add_section( $section );

					$control           = new JP_Widget_Area_Customize_Control(
			...
			foreach ( $sidebar_widget_ids as $i => $widget_id ) {
				...
				if ( ! $is_active_sidebar || ! isset( $jp_registered_widgets[ $widget_id ] ) ) {
					continue;
				}

				$registered_widget = $jp_registered_widgets[ $widget_id ];
				$setting_id        = $this->get_setting_id( $widget_id );
				$id_base           = $jp_registered_widget_controls[ $widget_id ]['id_base'];

				$control = new JP_Widget_Form_Customize_Control(
					$this->manager, $setting_id, array(
						...
						'priority'       => $i,
						'width'          => $jp_registered_widget_controls[ $widget_id ]['width'],
						'height'         => $jp_registered_widget_controls[ $widget_id ]['height'],
	 ...
	 * @since 4.4.0
	 *
	 * @see JP_Customize_Panel::$active_callback
	 *
	 * @global array $jp_registered_sidebars
	 * @return bool Active.
	 */
	public function is_panel_active() {
		...
		return ! empty( $jp_registered_sidebars );
	}
	 ...
	 * @return JP_Error|array Array containing a widget's id_base and number components,
	 *                        or a JP_Error object.
	 */
	public function parse_widget_setting_id( $setting_id ) {
		if ( ! preg_match( '/^(widget_(.+?))(?:\[(\d+)\])?$/', $setting_id, $matches ) ) {
			return new JP_Error( 'widget_setting_invalid_id' );
		}
	...
	public function enqueue_scripts() {
		global $jp_scripts, $jp_registered_sidebars, $jp_registered_widgets;

		jp_enqueue_style( 'customize-widgets' );
		jp_enqueue_script( 'customize-widgets' );
		...
		$registered_sidebar_count            = count( $jp_registered_sidebars );
		...
		$settings = array(
			'registeredSidebars'          => array_values( $jp_registered_sidebars ),
			'registeredWidgets'           => $jp_registered_widgets,
		...
		$jp_scripts->add_data(
			'customize-widgets',
			'data',
			sprintf( 'var _wpCustomizeWidgetsSettings = %s;', jp_json_encode( $settings ) )
		);
	}
	...
	public function get_available_widgets() {
		...
		global $jp_registered_widgets, $jp_registered_widget_controls;
		require_once ABSPATH . '/jp-admin/includes/widgets.php'; // for next_widget_id_number()

		$sort = $jp_registered_widgets;
		...
		foreach ( $sort as $widget ) {
			...
			$is_disabled     = false;
			$is_multi_widget = ( isset( $jp_registered_widget_controls[ $widget['id'] ]['id_base'] ) && isset( $widget['params'][0]['number'] ) );
			if ( $is_multi_widget ) {
				$id_base            = $jp_registered_widget_controls[ $widget['id'] ]['id_base'];
				...
				if ( $sidebar && 'jp_inactive_widgets' !== $sidebar ) {
					$is_disabled = true;
				}
				$id_base = $widget['id'];
			}

			$list_widget_controls_args = jp_list_widget_controls_dynamic_sidebar(
				array(
					0 => $args,
					1 => $widget['params'][0],
				)
			...
			$available_widget = array_merge(
				$available_widget, array(
					...
					'width'        => $jp_registered_widget_controls[ $widget['id'] ]['width'],
					'height'       => $jp_registered_widget_controls[ $widget['id'] ]['height'],
	...
	public function get_widget_control( $args ) {
		...
		ob_start();
		call_user_func_array( 'jp_widget_control', $args );
	...
	public function customize_preview_init() {
		add_action( 'jp_enqueue_scripts', array( $this, 'customize_preview_enqueue' ) );
		add_action( 'jp_print_styles', array( $this, 'print_preview_css' ), 1 );
		add_action( 'jp_footer', array( $this, 'export_preview_data' ), 20 );
	}
	...
	public function refresh_nonces( $nonces ) {
		$nonces['update-widget'] = jp_create_nonce( 'update-widget' );
		return $nonces;
	}
	 ...
	 *
	 * Because jp_get_sidebars_widgets() gets called early at {@see 'init' } (via
	 * jp_convert_widget_settings()) and can set global variable `$_jp_sidebars_widgets`
	...
	public function customize_preview_enqueue() {
		jp_enqueue_script( 'customize-preview-widgets' );
	}
	 ...
	 * and at the very end of the jp_footer,
	 *
	 * @since 3.9.0
	 *
	 * @global array $jp_registered_sidebars
	...
	public function export_preview_data() {
		...
		<script type="text/javascript">
			var _wpWidgetCustomizerPreviewSettings = <?php echo jp_json_encode( $settings ); ?>;
		</script>
		<?php
	}
	...
	protected function get_instance_hash_key( $serialized_instance ) {
		return jp_hash( $serialized_instance );
	}
	...
	public function sanitize_sidebar_widgets_js_instance( $widget_ids ) {
		...
		$widget_ids = array_values( array_intersect( $widget_ids, array_keys( $jp_registered_widgets ) ) );
		return $widget_ids;
	}
	 ...
	 *
	 * @param  string $widget_id Widget ID.
	 * @return JP_Error|array Array containing the updated widget information.
	 *                        A JP_Error object, otherwise.
	 */
	public function call_widget_update( $widget_id ) {
		global $jp_registered_widget_updates, $jp_registered_widget_controls;
		...
		if ( ! empty( $_POST['sanitized_widget_setting'] ) ) {
			...
			if ( false === $sanitized_widget_setting ) {
				...
				return new JP_Error( 'widget_setting_malformed' );
			}

			$instance = $this->sanitize_widget_instance( $sanitized_widget_setting );
			if ( is_null( $instance ) ) {
				$this->stop_capturing_option_updates();
				return new JP_Error( 'widget_setting_unsanitized' );
			}
			...
			if ( ! is_null( $parsed_id['number'] ) ) {
				...
				$_REQUEST[ $key ]              = $_POST[ $key ] = jp_slash( $value );
		...
		if ( 0 !== $this->count_captured_options() ) {
			if ( $this->count_captured_options() > 1 ) {
				$this->stop_capturing_option_updates();
				return new JP_Error( 'widget_setting_too_many_options' );
			}

			$updated_option_name = key( $this->get_captured_options() );
			if ( $updated_option_name !== $option_name ) {
				$this->stop_capturing_option_updates();
				return new JP_Error( 'widget_setting_unexpected_option' );
			}
		}
		 ...
		 * in place from JP_Customize_Setting::preview() will use this value
		...
		ob_start();
		$form = $jp_registered_widget_controls[ $widget_id ];
		if ( $form ) {
	 ...
	 *
	 * Most code here copied from jp_ajax_save_widget().
	 *
	 * @since 3.9.0
	 *
	 * @see jp_ajax_save_widget()
	 */
	public function jp_ajax_update_widget() {
		...
		if ( empty( $_POST['widget-id'] ) ) {
			jp_send_json_error( 'missing_widget-id' );
		}
		...
		if ( $is_updating_widget_template ) {
			jp_send_json_error( 'template_widget_not_updatable' );
		}

		$updated_widget = $this->call_widget_update( $widget_id ); // => {instance,form}
		if ( is_jp_error( $updated_widget ) ) {
			jp_send_json_error( $updated_widget->get_error_code() );
		}

		$form     = $updated_widget['form'];
		$instance = $this->sanitize_widget_js_instance( $updated_widget['instance'] );

		jp_send_json_success( compact( 'form', 'instance' ) );
	}
	...
	public function selective_refresh_init() {
		...
		add_filter( 'jp_kses_allowed_html', array( $this, 'filter_jp_kses_allowed_data_attributes' ) );
	 ...
	 * }
	 * @see JP_Customize_Nav_Menus::filter_jp_nav_menu_args()
	 *
	 * @return array Params.
	 */
	public function filter_dynamic_sidebar_params( $params ) {
		...
		$attributes                   .= sprintf( ' data-customize-partial-placement-context="%s"', esc_attr( jp_json_encode( $context ) ) );
	 ...
	 *
	 * This is used in the {@see 'filter_jp_kses_allowed_html'} filter to ensure that the
	 ...
	 *
	 * This is needed in case the `$before_widget` is run through jp_kses() when printed.
	 *
	 * @since 4.5.0
	 *
	 ...
	 */
	public function filter_jp_kses_allowed_data_attributes( $allowed_html ) {
	...
	public function capture_filter_pre_get_option( $value ) {
		...
		if ( isset( $this->_captured_options[ $option_name ] ) ) {
			...
			/** This filter is documented in jp-includes/option.php */
