...
jp_reset_vars( array( 'action', 'user_id', 'jp_http_referer' ) );
...
$profile_help = '<p>' . __( 'Your profile contains information about you (your &#8220;account&#8221;) as well as some personal options related to using JonPress.' ) . '</p>' .
	'<p>' . __( 'You can change your password, turn on keyboard shortcuts, change the color scheme of your JonPress administration screens, and turn off the WYSIWYG (Visual) editor, among other things. You can hide the Toolbar (formerly called the Admin Bar) from the front end of your site, however it cannot be disabled on the admin screens.' ) . '</p>' .
	'<p>' . __( 'You can select the language you wish to use while using the JonPress administration screen without affecting the language site visitors see.' ) . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://codex.jonpress.org/Users_Your_Profile_Screen">Documentation on User Profiles</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://jonpress.org/support/">Support Forums</a>' ) . '</p>'
);

$jp_http_referer = remove_query_arg( array( 'update', 'delete_count', 'user_id' ), $jp_http_referer );
...
if ( is_multisite()
	...
	jp_die( __( 'Sorry, you are not allowed to edit this user.' ) );
}
...
if ( IS_PROFILE_PAGE && isset( $_GET['newuseremail'] ) && $current_user->ID ) {
	...
	if ( $new_email && hash_equals( $new_email['hash'], $_GET['newuseremail'] ) ) {
		...
		jp_redirect( add_query_arg( array( 'updated' => 'true' ), self_admin_url( 'profile.php' ) ) );
		die();
	} else {
		jp_redirect( add_query_arg( array( 'error' => 'new-email' ), self_admin_url( 'profile.php' ) ) );
	}
...
switch ( $action ) {
	case 'update':
		...
		if ( ! is_jp_error( $errors ) ) {
			...
			if ( $jp_http_referer ) {
				$redirect = add_query_arg( 'jp_http_referer', urlencode( $jp_http_referer ), $redirect );
			}
	...
	default:
		...
		$sessions = JP_Session_Tokens::get_instance( $profileuser->ID );
...
<div id="message" class="updated notice is-dismissible">
	...
	<?php endif; ?>
	<?php if ( $jp_http_referer && false === strpos( $jp_http_referer, 'user-new.php' ) && ! IS_PROFILE_PAGE ) : ?>
	<p><a href="<?php echo esc_url( jp_validate_redirect( esc_url_raw( $jp_http_referer ), self_admin_url( 'users.php' ) ) ); ?>"><?php _e( '&larr; Back to Users' ); ?></a></p>
	<?php endif; ?>
</div>
<?php endif; ?>
...
<form id="your-profile" action="<?php echo esc_url( self_admin_url( IS_PROFILE_PAGE ? 'profile.php' : 'user-edit.php' ) ); ?>" method="post" novalidate="novalidate"
	...
	?>
	>
	<?php jp_nonce_field( 'update-user_' . $user_id ); ?>
<?php if ( $jp_http_referer ) : ?>
	<input type="hidden" name="jp_http_referer" value="<?php echo esc_url( $jp_http_referer ); ?>" />
<?php endif; ?>
<p>
...
<?php endif; ?>
<?php if ( count( $_jp_admin_css_colors ) > 1 && has_action( 'admin_color_scheme_picker' ) ) : ?>
...
<?php
endif; // $_jp_admin_css_colors
if ( ! ( IS_PROFILE_PAGE && ! $user_can_edit ) ) :
?>
<tr class="user-comment-shortcuts-wrap">
<th scope="row"><?php _e( 'Keyboard Shortcuts' ); ?></th>
<td><label for="comment_shortcuts"><input type="checkbox" name="comment_shortcuts" id="comment_shortcuts" value="true" <?php checked( 'true', $profileuser->comment_shortcuts ); ?> /> <?php _e( 'Enable keyboard shortcuts for comment moderation.' ); ?></label> <?php _e( '<a href="https://codex.jonpress.org/Keyboard_Shortcuts" target="_blank">More information</a>' ); ?></td>
</tr>
<?php endif; ?>
 ...
 * @since 2.7.0
 *
 * @param JP_User $profileuser The current JP_User object.
 */
...
<tr class="user-display-name-wrap">
	...
	<tr class="user-email-wrap">
		...
		printf(
			' <a href="%1$s">%2$s</a>',
			esc_url( jp_nonce_url( self_admin_url( 'profile.php?dismiss=' . $current_user->ID . '_new_email' ), 'dismiss-' . $current_user->ID . '_new_email' ) ),
			__( 'Cancel' )
		);
		?>
		</p>
	...
	<?php
	foreach ( jp_get_user_contact_methods( $profileuser ) as $name => $desc ) {
	?>
...
<?php endif; ?>
	 ...
	 * @param JP_User $profileuser User object for the current user to edit.
	 */
...
<tr id="password" class="user-pass1-wrap">
	...
	<td>
		...
		<button type="button" class="button jp-generate-pw hide-if-no-js"><?php _e( 'Generate Password' ); ?></button>
		<div class="jp-pwd hide-if-js">
			<span class="password-input-wrapper">
				<input type="password" name="pass1" id="pass1" class="regular-text" value="" autocomplete="off" data-pw="<?php echo esc_attr( jp_generate_password( 24 ) ); ?>" aria-describedby="pass-strength-result" />
			</span>
