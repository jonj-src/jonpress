...
if ( ! in_array( $tax->name, get_taxonomies( array( 'show_ui' => true ) ) ) ) {
	jp_die( __( 'Sorry, you are not allowed to edit terms in this taxonomy.' ) );
}
...
/**
 * $post_type is set when the JP_Terms_List_Table instance is created
 *
 * @global string $post_type
 */
global $post_type;

$jp_list_table = _get_list_table( 'JP_Terms_List_Table' );
...
if ( ! $referer ) { // For POST requests.
	$referer = jp_unslash( $_SERVER['REQUEST_URI'] );
}
$referer = remove_query_arg( array( '_jp_http_referer', '_wpnonce', 'error', 'message', 'paged' ), $referer );
switch ( $jp_list_table->current_action() ) {
	...
	case 'add-tag':
		...
		$ret = jp_insert_term( $_POST['tag-name'], $taxonomy, $_POST );
	...
	case 'delete':
		...
		jp_delete_term( $tag_ID, $taxonomy );
	...
	case 'edit':
		...
		if ( ! $term instanceof JP_Term ) {
			jp_die( __( 'You attempted to edit an item that doesn&#8217;t exist. Perhaps it was deleted?' ) );
		}

		jp_redirect( esc_url_raw( get_edit_term_link( $term_id, $taxonomy, $post_type ) ) );
		exit;
	...
	case 'editedtag':
		...
		$ret = jp_update_term( $tag_ID, $taxonomy, $_POST );
		...
		break;
	default:
		if ( ! $jp_list_table->current_action() || ! isset( $_REQUEST['delete_tags'] ) ) {
			break;
		}
		check_admin_referer( 'bulk-tags' );
		$tags = (array) $_REQUEST['delete_tags'];
		/** This action is documented in jp-admin/edit-comments.php */
		$location = apply_filters( 'handle_bulk_actions-' . get_current_screen()->id, $location, $jp_list_table->current_action(), $tags );
		break;
}

if ( ! $location && ! empty( $_REQUEST['_jp_http_referer'] ) ) {
	$location = remove_query_arg( array( '_jp_http_referer', '_wpnonce' ), jp_unslash( $_SERVER['REQUEST_URI'] ) );
}
...
if ( $location ) {
	 ...
	 */
	jp_redirect( apply_filters( 'redirect_term_location', $location, $tax ) );
	exit;
}
...
jp_enqueue_script( 'admin-tags' );
if ( current_user_can( $tax->cap->edit_terms ) ) {
	jp_enqueue_script( 'inline-edit-tax' );
}
...
if ( 'category' == $taxonomy || 'link_category' == $taxonomy || 'post_tag' == $taxonomy ) {
	...
	if ( 'category' == $taxonomy ) {
		$help .= '<p>' . __( '<a href="https://codex.jonpress.org/Posts_Categories_Screen">Documentation on Categories</a>' ) . '</p>';
	} elseif ( 'link_category' == $taxonomy ) {
		$help .= '<p>' . __( '<a href="https://codex.jonpress.org/Links_Link_Categories_Screen">Documentation on Link Categories</a>' ) . '</p>';
	} else {
		$help .= '<p>' . __( '<a href="https://codex.jonpress.org/Posts_Tags_Screen">Documentation on Tags</a>' ) . '</p>';
	}

	$help .= '<p>' . __( '<a href="https://jonpress.org/support/">Support Forums</a>' ) . '</p>';
...
require_once( ABSPATH . 'jp-admin/includes/edit-tag-messages.php' );
...
<h1 class="jp-heading-inline"><?php echo esc_html( $title ); ?></h1>

<?php
if ( isset( $_REQUEST['s'] ) && strlen( $_REQUEST['s'] ) ) {
	/* translators: %s: search keywords */
	printf( '<span class="subtitle">' . __( 'Search results for &#8220;%s&#8221;' ) . '</span>', esc_html( jp_unslash( $_REQUEST['s'] ) ) );
}
?>
...
<form class="search-form jp-clearfix" method="get">
<input type="hidden" name="taxonomy" value="<?php echo esc_attr( $taxonomy ); ?>" />
<input type="hidden" name="post_type" value="<?php echo esc_attr( $post_type ); ?>" />

<?php $jp_list_table->search_box( $tax->labels->search_items, 'tag' ); ?>

</form>

<div id="col-container" class="jp-clearfix">
...
<input type="hidden" name="taxonomy" value="<?php echo esc_attr( $taxonomy ); ?>" />
<input type="hidden" name="post_type" value="<?php echo esc_attr( $post_type ); ?>" />
<?php jp_nonce_field( 'add-tag', '_wpnonce_add-tag' ); ?>
...
<div class="form-field term-parent-wrap">
	...
	jp_dropdown_categories( $dropdown_args );
	?>
