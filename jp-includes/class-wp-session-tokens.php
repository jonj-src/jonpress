<?php
/**
 * Session API: JP_Session_Tokens class
 *
 * @package JonPress
 * @subpackage Session
 * @since 4.7.0
 ...
 * @since 4.0.0
 */
abstract class JP_Session_Tokens {
	 ...
	 * the session manager for a subclass of `JP_Session_Tokens`.
	 *
	 * @since 4.0.0
	 *
	 * @param int $user_id User whose session to manage.
	 * @return JP_User_Meta_Session_Tokens JP_User_Meta_Session_Tokens class instance by default.
	 */
	final public static function get_instance( $user_id ) {
		 ...
		 *                        Default 'JP_User_Meta_Session_Tokens'.
		 */
		$manager = apply_filters( 'session_token_manager', 'JP_User_Meta_Session_Tokens' );
	...
	final public function create( $expiration ) {
		...
		if ( ! empty( $_SERVER['HTTP_USER_AGENT'] ) ) {
			$session['ua'] = jp_unslash( $_SERVER['HTTP_USER_AGENT'] );
		}

		// Timestamp
		$session['login'] = time();

		$token = jp_generate_password( 43, false, false );
	...
	final public static function destroy_all_for_all_users() {
		/** This filter is documented in jp-includes/class-jp-session-tokens.php */
		$manager = apply_filters( 'session_token_manager', 'JP_User_Meta_Session_Tokens' );
