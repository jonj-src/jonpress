( function( window, $, undefined ) {
	...
	$( function() {
		var $wrap = $( '#postdivrich' ),
			$contentWrap = $( '#jp-content-wrap' ),
			$tools = $( '#jp-content-editor-tools' ),
			$visualTop = $(),
			$visualEditor = $(),
			$textTop = $( '#ed_toolbar' ),
			$textEditor = $( '#content' ),
		...
		$document.on( 'tinymce-editor-init.editor-expand', function( event, editor ) {
			...
			mceBind = function() {
				...
				editor.on( 'jp-toolbar-toggle', toggleAdvanced );

				// Adjust when the editor resizes.
				editor.on( 'setcontent jp-autoresize jp-toolbar-toggle', adjust );
			...
			mceUnbind = function() {
				...
				editor.off( 'jp-toolbar-toggle', toggleAdvanced );
				editor.off( 'setcontent jp-autoresize jp-toolbar-toggle', adjust );
			...
			if ( $wrap.hasClass( 'jp-editor-expand' ) ) {
		...
		function adjust( event ) {
			...
			if ( ! canPin ) {
				...
				if ( ( ! fixedBottom || ( resize && advanced ) ) &&
						// Add borderWidth for the border around the .jp-editor-container.
		...
		function on() {
			...
			$wrap.addClass( 'jp-editor-expand' );
			 ...
			 */
			$document.on( 'jp-collapse-menu.editor-expand postboxes-columnchange.editor-expand editor-classchange.editor-expand', adjust )
				.on( 'postbox-toggled.editor-expand postbox-moved.editor-expand', function() {
					...
					adjust();
				}).on( 'jp-window-resized.editor-expand', function() {
					if ( mceEditor && ! mceEditor.isHidden() ) {
			...
			if ( mceEditor ) {
				mceEditor.settings.jp_autoresize_on = true;
		...
		function off() {
			...
			$wrap.removeClass( 'jp-editor-expand' );
			...
			if ( mceEditor ) {
				mceEditor.settings.jp_autoresize_on = false;
		...
		$document.on( 'tinymce-editor-init.focus', function( event, editor ) {
			...
			if ( editor.id === 'content' ) {
				...
				mceBind = function() {
					...
					editor.on( 'blur', blur );
					editor.on( 'jp-autoresize', recalcEditorRect );
				};
				...
				mceUnbind = function() {
					...
					editor.off( 'jp-autoresize', recalcEditorRect );
				};
