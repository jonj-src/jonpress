...
wp.textWidgets = ( function( $ ) {
	...
	component.TextWidgetControl = Backbone.View.extend(/** @lends wp.textWidgets.TextWidgetControl.prototype */{
		...
		initialize: function initialize( options ) {
			...
			control.customHtmlWidgetPointer = control.$el.find( '.jp-pointer.custom-html-widget-pointer' );
			...
			control.pasteHtmlPointer = control.$el.find( '.jp-pointer.paste-html-pointer' );
		...
		dismissPointers: function dismissPointers( pointers ) {
			_.each( pointers, function( pointer ) {
				wp.ajax.post( 'dismiss-jp-pointer', {
					pointer: pointer
				});
		...
		initializeEditor: function initializeEditor() {
			...
			triggerChangeIfDirty = function() {
				...
				if ( control.editor.isDirty() ) {
					 ...
					 * having to make server round-trips to call the respective JP_Widget::update()
					 * callbacks. See <https://core.trac.jonpress.org/ticket/33507>.
					 */
			...
			function buildEditor() {
				...
				$( document ).one( 'jp-before-tinymce-init.text-widget-init', function( event, init ) {
