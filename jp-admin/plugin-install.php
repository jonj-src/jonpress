...
if ( ! current_user_can( 'install_plugins' ) ) {
	jp_die( __( 'Sorry, you are not allowed to install plugins on this site.' ) );
}

if ( is_multisite() && ! is_network_admin() ) {
	jp_redirect( network_admin_url( 'plugin-install.php' ) );
	exit();
}

$jp_list_table = _get_list_table( 'JP_Plugin_Install_List_Table' );
$pagenum       = $jp_list_table->get_pagenum();

if ( ! empty( $_REQUEST['_jp_http_referer'] ) ) {
	$location = remove_query_arg( '_jp_http_referer', jp_unslash( $_SERVER['REQUEST_URI'] ) );
 ...
 */
if ( 'upload' !== $tab ) {
	/** This action is documented in jp-admin/plugin-install.php */
...
get_current_screen()->add_help_tab(
	array(
		...
		'title'   => __( 'Overview' ),
		'content' =>
				 '<p>' . sprintf( __( 'Plugins hook into JonPress to extend its functionality with custom features. Plugins are developed independently from the core JonPress application by thousands of developers all over the world. All plugins in the official <a href="%s">JonPress Plugin Directory</a> are compatible with the license JonPress uses.' ), __( 'https://jonpress.org/plugins/' ) ) . '</p>' .
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
				 '<p>' . __( 'If you know what you&#8217;re looking for, Search is your best bet. The Search screen has options to search the JonPress Plugin Directory for a particular Term, Author, or Tag. You can also search the directory by selecting popular tags. Tags in larger type mean more plugins have been labeled with that tag.' ) . '</p>' .
				 ...
				 '<p>' . __( 'You can also browse a user&#8217;s favorite plugins, by using the Favorites link above the plugins list and entering their JonPress.org username.' ) . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://codex.jonpress.org/Plugins_Add_New_Screen">Documentation on Installing Plugins</a>' ) . '</p>' .
...
/**
 * JonPress Administration Template Header.
 */
...
/**
 * JonPress Administration Template Footer.
 */
