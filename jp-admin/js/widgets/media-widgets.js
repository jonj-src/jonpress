...
wp.mediaWidgets = ( function( $ ) {
	...
	component.MediaWidgetControl = Backbone.View.extend(/** @lends wp.mediaWidgets.MediaWidgetControl.prototype */{
		 ...
		 * exported from PHP to JS such as is done in JP_Widget_Media_Image::enqueue_admin_scripts().
		 *
		 * @type {Object}
		 */
		l10n: {
		 ...
		 * such as is done in JP_Widget_Media_Image::enqueue_admin_scripts(). If not,
		 ...
		 * such as is done in JP_Widget_Media_Image::enqueue_admin_scripts().
		 *
		 * @type {string}
		 */
		mime_type: '',
		...
		initialize: function initialize( options ) {
			 ...
			 * from the start, without having to sync with hidden fields. See <https://core.trac.jonpress.org/ticket/33507>.
			 */
		...
		selectMedia: function selectMedia() {
			...
			// Disable syncing of attachment changes back to server (except for deletions). See <https://core.trac.jonpress.org/ticket/40403>.
	...
	component.handleWidgetAdded = function handleWidgetAdded( event, widgetContainer ) {
		 ...
		 * and replaced with the rendered output of JP_Widget::form() which is
	 ...
	 * When JonPress enqueues this script, it should have an inline script
