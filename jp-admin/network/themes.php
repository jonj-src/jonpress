...
if ( ! current_user_can( 'manage_network_themes' ) ) {
	jp_die( __( 'Sorry, you are not allowed to manage network themes.' ) );
}
...
$referer                = remove_query_arg( $temp_args, jp_get_referer() );

if ( $action ) {
	switch ( $action ) {
		case 'enable':
			check_admin_referer( 'enable-theme_' . $_GET['theme'] );
			JP_Theme::network_enable_theme( $_GET['theme'] );
			if ( false === strpos( $referer, '/network/themes.php' ) ) {
				jp_redirect( network_admin_url( 'themes.php?enabled=1' ) );
			} else {
				jp_safe_redirect( add_query_arg( 'enabled', 1, $referer ) );
			}
			exit;
		case 'disable':
			check_admin_referer( 'disable-theme_' . $_GET['theme'] );
			JP_Theme::network_disable_theme( $_GET['theme'] );
			jp_safe_redirect( add_query_arg( 'disabled', '1', $referer ) );
			exit;
		case 'enable-selected':
			check_admin_referer( 'bulk-themes' );
			$themes = isset( $_POST['checked'] ) ? (array) $_POST['checked'] : array();
			if ( empty( $themes ) ) {
				jp_safe_redirect( add_query_arg( 'error', 'none', $referer ) );
				exit;
			}
			JP_Theme::network_enable_theme( (array) $themes );
			jp_safe_redirect( add_query_arg( 'enabled', count( $themes ), $referer ) );
			exit;
		case 'disable-selected':
			...
			JP_Theme::network_disable_theme( (array) $themes );
			jp_safe_redirect( add_query_arg( 'disabled', count( $themes ), $referer ) );
			exit;
		...
		case 'delete-selected':
			if ( ! current_user_can( 'delete_themes' ) ) {
				jp_die( __( 'Sorry, you are not allowed to delete themes for this site.' ) );
			}
			...
			if ( empty( $themes ) ) {
				jp_safe_redirect( add_query_arg( 'error', 'main', $referer ) );
				exit;
			}

			$theme_info = array();
			foreach ( $themes as $key => $theme ) {
				$theme_info[ $theme ] = jp_get_theme( $theme );
			}
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://codex.jonpress.org/Network_Admin_Themes_Screen">Documentation on Network Themes</a>' ) . '</p>' .
...
jp_enqueue_script( 'theme-preview' );
...
<form method="get">
<?php $jp_list_table->search_box( __( 'Search Installed Themes' ), 'theme' ); ?>
</form>
