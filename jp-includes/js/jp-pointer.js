...
(function($){
	...
	$.widget('wp.pointer',/** @lends $.widget.wp.pointer.prototype */{
		...
		_create: function() {
			...
			this.content = $('<div class="jp-pointer-content"></div>');
			this.arrow   = $('<div class="jp-pointer-arrow"><div class="jp-pointer-arrow-inner"></div></div>');
		...
		_update: function( event, content ) {
			...
			if ( buttons ) {
				buttons.wrap('<div class="jp-pointer-buttons" />').parent().appendTo( this.content );
			}
		...
		repoint: function() {
			...
			this.pointer[0].className = this.pointer[0].className.replace( /jp-pointer-[^\s'"]*/, '' );

			// Add arrow class.
			this.pointer.addClass( 'jp-pointer-' + edge );
		},
