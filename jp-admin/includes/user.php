 ...
 * @since 2.0.0
 *
 * @return int|JP_Error JP_Error or User ID.
 */
function add_user() {
	return edit_user();
}
 ...
 * @return int|JP_Error user id of the updated user
 */
function edit_user( $user_id = 0 ) {
	...
	if ( $user_id ) {
		...
		$user->user_login = jp_slash( $userdata->user_login );
	} else {
		$update = false;
	}
	...
	if ( isset( $_POST['role'] ) && current_user_can( 'promote_users' ) && ( ! $user_id || current_user_can( 'promote_user', $user_id ) ) ) {
		...
		if ( ! empty( $new_role ) && empty( $editable_roles[ $new_role ] ) ) {
			jp_die( __( 'Sorry, you are not allowed to give users that role.' ), 403 );
		}

		$potential_role = isset( $jp_roles->role_objects[ $new_role ] ) ? $jp_roles->role_objects[ $new_role ] : false;
	...
	if ( isset( $_POST['email'] ) ) {
		$user->user_email = sanitize_text_field( jp_unslash( $_POST['email'] ) );
	}
	if ( isset( $_POST['url'] ) ) {
		if ( empty( $_POST['url'] ) || $_POST['url'] == 'http://' ) {
			...
			$protocols      = implode( '|', array_map( 'preg_quote', jp_allowed_protocols() ) );
	...
	foreach ( jp_get_user_contact_methods( $user ) as $method => $name ) {
	...
	if ( false !== strpos( jp_unslash( $pass1 ), '\\' ) ) {
	 ...
	 * @since 2.8.0
	 *
	 * @param JP_Error $errors JP_Error object (passed by reference).
	...
	if ( $update ) {
		$user_id = jp_update_user( $user );
	} else {
		$user_id = jp_insert_user( $user );
		 ...
		 * @param string $notify  Type of notification that should happen. See jp_send_new_user_notifications()
 ...
 * list of roles in the $jp_roles object so that plugins can remove
 ...
 * @return JP_User|bool JP_User object on success, false on failure.
 */
 ...
 */
function jp_delete_user( $id, $reassign = null ) {
	global $wpdb;
...
%%s'
		), jp_specialchars_decode( get_bloginfo( 'name' ), ENT_QUOTES ), home_url(), jp_specialchars_decode( translate_user_role( $role['name'] ) )
	);
}
 ...
 * @return bool|JP_Error
 */
function _jp_privacy_resend_request( $request_id ) {
	$request_id = absint( $request_id );
	$request    = get_post( $request_id );

	if ( ! $request || 'user_request' !== $request->post_type ) {
		return new JP_Error( 'privacy_request_error', __( 'Invalid request.' ) );
	}

	$result = jp_send_user_request( $request_id );

	if ( is_jp_error( $result ) ) {
		return $result;
	} elseif ( ! $result ) {
		return new JP_Error( 'privacy_request_error', __( 'Unable to initiate confirmation request.' ) );
	}
 ...
 *
 * @param int $request_id Request ID.
 * @return int|JP_Error Request ID on succes or JP_Error.
 */
function _jp_privacy_completed_request( $request_id ) {
	$request_id   = absint( $request_id );
 ...
 * @access private
 */
function _jp_personal_data_handle_actions() {
	if ( isset( $_POST['privacy_action_email_retry'] ) ) { // WPCS: input var ok.
		check_admin_referer( 'bulk-privacy_requests' );

		$request_id = absint( current( array_keys( (array) jp_unslash( $_POST['privacy_action_email_retry'] ) ) ) ); // WPCS: input var ok, sanitization ok.
		$result     = _jp_privacy_resend_request( $request_id );
		...
		$action = isset( $_POST['action'] ) ? sanitize_key( jp_unslash( $_POST['action'] ) ) : ''; // WPCS: input var ok, CSRF ok.
		...
		switch ( $action ) {
			...
			case 'add_remove_personal_data_request':
				...
				$action_type               = sanitize_text_field( jp_unslash( $_POST['type_of_action'] ) ); // WPCS: input var ok.
				$username_or_email_address = sanitize_text_field( jp_unslash( $_POST['username_or_email_to_export'] ) ); // WPCS: input var ok.
				$email_address             = '';

				if ( ! in_array( $action_type, _jp_privacy_action_request_types(), true ) ) {
					add_settings_error(
						'action_type',
						'action_type',
						__( 'Invalid action.' ),
				...
				$request_id = jp_create_user_request( $email_address, $action_type );

				if ( is_jp_error( $request_id ) ) {
					add_settings_error(
						'username_or_email_to_export',
						'username_or_email_to_export',
				...
				jp_send_user_request( $request_id );
 ...
 * @access private
 */
function _jp_personal_data_cleanup_requests() {
 ...
 * @access private
 */
function _jp_personal_data_export_page() {
	if ( ! current_user_can( 'export_others_personal_data' ) ) {
		jp_die( __( 'Sorry, you are not allowed to export personal data on this site.' ) );
	}

	_jp_personal_data_handle_actions();
	_jp_personal_data_cleanup_requests();

	// "Borrow" xfn.js for now so we don't have to create new files.
	jp_enqueue_script( 'xfn' );

	$requests_table = new JP_Privacy_Data_Export_Requests_Table( array(
	...
	<div class="wrap nosubsub">
		...
		<hr class="jp-header-end" />

		<?php settings_errors(); ?>

		<form method="post" class="jp-privacy-request-form">
			...
			<div class="jp-privacy-request-form-field">
			...
			<?php jp_nonce_field( 'personal-data-request' ); ?>
		...
		<form class="search-form jp-clearfix">
 ...
 * @access private
 */
function _jp_personal_data_removal_page() {
	/*
	...
	if ( ! current_user_can( 'erase_others_personal_data' ) || ! current_user_can( 'delete_users' ) ) {
		jp_die( __( 'Sorry, you are not allowed to erase data on this site.' ) );
	}
	...
	$requests_table = new JP_Privacy_Data_Removal_Requests_Table( array(
		'plural'   => 'privacy_requests',
		'singular' => 'privacy_request',
	) );
 ...
 * @access private
 */
function _jp_privacy_hook_requests_page() {
	add_submenu_page( 'tools.php', __( 'Export Personal Data' ), __( 'Export Personal Data' ), 'export_others_personal_data', 'export_personal_data', '_jp_personal_data_export_page' );
	add_submenu_page( 'tools.php', __( 'Remove Personal Data' ), __( 'Remove Personal Data' ), 'erase_others_personal_data', 'remove_personal_data', '_jp_personal_data_removal_page' );
}

// TODO: move the following classes in new files.
if ( ! class_exists( 'JP_List_Table' ) ) {
	require_once( ABSPATH . 'jp-admin/includes/class-jp-list-table.php' );
}

/**
 * JP_Privacy_Requests_Table class.
 */
abstract class JP_Privacy_Requests_Table extends JP_List_Table {

	/**
	 * Action name for the requests this table will work with. Classes
	 * which inherit from JP_Privacy_Requests_Table should define this.
	...
	public function process_bulk_action() {
		...
		$request_ids = isset( $_REQUEST['request_id'] ) ? jp_parse_id_list( jp_unslash( $_REQUEST['request_id'] ) ) : array(); // WPCS: input var ok, CSRF ok.
		$count = 0;
		...
		switch ( $action ) {
			case 'delete':
				foreach ( $request_ids as $request_id ) {
					if ( jp_delete_post( $request_id, true ) ) {
						$count ++;
					}
				}
			...
			case 'resend':
				foreach ( $request_ids as $request_id ) {
					$resend = _jp_privacy_resend_request( $request_id );

					if ( $resend && ! is_jp_error( $resend ) ) {
						$count++;
					}
				}
	...
	public function prepare_items() {
		...
		$requests_query = new JP_Query( $args );
		$requests       = $requests_query->posts;

		foreach ( $requests as $request ) {
			$this->items[] = jp_get_user_request_data( $request->ID );
		}
	 ...
	 * @since 4.9.6
	 *
	 * @param JP_User_Request $item Item being shown.
	 * @return string
	 */
	 ...
	 * @since 4.9.6
	 *
	 * @param JP_User_Request $item The current item
	 */
...
/**
 * JP_Privacy_Data_Export_Requests_Table class.
 *
 * @since 4.9.6
 */
class JP_Privacy_Data_Export_Requests_Table extends JP_Privacy_Requests_Table {
	/**
	...
	public function column_email( $item ) {
		...
		$request_id      = $item->ID;
		$nonce           = jp_create_nonce( 'jp-privacy-export-personal-data-' . $request_id );
	...
	public function column_next_steps( $item ) {
		...
		switch ( $status ) {
			...
			case 'request-failed':
				...
				break;
			case 'request-completed':
				echo '<a href="' . esc_url( jp_nonce_url( add_query_arg( array(
					'action' => 'delete',
...
/**
 * JP_Privacy_Data_Removal_Requests_Table class.
 *
	 * @since 4.9.6
 */
class JP_Privacy_Data_Removal_Requests_Table extends JP_Privacy_Requests_Table {
	/**
	 * Action name for the requests this table will work with.
	 *
	 * @since 4.9.6
	...
	public function column_email( $item ) {
		...
		if ( 'request-confirmed' !== $status ) {
			...
			$request_id    = $item->ID;
			$nonce         = jp_create_nonce( 'jp-privacy-erase-personal-data-' . $request_id );
