...
class JP_Embed {
	 ...
	 * an Ajax request that will call JP_Embed::cache_oembed().
	 */
...
<?php
	 ...
	 *
	 * Do not use this function directly, use jp_embed_register_handler() instead.
	 *
	 ...
	 *
	 * Do not use this function directly, use jp_embed_unregister_handler() instead.
	 *
	 ...
	 * will be given to the JP_oEmbed class.
	 *
	 * @param array $attr {
	...
	public function shortcode( $attr, $url = '' ) {
		...
		$rawattr = $attr;
		$attr    = jp_parse_args( $attr, jp_embed_defaults( $url ) );

		$this->last_attr = $attr;

		// kses converts & into &amp; and we need to undo this
		// See https://core.trac.jonpress.org/ticket/11311
		$url = str_replace( '&amp;', '&', $url );
		...
		// Potentially set by JP_Embed::cache_oembed().
		...
		if ( $html ) {
			/** This filter is documented in jp-includes/class-jp-embed.php */
	...
	/**
	 * Passes any unlinked URLs that are on their own line to JP_Embed::shortcode() for potential embedding.
	 *
	 * @see JP_Embed::autoembed_callback()
	 *
	...
	public function autoembed( $content ) {
		...
		$content = jp_replace_in_html_tags( $content, array( "\n" => '<!-- jp-line-break -->' ) );
		...
		return str_replace( '<!-- jp-line-break -->', "\n", $content );
	}

	/**
	 * Callback function for JP_Embed::autoembed().
	 *
	...
	public function find_oembed_post_id( $cache_key ) {
		...
		$oembed_post_id = jp_cache_get( $cache_key, $cache_group );

		if ( $oembed_post_id && 'oembed_cache' === get_post_type( $oembed_post_id ) ) {
			return $oembed_post_id;
		}

		$oembed_post_query = new JP_Query(
			array(
				'post_type'              => 'oembed_cache',
				'post_status'            => 'publish',
				'name'                   => $cache_key,
		...
		if ( ! empty( $oembed_post_query->posts ) ) {
			...
			jp_cache_set( $cache_key, $oembed_post_id, $cache_group );
