 ...
 */
global $jp_scripts, $jp_customize;

if ( $jp_customize->changeset_post_id() ) {
	$changeset_post = get_post( $jp_customize->changeset_post_id() );
	...
	if ( $missed_schedule ) {
		/*
		 * Note that an Ajax request spawns here instead of just calling `jp_publish_post( $changeset_post->ID )`.
		 *
		 * Because JP_Customize_Manager is not instantiated for customize.php with the `settings_previewed=false`
		 ...
		 */
		$nonces       = $jp_customize->get_nonces();
		$request_args = array(
			'nonce'                      => $nonces['save'],
			'customize_changeset_uuid'   => $jp_customize->changeset_uuid(),
			'jp_customize'               => 'on',
			'customize_changeset_status' => 'publish',
		);
		ob_start();
		?>
		<?php jp_print_scripts( array( 'jp-util' ) ); ?>
		<script>
			wp.ajax.post( 'customize_save', <?php echo jp_json_encode( $request_args ); ?> );
		</script>
		<?php
		$script = ob_get_clean();
...
jp_reset_vars( array( 'url', 'return', 'autofocus' ) );
if ( ! empty( $url ) ) {
	$jp_customize->set_preview_url( jp_unslash( $url ) );
}
if ( ! empty( $return ) ) {
	$jp_customize->set_return_url( jp_unslash( $return ) );
}
if ( ! empty( $autofocus ) && is_array( $autofocus ) ) {
	$jp_customize->set_autofocus( jp_unslash( $autofocus ) );
}

$registered             = $jp_scripts->registered;
$jp_scripts             = new JP_Scripts;
$jp_scripts->registered = $registered;

add_action( 'customize_controls_print_scripts', 'print_head_scripts', 20 );
add_action( 'customize_controls_print_footer_scripts', '_jp_footer_scripts' );
...
jp_enqueue_script( 'heartbeat' );
jp_enqueue_script( 'customize-controls' );
jp_enqueue_style( 'customize-controls' );
...
$body_class = 'jp-core-ui jp-customizer js';
...
$admin_title = sprintf( $jp_customize->get_document_title_template(), __( 'Loading&hellip;' ) );

?>
<title><?php echo $admin_title; ?></title>

<script type="text/javascript">
var ajaxurl = <?php echo jp_json_encode( admin_url( 'admin-ajax.php', 'relative' ) ); ?>,
	pagenow = 'customize';
</script>
...
<div class="jp-full-overlay expanded">
	<form id="customize-controls" class="wrap jp-full-overlay-sidebar">
		<div id="customize-header-actions" class="jp-full-overlay-header">
			<?php $save_text = $jp_customize->is_theme_active() ? __( 'Publish' ) : __( 'Activate &amp; Publish' ); ?>
			...
			<a class="customize-controls-close" href="<?php echo esc_url( $jp_customize->get_return_url() ); ?>">
		...
		<div id="widgets-right" class="jp-clearfix"><!-- For Widget Customizer, many widgets try to look for instances under div#widgets-right, so we have to add that ID to a container div in the Customizer for compat -->
			<div id="customize-notifications-area" class="customize-control-notifications-container">
				<ul></ul>
			</div>
			<div class="jp-full-overlay-sidebar-content" tabindex="-1">
		...
		<div id="customize-footer-actions" class="jp-full-overlay-footer">
			...
			<?php $previewable_devices = $jp_customize->get_previewable_devices(); ?>
	...
	<div id="customize-preview" class="jp-full-overlay-main"></div>
	<?php
