<?php
/**
 * List Table API: JP_Posts_List_Table class
 *
 * @package JonPress
 ...
 * @see JP_List_Table
 */
class JP_Posts_List_Table extends JP_List_Table {
	...
	public function prepare_items() {
		global $avail_post_stati, $jp_query, $per_page, $mode;

		// is going to call wp()
		$avail_post_stati = jp_edit_posts_query();

		$this->set_hierarchical_display( is_post_type_hierarchical( $this->screen->post_type ) && 'menu_order title' === $jp_query->query['orderby'] );

		$post_type = $this->screen->post_type;
		$per_page  = $this->get_items_per_page( 'edit_' . $post_type . '_per_page' );

		/** This filter is documented in jp-admin/includes/post.php */
		$per_page = apply_filters( 'edit_posts_per_page', $per_page, $post_type );

		if ( $this->hierarchical_display ) {
			$total_items = $jp_query->post_count;
		} elseif ( $jp_query->found_posts || $this->get_pagenum() === 1 ) {
			$total_items = $jp_query->found_posts;
		} else {
			$post_counts = (array) jp_count_posts( $post_type, 'readable' );
	...
	protected function get_views() {
		...
		$status_links = array();
		$num_posts    = jp_count_posts( $post_type, 'readable' );
	...
	protected function categories_dropdown( $post_type ) {
		...
		if ( is_object_in_taxonomy( $post_type, 'category' ) ) {
			...
			jp_dropdown_categories( $dropdown_options );
		}
	}
...
if ( 'top' === $which && ! is_singular() ) {
	 ...
	 *                          'top' or 'bottom' for JP_Posts_List_Table,
	 *                          'bar' for JP_Media_List_Table.
	 */
...
<?php
	...
	public function get_columns() {
		...
		$taxonomies = jp_filter_object_list( $taxonomies, array( 'show_admin_column' => true ), 'and', 'name' );
	...
	public function display_rows( $posts = array(), $level = 0 ) {
		global $jp_query, $per_page;

		if ( empty( $posts ) ) {
			$posts = $jp_query->posts;
		}
	 ...
	 * @since 4.3.0
	 *
	 * @param JP_Post $post The current JP_Post object.
	 */
	...
	public function column_title( $post ) {
		...
		if ( $can_edit_post && $post->post_status != 'trash' ) {
			$lock_holder = jp_check_post_lock( $post->ID );
		...
		if ( current_user_can( 'delete_post', $post->ID ) ) {
			if ( 'trash' === $post->post_status ) {
				$actions['untrash'] = sprintf(
					'<a href="%s" aria-label="%s">%s</a>',
					jp_nonce_url( admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=untrash', $post->ID ) ), 'untrash-post_' . $post->ID ),
					/* translators: %s: post title */
...
if ( post_type_supports( $screen->post_type, 'author' ) ) :
	...
	if ( current_user_can( $post_type_object->cap->edit_others_posts ) ) :
		...
		if ( $authors = jp_dropdown_users( $users_opt ) ) :
...
if ( ! $bulk && $can_publish ) :
	?>
		...
		<div class="inline-edit-group jp-clearfix">
	...
	<?php foreach ( $hierarchical_taxonomies as $taxonomy ) : ?>
			...
			<ul class="cat-checklist <?php echo esc_attr( $taxonomy->name ); ?>-checklist">
				<?php jp_terms_checklist( null, array( 'taxonomy' => $taxonomy->name ) ); ?>
			</ul>
	...
	jp_dropdown_pages( $dropdown_args );
?>
		</label>
...
<?php	endif; // $bulk ?>
				<?php
				/** This filter is documented in jp-admin/includes/meta-boxes.php */
	...
	<?php foreach ( $flat_taxonomies as $taxonomy ) : ?>
		...
		if ( current_user_can( $taxonomy->cap->assign_terms ) ) :
			...
			<label class="inline-edit-tags">
				...
				<textarea data-jp-taxonomy="<?php echo $taxonomy_name; ?>" cols="22" rows="1" name="tax_input[<?php echo $taxonomy_name; ?>]" class="tax_input_<?php echo $taxonomy_name; ?>"></textarea>
			</label>
		<?php endif; ?>
	...
	foreach ( $columns as $column_name => $column_display_name ) {
		...
		if ( $bulk ) {
			 ...
			 * @param JP_Post $post_type   The post type slug.
			 */
	...
	?>
		<div class="submit inline-edit-save">
			...
			<?php
			if ( ! $bulk ) {
				jp_nonce_field( 'inlineeditnonce', '_inline_edit', false );
				?>
