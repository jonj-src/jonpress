<?php
/**
 * List Table API: JP_MS_Users_List_Table class
 *
 * @package JonPress
 ...
 * @see JP_List_Table
 */
class JP_MS_Users_List_Table extends JP_List_Table {
	/**
	 * @return bool
	 */
	...
	public function prepare_items() {
		 ...
		 */
		if ( ! $usersearch && jp_is_large_network( 'users' ) ) {
		...
		/** This filter is documented in jp-admin/includes/class-jp-users-list-table.php */
	 ...
	 * @since 4.3.0
	 *
	 * @param JP_User $user The current JP_User object.
	 */
	...
	public function column_username( $user ) {
		...
		if ( current_user_can( 'edit_user', $user->ID ) ) {
			$edit_link = esc_url( add_query_arg( 'jp_http_referer', urlencode( jp_unslash( $_SERVER['REQUEST_URI'] ) ), get_edit_user_link( $user->ID ) ) );
	...
	protected function handle_row_actions( $user, $column_name, $primary ) {
		...
		if ( current_user_can( 'delete_user', $user->ID ) && ! in_array( $user->user_login, $super_admins ) ) {
			$actions['delete'] = '<a href="' . $delete = esc_url( network_admin_url( add_query_arg( '_jp_http_referer', urlencode( jp_unslash( $_SERVER['REQUEST_URI'] ) ), jp_nonce_url( 'users.php', 'deleteuser' ) . '&amp;action=deleteuser&amp;id=' . $user->ID ) ) ) . '" class="delete">' . __( 'Delete' ) . '</a>';
		}
