 ...
 * @since 3.1.0
 *
 * @see JP_User_Query::prepare_query() for information on accepted arguments.
 */
class JP_User_Query {
	 ...
	 * @since 4.4.0
	 *
	 * @param array $args Query vars, as passed to `JP_User_Query`.
	...
	public static function fill_query_vars( $args ) {
		...
		return jp_parse_args( $args, $defaults );
	}
	...
	public function prepare_query( $query = array() ) {
		...
		/**
		 * Fires before the JP_User_Query has been parsed.
		 *
		 * The passed JP_User_Query object contains the query variables, not
		 * yet passed into SQL.
		 *
		 * @since 4.0.0
		 *
		 * @param JP_User_Query $this The current JP_User_Query instance,
		 *                            passed by reference.
		 */
		...
		if ( ! empty( $qv['include'] ) ) {
			$include = jp_parse_id_list( $qv['include'] );
		} else {
			$include = false;
		}
		...
		if ( $search ) {
			...
			if ( ! $search_columns ) {
				...
				} elseif ( preg_match( '|^https?://|', $search ) && ! ( is_multisite() && jp_is_large_network( 'users' ) ) ) {
			...
			/**
			 * Filters the columns to search in a JP_User_Query search.
			 *
		...
		if ( ! empty( $include ) ) {
			...
			$ids                = implode( ',', jp_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND $wpdb->users.ID NOT IN ($ids)";
		}

		// Date queries are allowed for the user_registered field.
		if ( ! empty( $qv['date_query'] ) && is_array( $qv['date_query'] ) ) {
			$date_query         = new JP_Date_Query( $qv['date_query'], 'user_registered' );
			$this->query_where .= $date_query->get_sql();
		}

		/**
		 * Fires after the JP_User_Query has been parsed, and before
		 * the query is executed.
		 *
		 * The passed JP_User_Query object contains SQL parts formed
	...
	public function query() {
		...
		/**
		 * Filters SELECT FOUND_ROWS() query for the current JP_User_Query instance.
		 *
		 * @since 3.2.0
		 *
		 * @global wpdb $wpdb JonPress database abstraction object.
		 *
		 * @param string $sql The SELECT FOUND_ROWS() query for the current JP_User_Query.
		 */
		if ( isset( $qv['count_total'] ) && $qv['count_total'] ) {
		...
		if ( 'all_with_meta' == $qv['fields'] ) {
			...
			foreach ( $this->results as $userid ) {
				$r[ $userid ] = new JP_User( $userid, '', $qv['blog_id'] );
			}

			$this->results = $r;
		} elseif ( 'all' == $qv['fields'] ) {
			foreach ( $this->results as $key => $user ) {
				$this->results[ $key ] = new JP_User( $user, '', $qv['blog_id'] );
			}
		}
	}
	...
	protected function parse_orderby( $orderby ) {
		...
		if ( in_array( $orderby, array( 'login', 'nicename', 'email', 'url', 'registered' ) ) ) {
			...
			$include     = jp_parse_id_list( $this->query_vars['include'] );
