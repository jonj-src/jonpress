<?php
/**
 * Canonical API to handle JonPress Redirecting
 *
 ...
 * not exist based on exact JonPress query. Will instead try to parse the URL
...
function redirect_canonical( $requested_url = null, $do_redirect = true ) {
	global $jp_rewrite, $is_IIS, $jp_query, $wpdb, $wp;

	if ( isset( $_SERVER['REQUEST_METHOD'] ) && ! in_array( strtoupper( $_SERVER['REQUEST_METHOD'] ), array( 'GET', 'HEAD' ) ) ) {
		return;
	}

	// If we're not in jp-admin and the post has been published and preview nonce
	// is non-existent or invalid then no need for preview in query
	if ( is_preview() && get_query_var( 'p' ) && 'publish' == get_post_status( get_query_var( 'p' ) ) ) {
		if ( ! isset( $_GET['preview_id'] )
			|| ! isset( $_GET['preview_nonce'] )
			|| ! jp_verify_nonce( $_GET['preview_nonce'], 'post_preview_' . (int) $_GET['preview_id'] ) ) {
			$jp_query->is_preview = false;
		}
	}
	...
	if ( is_singular() && 1 > $jp_query->post_count && ( $id = get_query_var( 'p' ) ) ) {
	...
	if ( is_404() ) {
		...
		if ( get_query_var( 'day' ) && get_query_var( 'monthnum' ) && get_query_var( 'year' ) ) {
			...
			if ( ! jp_checkdate( $month, $day, $year, $date ) ) {
		...
		if ( get_query_var( 'page' ) && $jp_query->post &&
			false !== strpos( $jp_query->post->post_content, '<!--nextpage-->' ) ) {
			$redirect['path']  = rtrim( $redirect['path'], (int) get_query_var( 'page' ) . '/' );
			$redirect['query'] = remove_query_arg( 'page', $redirect['query'] );
			$redirect_url      = get_permalink( $jp_query->post->ID );
		}
	} elseif ( is_object( $jp_rewrite ) && $jp_rewrite->using_permalinks() ) {
		// rewriting of old ?p=X, ?m=2004, ?m=200401, ?m=20040101
		if ( is_attachment() &&
			! array_diff( array_keys( $jp->query_vars ), array( 'attachment', 'attachment_id' ) ) &&
			! $redirect_url ) {
			...
			if ( $redirect_url = get_permalink( $jp_query->get_queried_object_id() ) ) {
			...
			$term_count = 0;
			foreach ( $jp_query->tax_query->queried_terms as $tax_query ) {
				$term_count += count( $tax_query['terms'] );
			}

			$obj = $jp_query->get_queried_object();
			if ( $term_count <= 1 && ! empty( $obj->term_id ) && ( $tax_url = get_term_link( (int) $obj->term_id, $obj->taxonomy ) ) && ! is_jp_error( $tax_url ) ) {
				if ( ! empty( $redirect['query'] ) ) {
					...
					$rewrite_vars = array_diff( array_keys( $jp_query->query ), array_keys( $_GET ) );
		...
		} elseif ( is_single() && strpos( $jp_rewrite->permalink_structure, '%category%' ) !== false && $cat = get_query_var( 'category_name' ) ) {
			$category = get_category_by_path( $cat );
			if ( ( ! $category || is_jp_error( $category ) ) || ! has_term( $category->term_id, 'category', $jp_query->get_queried_object_id() ) ) {
				$redirect_url = get_permalink( $jp_query->get_queried_object_id() );
			}
		}
		...
		if ( is_singular() && get_query_var( 'page' ) ) {
			...
			if ( $page > 1 ) {
				if ( is_front_page() ) {
					$redirect_url = trailingslashit( $redirect_url ) . user_trailingslashit( "$jp_rewrite->pagination_base/$page", 'paged' );
				} else {
		...
		if ( get_query_var( 'paged' ) || is_feed() || get_query_var( 'cpage' ) ) {
			while ( preg_match( "#/$jp_rewrite->pagination_base/?[0-9]+?(/+)?$#", $redirect['path'] ) || preg_match( '#/(comments/?)?(feed|rss|rdf|atom|rss2)(/+)?$#', $redirect['path'] ) || preg_match( "#/{$jp_rewrite->comments_pagination_base}-[0-9]+(/+)?$#", $redirect['path'] ) ) {
				// Strip off paging and feed
				$redirect['path'] = preg_replace( "#/$jp_rewrite->pagination_base/?[0-9]+?(/+)?$#", '/', $redirect['path'] ); // strip off any existing paging
				$redirect['path'] = preg_replace( '#/(comments/?)?(feed|rss2?|rdf|atom)(/+|$)#', '/', $redirect['path'] ); // strip off feed endings
				$redirect['path'] = preg_replace( "#/{$jp_rewrite->comments_pagination_base}-[0-9]+?(/+)?$#", '/', $redirect['path'] ); // strip off any existing comment paging
			}

			$addl_path = '';
			if ( is_feed() && in_array( get_query_var( 'feed' ), $jp_rewrite->feeds ) ) {
				...
				$old_feed_files = array(
					'jp-atom.php'         => 'atom',
					'jp-commentsrss2.php' => 'comments_rss2',
					'jp-feed.php'         => get_default_feed(),
					'jp-rdf.php'          => 'rdf',
					'jp-rss.php'          => 'rss2',
					'jp-rss2.php'         => 'rss2',
				);
			...
			if ( get_query_var( 'paged' ) > 0 ) {
				...
				if ( ! is_feed() ) {
					if ( $paged > 1 && ! is_single() ) {
						$addl_path = ( ! empty( $addl_path ) ? trailingslashit( $addl_path ) : '' ) . user_trailingslashit( "$jp_rewrite->pagination_base/$paged", 'paged' );
					} elseif ( ! is_single() ) {
						$addl_path = ! empty( $addl_path ) ? trailingslashit( $addl_path ) : '';
					}
				} elseif ( $paged > 1 ) {
			...
			( 'newest' != get_option( 'default_comments_page' ) && get_query_var( 'cpage' ) > 1 )
				...
				$addl_path         = ( ! empty( $addl_path ) ? trailingslashit( $addl_path ) : '' ) . user_trailingslashit( $jp_rewrite->comments_pagination_base . '-' . get_query_var( 'cpage' ), 'commentpaged' );
				$redirect['query'] = remove_query_arg( 'cpage', $redirect['query'] );
			}

			$redirect['path'] = user_trailingslashit( preg_replace( '|/' . preg_quote( $jp_rewrite->index, '|' ) . '/?$|', '/', $redirect['path'] ) ); // strip off trailing /index.php/
			if ( ! empty( $addl_path ) && $jp_rewrite->using_index_permalinks() && strpos( $redirect['path'], '/' . $jp_rewrite->index . '/' ) === false ) {
				$redirect['path'] = trailingslashit( $redirect['path'] ) . $jp_rewrite->index . '/';
			}
			if ( ! empty( $addl_path ) ) {
		...
		if ( 'jp-register.php' == basename( $redirect['path'] ) ) {
			if ( is_multisite() ) {
				/** This filter is documented in jp-login.php */
				$redirect_url = apply_filters( 'jp_signup_location', network_site_url( 'jp-signup.php' ) );
			} else {
				$redirect_url = jp_registration_url();
			}
	...
	// trailing /index.php
	$redirect['path'] = preg_replace( '|/' . preg_quote( $jp_rewrite->index, '|' ) . '/*?$|', '/', $redirect['path'] );
	...
	if ( ! $jp_rewrite->using_index_permalinks() ) {
		$redirect['path'] = str_replace( '/' . $jp_rewrite->index . '/', '/', $redirect['path'] );
	}

	// trailing slashes
	if ( is_object( $jp_rewrite ) && $jp_rewrite->using_permalinks() && ! is_404() && ( ! is_front_page() || ( is_front_page() && ( get_query_var( 'paged' ) > 1 ) ) ) ) {
		$user_ts_type = '';
		if ( get_query_var( 'paged' ) > 0 ) {
			$user_ts_type = 'paged';
		} else {
 ...
 *
 * If a user visits example.com/admin, they'll be redirected to /jp-admin.
 * Visiting /login redirects to /jp-login.php, and so on.
 *
 * @since 3.4.0
 *
 * @global JP_Rewrite $jp_rewrite
 */
function jp_redirect_admin_locations() {
	global $jp_rewrite;
	if ( ! ( is_404() && $jp_rewrite->using_permalinks() ) ) {
		return;
	}

	$admins = array(
		home_url( 'jp-admin', 'relative' ),
	...
	if ( in_array( untrailingslashit( $_SERVER['REQUEST_URI'] ), $admins ) ) {
		jp_redirect( admin_url() );
		exit;
	}

	$logins = array(
		home_url( 'jp-login.php', 'relative' ),
		home_url( 'login', 'relative' ),
		site_url( 'login', 'relative' ),
	);
	if ( in_array( untrailingslashit( $_SERVER['REQUEST_URI'] ), $logins ) ) {
		jp_redirect( jp_login_url() );
		exit;
	}
}
