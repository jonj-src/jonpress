<?php
/**
 * Feed API: JP_SimplePie_File class
 *
 * @package JonPress
 * @subpackage Feed
 * @since 4.7.0
 ...
 * @see SimplePie_File
 */
class JP_SimplePie_File extends SimplePie_File {
	...
	public function __construct( $url, $timeout = 10, $redirects = 5, $headers = null, $useragent = null, $force_fsockopen = false ) {
		...
		if ( preg_match( '/^http(s)?:\/\//i', $url ) ) {
			...
			$res = jp_safe_remote_request( $url, $args );

			if ( is_jp_error( $res ) ) {
				$this->error   = 'WP HTTP Error: ' . $res->get_error_message();
				$this->success = false;
			} else {
				$this->headers     = jp_remote_retrieve_headers( $res );
				$this->body        = jp_remote_retrieve_body( $res );
				$this->status_code = jp_remote_retrieve_response_code( $res );
			}
		} else {
			$this->error   = '';
			$this->success = false;
