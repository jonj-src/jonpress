...
wpList = {
	 ...
	 *
	 * Example 1: data-jp-lists="delete:the-comment-list:comment-{comment_ID}:66cc66:unspam=1"
	 * Example 2: data-jp-lists="dim:the-comment-list:comment-{comment_ID}:unapproved:e7e7d3:e7e7d3:new=approved"
	 *
	...
	parseData: function( element, type ) {
		...
		try {
			wpListsData = $( element ).data( 'jp-lists' ) || '';
	...
	process: function( elementId ) {
		...
		$element.on( 'submit', 'form[data-jp-lists^="add:' + list.id + ':"]', function() {
			return list.wpList.add( this );
		} );

		$element.on( 'click', 'a[data-jp-lists^="add:' + list.id + ':"], input[data-jp-lists^="add:' + list.id + ':"]', function() {
			return list.wpList.add( this );
		} );

		$element.on( 'click', '[data-jp-lists^="delete:' + list.id + ':"]', function() {
			return list.wpList.del( this );
		} );

		$element.on( 'click', '[data-jp-lists^="dim:' + list.id + ':"]', function() {
