...
final class JP_Site {
	 ...
	 *
	 * Handling for this does not exist throughout JonPress core, but custom
	 ...
	 * @return JP_Site|false The site's object if found. False if not.
	 */
	public static function get_instance( $site_id ) {
		...
		$_site = jp_cache_get( $site_id, 'sites' );

		if ( ! $_site ) {
			$_site = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->blogs} WHERE blog_id = %d LIMIT 1", $site_id ) );

			if ( empty( $_site ) || is_jp_error( $_site ) ) {
				return false;
			}

			jp_cache_add( $site_id, $_site, 'sites' );
		}

		return new JP_Site( $_site );
	}

	/**
	 * Creates a new JP_Site object.
	 *
	 ...
	 * @since 4.5.0
	 *
	 * @param JP_Site|object $site A site object.
	 */
	...
	private function get_details() {
		$details = jp_cache_get( $this->blog_id, 'site-details' );
		...
		if ( false === $details ) {
			...
			jp_cache_set( $this->blog_id, $details, 'site-details' );
		}
