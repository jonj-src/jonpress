<?php
/**
 * REST API: JP_REST_Post_Statuses_Controller class
 *
 * @package JonPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class JP_REST_Post_Statuses_Controller extends JP_REST_Controller {
	...
	public function get_items_permissions_check( $request ) {
		if ( 'edit' === $request['context'] ) {
			...
			return new JP_Error( 'rest_cannot_view', __( 'Sorry, you are not allowed to manage post statuses.' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( empty( $status ) ) {
			return new JP_Error( 'rest_status_invalid', __( 'Invalid status.' ), array( 'status' => 404 ) );
		}

		$check = $this->check_read_permission( $status );

		if ( ! $check ) {
			return new JP_Error( 'rest_cannot_read_status', __( 'Cannot view status.' ), array( 'status' => rest_authorization_required_code() ) );
		}
	 ...
	 * @param JP_REST_Request $request Full details about the request.
	 * @return JP_REST_Response Post status data.
	 */
