<?php
/**
 * JonPress Widgets Administration API
 *
 * @package JonPress
...
function jp_list_widgets() {
	...
	foreach ( $sort as $widget ) {
		...
		if ( isset( $jp_registered_widget_controls[ $widget['id'] ]['id_base'] ) && isset( $widget['params'][0]['number'] ) ) {
		...
		$args = jp_list_widget_controls_dynamic_sidebar(
			array(
				0 => $args,
				1 => $widget['params'][0],
			)
 ...
 */
function jp_list_widget_controls( $sidebar, $sidebar_name = '' ) {
	add_filter( 'dynamic_sidebar_params', 'jp_list_widget_controls_dynamic_sidebar' );

	$description = jp_sidebar_description( $sidebar );
 ...
 * @return array
 */
function jp_list_widget_controls_dynamic_sidebar( $params ) {
	...
	if ( is_callable( $jp_registered_widgets[ $widget_id ]['callback'] ) ) {
		$jp_registered_widgets[ $widget_id ]['_callback'] = $jp_registered_widgets[ $widget_id ]['callback'];
		$jp_registered_widgets[ $widget_id ]['callback']  = 'jp_widget_control';
	}
 ...
 * @return array
 */
function jp_widget_control( $sidebar_args ) {
	global $jp_registered_widgets, $jp_registered_widget_controls, $sidebars_widgets;

	$widget_id  = $sidebar_args['widget_id'];
	$sidebar_id = isset( $sidebar_args['id'] ) ? $sidebar_args['id'] : false;
	$key        = $sidebar_id ? array_search( $widget_id, $sidebars_widgets[ $sidebar_id ] ) : '-1'; // position of widget in sidebar
	$control    = isset( $jp_registered_widget_controls[ $widget_id ] ) ? $jp_registered_widget_controls[ $widget_id ] : array();
	$widget     = $jp_registered_widgets[ $widget_id ];
	...
	$jp_registered_widgets[ $widget_id ]['callback'] = $jp_registered_widgets[ $widget_id ]['_callback'];
	unset( $jp_registered_widgets[ $widget_id ]['_callback'] );
...
<?php echo ( $widget_description = jp_widget_description( $widget_id ) ) ? "$widget_description\n" : "$widget_title\n"; ?>
	</div>
<?php
 ...
 * @return string
 */
function jp_widgets_access_body_class( $classes ) {
