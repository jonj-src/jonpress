...
add_action( 'network_site_new_created_user', 'jp_send_new_user_notifications' );
add_action( 'network_site_users_created_user', 'jp_send_new_user_notifications' );
add_action( 'network_user_new_created_user', 'jp_send_new_user_notifications' );
...
// Counts
add_action( 'admin_init', 'jp_schedule_update_network_counts' );
add_action( 'update_network_counts', 'jp_update_network_counts', 10, 0 );
foreach ( array( 'user_register', 'deleted_user', 'wpmu_new_user', 'make_spam_user', 'make_ham_user' ) as $action ) {
	add_action( $action, 'jp_maybe_update_network_user_counts', 10, 0 );
}
foreach ( array( 'make_spam_blog', 'make_ham_blog', 'archive_blog', 'unarchive_blog', 'make_delete_blog', 'make_undelete_blog' ) as $action ) {
	add_action( $action, 'jp_maybe_update_network_site_counts', 10, 0 );
}
unset( $action );

// Files
add_filter( 'jp_upload_bits', 'upload_is_file_too_big' );
...
// JP_HOME and JP_SITEURL should not have any effect in MS
remove_filter( 'option_siteurl', '_config_jp_siteurl' );
remove_filter( 'option_home', '_config_jp_home' );
