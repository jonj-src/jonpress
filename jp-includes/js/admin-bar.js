...
if ( typeof(jQuery) != 'undefined' ) {
	if ( typeof(jQuery.fn.hoverIntent) == 'undefined' ) {
		/* jshint ignore:start */
		// hoverIntent v1.8.1 - Copy of jp-includes/js/hoverIntent.min.js
	...
	jQuery(document).ready(function($){
		...
		touchOpen = function(unbind) {
			adminbar.find('li.menupop').on('click.jp-mobile-hover', function(e) {
				var el = $(this);

				if ( el.parent().is('#jp-admin-bar-root-default') && !el.hasClass('hover') ) {
					e.preventDefault();
				...
				if ( unbind ) {
					$('li.menupop').off('click.jp-mobile-hover');
		...
		touchClose = function() {
			...
			$(document.body).on( mobileEvent+'.jp-mobile-hover', function(e) {
		...
		$('#jp-admin-bar-get-shortlink').click(function(e){
			e.preventDefault();
			$(this).addClass('selected').children('.shortlink-input').blur(function(){
				$(this).parents('#jp-admin-bar-get-shortlink').removeClass('selected');
			}).focus().select();
		});
		...
		adminbar.click( function(e) {
			if ( e.target.id != 'wpadminbar' && e.target.id != 'jp-admin-bar-top-secondary' ) {
				return;
			}
		...
		if ( 'sessionStorage' in window ) {
			$('#jp-admin-bar-logout a').click( function() {
				try {
					for ( var key in sessionStorage ) {
						if ( key.indexOf('jp-autosave-') != -1 )
	...
	(function(d, w) {
		...
		clickShortlink = function(e) {
			...
			while ( true ) {
				...
				if ( t.id && t.id == 'jp-admin-bar-get-shortlink' )
					break;
				t = t.parentNode;
			}
		...
		scrollToTop = function(t) {
			...
			if ( t.id != 'wpadminbar' && t.id != 'jp-admin-bar-top-secondary' )
				return;
		...
		addEvent(w, 'load', function() {
			...
			if ( d.body && aB ) {
				...
				addEvent( document.getElementById('jp-admin-bar-logout'), 'click', function() {
					if ( 'sessionStorage' in window ) {
						try {
							for ( var key in sessionStorage ) {
								if ( key.indexOf('jp-autosave-') != -1 )
