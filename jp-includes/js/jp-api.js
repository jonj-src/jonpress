	...
	wp.api.utils.addMixinsAndHelpers = function( model, modelClassName, loadingObjects ) {
		...
		var hasDate = false,
			...
			TimeStampedMixin = {
				 ...
				 *
				 * @param {Date}   date   A JavaScript date object. JonPress expects dates in UTC.
				 ...
				 *
				 * JonPress returns 'date' and 'date_modified' in the timezone of the server as well as
				...
				if ( 'read' === method ) {
					...
					options.success = function( data, textStatus, request ) {
						if ( ! _.isUndefined( request ) ) {
							self.state.totalPages   = parseInt( request.getResponseHeader( 'x-jp-totalpages' ), 10 );
							self.state.totalObjects = parseInt( request.getResponseHeader( 'x-jp-total' ), 10 );
						}
...
( function() {
	...
	if ( _.isEmpty( wpApiSettings ) ) {
		wpApiSettings.root = window.location.origin + '/jp-json/';
	}
	...
	Endpoint = Backbone.Model.extend(/** @lends Endpoint.prototype */{
		...
		initialize: function() {
			...
			if ( model.get( 'schema' ) ) {
				...
				sessionStorage.getItem( 'jp-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ) )
			) {

				// Used a cached copy of the schema model if available.
				model.schemaModel.set( model.schemaModel.parse( JSON.parse( sessionStorage.getItem( 'jp-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ) ) ) ) );
			} else {
				model.schemaModel.fetch( {
					...
					success: function( newSchemaModel ) {
						...
						if ( ! _.isUndefined( sessionStorage ) && ( _.isUndefined( wpApiSettings.cacheSchema ) || wpApiSettings.cacheSchema ) ) {
							try {
								sessionStorage.setItem( 'jp-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ), JSON.stringify( newSchemaModel ) );
							} catch ( error ) {
	...
	/**
	 * Initialize the jp-api, optionally passing the API root.
	 *
	 * @param {object} [args]
	...
	wp.api.init = function( args ) {
		...
		attributes.apiRoot        = args.apiRoot || wpApiSettings.root || '/jp-json';
