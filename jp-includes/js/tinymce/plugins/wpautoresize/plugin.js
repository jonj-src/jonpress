...
// Forked for JonPress so it can be turned on/off after loading.
...
tinymce.PluginManager.add( 'wpautoresize', function( editor ) {
	...
	function resize( e ) {
		...
		if (resizeHeight !== oldSize) {
			...
			editor.fire( 'jp-autoresize', { height: resizeHeight, deltaHeight: e.type === 'nodechange' ? deltaSize : null } );
		}
	}
	...
	function on() {
		if ( ! editor.dom.hasClass( editor.getBody(), 'jp-autoresize' ) ) {
			isActive = true;
			editor.dom.addClass( editor.getBody(), 'jp-autoresize' );
	...
	function off() {
		...
		if ( ! settings.jp_autoresize_on ) {
			isActive = false;
			doc = editor.getDoc();
			editor.dom.removeClass( editor.getBody(), 'jp-autoresize' );
