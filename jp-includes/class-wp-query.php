...
/**
 * The JonPress Query class.
 *
 * @link https://codex.jonpress.org/Function_Reference/JP_Query Codex page.
 *
 * @since 1.5.0
...
class JP_Query {
	 ...
	 *
	 * The query flags are what page info JonPress was able to figure out.
	 *
	 * @since 2.0.0
	 */
	 ...
	 *     @type array        $date_query              An associative array of JP_Date_Query arguments.
	 *                                                 See JP_Date_Query::__construct().
	 ...
	 *     @type array        $meta_query              An associative array of JP_Meta_Query arguments. See JP_Meta_Query.
	 ...
	 *                                                 the 'jp_query_search_exclusion_prefix' filter.
	 ...
	 *     @type array        $tax_query               An associative array of JP_Tax_Query arguments.
	 *                                                 See JP_Tax_Query->queries.
	...
	public function parse_query( $query = '' ) {
		if ( ! empty( $query ) ) {
			$this->init();
			$this->query = $this->query_vars = jp_parse_args( $query );
		...
		if ( ( '' != $qv['attachment'] ) || ! empty( $qv['attachment_id'] ) ) {
			...
			if ( $qv['day'] ) {
				if ( ! $this->is_date ) {
					...
					if ( $qv['monthnum'] && $qv['year'] && ! jp_checkdate( $qv['monthnum'], $qv['day'], $qv['year'], $date ) ) {
						$qv['error'] = '404';
					} else {
						$this->is_day  = true;
						$this->is_date = true;
		...
		if ( $this->is_home && 'page' == get_option( 'show_on_front' ) && get_option( 'page_on_front' ) ) {
			$_query = jp_parse_args( $this->query );
		 ...
		 * @since 1.5.0
		 *
		 * @param JP_Query $this The JP_Query instance (passed by reference).
		 */
	...
	public function parse_tax_query( &$q ) {
		...
		foreach ( get_taxonomies( array(), 'objects' ) as $taxonomy => $t ) {
			...
			if ( $t->query_var && ! empty( $q[ $t->query_var ] ) ) {
				...
				if ( isset( $t->rewrite['hierarchical'] ) && $t->rewrite['hierarchical'] ) {
					$q[ $t->query_var ] = jp_basename( $q[ $t->query_var ] );
				}
		...
		$this->tax_query = new JP_Tax_Query( $tax_query );
		 ...
		 * @since 3.7.0
		 *
		 * @param JP_Query $this The JP_Query instance.
		 */
	...
	protected function parse_search( &$q ) {
		 ...
		 */
		$exclusion_prefix = apply_filters( 'jp_query_search_exclusion_prefix', '-' );
	...
	protected function get_search_stopwords() {
		 ...
		 */
		$this->stopwords = apply_filters( 'jp_search_stopwords', $stopwords );
		return $this->stopwords;
	}
	...
	public function get_posts() {
		 ...
		 * like is_main_query() test against the global $jp_query instance, not the passed one.
		 *
		 * @since 2.0.0
		 *
		 * @param JP_Query $this The JP_Query instance (passed by reference).
		...
		// Parse meta query
		$this->meta_query = new JP_Meta_Query();
		...
		if ( $date_parameters ) {
			$date_query = new JP_Date_Query( array( $date_parameters ) );
		...
		if ( ! empty( $q['date_query'] ) ) {
			$this->date_query = new JP_Date_Query( $q['date_query'] );
		...
		if ( '' != $q['name'] ) {
			...
			if ( ( 'page' != get_option( 'show_on_front' ) ) || empty( $page_for_posts ) || ( $reqpage != $page_for_posts ) ) {
				$q['pagename'] = sanitize_title_for_query( jp_basename( $q['pagename'] ) );
				$q['name']     = $q['pagename'];
			...
			$q['attachment'] = sanitize_title_for_query( jp_basename( $q['attachment'] ) );
			$q['name']       = $q['attachment'];
		...
		if ( ! $q['suppress_filters'] ) {
			/**
			 * Filters the search SQL that is used in the WHERE clause of JP_Query.
			 *
			 * @since 3.0.0
			 *
			 * @param string   $search Search SQL for WHERE clause.
			 * @param JP_Query $this   The current JP_Query object.
			 */
		...
		if ( isset( $q['post_mime_type'] ) && '' != $q['post_mime_type'] ) {
			$whichmimetype = jp_post_mime_type_where( $q['post_mime_type'], $wpdb->posts );
		}
		...
		if ( ! empty( $q['s'] ) ) {
			...
			if ( ! $q['suppress_filters'] ) {
				 ...
				 * @param JP_Query $this           The current JP_Query instance.
				 */
		 ...
		 *
		 * Return a non-null value to bypass JonPress's default post queries.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `found_posts` and `max_num_pages` properties of the JP_Query object,
		 * passed to the filter by reference. If JP_Query does not perform a database
		...
		if ( ! $q['suppress_filters'] ) {
			 ...
			 * @since 2.3.0
			 *
			 * @param JP_Post[] $posts Array of post objects.
			 * @param JP_Query  $this  The JP_Query instance (passed by reference).
			 */
		...
		if ( ! empty( $this->posts ) && $this->is_comment_feed && $this->is_singular ) {
			/** This filter is documented in jp-includes/query.php */
		...
		if ( ! empty( $this->posts ) && ( $this->is_single || $this->is_page ) ) {
			...
			if ( $this->is_preview && $this->posts && current_user_can( $edit_cap, $this->posts[0]->ID ) ) {
				 ...
				 * @since 2.7.0
				 *
				 * @param JP_Post  $post_preview  The Post object.
				 * @param JP_Query $this          The JP_Query instance (passed by reference).
				 */
		...
		if ( ! empty( $this->comments ) ) {
			jp_queue_comments_for_comment_meta_lazyload( $this->comments );
		}
		...
		// of the type JP_Post and are filtered.
		if ( $this->posts ) {
			$this->post_count = count( $this->posts );
		...
		if ( $q['lazy_load_term_meta'] ) {
			jp_queue_posts_for_term_meta_lazyload( $this->posts );
		}
	...
	/**
	 * Iterate current comment index and return JP_Comment object.
	 *
	 * @since 2.2.0
	 *
	 * @return JP_Comment Comment object.
	 */
	...
	/**
	 * Sets up the JonPress query by parsing query string.
	 *
	 * @since 1.5.0
	 *
	 ...
	 * Constructor.
	 *
	 * Sets up the JonPress query, if parameter is not empty.
	 *
	 * @since 1.5.0
	 *
	...
	public function is_tax( $taxonomy = '', $term = '' ) {
		...
		$tax_array      = array_intersect( array_keys( $jp_taxonomies ), (array) $taxonomy );
	 ...
	 *
	 * Otherwise the same as @see JP_Query::is_home()
	 *
	 * @since 3.1.0
	 *
	 ...
	 *
	 * @see JP_Query::is_single()
	 * @see JP_Query::is_singular()
	 *
	 * @since 3.1.0
	 *
	...
	public function setup_postdata( $post ) {
		 ...
		 *
		 * @param JP_Post  $post The Post object (passed by reference).
		 * @param JP_Query $this The current Query object (passed by reference).
		 */
	 ...
	 *
	 * @since 4.4.0
	 * @deprecated 4.5.0 See jp_queue_posts_for_term_meta_lazyload().
	 *
	 * @param mixed $check
	 * @param int   $term_id
	 * @return mixed
	 ...
	 *
	 * @since 4.4.0
	 * @deprecated 4.5.0 See jp_queue_comments_for_comment_meta_lazyload().
	 *
	 * @param mixed $check
