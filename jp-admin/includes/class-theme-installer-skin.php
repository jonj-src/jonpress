...
/**
 * Theme Installer Skin for the JonPress Theme Installer.
 *
 * @since 2.8.0
 ...
 */
class Theme_Installer_Skin extends JP_Upgrader_Skin {
	public $api;
	public $type;
	...
	public function after() {
		...
		$activate_link = jp_nonce_url( $activate_link, 'switch-theme_' . $stylesheet );
		...
		if ( is_network_admin() && current_user_can( 'manage_network_themes' ) ) {
			$install_actions['network_enable'] = '<a href="' . esc_url( jp_nonce_url( 'themes.php?action=enable&amp;theme=' . urlencode( $stylesheet ), 'enable-theme_' . $stylesheet ) ) . '" target="_parent">' . __( 'Network Enable' ) . '</a>';
		}
		...
		if ( ! $this->result || is_jp_error( $this->result ) || is_network_admin() || ! current_user_can( 'switch_themes' ) ) {
		 ...
		 * @param object   $api             Object containing JonPress.org API theme data.
		 * @param string   $stylesheet      Theme directory name.
		 * @param JP_Theme $theme_info      Theme object.
		 */
