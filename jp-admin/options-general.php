...
/** JonPress Translation Installation API */
require_once( ABSPATH . 'jp-admin/includes/translation-install.php' );

if ( ! current_user_can( 'manage_options' ) ) {
	jp_die( __( 'Sorry, you are not allowed to manage options for this site.' ) );
}
...
if ( ! is_multisite() ) {
	$options_help .= '<p>' . __( 'The JonPress URL and the Site URL can be the same (example.com) or different; for example, having the JonPress core files (example.com/jonpress) in a subdirectory instead of the root directory.' ) . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://codex.jonpress.org/Settings_General_Screen">Documentation on General Settings</a>' ) . '</p>' .
...
<?php
if ( ! is_multisite() ) {
	$jp_site_url_class = $jp_home_class = '';
	if ( defined( 'JP_SITEURL' ) ) {
		$jp_site_url_class = ' disabled';
	}
	if ( defined( 'JP_HOME' ) ) {
		$jp_home_class = ' disabled';
	}
?>

<tr>
<th scope="row"><label for="siteurl"><?php _e( 'JonPress Address (URL)' ); ?></label></th>
<td><input name="siteurl" type="url" id="siteurl" value="<?php form_option( 'siteurl' ); ?>"<?php disabled( defined( 'JP_SITEURL' ) ); ?> class="regular-text code<?php echo $jp_site_url_class; ?>" /></td>
</tr>

<tr>
<th scope="row"><label for="home"><?php _e( 'Site Address (URL)' ); ?></label></th>
<td><input name="home" type="url" id="home" aria-describedby="home-description" value="<?php form_option( 'home' ); ?>"<?php disabled( defined( 'JP_HOME' ) ); ?> class="regular-text code<?php echo $jp_home_class; ?>" />
<?php if ( ! defined( 'JP_HOME' ) ) : ?>
<p class="description" id="home-description">
<?php
	printf(
		/* translators: %s: Codex URL */
		__( 'Enter the address here if you <a href="%s">want your site home page to be different from your JonPress installation directory</a>.' ),
		__( 'https://codex.jonpress.org/Giving_JonPress_Its_Own_Directory' )
	);
?>
</p>
<?php endif; ?>
...
?>
	...
	<?php
		...
		printf(
			' <a href="%1$s">%2$s</a>',
			esc_url( jp_nonce_url( admin_url( 'options.php?dismiss=new_admin_email' ), 'dismiss-' . get_current_blog_id() . '-new_admin_email' ) ),
			__( 'Cancel' )
		);
	?>
	</p>
...
<td>
<select name="default_role" id="default_role"><?php jp_dropdown_roles( get_option( 'default_role' ) ); ?></select>
</td>
</tr>
		...
		<td>
			...
			jp_dropdown_languages(
				array(
					...
					'show_available_translations' => current_user_can( 'install_languages' ) && jp_can_install_language_pack(),
				)
			);
			...
			if ( defined( 'WPLANG' ) && ( '' !== WPLANG ) && $locale !== WPLANG ) {
				if ( is_multisite() && current_user_can( 'manage_network_options' )
					...
					?>
					<p class="description">
						<strong><?php _e( 'Note:' ); ?></strong> <?php printf( __( 'The %1$s constant in your %2$s file is no longer needed.' ), '<code>WPLANG</code>', '<code>jp-config.php</code>' ); ?>
					</p>
					<?php
				}
				_deprecated_argument( 'define()', '4.0.0', sprintf( __( 'The %1$s constant in your %2$s file is no longer needed.' ), 'WPLANG', 'jp-config.php' ) );
			}
			?>
		</td>
	</tr>
...
<select id="timezone_string" name="timezone_string" aria-describedby="timezone-description">
	<?php echo jp_timezone_choice( $tzstring, get_user_locale() ); ?>
</select>
...
foreach ( $time_formats as $format ) {
	...
	echo "\t<p class='date-time-doc'>" . __( '<a href="https://codex.jonpress.org/Formatting_Date_and_Time">Documentation on date and time formatting</a>.' ) . "</p>\n";
?>
	</fieldset>
</td>
</tr>
...
for ( $day_index = 0; $day_index <= 6; $day_index++ ) :
	...
	echo "\n\t<option value='" . esc_attr( $day_index ) . "' $selected>" . $jp_locale->get_weekday( $day_index ) . '</option>';
endfor;
?>
</select></td>
</tr>
