...
if ( ! current_user_can( 'create_sites' ) ) {
	jp_die( __( 'Sorry, you are not allowed to add sites to this network.' ) );
}
...
if ( isset( $_REQUEST['action'] ) && 'add-site' == $_REQUEST['action'] ) {
	...
	if ( ! is_array( $_POST['blog'] ) ) {
		jp_die( __( 'Can&#8217;t create an empty site.' ) );
	}
	...
	if ( ! is_subdomain_install() ) {
		...
		if ( in_array( $domain, $subdirectory_reserved_names ) ) {
			jp_die(
				...
				sprintf(
					__( 'The following words are reserved for use by JonPress functions and cannot be used as blog names: %s' ),
	...
	if ( isset( $_POST['WPLANG'] ) ) {
		...
		} elseif ( current_user_can( 'install_languages' ) && jp_can_install_language_pack() ) {
			$language = jp_download_language_pack( jp_unslash( $_POST['WPLANG'] ) );
			if ( $language ) {
	...
	if ( empty( $domain ) ) {
		jp_die( __( 'Missing or invalid site address.' ) );
	}

	if ( isset( $blog['email'] ) && '' === trim( $blog['email'] ) ) {
		jp_die( __( 'Missing email address.' ) );
	}

	$email = sanitize_email( $blog['email'] );
	if ( ! is_email( $email ) ) {
		jp_die( __( 'Invalid email address.' ) );
	}
	...
	if ( ! $user_id ) { // Create a new user with a random password
		...
		if ( $user_id ) {
			jp_die( __( 'The domain or path entered conflicts with an existing username.' ) );
		}
		$password = jp_generate_password( 12, false );
		$user_id  = wpmu_create_user( $domain, $password, $email );
		if ( false === $user_id ) {
			jp_die( __( 'There was an error creating the user.' ) );
		}
...
<?php jp_nonce_field( 'add-blog', '_wpnonce_add-blog' ); ?>
...
<?php
		...
		<tr class="form-field form-required">
			...
			<td><input name="blog[email]" type="email" class="regular-text jp-suggest-user" id="admin-email" data-autocomplete-type="search" data-autocomplete-field="user_email" aria-describedby="site-admin-email" required /></td>
		</tr>
		<tr class="form-field">
