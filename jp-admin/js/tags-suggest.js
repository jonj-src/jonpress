...
( function( $ ) {
	 ...
	 *
	 * The taxonomy can be passed in a `data-jp-taxonomy` attribute on the element or
	...
	$.fn.wpTagsSuggest = function( options ) {
		...
		var taxonomy = options.taxonomy || $element.attr( 'data-jp-taxonomy' ) || 'post_tag';
		...
		options = $.extend( {
			...
			focus: function( event, ui ) {
				$element.attr( 'aria-activedescendant', 'jp-tags-autocomplete-' + ui.item.id );
		...
		.autocomplete( 'instance' )._renderItem = function( ul, item ) {
			return $( '<li role="option" id="jp-tags-autocomplete-' + item.id + '">' )
				.text( item.name )
				.appendTo( ul );
		};
