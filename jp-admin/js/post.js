...
(function($) {
	...
	$(document).on( 'heartbeat-send.jp-refresh-nonces', function( e, data ) {
		var post_id,
			$authCheck = $('#jp-auth-check-wrap');

		if ( check || ( $authCheck.length && ! $authCheck.hasClass( 'hidden' ) ) ) {
			if ( ( post_id = $('#post_ID').val() ) && $('#_wpnonce').val() ) {
				data['jp-refresh-post-nonces'] = {
					post_id: post_id
				};
			}
		}
	}).on( 'heartbeat-tick.jp-refresh-nonces', function( e, data ) {
		var nonces = data['jp-refresh-post-nonces'];
...
jQuery(document).ready( function($) {
	...
	$('#post-lock-dialog .notification-dialog').on( 'keydown', function(e) {
		...
		if ( target.hasClass('jp-tab-first') && e.shiftKey ) {
			$(this).find('.jp-tab-last').focus();
			e.preventDefault();
		// [tab] on last tab cycles back to first tab.
		} else if ( target.hasClass('jp-tab-last') && ! e.shiftKey ) {
			$(this).find('.jp-tab-first').focus();
			e.preventDefault();
		}
	}).filter(':visible').find('.jp-tab-first').focus();
	...
	$('#post-preview').on( 'click.post-preview', function( event ) {
		var $this = $(this),
			$form = $('form#post'),
			$previewField = $('input#jp-preview'),
			target = $this.attr('target') || 'jp-preview',
	...
	$('.categorydiv').each( function(){
		...
		// TODO: move to jQuery 1.3+, support for multiple hierarchical taxonomies, see jp-lists.js
		...
		$('#' + taxonomy + '-add-toggle').click( function( e ) {
			e.preventDefault();
			$('#' + taxonomy + '-adder').toggleClass( 'jp-hidden-children' );
	...
	( function() {
		...
		function dragging( event ) {
			if ( $postdivrich.hasClass( 'jp-editor-expand' ) ) {
				return;
			}
		...
		function endDrag() {
			...
			if ( mce ) {
				editor.focus();
				toolbarHeight = parseInt( $( '#jp-content-editor-container .mce-toolbar-grp' ).height(), 10 );
		...
		$handle.on( 'mousedown.jp-editor-resize', function( event ) {
			...
			$document.on( 'mousemove.jp-editor-resize', dragging )
				.on( 'mouseup.jp-editor-resize mouseleave.jp-editor-resize', endDrag );

			event.preventDefault();
		}).on( 'mouseup.jp-editor-resize', endDrag );
	})();
	...
	$textarea.on( 'keydown.jp-autosave', function( event ) {
		// Key [s] has code 83.
		if ( event.which === 83 ) {
	...
	if ( $( '#original_post_status' ).val() === 'auto-draft' && window.history.replaceState ) {
		...
		$( '#publish' ).on( 'click', function() {
			...
			location += 'jp-post-new-reload=true';
...
( function( $, counter ) {
	$( function() {
		var $content = $( '#content' ),
			$count = $( '#jp-word-count' ).find( '.word-count' ),
			prevCount = 0,
			contentEditor;
