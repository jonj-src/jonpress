 ...
 * @see JP_Upgrader
 */
class Theme_Upgrader extends JP_Upgrader {
	 ...
	 * }
	 *
	 * @return bool|JP_Error True if the installation was successful, false or a JP_Error object otherwise.
	 */
	public function install( $package, $args = array() ) {
		...
		if ( $parsed_args['clear_update_cache'] ) {
			// Clear cache so jp_update_themes() knows about the new theme.
			add_action( 'upgrader_process_complete', 'jp_clean_themes_cache', 9, 0 );
		}
		...
		remove_action( 'upgrader_process_complete', 'jp_clean_themes_cache', 9 );
		...
		jp_clean_themes_cache( $parsed_args['clear_update_cache'] );
	...
	public function bulk_upgrade( $themes, $args = array() ) {
		...
		$res = $this->fs_connect( array( JP_CONTENT_DIR ) );
		if ( ! $res ) {
			$this->skin->footer();
			return false;
		}
	 ...
	 * @return string|JP_Error The source or a JP_Error.
	 */
	 ...
	 * @since 2.8.0
	 *
	 * @param bool|JP_Error  $return
	 * @param array          $theme
	 * @return bool|JP_Error
	 */
	public function current_before( $return, $theme ) {
		if ( is_jp_error( $return ) ) {
			return $return;
		}
	...
	public function delete_old_theme( $removed, $local_destination, $remote_destination, $theme ) {
		...
		$theme      = $theme['theme'];
		$themes_dir = trailingslashit( $jp_filesystem->jp_themes_dir( $theme ) );
		if ( $jp_filesystem->exists( $themes_dir . $theme ) ) {
			if ( ! $jp_filesystem->delete( $themes_dir . $theme, true ) ) {
				return false;
			}
		}
	...
	/**
	 * Get the JP_Theme object for a theme.
	 *
	 * @since 2.8.0
	 ...
	 * @return JP_Theme|false The theme's info object, or false `$theme` is not supplied
