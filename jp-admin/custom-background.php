...
class Custom_Background {
	...
	public function __construct( $admin_header_callback = '', $admin_image_div_callback = '' ) {
		...
		add_action( 'jp_ajax_custom-background-add', array( $this, 'ajax_background_add' ) );

		// Unused since 3.5.0.
		add_action( 'jp_ajax_set-background-image', array( $this, 'jp_set_background_image' ) );
	}
	...
	public function admin_load() {
		...
		get_current_screen()->set_help_sidebar(
			...
			'<p>' . __( '<a href="https://codex.jonpress.org/Appearance_Background_Screen">Documentation on Custom Background</a>' ) . '</p>' .
			'<p>' . __( '<a href="https://jonpress.org/support/">Support Forums</a>' ) . '</p>'
		);

		jp_enqueue_media();
		jp_enqueue_script( 'custom-background' );
	...
	public function take_action() {
		...
		if ( isset( $_POST['remove-background'] ) ) {
			...
			$this->updated = true;
			jp_safe_redirect( $_POST['_jp_http_referer'] );
			return;
		}
...
<td>
<form method="post">
<?php jp_nonce_field( 'custom-background-remove', '_wpnonce-custom-background-remove' ); ?>
...
<td>
<form method="post">
<?php jp_nonce_field( 'custom-background-reset', '_wpnonce-custom-background-reset' ); ?>
...
<td><form enctype="multipart/form-data" id="upload-form" class="jp-upload-form" method="post">
	<p>
		<label for="upload"><?php _e( 'Choose an image from your computer:' ); ?></label><br />
		<input type="file" id="upload" name="import" />
		<input type="hidden" name="action" value="save" />
		<?php jp_nonce_field( 'custom-background-upload', '_wpnonce-custom-background-upload' ); ?>
...
<?php jp_nonce_field( 'custom-background' ); ?>
...
<?php
	...
	public function handle_upload() {
		...
		// Save the data
		$id = jp_insert_attachment( $object, $file );

		// Add the meta-data
		jp_update_attachment_metadata( $id, jp_generate_attachment_metadata( $id, $file ) );
		update_post_meta( $id, '_jp_attachment_is_custom_background', get_option( 'stylesheet' ) );

		set_theme_mod( 'background_image', esc_url_raw( $url ) );

		$thumbnail = jp_get_attachment_image_src( $id, 'thumbnail' );
		set_theme_mod( 'background_image_thumb', esc_url_raw( $thumbnail[0] ) );

		/** This action is documented in jp-admin/custom-header.php */
		do_action( 'jp_create_file_in_uploads', $file, $id ); // For replication
		$this->updated = true;
	}
	...
	public function ajax_background_add() {
		...
		update_post_meta( $attachment_id, '_jp_attachment_is_custom_background', get_stylesheet() );
	 ...
	 * @deprecated 3.5.0
	 */
	public function jp_set_background_image() {
		...
		update_post_meta( $attachment_id, '_jp_attachment_is_custom_background', get_option( 'stylesheet' ) );
		$url       = jp_get_attachment_image_src( $attachment_id, $size );
		$thumbnail = jp_get_attachment_image_src( $attachment_id, 'thumbnail' );
