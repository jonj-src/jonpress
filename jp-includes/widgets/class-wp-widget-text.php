<?php
/**
 * Widget API: JP_Widget_Text class
 *
 * @package JonPress
 * @subpackage Widgets
 * @since 4.4.0
 ...
 * @see JP_Widget
 */
class JP_Widget_Text extends JP_Widget {
	...
	public function _register_one( $number = -1 ) {
		...
		jp_add_inline_script( 'text-widgets', sprintf( 'wp.textWidgets.idBases.push( %s );', jp_json_encode( $this->id_base ) ) );

		if ( $this->is_preview() ) {
			add_action( 'jp_enqueue_scripts', array( $this, 'enqueue_preview_scripts' ) );
		}

		// Note that the widgets component in the customizer will also do the 'admin_print_scripts-widgets.php' action in JP_Customize_Widgets::print_scripts().
		add_action( 'admin_print_scripts-widgets.php', array( $this, 'enqueue_admin_scripts' ) );

		// Note that the widgets component in the customizer will also do the 'admin_footer-widgets.php' action in JP_Customize_Widgets::print_footer_scripts().
		add_action( 'admin_footer-widgets.php', array( 'JP_Widget_Text', 'render_control_template_scripts' ) );
	}
	...
	public function widget( $args, $instance ) {
		 ...
		 * @since 4.8.1 The `$this` param may now be a `JP_Widget_Custom_HTML` object in addition to a `JP_Widget_Text` object.
		 *
		 * @param string                               $text     The widget content.
		 * @param array                                $instance Array of settings for the current widget.
		 * @param JP_Widget_Text|JP_Widget_Custom_HTML $this     Current Text widget instance.
		 */
		...
		if ( $is_visual_text_widget ) {
			 ...
			 * @param string         $text     The widget content.
			 * @param array          $instance Array of settings for the current widget.
			 * @param JP_Widget_Text $this     Current Text widget instance.
			 */
	 ...
	 * @since 4.9.0
	 *
	 * @see JP_Widget_Media_Video::inject_video_max_width_style()
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( current_user_can( 'unfiltered_html' ) ) {
			...
			$instance['text'] = jp_kses_post( $new_instance['text'] );
		}
	...
	public function enqueue_preview_scripts() {
		...
		jp_playlist_scripts( 'audio' );
		jp_playlist_scripts( 'video' );
	}
	...
	public function enqueue_admin_scripts() {
		jp_enqueue_editor();
		jp_enqueue_media();
		jp_enqueue_script( 'text-widgets' );
		jp_add_inline_script( 'text-widgets', 'wp.textWidgets.init();', 'after' );
	}
	 ...
	 * @see JP_Widget_Text::render_control_template_scripts()
	...
	public function form( $instance ) {
		...
		<?php if ( ! $this->is_legacy_instance( $instance ) ) : ?>
			...
			/** This filter is documented in jp-includes/class-jp-editor.php */
	...
	public static function render_control_template_scripts() {
		...
		<script type="text/html" id="tmpl-widget-text-control-fields">
			...
			<?php if ( ! in_array( 'text_widget_custom_html', $dismissed_pointers, true ) ) : ?>
				<div hidden class="jp-pointer custom-html-widget-pointer jp-pointer-top">
					<div class="jp-pointer-content">
						...
						<?php endif; ?>
						<div class="jp-pointer-buttons">
							<a class="close" href="#"><?php _e( 'Dismiss' ); ?></a>
						</div>
					</div>
					<div class="jp-pointer-arrow">
						<div class="jp-pointer-arrow-inner"></div>
					</div>
				</div>
			<?php endif; ?>

			<?php if ( ! in_array( 'text_widget_paste_html', $dismissed_pointers, true ) ) : ?>
				<div hidden class="jp-pointer paste-html-pointer jp-pointer-top">
					<div class="jp-pointer-content">
			...
			<p>
				...
				<textarea id="{{ elementIdPrefix }}text" class="widefat text jp-editor-area" style="height: 200px" rows="16" cols="20"></textarea>
			</p>
		</script>
		<?php
	}
