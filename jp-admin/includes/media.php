<?php
/**
 * JonPress Administration Media API.
 *
 * @package JonPress
...
function the_media_upload_tabs() {
	...
	if ( ! empty( $tabs ) ) {
		...
		if ( isset( $redir_tab ) && array_key_exists( $redir_tab, $tabs ) ) {
			...
			/** This filter is documented in jp-admin/media-upload.php */
...
function get_image_send_to_editor( $id, $caption, $title, $align, $url = '', $rel = false, $size = 'medium', $alt = '' ) {
	...
	if ( $rel ) {
		if ( is_string( $rel ) ) {
			...
			$rel = ' rel="attachment jp-att-' . intval( $id ) . '"';
		}
	} else {
		$rel = '';
	}
...
win.send_to_editor( <?php echo jp_json_encode( $html ); ?> );
</script>
<?php
	exit;
}
 ...
 * @param array  $overrides Override the jp_handle_upload() behavior. Optional.
 * @return int|JP_Error ID of the attachment or a JP_Error object on failure.
 */
function media_handle_upload( $file_id, $post_id, $post_data = array(), $overrides = array( 'test_form' => false ) ) {
	...
	$file = jp_handle_upload( $_FILES[ $file_id ], $overrides, $time );

	if ( isset( $file['error'] ) ) {
		return new JP_Error( 'upload_error', $file['error'] );
	}

	$name = $_FILES[ $file_id ]['name'];
	$ext  = pathinfo( $name, PATHINFO_EXTENSION );
	$name = jp_basename( $name, ".$ext" );
	...
	if ( preg_match( '#^audio#', $type ) ) {
		$meta = jp_read_audio_metadata( $file );
	...
	} elseif ( 0 === strpos( $type, 'image/' ) && $image_meta = jp_read_image_metadata( $file ) ) {
	...
	// Save the data
	$id = jp_insert_attachment( $attachment, $file, $post_id, true );
	if ( ! is_jp_error( $id ) ) {
		jp_update_attachment_metadata( $id, jp_generate_attachment_metadata( $id, $file ) );
	}
 ...
 * @return int|object The ID of the attachment or a JP_Error on failure.
 */
function media_handle_sideload( $file_array, $post_id, $desc = null, $post_data = array() ) {
	...
	$file = jp_handle_sideload( $file_array, $overrides, $time );
	if ( isset( $file['error'] ) ) {
		return new JP_Error( 'upload_error', $file['error'] );
	}
	...
	// Use image exif/iptc data for title and caption defaults if possible.
	if ( $image_meta = jp_read_image_metadata( $file ) ) {
		if ( trim( $image_meta['title'] ) && ! is_numeric( sanitize_title( $image_meta['title'] ) ) ) {
			$title = $image_meta['title'];
		}
		if ( trim( $image_meta['caption'] ) ) {
	...
	$id = jp_insert_attachment( $attachment, $file, $post_id );
	if ( ! is_jp_error( $id ) ) {
		jp_update_attachment_metadata( $id, jp_generate_attachment_metadata( $id, $file ) );
	}
 ...
 */
function jp_iframe( $content_func /* ... */ ) {
	_jp_admin_html_begin();
?>
<title><?php bloginfo( 'name' ); ?> &rsaquo; <?php _e( 'Uploads' ); ?> &#8212; <?php _e( 'JonPress' ); ?></title>
<?php

jp_enqueue_style( 'colors' );
// Check callback name for 'media'
if ( ( is_array( $content_func ) && ! empty( $content_func[1] ) && 0 === strpos( (string) $content_func[1], 'media' ) )
	|| ( ! is_array( $content_func ) && 0 === strpos( $content_func, 'media' ) ) ) {
	jp_enqueue_style( 'deprecated-media' );
}
jp_enqueue_style( 'ie' );
?>
<script type="text/javascript">
...
<?php
	/** This action is documented in jp-admin/admin-header.php */
...
<body<?php echo $body_id_attr; ?> class="jp-core-ui no-js">
<script type="text/javascript">
...
<?php
	...
	/** This action is documented in jp-admin/admin-footer.php */
...
function media_buttons( $editor_id = 'content' ) {
	...
	$img = '<span class="jp-media-buttons-icon"></span> ';
 ...
 * @since 2.5.0
 *
 * @return mixed void|object JP_Error on failure
 */
function media_upload_form_handler() {
	...
	if ( ! empty( $_POST['attachments'] ) ) {
		foreach ( $_POST['attachments'] as $attachment_id => $attachment ) {
			 ...
			 * @since 2.5.0
			 *
			 * @see jp_get_attachment_metadata()
			 *
			...
			if ( isset( $attachment['image_alt'] ) ) {
				$image_alt = jp_unslash( $attachment['image_alt'] );
				if ( $image_alt != get_post_meta( $attachment_id, '_jp_attachment_image_alt', true ) ) {
					$image_alt = jp_strip_all_tags( $image_alt, true );

					// Update_meta expects slashed.
					update_post_meta( $attachment_id, '_jp_attachment_image_alt', jp_slash( $image_alt ) );
				}
			}
			...
			foreach ( get_attachment_taxonomies( $post ) as $t ) {
				if ( isset( $attachment[ $t ] ) ) {
					jp_set_object_terms( $attachment_id, array_map( 'trim', preg_split( '/,+/', $attachment[ $t ] ) ), $t, false );
				}
			}
		}
	}
	...
	if ( isset( $send_id ) ) {
		$attachment = jp_unslash( $_POST['attachments'][ $send_id ] );

		$html = isset( $attachment['post_title'] ) ? $attachment['post_title'] : '';
		if ( ! empty( $attachment['url'] ) ) {
			$rel = '';
			if ( strpos( $attachment['url'], 'attachment_id' ) || get_attachment_link( $send_id ) == $attachment['url'] ) {
				$rel = " rel='attachment jp-att-" . esc_attr( $send_id ) . "'";
			}
 ...
 * @return null|string
 */
function jp_media_upload_handler() {
	$errors = array();
	$id     = 0;
	...
	if ( ! empty( $_POST['insertonlybutton'] ) ) {
		...
		if ( isset( $_POST['media_type'] ) && 'image' != $_POST['media_type'] ) {
			$title = esc_html( jp_unslash( $_POST['title'] ) );
			if ( empty( $title ) ) {
			...
			$align = '';
			$alt   = esc_attr( jp_unslash( $_POST['alt'] ) );
			if ( isset( $_POST['align'] ) ) {
				$align = esc_attr( jp_unslash( $_POST['align'] ) );
	...
	if ( isset( $_POST['save'] ) ) {
		...
		jp_enqueue_script( 'admin-gallery' );
		return jp_iframe( 'media_upload_gallery_form', $errors );
	...
	if ( isset( $_GET['tab'] ) && $_GET['tab'] == 'type_url' ) {
		...
		return jp_iframe( 'media_upload_type_url_form', $type, $errors, $id );
	}

	return jp_iframe( 'media_upload_type_form', 'image', $errors, $id );
}
 ...
 * @return string|JP_Error Populated HTML img tag on success, JP_Error object otherwise.
 */
function media_sideload_image( $file, $post_id, $desc = null, $return = 'html' ) {
	if ( ! empty( $file ) ) {
		...
		if ( ! $matches ) {
			return new JP_Error( 'image_sideload_failed', __( 'Invalid image URL' ) );
		}
		...
		if ( is_jp_error( $file_array['tmp_name'] ) ) {
	...
	if ( ! empty( $src ) ) {
		...
		return new JP_Error( 'image_sideload_failed' );
	}
}
...
function media_upload_library() {
	...
	return jp_iframe( 'media_upload_library_form', $errors );
}
...
function image_link_input_fields( $post, $url_type = '' ) {
	...
	$file = jp_get_attachment_url( $post->ID );
 ...
 * @since 3.4.0
 *
 * @param JP_Post $edit_post Attachment JP_Post object.
 * @return string HTML markup for the textarea element.
 */
function jp_caption_input_textarea( $edit_post ) {
 ...
 * @param JP_Post $post        The JP_Post attachment object.
 ...
 * @since 2.5.0
 *
 * @param array $post       The JP_Post attachment object converted to an array.
 * @param array $attachment An array of attachment metadata.
...
function image_attachment_fields_to_save( $post, $attachment ) {
	if ( substr( $post['post_mime_type'], 0, 5 ) == 'image' ) {
		if ( strlen( trim( $post['post_title'] ) ) == 0 ) {
			...
			$post['post_title']                       = preg_replace( '/\.\w+$/', '', jp_basename( $attachment_url ) );
...
function get_attachment_fields_to_edit( $post, $errors = null ) {
	...
	if ( is_array( $post ) ) {
		$post = new JP_Post( (object) $post );
	}

	$image_url = jp_get_attachment_url( $post->ID );
	...
	$form_fields = array(
		...
		'post_excerpt' => array(
			'label' => __( 'Caption' ),
			'input' => 'html',
			'html'  => jp_caption_input_textarea( $edit_post ),
		),
		'post_content' => array(
		...
		'image_url'    => array(
			...
			'value' => jp_get_attachment_url( $post->ID ),
	...
	if ( substr( $post->post_mime_type, 0, 5 ) == 'image' ) {
		$alt = get_post_meta( $post->ID, '_jp_attachment_image_alt', true );
		if ( empty( $alt ) ) {
			$alt = '';
		}
...
function get_media_items( $post_id, $errors ) {
	...
	if ( $post_id ) {
		...
		if ( is_array( $GLOBALS['jp_the_query']->posts ) ) {
			foreach ( $GLOBALS['jp_the_query']->posts as $attachment ) {
...
function get_media_item( $attachment_id, $args = null ) {
	...
	if ( ( $attachment_id = intval( $attachment_id ) ) && $thumb_url = jp_get_attachment_image_src( $attachment_id, 'thumbnail', true ) ) {
	...
	$filename = esc_html( jp_basename( $file ) );
	$title    = esc_attr( $post->post_title );

	$post_mime_types = get_post_mime_types();
	$keys            = array_keys( jp_match_mime_types( array_keys( $post_mime_types ), $post->post_mime_type ) );
	$type            = reset( $keys );
	...
	$display_title = $r['show_title'] ? "<div class='filename new'><span class='title'>" . jp_html_excerpt( $display_title, 60, '&hellip;' ) . '</span></div>' : '';
	...
	$media_dims = '';
	$meta       = jp_get_attachment_metadata( $post->ID );
	...
	$image_edit_button = '';
	if ( jp_attachment_is_image( $post->ID ) && jp_image_editor_supports( array( 'mime_type' => $post->post_mime_type ) ) ) {
		$nonce             = jp_create_nonce( "image_editor-$post->ID" );
	...
	if ( $delete && current_user_can( 'delete_post', $attachment_id ) ) {
		if ( ! EMPTY_TRASH_DAYS ) {
			$delete = "<a href='" . jp_nonce_url( "post.php?action=delete&amp;post=$attachment_id", 'delete-post_' . $attachment_id ) . "' id='del[$attachment_id]' class='delete-permanently'>" . __( 'Delete Permanently' ) . '</a>';
		} elseif ( ! MEDIA_TRASH ) {
			$delete = "<a href='#' class='del-link' onclick=\"document.getElementById('del_attachment_$attachment_id').style.display='block';return false;\">" . __( 'Delete' ) . "</a>
				...
				<a href='" . jp_nonce_url( "post.php?action=delete&amp;post=$attachment_id", 'delete-post_' . $attachment_id ) . "' id='del[$attachment_id]' class='button'>" . __( 'Continue' ) . "</a>
				<a href='#' class='button' onclick=\"this.parentNode.style.display='none';return false;\">" . __( 'Cancel' ) . '</a>
				</div>';
		} else {
			$delete = "<a href='" . jp_nonce_url( "post.php?action=trash&amp;post=$attachment_id", 'trash-post_' . $attachment_id ) . "' id='del[$attachment_id]' class='delete'>" . __( 'Move to Trash' ) . "</a>
			<a href='" . jp_nonce_url( "post.php?action=untrash&amp;post=$attachment_id", 'untrash-post_' . $attachment_id ) . "' id='undo[$attachment_id]' class='undo hidden'>" . __( 'Undo' ) . '</a>';
		}
	} else {
		$delete = '';
	}
	...
	if ( 'image' == $type && $calling_post_id && current_theme_supports( 'post-thumbnails', get_post_type( $calling_post_id ) )
		...
		$ajax_nonce = jp_create_nonce( "set_post_thumbnail-$calling_post_id" );
		$thumbnail  = "<a class='jp-post-thumbnail' id='jp-post-thumbnail-" . $attachment_id . "' href='#' onclick='WPSetAsThumbnail(\"$attachment_id\", \"$ajax_nonce\");return false;'>" . esc_html( $calling_post_type_object->labels->use_featured_image ) . '</a>';
	}
...
function media_upload_form( $errors = null ) {
	...
	if ( ! _device_can_upload() ) {
		echo '<p>' . sprintf( __( 'The web browser on your device cannot be used to upload files. You may be able to use the <a href="%s">native app for your device</a> instead.' ), 'https://apps.jonpress.org/' ) . '</p>';
		return;
	}
...
if ( isset( $errors['upload_error'] ) && is_jp_error( $errors['upload_error'] ) ) {
...
$post_params = array(
	'post_id'  => $post_id,
	'_wpnonce' => jp_create_nonce( 'media-form' ),
	'type'     => $_type,
	'tab'      => $_tab,
	'short'    => '1',
);
...
wpUploaderInit = <?php echo jp_json_encode( $plupload_init ); ?>;
</script>
 ...
 */
echo apply_filters( 'type_url_form_media', jp_media_insert_url_form( $type ) );
?>
</div>
</div>
</form>
...
function media_upload_library_form( $errors ) {
	global $wpdb, $jp_query, $jp_locale, $type, $tab, $post_mime_types;
	...
	list($post_mime_types, $avail_post_mime_types) = jp_edit_attachments_query( $q );
...
<?php
$type_links = array();
$_num_posts = (array) jp_count_attachments();
$matches    = jp_match_mime_types( array_keys( $post_mime_types ), array_keys( $_num_posts ) );
...
if ( empty( $_GET['post_mime_type'] ) && ! empty( $num_posts[ $type ] ) ) {
	...
	list($post_mime_types, $avail_post_mime_types) = jp_edit_attachments_query();
}
...
foreach ( $post_mime_types as $mime_type => $label ) {
	...
	if ( ! jp_match_mime_types( $mime_type, $avail_post_mime_types ) ) {
		continue;
	}

	if ( isset( $_GET['post_mime_type'] ) && jp_match_mime_types( $mime_type, $_GET['post_mime_type'] ) ) {
		$class = ' class="current"';
	}
...
$page_links = paginate_links(
	array(
		...
		'prev_text' => __( '&laquo;' ),
		'next_text' => __( '&raquo;' ),
		'total'     => ceil( $jp_query->found_posts / 10 ),
		'current'   => $q['paged'],
	)
);
...
foreach ( $arc_result as $arc_row ) {
	...
	echo esc_html( $jp_locale->get_month( $arc_row->mmonth ) . " $arc_row->yyear" );
	echo "</option>\n";
}
?>
</select>
 ...
 */
function jp_media_insert_url_form( $default_view = 'image' ) {
	/** This filter is documented in jp-admin/includes/media.php */
	if ( ! apply_filters( 'disable_captions', '' ) ) {
		$caption = '
		<tr class="image-only">
...
function media_upload_html_bypass() {
	...
	<p class="upload-html-bypass hide-if-no-js">
		<?php _e( 'You are using the browser&#8217;s built-in file uploader. The JonPress uploader includes multiple file selection and drag and drop capability. <a href="#">Switch to the multi-file uploader</a>.' ); ?>
	</p>
	<?php
}
 ...
 * @since 3.5.0
 *
 * @param JP_Post $post A post object.
 */
function edit_form_image_editor( $post ) {
	$open = isset( $_GET['image-editor'] );
	if ( $open ) {
		require_once ABSPATH . 'jp-admin/includes/image-edit.php';
	}

	$thumb_url = false;
	if ( $attachment_id = intval( $post->ID ) ) {
		$thumb_url = jp_get_attachment_image_src( $attachment_id, array( 900, 450 ), true );
	}

	$alt_text = get_post_meta( $post->ID, '_jp_attachment_image_alt', true );

	$att_url = jp_get_attachment_url( $post->ID );
	?>
	<div class="jp_attachment_holder jp-clearfix">
	<?php
	if ( jp_attachment_is_image( $post->ID ) ) :
		$image_edit_button = '';
		if ( jp_image_editor_supports( array( 'mime_type' => $post->post_mime_type ) ) ) {
			$nonce             = jp_create_nonce( "image_editor-$post->ID" );
			$image_edit_button = "<input type='button' id='imgedit-open-btn-$post->ID' onclick='imageEdit.open( $post->ID, \"$nonce\" )' class='button' value='" . esc_attr__( 'Edit Image' ) . "' /> <span class='spinner'></span>";
		}
	...
	?>
		...
		<div<?php echo $open_style; ?> class="jp_attachment_image jp-clearfix" id="media-head-<?php echo $attachment_id; ?>">
		...
		<div<?php echo $not_open_style; ?> class="image-editor" id="image-editor-<?php echo $attachment_id; ?>">
			<?php
			if ( $open ) {
				jp_image_editor( $attachment_id );}
?>
		</div>
	<?php
	elseif ( $attachment_id && jp_attachment_is( 'audio', $post ) ) :

		jp_maybe_generate_attachment_metadata( $post );

		echo jp_audio_shortcode( array( 'src' => $att_url ) );

	elseif ( $attachment_id && jp_attachment_is( 'video', $post ) ) :
		...
		if ( ! empty( $thumb_id ) ) {
			$attr['poster'] = jp_get_attachment_url( $thumb_id );
		}

		echo jp_video_shortcode( $attr );

	elseif ( isset( $thumb_url[0] ) ) :

		?>
		<div class="jp_attachment_image jp-clearfix" id="media-head-<?php echo $attachment_id; ?>">
		 ...
		 * @param JP_Post $post A post object.
		 */
		do_action( 'jp_edit_form_attachment_display', $post );

	endif;
	?>
	</div>
	<div class="jp_attachment_details edit-form-section">
		<p>
	...
	<?php if ( 'image' === substr( $post->post_mime_type, 0, 5 ) ) : ?>
		<p>
			...
			<input type="text" class="widefat" name="_jp_attachment_image_alt" id="attachment_alt" value="<?php echo esc_attr( $alt_text ); ?>" />
		</p>
	<?php endif; ?>
	...
	<?php jp_editor( $post->post_content, 'attachment_content', $editor_args ); ?>
...
?>
	...
	if ( preg_match( '#^(audio|video)/#', $post->post_mime_type ) ) {
		 ...
		 * @param JP_Post $post   JP_Post object for the current attachment.
		 */
 ...
 */
function jp_add_id3_tag_data( &$metadata, $data ) {
	foreach ( array( 'id3v2', 'id3v1' ) as $version ) {
		if ( ! empty( $data[ $version ]['comments'] ) ) {
			foreach ( $data[ $version ]['comments'] as $key => $list ) {
				if ( 'length' !== $key && ! empty( $list ) ) {
					$metadata[ $key ] = jp_kses_post( reset( $list ) );
 ...
 */
function jp_read_video_metadata( $file ) {
	if ( ! file_exists( $file ) ) {
		return false;
	}
	...
	if ( empty( $metadata['created_timestamp'] ) ) {
		$created_timestamp = jp_get_media_creation_timestamp( $data );
	...
	jp_add_id3_tag_data( $metadata, $data );
	 ...
	 */
	return apply_filters( 'jp_read_video_metadata', $metadata, $file, $file_format, $data );
}
 ...
 * @return array|bool Returns array of metadata, if found.
 */
function jp_read_audio_metadata( $file ) {
	if ( ! file_exists( $file ) ) {
		return false;
	}
	$metadata = array();
 ...
 */
function jp_get_media_creation_timestamp( $metadata ) {
	$creation_date = false;
 ...
 *                          Default 'attach'.
 */
function jp_media_attach_action( $parent_id, $action = 'attach' ) {
	global $wpdb;
