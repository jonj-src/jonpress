 ...
 * @return string|JP_Error A list of category parents on success, JP_Error on failure.
 */
 ...
 * @return JP_Term[] Array of JP_Term objects, one for each category assigned to the post.
 */
function get_the_category( $id = false ) {
	$categories = get_the_terms( $id, 'category' );
	if ( ! $categories || is_jp_error( $categories ) ) {
		$categories = array();
	}
	 ...
	 *
	 * @param JP_Term[] $categories An array of categories to return for the post.
 ...
 * @return string|JP_Error Category name on success, JP_Error on failure.
 */
...
function get_the_category_list( $separator = '', $parents = '', $post_id = false ) {
	 ...
	 * @since 4.4.0
	 *
	 * @param JP_Term[] $categories An array of the post's categories.
	...
	$rel = ( is_object( $jp_rewrite ) && $jp_rewrite->using_permalinks() ) ? 'rel="category tag"' : 'rel="category"';
 ...
 *
 * Prior to v2.5 of JonPress, category names were not supported.
 * Prior to v2.7, category slugs were not supported.
 * Prior to v2.7, only one category could be compared: in_category( $single_category ).
 * Prior to v2.7, this function could only be used in the JonPress Loop.
 ...
 *     Optional. Array or string of arguments to generate a categories drop-down element. See JP_Term_Query::__construct()
 ...
 */
function jp_dropdown_categories( $args = '' ) {
	$defaults = array(
		'show_option_all'   => '',
		'show_option_none'  => '',
		'orderby'           => 'id',
	...
	if ( empty( $categories ) && ! $r['hide_if_empty'] && ! empty( $r['show_option_none'] ) ) {
		 ...
		 * @param JP_Term|null $category The category object, or null if there's no corresponding category.
		 */
	 ...
	 */
	$output = apply_filters( 'jp_dropdown_cats', $output, $r );
 ...
 */
function jp_list_categories( $args = '' ) {
	$defaults = array(
		'child_of'            => 0,
		'current_category'    => 0,
		'depth'               => 0,
	...
	if ( true == $r['hierarchical'] ) {
		...
		if ( $r['exclude_tree'] ) {
			$exclude_tree = array_merge( $exclude_tree, jp_parse_id_list( $r['exclude_tree'] ) );
		}

		if ( $r['exclude'] ) {
			$exclude_tree = array_merge( $exclude_tree, jp_parse_id_list( $r['exclude'] ) );
		}
	 ...
	 */
	$html = apply_filters( 'jp_list_categories', $output, $args );
 ...
 *     Optional. Array or string of arguments for displaying a tag cloud. See jp_generate_tag_cloud()
...
function jp_tag_cloud( $args = '' ) {
	...
	if ( empty( $tags ) || is_jp_error( $tags ) ) {
		return;
	}
	...
	$return = jp_generate_tag_cloud( $tags, $args ); // Here's where those top tags get sorted according to $args
	 ...
	 */
	$return = apply_filters( 'jp_tag_cloud', $return, $args );
 ...
 * @since 4.8.0 Added the `show_count` argument.
 *
 * @param JP_Term[]    $tags Array of JP_Term objects to generate the tag cloud for.
 * @param string|array $args {
 ...
 *                                                via {@see 'jp_generate_tag_cloud'}. Default 1|true.
 ...
 */
function jp_generate_tag_cloud( $tags, $args = '' ) {
	$defaults = array(
		'smallest'                   => 8,
		'largest'                    => 22,
		'unit'                       => 'pt',
	 ...
	 * @since 2.8.0
	 *
	 * @param JP_Term[] $tags Ordered array of terms.
	 * @param array     $args An array of tag cloud arguments.
	 */
	...
	if ( $tags_sorted !== $tags ) {
		...
		if ( 'RAND' === $args['order'] ) {
			...
			if ( 'name' === $args['orderby'] ) {
				uasort( $tags, '_jp_object_name_sort_cb' );
			} else {
				uasort( $tags, '_jp_object_count_sort_cb' );
			}
	 ...
	 */
	$tags_data = apply_filters( 'jp_generate_tag_cloud_data', $tags_data );
	...
	switch ( $args['format'] ) {
		...
		case 'list':
			 ...
			 */
			$return  = "<ul class='jp-tag-cloud' role='list'>\n\t<li>";
			$return .= join( "</li>\n\t<li>", $a );
			$return .= "</li>\n</ul>\n";
			break;
		default:
	...
	if ( $args['filter'] ) {
		 ...
		 * to the $filter argument in jp_generate_tag_cloud().
		 *
		 * @since 2.3.0
		 *
		 * @see jp_generate_tag_cloud()
		 *
		 * @param array|string $return String containing the generated HTML tag cloud output
		 *                             or an array of tag links if the 'format' argument
		 *                             equals 'array'.
		 * @param JP_Term[]    $tags   An array of terms used in the tag cloud.
		 * @param array        $args   An array of jp_generate_tag_cloud() arguments.
		 */
		return apply_filters( 'jp_generate_tag_cloud', $return, $tags, $args );
	} else {
		return $return;
	}
}
 ...
 */
function _jp_object_name_sort_cb( $a, $b ) {
 ...
 */
function _jp_object_count_sort_cb( $a, $b ) {
 ...
 * @return array|false|JP_Error Array of tag objects on success, false on failure.
 */
function get_the_tags( $id = 0 ) {
	 ...
	 * @see get_the_terms()
	 *
	 * @param JP_Term[] $terms An array of tags for the given post.
	 */
 ...
 * @return string|false|JP_Error A list of tags on success, false if there are no terms, JP_Error on failure.
 */
...
function term_description( $term = 0, $deprecated = null ) {
	...
	return is_jp_error( $description ) ? '' : $description;
}
 ...
 * @return JP_Term[]|false|JP_Error Array of JP_Term objects on success, false if there are no terms
 *                                  or the post does not exist, JP_Error on failure.
 */
function get_the_terms( $post, $taxonomy ) {
	...
	if ( false === $terms ) {
		$terms = jp_get_object_terms( $post->ID, $taxonomy );
		if ( ! is_jp_error( $terms ) ) {
			$term_ids = jp_list_pluck( $terms, 'term_id' );
			jp_cache_add( $post->ID, $term_ids, $taxonomy . '_relationships' );
		}
	}
	 ...
	 * @since 3.1.0
	 *
	 * @param JP_Term[]|JP_Error $terms    Array of attached terms, or JP_Error on failure.
 ...
 * @param string $sep Optional. Separate items using this.
 * @param string $after Optional. After list.
 * @return string|false|JP_Error A list of terms on success, false if there are no terms, JP_Error on failure.
 */
 ...
 * }
 * @return string|JP_Error A list of term parents on success, JP_Error or empty string on failure.
 */
function get_term_parents_list( $term_id, $taxonomy, $args = array() ) {
	...
	foreach ( array( 'link', 'inclusive' ) as $bool ) {
		$args[ $bool ] = jp_validate_boolean( $args[ $bool ] );
	}
 ...
 * @param string $sep Optional. Separate items using this.
 * @param string $after Optional. After list.
 * @return false|void False on JonPress error.
 */
function the_terms( $id, $taxonomy, $before = '', $sep = ', ', $after = '' ) {
	$term_list = get_the_term_list( $id, $taxonomy, $before, $sep, $after );

	if ( is_jp_error( $term_list ) ) {
		return false;
	}
 ...
 *
 * Prior to v2.7 of JonPress, tags given as integers would also be checked against the post's tags' names and slugs (in addition to term_ids)
 * Prior to v2.7, this function could only be used in the JonPress Loop.
 * As of 2.7, the function can be used anywhere if it is provided a post ID or post object.
 *
 * For more information on this and similar theme functions, check out
