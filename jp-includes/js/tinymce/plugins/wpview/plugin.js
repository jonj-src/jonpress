...
( function( tinymce, wp ) {
	tinymce.PluginManager.add( 'wpview', function( editor ) {
		...
		function resetViews( content ) {
			...
			return content
				.replace( /<div[^>]+data-jpview-text="([^"]+)"[^>]*>(?:\.|[\s\S]+?wpview-end[^>]+>\s*<\/span>\s*)?<\/div>/g, callback )
				.replace( /<p[^>]+data-jpview-marker="([^"]+)"[^>]*>[\s\S]*?<\/p>/g, callback );
		}
		...
		editor.on( 'init', function() {
			...
			if ( MutationObserver ) {
				new MutationObserver( function() {
					editor.fire( 'jp-body-class-change' );
				} )
			...
			editor.on( 'jp-body-class-change', function() {
		...
		editor.on( 'preprocess hide', function( event ) {
			editor.$( 'div[data-jpview-text], p[data-jpview-marker]', event.node ).each( function( i, node ) {
				node.innerHTML = '.';
			} );
		}, true );
		...
		editor.on( 'drop objectselected', function( event ) {
			if ( isView( event.targetClone ) ) {
				event.targetClone = editor.getDoc().createTextNode(
					window.decodeURIComponent( editor.dom.getAttrib( event.targetClone, 'data-jpview-text' ) )
				);
			}
		} );
		...
		editor.on( 'resolvename', function( event ) {
			if ( isView( event.target ) ) {
				event.name = editor.dom.getAttrib( event.target, 'data-jpview-type' ) || 'object';
			}
		} );
		...
		editor.addButton( 'jp_view_remove', {
			tooltip: 'Remove',
