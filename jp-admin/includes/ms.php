...
function check_upload_size( $file ) {
	...
	if ( $file['error'] != '0' && ! isset( $_POST['html-upload'] ) && ! jp_doing_ajax() ) {
		jp_die( $file['error'] . ' <a href="javascript:history.go(-1)">' . __( 'Back' ) . '</a>' );
	}
...
function wpmu_delete_blog( $blog_id, $drop = false ) {
	...
	// If ms_files_rewriting is enabled and upload_path is empty, jp_upload_dir is not reliable.
	...
	if ( $drop ) {
		 ...
		 * @since MU (3.0.0)
		 *
		 * @param string $uploads['basedir'] Uploads path without subdirectory. @see jp_upload_dir()
		 * @param int    $blog_id            The site ID.
		 */
 ...
 * @since 3.0.0
 *
 * @todo Merge with jp_delete_user() ?
 *
 * @global wpdb $wpdb JonPress database abstraction object.
 *
...
function wpmu_delete_user( $id ) {
	...
	/** This action is documented in jp-admin/includes/user.php */
 ...
 *
 * @param int    $id         The user ID.
 * @param string $pref       The column in the jp_users table to update the user's status
...
function _access_denied_splash() {
	...
	if ( jp_list_filter( $blogs, array( 'userblog_id' => get_current_blog_id() ) ) ) {
		return;
	}
	...
	if ( empty( $blogs ) ) {
		jp_die( sprintf( __( 'You attempted to access the "%1$s" dashboard, but you do not currently have privileges on this site. If you believe you should be able to access the "%1$s" dashboard, please contact your network administrator.' ), $blog_name ), 403 );
	}
...
function mu_dropdown_languages( $lang_files = array(), $current = '' ) {
	...
	if ( $flag === false ) { // JonPress english
 ...
 * @since 3.0.0
 *
 * @global int    $jp_db_version The version number of the database.
 * @global string $pagenow
 *
 * @return false False if the current user is not a super admin.
 */
function site_admin_notice() {
	global $jp_db_version, $pagenow;
	...
	if ( get_site_option( 'wpmu_upgrade_site' ) != $jp_db_version ) {
...
function confirm_delete_users( $users ) {
	...
	<?php
	jp_nonce_field( 'ms-users-delete' );
	...
	foreach ( ( $allusers = (array) $_POST['allusers'] ) as $user_id ) {
		if ( $user_id != '' && $user_id != '0' ) {
			...
			if ( ! current_user_can( 'delete_user', $delete_user->ID ) ) {
				jp_die( sprintf( __( 'Warning! User %s cannot be deleted.' ), $delete_user->user_login ) );
			}

			if ( in_array( $delete_user->user_login, $site_admins ) ) {
				jp_die( sprintf( __( 'Warning! User cannot be deleted. The user %s is a network administrator.' ), '<em>' . $delete_user->user_login . '</em>' ) );
			}
			?>
			<tr>
	...
	<?php
	/** This action is documented in jp-admin/users.php */
...
function network_edit_site_nav( $args = array() ) {
	...
	// All done!
	echo '<h2 class="nav-tab-wrapper jp-clearfix">';
...
function get_site_screen_help_tab_args() {
	return array(
		...
		'content' =>
			...
			'<p>' . __( '<strong>Settings</strong> &mdash; This page shows a list of all settings associated with this site. Some are created by JonPress and others are created by plugins you activate. Note that some fields are grayed out and say Serialized Data. You cannot modify these values due to the way the setting is stored in the database.' ) . '</p>',
	);
}
...
function get_site_screen_help_sidebar_content() {
	return '<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
		'<p>' . __( '<a href="https://codex.jonpress.org/Network_Admin_Sites_Screen">Documentation on Site Management</a>' ) . '</p>' .
		'<p>' . __( '<a href="https://jonpress.org/support/forum/multisite/">Support Forums</a>' ) . '</p>';
}
