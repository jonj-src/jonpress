...
switch ( $action ) {
	case 'editattachment':
		...
		if ( ! current_user_can( 'edit_post', $attachment_id ) ) {
			jp_die( __( 'Sorry, you are not allowed to edit this attachment.' ) );
		}
		...
		if ( empty( $errors ) ) {
			$location = 'media.php';
			if ( $referer = jp_get_original_referer() ) {
	...
	case 'edit':
		...
		if ( empty( $_GET['attachment_id'] ) ) {
			jp_redirect( admin_url( 'upload.php' ) );
			exit();
		}
		...
		if ( empty( $att->ID ) ) {
			jp_die( __( 'You attempted to edit an attachment that doesn&#8217;t exist. Perhaps it was deleted?' ) );
		}
		if ( 'attachment' !== $att->post_type ) {
			jp_die( __( 'You attempted to edit an item that isn&#8217;t an attachment. Please go back and try again.' ) );
		}
		if ( $att->post_status == 'trash' ) {
			jp_die( __( 'You can&#8217;t edit this attachment because it is in the Trash. Please move it out of the Trash and try again.' ) );
		}
...
?>
	...
	<?php jp_original_referer_field( true, 'previous' ); ?>
