...
class Featured_Content {
	...
	/**
	 * Conditionally hook into JonPress.
	 *
	 * Theme must declare that they support this module by adding
	 * add_theme_support( 'featured-content' ); during after_setup_theme.
	 *
	 * If no theme support is found there is no need to hook into JonPress.
	...
	public static function init() {
		...
		add_action( 'jp_loaded', array( __CLASS__, 'jp_loaded' ) );
	}

	/**
	 * Hide "featured" tag from the front end.
	 *
	 * Has to run on jp_loaded so that the preview filters of the Customizer
	 * have a chance to alter the value.
	 *
	 * @since Twenty Fourteen 1.0
	 */
	public static function jp_loaded() {
	 ...
	 * @since Twenty Fourteen 1.0
	 *
	 * @param JP_Query $query JP_Query object.
	 * @return JP_Query Possibly-modified JP_Query.
	 */
	 ...
	 */
	public static function customize_register( $jp_customize ) {
	...
	public static function enqueue_scripts() {
		jp_enqueue_script( 'featured-content-suggest', get_template_directory_uri() . '/js/featured-content-admin.js', array( 'jquery', 'suggest' ), '20131022', true );
	}
	...
	public static function get_setting( $key = 'all' ) {
		...
		$options = jp_parse_args( $saved, $defaults );
	...
	public static function validate_settings( $input ) {
		...
		if ( empty( $input['tag-name'] ) ) {
			...
			if ( $term ) {
				...
				$new_tag = jp_create_tag( $input['tag-name'] );

				if ( ! is_jp_error( $new_tag ) && isset( $new_tag['term_id'] ) ) {
