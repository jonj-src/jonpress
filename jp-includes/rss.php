...
define('RSS', 'RSS');
define('ATOM', 'Atom');
define('MAGPIE_USER_AGENT', 'JonPress/' . $GLOBALS['jp_version']);
...
function _fetch_remote_file($url, $headers = "" ) {
	$resp = jp_safe_remote_request( $url, array( 'headers' => $headers, 'timeout' => MAGPIE_FETCH_TIME_OUT ) );
	if ( is_jp_error($resp) ) {
	...
	// Also note, JP_HTTP lowercases all keys, Snoopy did not.
	$return_headers = array();
	foreach ( jp_remote_retrieve_headers( $resp ) as $key => $value ) {
		if ( !is_array($value) ) {
	...
	$response->status = jp_remote_retrieve_response_code( $resp );
	$response->response_code = jp_remote_retrieve_response_code( $resp );
	$response->headers = $return_headers;
	$response->results = jp_remote_retrieve_body( $resp );
...
function init () {
	...
	if ( !defined('MAGPIE_USER_AGENT') ) {
		$ua = 'JonPress/' . $GLOBALS['jp_version'];
...
class RSSCache {
	...
	function __construct( $base = '', $age = '' ) {
		$this->BASE_CACHE = JP_CONTENT_DIR . '/cache';
		if ( $base ) {
 ...
 */
function jp_rss( $url, $num_items = -1 ) {
 ...
 * to display. You can't display all of them like you can with jp_rss()
 * function.
 *
 * @since 1.5.0
 * @package External
