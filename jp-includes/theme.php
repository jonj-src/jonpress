...
/**
 * Returns an array of JP_Theme objects based on the arguments.
 *
 * Despite advances over get_themes(), this function is quite expensive, and grows
 * linearly with additional themes. Stick to jp_get_theme() if possible.
 *
 * @since 3.4.0
 *
 ...
 * }
 * @return JP_Theme[] Array of JP_Theme objects.
 */
function jp_get_themes( $args = array() ) {
	global $jp_theme_directories;
	...
	if ( is_array( $jp_theme_directories ) && count( $jp_theme_directories ) > 1 ) {
		// Make sure the current theme wins out, in case search_theme_directories() picks the wrong
		// one in the case of a conflict. (Normally, last registered theme root wins.)
		$current_theme = get_stylesheet();
		if ( isset( $theme_directories[ $current_theme ] ) ) {
			$root_of_current_theme = get_raw_theme_root( $current_theme );
			if ( ! in_array( $root_of_current_theme, $jp_theme_directories ) ) {
				$root_of_current_theme = JP_CONTENT_DIR . $root_of_current_theme;
			}
	...
	if ( is_multisite() && null !== $args['allowed'] ) {
		...
		if ( 'network' === $allowed ) {
			$theme_directories = array_intersect_key( $theme_directories, JP_Theme::get_allowed_on_network() );
		} elseif ( 'site' === $allowed ) {
			$theme_directories = array_intersect_key( $theme_directories, JP_Theme::get_allowed_on_site( $args['blog_id'] ) );
		} elseif ( $allowed ) {
			$theme_directories = array_intersect_key( $theme_directories, JP_Theme::get_allowed( $args['blog_id'] ) );
		} else {
			$theme_directories = array_diff_key( $theme_directories, JP_Theme::get_allowed( $args['blog_id'] ) );
		}
	}
	...
	foreach ( $theme_directories as $theme => $theme_root ) {
		if ( isset( $_themes[ $theme_root['theme_root'] . '/' . $theme ] ) ) {
			...
			$themes[ $theme ] = $_themes[ $theme_root['theme_root'] . '/' . $theme ] = new JP_Theme( $theme, $theme_root['theme_root'] );
		}
	}

	if ( null !== $args['errors'] ) {
		foreach ( $themes as $theme => $jp_theme ) {
			if ( $jp_theme->errors() != $args['errors'] ) {
				unset( $themes[ $theme ] );
			}
		}
	}
...
/**
 * Gets a JP_Theme object for a theme.
 *
 * @since 3.4.0
 *
 * @global array $jp_theme_directories
 ...
 * @return JP_Theme Theme object. Be sure to check the object's exists() method if you need to confirm the theme's existence.
 */
function jp_get_theme( $stylesheet = null, $theme_root = null ) {
	global $jp_theme_directories;
	...
	if ( empty( $theme_root ) ) {
		...
		if ( false === $theme_root ) {
			$theme_root = JP_CONTENT_DIR . '/themes';
		} elseif ( ! in_array( $theme_root, (array) $jp_theme_directories ) ) {
			$theme_root = JP_CONTENT_DIR . $theme_root;
		}
	}

	return new JP_Theme( $stylesheet, $theme_root );
}

/**
 * Clears the cache held by get_theme_roots() and JP_Theme.
 *
 * @since 3.5.0
 * @param bool $clear_update_cache Whether to clear the Theme updates cache
 */
function jp_clean_themes_cache( $clear_update_cache = true ) {
	if ( $clear_update_cache ) {
		delete_site_transient( 'update_themes' );
	}
	search_theme_directories( true );
	foreach ( jp_get_themes( array( 'errors' => null ) ) as $theme ) {
		$theme->cache_delete();
	}
}
 ...
 * JonPress workflow, then change the former. If you just have the locale in a
...
function get_locale_stylesheet_uri() {
	...
	} elseif ( ! empty( $jp_locale->text_direction ) && file_exists( "$dir/{$jp_locale->text_direction}.css" ) ) {
		$stylesheet_uri = "$stylesheet_dir_uri/{$jp_locale->text_direction}.css";
	} else {
		$stylesheet_uri = '';
	}
	/**
 ...
 * @global array $jp_theme_directories
 *
 * @param string $directory Either the full filesystem path to a theme folder or a folder within JP_CONTENT_DIR
 * @return bool
 */
function register_theme_directory( $directory ) {
	...
	if ( ! file_exists( $directory ) ) {
		...
		$directory = JP_CONTENT_DIR . '/' . $directory;
	...
	if ( ! is_array( $jp_theme_directories ) ) {
		$jp_theme_directories = array();
	}

	$untrailed = untrailingslashit( $directory );
	if ( ! empty( $untrailed ) && ! in_array( $untrailed, $jp_theme_directories ) ) {
		$jp_theme_directories[] = $untrailed;
	}
...
function search_theme_directories( $force = false ) {
	...
	$jp_theme_directories = (array) $jp_theme_directories;
	$relative_theme_roots = array();

	// Set up maybe-relative, maybe-absolute array of theme directories.
	// We always want to return absolute, but we need to cache relative
	// to use in get_theme_root().
	foreach ( $jp_theme_directories as $theme_root ) {
		if ( 0 === strpos( $theme_root, JP_CONTENT_DIR ) ) {
			$relative_theme_roots[ str_replace( JP_CONTENT_DIR, '', $theme_root ) ] = $theme_root;
		} else {
	 ...
	 */
	if ( $cache_expiration = apply_filters( 'jp_cache_themes_persistently', false, 'search_theme_directories' ) ) {
	...
	foreach ( $jp_theme_directories as $theme_root ) {
		...
		foreach ( $dirs as $dir ) {
			...
			if ( file_exists( $theme_root . '/' . $dir . '/style.css' ) ) {
				// jp-content/themes/a-single-theme
				// jp-content/themes is $theme_root, a-single-theme is $dir
				...
				$found_theme = false;
				// jp-content/themes/a-folder-of-themes/*
				// jp-content/themes is $theme_root, a-folder-of-themes is $dir, then themes are $sub_dirs
				...
				// Return it; JP_Theme will catch the error.
				if ( ! $found_theme ) {
					$found_themes[ $dir ] = array(
						'theme_file' => $dir . '/style.css',
						'theme_root' => $theme_root,
...
function get_theme_root( $stylesheet_or_template = false ) {
	...
	if ( $stylesheet_or_template && $theme_root = get_raw_theme_root( $stylesheet_or_template ) ) {
		// Always prepend JP_CONTENT_DIR unless the root currently registered as a theme directory.
		// This gives relative theme roots the benefit of the doubt when things go haywire.
		if ( ! in_array( $theme_root, (array) $jp_theme_directories ) ) {
			$theme_root = JP_CONTENT_DIR . $theme_root;
		}
	} else {
		$theme_root = JP_CONTENT_DIR . '/themes';
...
function get_theme_root_uri( $stylesheet_or_template = false, $theme_root = false ) {
	...
	if ( $stylesheet_or_template && $theme_root ) {
		if ( in_array( $theme_root, (array) $jp_theme_directories ) ) {
			...
			if ( 0 === strpos( $theme_root, JP_CONTENT_DIR ) ) {
				$theme_root_uri = content_url( str_replace( JP_CONTENT_DIR, '', $theme_root ) );
			} elseif ( 0 === strpos( $theme_root, ABSPATH ) ) {
				$theme_root_uri = site_url( str_replace( ABSPATH, '', $theme_root ) );
			} elseif ( 0 === strpos( $theme_root, JP_PLUGIN_DIR ) || 0 === strpos( $theme_root, WPMU_PLUGIN_DIR ) ) {
	 ...
	 * @param string $siteurl                JonPress web address which is set in General Options.
...
function switch_theme( $stylesheet ) {
	global $jp_theme_directories, $jp_customize, $sidebars_widgets;

	$_sidebars_widgets = null;
	if ( 'jp_ajax_customize_save' === current_action() ) {
		$old_sidebars_widgets_data_setting = $jp_customize->get_setting( 'old_sidebars_widgets_data' );
		if ( $old_sidebars_widgets_data_setting ) {
			$_sidebars_widgets = $jp_customize->post_value( $old_sidebars_widgets_data_setting );
		}
	...
	$old_theme = jp_get_theme();
	$new_theme = jp_get_theme( $stylesheet );
	$template  = $new_theme->get_template();

	update_option( 'template', $template );
	update_option( 'stylesheet', $stylesheet );

	if ( count( $jp_theme_directories ) > 1 ) {
	...
	if ( is_admin() && false === get_option( 'theme_mods_' . $stylesheet ) ) {
		 ...
		 * the Customizer when accessing jp-admin/widgets.php.
		 */
		if ( 'jp_ajax_customize_save' === current_action() ) {
	 ...
	 * @param JP_Theme $new_theme JP_Theme instance of the new theme.
	 * @param JP_Theme $old_theme JP_Theme instance of the old theme.
	 */
...
function validate_current_theme() {
	 ...
	 */
	if ( jp_installing() || ! apply_filters( 'validate_current_theme', true ) ) {
		return true;
	}
	...
	$default = jp_get_theme( JP_DEFAULT_THEME );
	if ( $default->exists() ) {
	...
	/**
	 * If we're in an invalid state but JP_DEFAULT_THEME doesn't exist,
	 * switch to the latest core default theme that's installed.
	 * If it turns out that this latest core default theme is our current
	 * theme, then there's nothing we can do about that, so we have to bail,
	 * rather than going into an infinite loop. (This is why there are
	 * checks against JP_DEFAULT_THEME above, also.) We also can't do anything
	 * if it turns out there is no default theme installed. (That's `false`.)
	 */
	$default = JP_Theme::get_core_default_theme();
...
function get_theme_mods() {
	...
	if ( false === $mods ) {
		...
		if ( false === $theme_name ) {
			$theme_name = jp_get_theme()->get( 'Name' );
		}
...
function get_theme_mod( $name, $default = false ) {
	...
	/** This filter is documented in jp-includes/theme.php */
...
function get_header_image_tag( $attr = array() ) {
	...
	if ( empty( $attr['srcset'] ) && ! empty( $header->attachment_id ) ) {
		$image_meta = get_post_meta( $header->attachment_id, '_jp_attachment_metadata', true );
		$size_array = array( $width, $height );

		if ( is_array( $image_meta ) ) {
			$srcset = jp_calculate_image_srcset( $size_array, $header->url, $image_meta, $header->attachment_id );
			$sizes  = ! empty( $attr['sizes'] ) ? $attr['sizes'] : jp_calculate_image_sizes( $size_array, $header->url, $image_meta, $header->attachment_id );
 ...
 * @access private
 *
 * @global array  $_jp_default_headers
 * @staticvar object $_jp_random_header
 *
 * @return object
 */
function _get_random_header_data() {
	static $_jp_random_header = null;

	if ( empty( $_jp_random_header ) ) {
		global $_jp_default_headers;
		$header_image_mod = get_theme_mod( 'header_image', '' );
		$headers          = array();

		if ( 'random-uploaded-image' == $header_image_mod ) {
			$headers = get_uploaded_header_images();
		} elseif ( ! empty( $_jp_default_headers ) ) {
			if ( 'random-default-image' == $header_image_mod ) {
				$headers = $_jp_default_headers;
			} else {
		...
		$_jp_random_header = (object) $headers[ array_rand( $headers ) ];

		$_jp_random_header->url           = sprintf( $_jp_random_header->url, get_template_directory_uri(), get_stylesheet_directory_uri() );
		$_jp_random_header->thumbnail_url = sprintf( $_jp_random_header->thumbnail_url, get_template_directory_uri(), get_stylesheet_directory_uri() );
	}
	return $_jp_random_header;
}
...
function get_uploaded_header_images() {
	...
	$headers = get_posts(
		array(
			...
			'meta_key'   => '_jp_attachment_is_custom_header',
	...
	foreach ( (array) $headers as $header ) {
		$url          = esc_url_raw( jp_get_attachment_url( $header->ID ) );
		$header_data  = jp_get_attachment_metadata( $header->ID );
		...
		$header_images[ $header_index ]['alt_text']          = get_post_meta( $header->ID, '_jp_attachment_image_alt', true );
...
function get_custom_header() {
	...
	if ( is_random_header_image() ) {
		...
		if ( ! $data && current_theme_supports( 'custom-header', 'default-image' ) ) {
			...
			if ( ! empty( $_jp_default_headers ) ) {
				foreach ( (array) $_jp_default_headers as $default_header ) {
	...
	return (object) jp_parse_args( $data, $default );
}
...
function register_default_headers( $headers ) {
	...
	$_jp_default_headers = array_merge( (array) $_jp_default_headers, (array) $headers );
}
...
function unregister_default_headers( $header ) {
	...
	} elseif ( isset( $_jp_default_headers[ $header ] ) ) {
		unset( $_jp_default_headers[ $header ] );
		return true;
	} else {
		return false;
	}
...
function get_header_video_settings() {
	...
	$video_type = jp_check_filetype( $video_url, jp_get_mime_types() );
...
function get_custom_header_markup() {
	...
	return sprintf(
		'<div id="jp-custom-header" class="jp-custom-header">%s</div>',
...
function the_custom_header_markup() {
	...
	if ( is_header_video_active() && ( has_header_video() || is_customize_preview() ) ) {
		jp_enqueue_script( 'jp-custom-header' );
		jp_localize_script( 'jp-custom-header', '_wpCustomHeaderSettings', get_header_video_settings() );
	}
}
 ...
 * @since 4.7.0
 */
function jp_custom_css_cb() {
	$styles = jp_get_custom_css();
	if ( $styles || is_customize_preview() ) :
	?>
		<style type="text/css" id="jp-custom-css">
 ...
 * @return JP_Post|null The custom_css post or null if none exists.
 */
function jp_get_custom_css_post( $stylesheet = '' ) {
	if ( empty( $stylesheet ) ) {
		$stylesheet = get_stylesheet();
	}
	...
	if ( get_stylesheet() === $stylesheet ) {
		...
		if ( ! $post && -1 !== $post_id ) {
			$query = new JP_Query( $custom_css_query_vars );
			$post  = $query->post;
			/*
			 * Cache the lookup. See jp_update_custom_css_post().
 ...
 */
function jp_get_custom_css( $stylesheet = '' ) {
	$css = '';
	...
	$post = jp_get_custom_css_post( $stylesheet );
	if ( $post ) {
	 ...
	 */
	$css = apply_filters( 'jp_get_custom_css', $css, $stylesheet );
 ...
 * }
 * @return JP_Post|JP_Error Post on success, error on failure.
 */
function jp_update_custom_css_post( $css, $args = array() ) {
	$args = jp_parse_args(
		$args, array(
			'preprocessed' => '',
			'stylesheet'   => get_stylesheet(),
	 ...
	 *     $post = jp_get_custom_css_post( $setting->stylesheet );
	 ...
	 * }
	 * @param array $args {
	 *     The args passed into `jp_update_custom_css_post()` merged with defaults.
	 *
	...
	$post = jp_get_custom_css_post( $args['stylesheet'] );
	if ( $post ) {
		$post_data['ID'] = $post->ID;
		$r               = jp_update_post( jp_slash( $post_data ), true );
	} else {
		$r = jp_insert_post( jp_slash( $post_data ), true );
		...
		if ( ! is_jp_error( $r ) ) {
			...
			if ( 0 === count( jp_get_post_revisions( $r ) ) ) {
				jp_save_post_revision( $r );
			}
		}
	}
	...
	$core_content = array(
		...
		'nav_menus' => array(
			...
			'link_email'      => array(
				...
				'url'   => 'mailto:jonpress@example.com',
			),
			'link_facebook'   => array(
				'title' => _x( 'Facebook', 'Theme starter content' ),
				'url'   => 'https://www.facebook.com/jonpress',
			),
			...
			'link_twitter'    => array(
				...
				'url'   => 'https://twitter.com/jonpress',
			),
			'link_yelp'       => array(
			...
			case 'posts':
				foreach ( $config[ $type ] as $id => $item ) {
					if ( is_array( $item ) ) {
						...
						$content[ $type ][ $id ] = jp_array_slice_assoc(
							$item,
							array(
								'post_type',
								'post_title',
 ...
 *
 * @global array $_jp_theme_features
 *
...
function add_theme_support( $feature ) {
	...
	switch ( $feature ) {
		case 'post-thumbnails':
			 ...
			 * for post thumbnails.
			 */
			if ( is_array( $args[0] ) && isset( $_jp_theme_features['post-thumbnails'] ) ) {
				$args[0] = array_unique( array_merge( $_jp_theme_features['post-thumbnails'][0], $args[0] ) );
			}
		...
		case 'html5':
			...
			if ( isset( $_jp_theme_features['html5'] ) ) {
				$args[0] = array_merge( $_jp_theme_features['html5'][0], $args[0] );
			}
			break;
		...
		case 'custom-logo':
			...
			$args[0]  = jp_parse_args( array_intersect_key( $args[0], $defaults ), $defaults );
		...
		case 'custom-header':
			...
			if ( isset( $_jp_theme_features['custom-header'] ) ) {
				$args[0] = jp_parse_args( $_jp_theme_features['custom-header'][0], $args[0] );
			}

			// Load in the defaults at the end, as we need to insure first one wins.
			// This will cause all constants to be defined, as each arg will then be set to the default.
			if ( $jit ) {
				$args[0] = jp_parse_args( $args[0], $defaults );
			}

			// If a constant was defined, use that value. Otherwise, define the constant to ensure
			// the constant is always accurate (and is not defined later,  overriding our value).
			// As stated above, the first value wins.
			// Once we get to jp_loaded (just-in-time), define any constants we haven't already.
		...
		case 'custom-background':
			...
			$defaults = array(
				...
				'default-color'          => '',
				'jp-head-callback'       => '_custom_background_cb',
				'admin-head-callback'    => '',
				'admin-preview-callback' => '',
			);
			...
			if ( isset( $_jp_theme_features['custom-background'] ) ) {
				$args[0] = jp_parse_args( $_jp_theme_features['custom-background'][0], $args[0] );
			}
		...
		case 'title-tag':
			// Can be called in functions.php but must happen before jp_loaded, i.e. not in header.php.
			if ( did_action( 'jp_loaded' ) ) {
				/* translators: 1: title-tag, 2: jp_loaded */
				_doing_it_wrong(
					"add_theme_support( 'title-tag' )", sprintf(
						__( 'Theme support for %1$s should be registered before the %2$s hook.' ),
						'<code>title-tag</code>', '<code>jp_loaded</code>'
					), '4.1.0'
				);
	...
	$_jp_theme_features[ $feature ] = $args;
}
...
function _custom_header_background_just_in_time() {
	...
	if ( current_theme_supports( 'custom-header' ) ) {
		...
		if ( $args[0]['jp-head-callback'] ) {
			add_action( 'jp_head', $args[0]['jp-head-callback'] );
		}

		if ( is_admin() ) {
			require_once( ABSPATH . 'jp-admin/custom-header.php' );
	...
	if ( current_theme_supports( 'custom-background' ) ) {
		...
		if ( is_admin() ) {
			require_once( ABSPATH . 'jp-admin/custom-background.php' );
...
function get_theme_support( $feature ) {
	global $_jp_theme_features;
	if ( ! isset( $_jp_theme_features[ $feature ] ) ) {
		return false;
	}

	if ( func_num_args() <= 1 ) {
		return $_jp_theme_features[ $feature ];
	}
	...
	switch ( $feature ) {
		...
		case 'custom-header':
		case 'custom-background':
			if ( isset( $_jp_theme_features[ $feature ][0][ $args[0] ] ) ) {
				return $_jp_theme_features[ $feature ][0][ $args[0] ];
			}
			return false;
...
function _remove_theme_support( $feature ) {
	...
	switch ( $feature ) {
		case 'custom-header':
			...
			if ( isset( $support[0]['jp-head-callback'] ) ) {
				remove_action( 'jp_head', $support[0]['jp-head-callback'] );
			}
	...
	unset( $_jp_theme_features[ $feature ] );
	return true;
}
...
function current_theme_supports( $feature ) {
	...
	switch ( $feature ) {
		case 'post-thumbnails':
			...
			if ( true === $_jp_theme_features[ $feature ] ) {  // Registered for all types
				return true;
			}
			$content_type = $args[0];
			return in_array( $content_type, $_jp_theme_features[ $feature ][0] );
		...
		case 'post-formats':
			...
			$type = $args[0];
			return in_array( $type, $_jp_theme_features[ $feature ][0] );

		case 'custom-logo':
		case 'custom-header':
		case 'custom-background':
			// Specific capabilities can be registered by passing an array to add_theme_support().
			return ( isset( $_jp_theme_features[ $feature ][0][ $args[0] ] ) && $_jp_theme_features[ $feature ][0][ $args[0] ] );
	}
	 ...
	 */
	return apply_filters( "current_theme_supports-{$feature}", true, $args, $_jp_theme_features[ $feature ] );
}
...
function _delete_attachment_theme_mod( $id ) {
	$attachment_image = jp_get_attachment_url( $id );
...
function check_theme_switched() {
	if ( $stylesheet = get_option( 'theme_switched' ) ) {
		$old_theme = jp_get_theme( $stylesheet );

		// Prevent widget & menu mapping from running since Customizer already called it up front
		if ( get_option( 'theme_switched_via_customizer' ) ) {
			remove_action( 'after_switch_theme', '_jp_menus_changed' );
			remove_action( 'after_switch_theme', '_jp_sidebars_changed' );
		...
		if ( $old_theme->exists() ) {
			...
			/** This action is documented in jp-includes/theme.php */
...
/**
 * Includes and instantiates the JP_Customize_Manager class.
 *
 * Loads the Customizer at plugins_loaded when accessing the customize.php admin
 * page or when any request includes a jp_customize=on param or a customize_changeset
 * param (a UUID). This param is a signal for whether to bootstrap the Customizer when
 * JonPress is loading, especially in the Customizer preview
 ...
 * @global JP_Customize_Manager $jp_customize
 */
function _jp_customize_include() {

	$is_customize_admin_page = ( is_admin() && 'customize.php' == basename( $_SERVER['PHP_SELF'] ) );
	$should_include          = (
		$is_customize_admin_page
		||
		( isset( $_REQUEST['jp_customize'] ) && 'on' == $_REQUEST['jp_customize'] )
		||
	...
	/*
	 * Note that jp_unslash() is not being used on the input vars because it is
	 * called before jp_magic_quotes() gets called. Besides this fact, none of
	...
	$input_vars = array_merge(
		jp_array_slice_assoc( $_GET, $keys ),
		jp_array_slice_assoc( $_POST, $keys )
	);
	...
	// Note that theme will be sanitized via JP_Theme.
	...
	$is_customize_save_action = (
		...
		&&
		'customize_save' === jp_unslash( $_REQUEST['action'] )
	);
	$settings_previewed       = ! $is_customize_save_action;

	require_once ABSPATH . WPINC . '/class-jp-customize-manager.php';
	$GLOBALS['jp_customize'] = new JP_Customize_Manager( compact( 'changeset_uuid', 'theme', 'messenger_channel', 'settings_previewed', 'autosaved', 'branching' ) );
}
 ...
 * @global JP_Customize_Manager $jp_customize Customizer instance.
 *
 * @param string  $new_status     New post status.
 * @param string  $old_status     Old post status.
 * @param JP_Post $changeset_post Changeset post object.
 */
function _jp_customize_publish_changeset( $new_status, $old_status, $changeset_post ) {
	global $jp_customize, $wpdb;
	...
	if ( empty( $jp_customize ) ) {
		require_once ABSPATH . WPINC . '/class-jp-customize-manager.php';
		$jp_customize = new JP_Customize_Manager(
			array(
	...
	if ( ! did_action( 'customize_register' ) ) {
		 ...
		 * will have a $jp_customize with those settings registered since they
		 ...
		 */
		remove_action( 'customize_register', array( $jp_customize, 'register_controls' ) );
		$jp_customize->register_controls();

		/** This filter is documented in /jp-includes/class-jp-customize-manager.php */
		do_action( 'customize_register', $jp_customize );
	}
	$jp_customize->_publish_changeset_values( $changeset_post->ID );
	 ...
	 */
	if ( ! jp_revisions_enabled( $changeset_post ) ) {
		$jp_customize->trash_changeset_post( $changeset_post->ID );
	}
}
 ...
 */
function _jp_customize_changeset_filter_insert_post_data( $post_data, $supplied_post_data ) {
 ...
 * @since 3.4.0
 */
function _jp_customize_loader_settings() {
	...
	$browser = array(
		'mobile' => jp_is_mobile(),
		'ios'    => jp_is_mobile() && preg_match( '/iPad|iPod|iPhone/', $_SERVER['HTTP_USER_AGENT'] ),
	);
	...
	$script = 'var _wpCustomizeLoaderSettings = ' . jp_json_encode( $settings ) . ';';

	$jp_scripts = jp_scripts();
	$data       = $jp_scripts->get_data( 'customize-loader', 'data' );
	if ( $data ) {
		$script = "$data\n$script";
	}

	$jp_scripts->add_data( 'customize-loader', 'data', $script );
}
 ...
 * @return string
 */
function jp_customize_url( $stylesheet = null ) {
 ...
 */
function jp_customize_support_script() {
	$admin_origin = parse_url( admin_url() );
	$home_origin  = parse_url( home_url() );
	$cross_domain = ( strtolower( $admin_origin['host'] ) != strtolower( $home_origin['host'] ) );
...
function is_customize_preview() {
	...
	return ( $jp_customize instanceof JP_Customize_Manager ) && $jp_customize->is_preview();
}
 ...
 * garbage-collected at the same time by `jp_delete_auto_drafts()`. Otherwise,
 ...
 *
 * @link https://core.trac.jonpress.org/ticket/39752
 *
 * @since 4.8.0
 * @access private
 * @see jp_delete_auto_drafts()
 *
 ...
 * @global wpdb $wpdb
 */
function _jp_keep_alive_customize_changeset_dependent_auto_drafts( $new_status, $old_status, $post ) {
	global $wpdb;
	unset( $old_status );
	 ...
	 * _jp_delete_customize_changeset_dependent_auto_drafts() will be called, since they need to be
	...
	if ( 'auto-draft' === $new_status ) {
		 ...
		 */
		$post_args['post_date'] = $post->post_date; // Note jp_delete_auto_drafts() only looks at this date.
	} else {
		/*
