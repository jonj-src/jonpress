<?php
/**
 * List Table API: JP_MS_Themes_List_Table class
 *
 * @package JonPress
 ...
 * @see JP_List_Table
 */
class JP_MS_Themes_List_Table extends JP_List_Table {
	...
	public function prepare_items() {
		...
		jp_reset_vars( array( 'orderby', 'order', 's' ) );

		$themes = array(
			/**
			 * Filters the full array of JP_Theme objects to list in the Multisite
			 * themes list table.
			 *
			 * @since 3.1.0
			 *
			 * @param JP_Theme[] $all Array of JP_Theme objects to display in the list table.
			 */
			'all'      => apply_filters( 'all_themes', jp_get_themes() ),
			'search'   => array(),
			'enabled'  => array(),
			'disabled' => array(),
			'upgrade'  => array(),
			'broken'   => $this->is_site_themes ? array() : jp_get_themes( array( 'errors' => true ) ),
		);
		...
		JP_Theme::sort_by_name( $this->items );
	 ...
	 * @since 4.3.0
	 *
	 * @param JP_Theme $theme The current JP_Theme object.
	 */
	...
	public function column_name( $theme ) {
		...
		if ( ! $allowed ) {
			if ( ! $theme->errors() ) {
				...
				$actions['enable'] = sprintf(
					...
					esc_url( jp_nonce_url( $url, 'enable-theme_' . $stylesheet ) ),
					esc_attr( $aria_label ),
			...
			$actions['disable'] = sprintf(
				...
				esc_url( jp_nonce_url( $url, 'disable-theme_' . $stylesheet ) ),
				esc_attr( $aria_label ),
		...
		if ( ! $allowed && current_user_can( 'delete_themes' ) && ! $this->is_site_themes && $stylesheet != get_option( 'stylesheet' ) && $stylesheet != get_option( 'template' ) ) {
			...
			$actions['delete'] = sprintf(
				...
				esc_url( jp_nonce_url( $url, 'bulk-themes' ) ),
				esc_attr( $aria_label ),
				__( 'Delete' )
			);
		}
	...
	public function column_default( $theme, $column_name ) {
		 ...
		 * @param string   $stylesheet  Directory name of the theme.
		 * @param JP_Theme $theme       Current JP_Theme object.
		 */
	 ...
	 * @since 4.3.0
	 *
	 * @param JP_Theme $item The current JP_Theme object.
	 */
	...
	public function single_row( $theme ) {
		...
		if ( $this->is_site_themes ) {
			remove_action( "after_theme_row_$stylesheet", 'jp_theme_update_row' );
		}
