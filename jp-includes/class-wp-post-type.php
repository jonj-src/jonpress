...
final class JP_Post_Type {
	 ...
	 * to use a font icon, e.g. 'dashicons-chart-pie'. Pass 'none' to leave div.jp-menu-image empty
	 ...
	 *
	 * Custom controllers must extend JP_REST_Controller.
	 *
	 * @since 4.7.4
	...
	public function add_rewrite_rules() {
		...
		if ( false !== $this->query_var && $wp && is_post_type_viewable( $this ) ) {
			$jp->add_query_var( $this->query_var );
		}
		...
		if ( false !== $this->rewrite && ( is_admin() || '' != get_option( 'permalink_structure' ) ) ) {
			...
			if ( $this->has_archive ) {
				...
				if ( $this->rewrite['with_front'] ) {
					$archive_slug = substr( $jp_rewrite->front, 1 ) . $archive_slug;
				} else {
					$archive_slug = $jp_rewrite->root . $archive_slug;
				}

				add_rewrite_rule( "{$archive_slug}/?$", "index.php?post_type=$this->name", 'top' );
				if ( $this->rewrite['feeds'] && $jp_rewrite->feeds ) {
					$feeds = '(' . trim( implode( '|', $jp_rewrite->feeds ) ) . ')';
				...
				if ( $this->rewrite['pages'] ) {
					add_rewrite_rule( "{$archive_slug}/{$jp_rewrite->pagination_base}/([0-9]{1,})/?$", "index.php?post_type=$this->name" . '&paged=$matches[1]', 'top' );
				}
			}
	...
	public function remove_supports() {
		...
		unset( $_jp_post_type_features[ $this->name ] );
	}
	...
	public function remove_rewrite_rules() {
		global $wp, $jp_rewrite, $post_type_meta_caps;

		// Remove query var.
		if ( false !== $this->query_var ) {
			$jp->remove_query_var( $this->query_var );
		}
		...
		if ( false !== $this->rewrite ) {
			...
			foreach ( $jp_rewrite->extra_rules_top as $regex => $query ) {
				if ( false !== strpos( $query, "index.php?post_type=$this->name" ) ) {
					unset( $jp_rewrite->extra_rules_top[ $regex ] );
				}
			}
		}
