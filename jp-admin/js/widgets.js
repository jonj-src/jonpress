...
wpWidgets = {
	 ...
	 * Translations.
	 *
	 * Exported from PHP in jp_default_scripts().
	 *
	 * @var {object}
	 */
	l10n: {
	...
	init : function() {
		...
		$( '#widgets-right .sidebar-name' )
			...
			.click( function() {
				...
				$document.triggerHandler( 'jp-pin-menu' );
			})
			/*
	...
	saveOrder : function( sidebarId ) {
		...
		$.post( ajaxurl, data, function() {
			$( '#inactive-widgets-control-remove' ).prop( 'disabled' , ! $( '#jp_inactive_widgets .widget' ).length );
		...
		$.post( ajaxurl, data, function(r) {
			...
			if ( del ) {
				...
				if ( animate ) {
					...
					if ( sidebarId === 'jp_inactive_widgets' ) {
						$( '#inactive-widgets-control-remove' ).prop( 'disabled' , ! $( '#jp_inactive_widgets .widget' ).length );
					}
				}
			} else {
	...
	removeInactiveWidgets : function() {
		...
		$.post( ajaxurl, data, function() {
			$( '#jp_inactive_widgets .widget' ).each(function() {
				var $widget = $( this );
