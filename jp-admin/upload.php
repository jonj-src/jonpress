...
if ( ! current_user_can( 'upload_files' ) ) {
	jp_die( __( 'Sorry, you are not allowed to upload files.' ) );
}
...
if ( 'grid' === $mode ) {
	jp_enqueue_media();
	jp_enqueue_script( 'media-grid' );
	jp_enqueue_script( 'media' );

	remove_action( 'admin_head', 'jp_admin_canonical_url' );

	$q = $_GET;
	// let JS handle this
	unset( $q['s'] );
	$vars   = jp_edit_attachments_query_vars( $q );
	...
	get_current_screen()->set_help_sidebar(
		...
		'<p>' . __( '<a href="https://codex.jonpress.org/Media_Library_Screen">Documentation on Media Library</a>' ) . '</p>' .
	...
	?>
	<div class="wrap" id="jp-media-grid" data-search="<?php _admin_search_query(); ?>">
...
$jp_list_table = _get_list_table( 'JP_Media_List_Table' );
...
if ( $doaction ) {
	...
	switch ( $doaction ) {
		case 'detach':
			jp_media_attach_action( $_REQUEST['parent_post_id'], 'detach' );
			break;

		case 'attach':
			jp_media_attach_action( $_REQUEST['found_post_id'] );
			break;
		...
		case 'delete':
			...
			foreach ( (array) $post_ids as $post_id_del ) {
				...
				if ( ! jp_delete_attachment( $post_id_del ) ) {
...
if ( ! empty( $_GET['trashed'] ) && $trashed = absint( $_GET['trashed'] ) ) {
	...
	$message               .= ' <a href="' . esc_url( jp_nonce_url( 'upload.php?doaction=undo&action=untrash&ids=' . ( isset( $_GET['ids'] ) ? $_GET['ids'] : '' ), 'bulk-media' ) ) . '">' . __( 'Undo' ) . '</a>';
...
$messages[4] = __( 'Media file moved to the trash.' ) . ' <a href="' . esc_url( jp_nonce_url( 'upload.php?doaction=undo&action=untrash&ids=' . ( isset( $_GET['ids'] ) ? $_GET['ids'] : '' ), 'bulk-media' ) ) . '">' . __( 'Undo' ) . '</a>';
