...
function get_bookmark( $bookmark, $output = OBJECT, $filter = 'raw' ) {
	...
	if ( empty( $bookmark ) ) {
		...
		jp_cache_add( $bookmark->link_id, $bookmark, 'bookmark' );
		$_bookmark = $bookmark;
	} else {
		if ( isset( $GLOBALS['link'] ) && ( $GLOBALS['link']->link_id == $bookmark ) ) {
			$_bookmark = & $GLOBALS['link'];
		} elseif ( ! $_bookmark = jp_cache_get( $bookmark, 'bookmark' ) ) {
			$_bookmark = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $wpdb->links WHERE link_id = %d LIMIT 1", $bookmark ) );
			if ( $_bookmark ) {
				$_bookmark->link_category = array_unique( jp_get_object_terms( $_bookmark->link_id, 'link_category', array( 'fields' => 'ids' ) ) );
				jp_cache_add( $_bookmark->link_id, $_bookmark, 'bookmark' );
			}
		}
	}
 ...
 * @return string|JP_Error
 */
function get_bookmark_field( $field, $bookmark, $context = 'display' ) {
	$bookmark = (int) $bookmark;
	$bookmark = get_bookmark( $bookmark );

	if ( is_jp_error( $bookmark ) ) {
		return $bookmark;
	}
...
function get_bookmarks( $args = '' ) {
	...
	$key   = md5( serialize( $r ) );
	$cache = false;
	if ( 'rand' !== $r['orderby'] && $cache = jp_cache_get( 'get_bookmarks', 'bookmark' ) ) {
	...
	if ( ! empty( $r['category_name'] ) ) {
		if ( $r['category'] = get_term_by( 'name', $r['category_name'], 'link_category' ) ) {
			...
			$cache[ $key ] = array();
			jp_cache_set( 'get_bookmarks', $cache, 'bookmark' );
			/** This filter is documented in jp-includes/bookmark.php */
...
function clean_bookmark_cache( $bookmark_id ) {
	jp_cache_delete( $bookmark_id, 'bookmark' );
	jp_cache_delete( 'get_bookmarks', 'bookmark' );
