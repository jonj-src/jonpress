 ...
 *                             Default false.
 */
function jp_image_editor( $post_id, $msg = false ) {
	$nonce     = jp_create_nonce( "image_editor-$post_id" );
	...
	$backup_sizes = get_post_meta( $post_id, '_jp_attachment_backup_sizes', true );
	$can_restore  = false;
	...
	?>
	<div class="imgedit-wrap jp-clearfix">
...
<?php } ?>
	...
	<?php
	if ( $thumb && $sub_sizes ) {
		$thumb_img = jp_constrain_dimensions( $thumb['width'], $thumb['height'], 160, 120 );
	?>
	...
	<div class="imgedit-panel-content jp-clearfix">
		<?php echo $note; ?>
		<div class="imgedit-menu jp-clearfix">
...
/**
 * Streams image in JP_Image_Editor to browser.
 *
 * @since 2.9.0
 *
 * @param JP_Image_Editor $image         The image editor instance.
 * @param string          $mime_type     The mime type of the image.
 * @param int             $attachment_id The image's attachment post ID.
 * @return bool True on success, false on failure.
 */
function jp_stream_image( $image, $mime_type, $attachment_id ) {
	if ( $image instanceof JP_Image_Editor ) {

		/**
		 * Filters the JP_Image_Editor instance for the image to be streamed to the browser.
		 *
		 * @since 3.5.0
		 *
		 * @param JP_Image_Editor $image         The image editor instance.
		...
		if ( is_jp_error( $image->stream( $mime_type ) ) ) {
			return false;
		}

		return true;
	} else {
		_deprecated_argument( __FUNCTION__, '3.5.0', __( '$image needs to be an JP_Image_Editor object' ) );
 ...
 * @param JP_Image_Editor $image
 * @param string $mime_type
 * @param int $post_id
 * @return bool
 */
function jp_save_image_file( $filename, $image, $mime_type, $post_id ) {
	if ( $image instanceof JP_Image_Editor ) {

		/** This filter is documented in jp-admin/includes/image-edit.php */
		 ...
		 * @param JP_Image_Editor $image     JP_Image_Editor instance.
		 * @param string          $mime_type Image mime type.
		 * @param int             $post_id   Post ID.
		 */
		$saved = apply_filters( 'jp_save_image_editor_file', null, $filename, $image, $mime_type, $post_id );
		 ...
		 *
		 * @since 2.9.0
		 * @deprecated 3.5.0 Use jp_save_image_editor_file instead.
		 *
		 * @param mixed           $override  Value to return instead of saving. Default null.
		 * @param string          $filename  Name of the file to be saved.
		 * @param JP_Image_Editor $image     JP_Image_Editor instance.
		 ...
		 * @param int             $post_id   Post ID.
		 */
		$saved = apply_filters( 'jp_save_image_file', null, $filename, $image, $mime_type, $post_id );
		...
		switch ( $mime_type ) {
			case 'image/jpeg':
				/** This filter is documented in jp-includes/class-jp-image-editor.php */
 ...
 *
 * @since 2.9.0
 * @deprecated 3.5.0 Use JP_Image_Editor::rotate()
 * @see JP_Image_Editor::rotate()
 *
 * @ignore
...
function _rotate_image_resource( $img, $angle ) {
	_deprecated_function( __FUNCTION__, '3.5.0', 'JP_Image_Editor::rotate()' );
 ...
 *
 * @since 2.9.0
 * @deprecated 3.5.0 Use JP_Image_Editor::flip()
 * @see JP_Image_Editor::flip()
 *
 * @ignore
 * @param resource $img  Image resource.
...
function _flip_image_resource( $img, $horz, $vert ) {
	_deprecated_function( __FUNCTION__, '3.5.0', 'JP_Image_Editor::flip()' );
	$w   = imagesx( $img );
	$h   = imagesy( $img );
	$dst = jp_imagecreatetruecolor( $w, $h );
	if ( is_resource( $dst ) ) {
		$sx = $vert ? ( $w - 1 ) : 0;
		$sy = $horz ? ( $h - 1 ) : 0;
		$sw = $vert ? -$w : $w;
 ...
 * @return JP_Image_Editor JP_Image_Editor instance with changes applied.
 */
function image_edit_apply_changes( $image, $changes ) {
	...
	if ( $image instanceof JP_Image_Editor ) {
		...
		/**
		 * Filters the JP_Image_Editor instance before applying changes to the image.
		 *
		 * @since 3.5.0
		 *
		 * @param JP_Image_Editor $image   JP_Image_Editor instance.
		 * @param array           $changes Array of change operations.
		 */
		$image = apply_filters( 'jp_image_editor_before_change', $image, $changes );
		 ...
		 *
		 * @since 2.9.0
		 * @deprecated 3.5.0 Use jp_image_editor_before_change instead.
		 *
...
function stream_preview_image( $post_id ) {
	...
	$img = jp_get_image_editor( _load_image_to_edit_path( $post_id ) );

	if ( is_jp_error( $img ) ) {
		return false;
	}

	$changes = ! empty( $_REQUEST['history'] ) ? json_decode( jp_unslash( $_REQUEST['history'] ) ) : null;
	if ( $changes ) {
	...
	if ( is_jp_error( $img->resize( $w2, $h2 ) ) ) {
		return false;
	}

	return jp_stream_image( $img, $post->post_mime_type, $post_id );
}
 ...
 */
function jp_restore_image( $post_id ) {
	$meta         = jp_get_attachment_metadata( $post_id );
	$file         = get_attached_file( $post_id );
	$backup_sizes = $old_backup_sizes = get_post_meta( $post_id, '_jp_attachment_backup_sizes', true );
	$restored     = false;
	$msg          = new stdClass;
	...
	if ( isset( $backup_sizes['full-orig'] ) && is_array( $backup_sizes['full-orig'] ) ) {
		...
		$meta['file']   = _jp_relative_upload_path( $restored_file );
	...
	if ( ! jp_update_attachment_metadata( $post_id, $meta ) ||
		( $old_backup_sizes !== $backup_sizes && ! update_post_meta( $post_id, '_jp_attachment_backup_sizes', $backup_sizes ) ) ) {
 ...
 * @return \stdClass
 */
function jp_save_image( $post_id ) {
	...
	$img = jp_get_image_editor( _load_image_to_edit_path( $post_id, 'full' ) );
	if ( is_jp_error( $img ) ) {
	...
	if ( $scale && $fwidth > 0 && $fheight > 0 ) {
		...
		$changes = json_decode( jp_unslash( $_REQUEST['history'] ) );
		if ( $changes ) {
			$img = image_edit_apply_changes( $img, $changes );
		}
	} else {
	...
	if ( ! jp_save_image_file( $new_path, $img, $post->post_mime_type, $post_id ) ) {
	...
	if ( 'nothumb' === $target || 'all' === $target || 'full' === $target || $scaled ) {
		...
		$meta['file'] = _jp_relative_upload_path( $new_path );
	 ...
	 * https://core.trac.jonpress.org/ticket/32171
	 */
	...
	if ( isset( $sizes ) ) {
		...
		foreach ( $sizes as $size ) {
			...
			if ( isset( $_jp_additional_image_sizes[ $size ] ) ) {
				$width  = intval( $_jp_additional_image_sizes[ $size ]['width'] );
				$height = intval( $_jp_additional_image_sizes[ $size ]['height'] );
				$crop   = ( $nocrop ) ? false : $_jp_additional_image_sizes[ $size ]['crop'];
			} else {
	...
	if ( $success ) {
		jp_update_attachment_metadata( $post_id, $meta );
		update_post_meta( $post_id, '_jp_attachment_backup_sizes', $backup_sizes );

		if ( $target == 'thumbnail' || $target == 'all' || $target == 'full' ) {
			// Check if it's an image edit from attachment edit screen
			if ( ! empty( $_REQUEST['context'] ) && 'edit-attachment' == $_REQUEST['context'] ) {
				$thumb_url         = jp_get_attachment_image_src( $post_id, array( 900, 600 ), true );
