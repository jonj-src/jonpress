<?php
/**
 * Network API: JP_Network_Query class
 *
 * @package JonPress
 ...
 * @since 4.6.0
 *
 * @see JP_Network_Query::__construct() for accepted arguments.
 */
class JP_Network_Query {
	 ...
	 * @since 4.6.0
	 *
	 * @param string|array $query JP_Network_Query arguments. See JP_Network_Query::__construct()
	 */
	public function parse_query( $query = '' ) {
		 ...
		 * @since 4.6.0
		 *
		 * @param JP_Network_Query $this The JP_Network_Query instance (passed by reference).
		 */
		do_action_ref_array( 'parse_network_query', array( &$this ) );
	}

	/**
	 * Sets up the JonPress query for retrieving networks.
	 *
	 * @since 4.6.0
	 *
	...
	public function get_networks() {
		 ...
		 * @since 4.6.0
		 *
		 * @param JP_Network_Query $this Current instance of JP_Network_Query (passed by reference).
		 */
		...
		$last_changed = jp_cache_get_last_changed( 'networks' );

		$cache_key   = "get_network_ids:$key:$last_changed";
		$cache_value = jp_cache_get( $cache_key, 'networks' );
		...
		if ( false === $cache_value ) {
			...
			jp_cache_add( $cache_key, $cache_value, 'networks' );
		} else {
		 ...
		 * @since 4.6.0
		 *
		 * @param JP_Network[]     $_networks An array of JP_Network objects.
		 * @param JP_Network_Query $this      Current instance of JP_Network_Query (passed by reference).
		 */
		$_networks = apply_filters_ref_array( 'the_networks', array( $_networks, &$this ) );

		// Convert to JP_Network instances
	...
	protected function get_network_ids() {
		...
		if ( ! empty( $this->query_vars['network__in'] ) ) {
			$this->sql_clauses['where']['network__in'] = "$wpdb->site.id IN ( " . implode( ',', jp_parse_id_list( $this->query_vars['network__in'] ) ) . ' )';
		}

		// Parse network IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['network__not_in'] ) ) {
			$this->sql_clauses['where']['network__not_in'] = "$wpdb->site.id NOT IN ( " . implode( ',', jp_parse_id_list( $this->query_vars['network__not_in'] ) ) . ' )';
		}
	...
	private function set_found_networks() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			 ...
			 * @param JP_Network_Query $network_query        The `JP_Network_Query` instance.
			 */
