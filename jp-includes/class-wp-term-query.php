...
/**
 * Taxonomy API: JP_Term_Query class.
 *
 * @package JonPress
 * @subpackage Taxonomy
 * @since 4.6.0
 ...
 * @since 4.6.0
 *
 * @see JP_Term_Query::__construct() for accepted arguments.
 */
class JP_Term_Query {
	 ...
	 * @since 4.6.0
	 *
	 * @param string|array $query JP_Term_Query arguments. See JP_Term_Query::__construct()
	 */
	public function parse_query( $query = '' ) {
		...
		$query = jp_parse_args( $query, $this->query_var_defaults );
		 ...
		 * @since 4.6.0
		 *
		 * @param JP_Term_Query $this Current instance of JP_Term_Query.
		 */
	...
	public function get_terms() {
		...
		if ( ! empty( $include ) ) {
			$exclude      = '';
			$exclude_tree = '';
			$inclusions   = implode( ',', jp_parse_id_list( $include ) );
		}
		...
		if ( ! empty( $exclude_tree ) ) {
			$exclude_tree      = jp_parse_id_list( $exclude_tree );
		...
		if ( ! empty( $exclude ) ) {
			$exclusions = array_merge( jp_parse_id_list( $exclude ), $exclusions );
		}
		...
		$key          = md5( serialize( jp_array_slice_assoc( $args, array_keys( $this->query_var_defaults ) ) ) . serialize( $taxonomies ) . $this->request );
		...
		if ( 'count' == $_fields ) {
			...
			jp_cache_set( $cache_key, $count, 'terms' );
			return $count;
		}
		...
		if ( empty( $terms ) ) {
			jp_cache_add( $cache_key, array(), 'terms', DAY_IN_SECONDS );
			return array();
		}
		...
		jp_cache_add( $cache_key, $terms, 'terms', DAY_IN_SECONDS );
	...
	protected function parse_orderby( $orderby_raw ) {
		...
		if ( in_array( $_orderby, array( 'term_id', 'name', 'slug', 'term_group' ), true ) ) {
			...
			$include = implode( ',', jp_parse_id_list( $this->query_vars['include'] ) );
	 ...
	 * @since 4.6.0
	 *
	 * @param string $orderby_raw Raw 'orderby' value passed to JP_Term_Query.
