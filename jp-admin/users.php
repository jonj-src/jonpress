...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://codex.jonpress.org/Users_Screen">Documentation on Managing Users</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://codex.jonpress.org/Roles_and_Capabilities">Descriptions of Roles and Capabilities</a>' ) . '</p>' .
...
if ( empty( $_REQUEST ) ) {
	$referer = '<input type="hidden" name="jp_http_referer" value="' . esc_attr( jp_unslash( $_SERVER['REQUEST_URI'] ) ) . '" />';
} elseif ( isset( $_REQUEST['jp_http_referer'] ) ) {
	$redirect = remove_query_arg( array( 'jp_http_referer', 'updated', 'delete_count' ), jp_unslash( $_REQUEST['jp_http_referer'] ) );
	$referer  = '<input type="hidden" name="jp_http_referer" value="' . esc_attr( $redirect ) . '" />';
} else {
	$redirect = 'users.php';
	$referer  = '';
}
...
switch ( $jp_list_table->current_action() ) {
	...
	case 'promote':
		...
		if ( ! current_user_can( 'promote_users' ) ) {
			jp_die( __( 'Sorry, you are not allowed to edit this user.' ), 403 );
		}
		...
		foreach ( $userids as $id ) {
			...
			if ( $id == $current_user->ID && ! $jp_roles->role_objects[ $role ]->has_cap( 'promote_users' )
		...
		jp_redirect( add_query_arg( 'update', $update, $redirect ) );
		exit();

	case 'dodelete':
		if ( is_multisite() ) {
			jp_die( __( 'User deletion is not allowed from this screen.' ), 400 );
		}
		...
		if ( empty( $_REQUEST['delete_option'] ) ) {
			...
			$url = str_replace( '&amp;', '&', jp_nonce_url( $url, 'bulk-users' ) );
			jp_redirect( $url );
			exit;
		}

		if ( ! current_user_can( 'delete_users' ) ) {
			jp_die( __( 'Sorry, you are not allowed to delete users.' ), 403 );
		}
		...
		foreach ( $userids as $id ) {
			if ( ! current_user_can( 'delete_user', $id ) ) {
				jp_die( __( 'Sorry, you are not allowed to delete that user.' ), 403 );
			}
			...
			switch ( $_REQUEST['delete_option'] ) {
				case 'delete':
					...
					break;
				case 'reassign':
					jp_delete_user( $id, $_REQUEST['reassign_user'] );
					break;
			}
			++$delete_count;
		}
	...
	case 'delete':
		...
		if ( ! current_user_can( 'delete_users' ) ) {
			$errors = new JP_Error( 'edit_users', __( 'Sorry, you are not allowed to delete users.' ) );
		}
	...
	<?php jp_nonce_field( 'delete-users' ); ?>
<?php echo $referer; ?>
...
foreach ( $userids as $id ) {
	 ...
	 *
	 * @param JP_User $current_user JP_User object for the current user.
	...
	case 'doremove':
		...
		if ( ! is_multisite() ) {
			jp_die( __( 'You can&#8217;t remove users.' ), 400 );
		}
		...
		if ( ! current_user_can( 'remove_users' ) ) {
			jp_die( __( 'Sorry, you are not allowed to remove users.' ), 403 );
		}
	...
	case 'remove':
		...
		if ( ! current_user_can( 'remove_users' ) ) {
			$error = new JP_Error( 'edit_users', __( 'Sorry, you are not allowed to remove users.' ) );
		}
	...
	?>
	<form method="post" name="updateusers" id="updateusers">
	<?php jp_nonce_field( 'remove-users' ); ?>
<?php echo $referer; ?>
...
foreach ( $userids as $id ) {
	...
	default:
		if ( ! empty( $_GET['_jp_http_referer'] ) ) {
			jp_redirect( remove_query_arg( array( '_jp_http_referer', '_wpnonce' ), jp_unslash( $_SERVER['REQUEST_URI'] ) ) );
			exit;
		}

		if ( $jp_list_table->current_action() && ! empty( $_REQUEST['users'] ) ) {
			$userids  = $_REQUEST['users'];
			$sendback = jp_get_referer();

			/** This action is documented in jp-admin/edit-comments.php */
			$sendback = apply_filters( 'handle_bulk_actions-' . get_current_screen()->id, $sendback, $jp_list_table->current_action(), $userids );
		...
		$total_pages = $jp_list_table->get_pagination_arg( 'total_pages' );
		if ( $pagenum > $total_pages && $total_pages > 0 ) {
			jp_redirect( add_query_arg( 'paged', $total_pages ) );
			exit;
		}
				...
				case 'add':
					if ( isset( $_GET['id'] ) && ( $user_id = $_GET['id'] ) && current_user_can( 'edit_user', $user_id ) ) {
						...
						$messages[] = '<div id="message" class="updated notice is-dismissible"><p>' . sprintf(
							...
							esc_url(
								add_query_arg(
									'jp_http_referer', urlencode( jp_unslash( $_SERVER['REQUEST_URI'] ) ),
...
<?php $jp_list_table->search_box( __( 'Search Users' ), 'user' ); ?>
